...
mod windows_tests {
    ...
    fn smart_paste_with_text() {
        ...
        eprintln!("[INFO] smart_paste returned: type={}, content={}", result.paste_type, result.content);
    ...
    fn smart_paste_with_text_wsl_profile() {
        ...
        eprintln!("[INFO] smart_paste WSL returned: type={}, content={}", result.paste_type, result.content);
    ...
    fn diagnostic_clipboard_state() {
        ...
        for (id, name) in &[(1, "CF_TEXT"), (2, "CF_BITMAP"), (7, "CF_OEMTEXT"), (8, "CF_DIB"), (13, "CF_UNICODETEXT"), (15, "CF_HDROP"), (17, "CF_DIBV5")] {
            let available = clipboard_win::raw::is_format_avail(*id);
            eprintln!("  Format {} ({}): {}", id, name, if available { "YES" } else { "no" });
        }

        // Try reading text
        match get_clipboard::<String, _>(formats::Unicode) {
            Ok(text) => eprintln!("  Unicode text: {:?} ({} chars)", &text[..text.len().min(100)], text.len()),
        ...
        match get_clipboard::<Vec<u8>, _>(formats::Bitmap) {
            Ok(data) => {
                ...
                if data.len() >= 40 {
                    ...
                    eprintln!("    header_size={} width={} height={} bits={} compression={}", header_size, width, height, bit_count, compression);
                }
            }
        ...
        eprintln!("  smart_paste result: type={} content={}", result.paste_type, &result.content[..result.content.len().min(200)]);
    ...
    fn smart_paste_with_bitmap() {
        ...
        eprintln!("[INFO] First 20 bytes: {:?}", &bmp_data[..20.min(bmp_data.len())]);
        // Check if starts with "BM" (BMP file header)
        if bmp_data.len() >= 2 {
            eprintln!("[INFO] Starts with BM? {} (0x{:02X} 0x{:02X})", bmp_data[0] == b'B' && bmp_data[1] == b'M', bmp_data[0], bmp_data[1]);
        }
        if bmp_data.len() >= 40 {
            ...
            let img_size = u32::from_le_bytes([bmp_data[20], bmp_data[21], bmp_data[22], bmp_data[23]]);
            eprintln!("[INFO] DIB header: size={} w={} h={} planes={} bits={} comp={} img_size={}", hs, w, h, planes, bits, comp, img_size);
            let pixel_offset = if comp == 3 && hs <= 40 { hs as usize + 12 } else { hs as usize };
            let row_stride = ((w as usize * (bits as usize / 8) + 3) / 4) * 4;
            let expected = pixel_offset + row_stride * h.unsigned_abs() as usize;
            eprintln!("[INFO] pixel_offset={} row_stride={} expected_total={} actual={}", pixel_offset, row_stride, expected, bmp_data.len());
        }
        ...
        let tmp = tempfile::tempdir().unwrap();
        let result = smart_paste(tmp.path().to_str().unwrap(), "PowerShell").unwrap();
        eprintln!("[INFO] smart_paste bitmap returned: type={}, content={}", result.paste_type, result.content);

        assert_eq!(result.paste_type, "path");
        assert!(Path::new(&result.content).exists(), "Saved PNG should exist: {}", result.content);
    ...
    fn smart_paste_with_bitmap_wsl_path() {
        ...
        eprintln!("[INFO] smart_paste bitmap WSL returned: type={}, content={}", result.paste_type, result.content);

        assert_eq!(result.paste_type, "path");
        assert!(result.content.starts_with("/mnt/"), "WSL path should start with /mnt/, got: {}", result.content);
