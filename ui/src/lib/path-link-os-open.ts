 ...
 * - `Ctrl` → `osOpen`.
 * - `Ctrl+Shift` → `osReveal`.
 * - `Alt` 가 끼면 path-link 가 소유하지 않는다(#352 TUI 우회와 xterm 이 쓴다) →
 *   기존 동작으로 떨어진다.
...
export function decidePathLinkClickAction(
  ...
  if (!mods.ctrlKey || mods.altKey) return plain;
  return mods.shiftKey ? "osReveal" : "osOpen";
}

/**
 * 사용자 설정과 무관하게 **항상** 확인을 받는 확장자(ADR-0099 Decision 3 의
 * 하드 클래스). 호스트에서 곧바로 실행·설치·레지스트리 병합으로 이어지는
 * 것들이다.
 *
 * `.js`/`.jse`/`.vbs`/`.wsf` 가 특히 중요하다 — Windows 는 이들을 편집기가
 * 아니라 Windows Script Host 로 **실행**하므로, node 프로젝트에서 소스를
 * 열려던 클릭이 스크립트 실행이 된다.
 *
