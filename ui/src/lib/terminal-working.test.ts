...
import { hasSleepInhibitingTerminal, isTerminalWorking } from "./terminal-working";
...
describe("공통 작업 절전 정책", () => {
  it("작업 수는 지연된 running을 유지하고 출력만 있는 셸을 세지 않는다", () => {
    const task = observeTask(
      undefined,
      { source: "s", taskId: "t", sequence: 1, state: "running" },
      0,
    );
    const stale = observeTask(task, { source: "s", taskId: "t", sequence: 2, state: undefined }, 1);
    expect(isTerminalWorking(terminal({ task: stale }))).toBe(true);
    expect(isTerminalWorking(terminal({ outputActive: true }))).toBe(false);
  });
  ...
  it("비통합 셸과 미지원 TUI의 출력 예외는 유지한다", () => {
    expect(hasSleepInhibitingTerminal([terminal({ outputActive: true })])).toBe(true);
    expect(
      hasSleepInhibitingTerminal([
  ...
  it("다른 pane의 억제 근거를 보존한다", () => {
    expect(hasSleepInhibitingTerminal([])).toBe(false);
    expect(
      hasSleepInhibitingTerminal([terminal(), terminal({ id: "other", outputActive: true })]),
    ).toBe(true);
  });
});
