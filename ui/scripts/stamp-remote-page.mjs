...
const lines = [
  ...
  `Source-SHA256: remote-app.js=${assetSourceHash("remote-app.js")} remote-app.css=${assetSourceHash("remote-app.css")} remote-icons.js=${fileHash(REMOTE_ICONS_SOURCE)} file-kind-icon.ts=${fileHash(path.resolve(SCRIPT_DIR, "../src/lib/file-kind-icon.ts"))} task-status-glyph.ts=${fileHash(path.resolve(SCRIPT_DIR, "../src/lib/task-status-glyph.ts"))} codex-transcript-wheel.ts=${fileHash(CODEX_TRANSCRIPT_WHEEL_SOURCE)} path-link-lines.ts=${fileHash(path.resolve(SCRIPT_DIR, "../src/lib/path-link-lines.ts"))} terminal-cell-map.ts=${fileHash(path.resolve(SCRIPT_DIR, "../src/lib/terminal-cell-map.ts"))} composer-editor.js=${fileHash(path.resolve(SCRIPT_DIR, "../src/remote/composer-editor.js"))} remote-settings-mcp.js=${fileHash(REMOTE_SETTINGS_SOURCE)} remote-settings-schema.json=${fileHash(REMOTE_SETTINGS_SCHEMA)} remote-memo.js=${fileHash(path.resolve(SCRIPT_DIR, "../src/remote/remote-memo.js"))} remote-tool-swipe.js=${fileHash(path.resolve(SCRIPT_DIR, "../src/remote/remote-tool-swipe.js"))} memo-document.ts=${fileHash(path.resolve(SCRIPT_DIR, "../src/lib/memo-document.ts"))} lucide-package=${packageInputHash("lucide")}`,
