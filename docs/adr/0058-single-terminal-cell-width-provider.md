...
- `\p{Mn}\p{Me}\p{Cf}` 판정은 엔진 Unicode 데이터를 쓰므로 폭 0 집합은 Chromium/Node 버전에 따라 미세하게 달라질 수 있다. 폭 2 집합은 명시 테이블이므로 새 Unicode emoji 블록은 수동으로 추가해야 한다. `charProperties` 는 출력 코드포인트마다 호출되므로 조회는 Unicode 전 범위(`U+0`–`U+10FFFF`)를 덮는 lazy `Uint8Array` 캐시로 상시 O(1) 을 유지한다. 캐시 상한을 BMP+SMP 로 두면 CJK 확장 B–D 와 tag/variation selector supplement 가 캐시를 통째로 우회해 코드포인트마다 정규식 + 문자열 할당을 돌게 되므로, 상한을 낮춰 메모리를 아끼는 선택은 하지 않는다(엔트리당 1 바이트 = 약 1.1 MB, 앱 전체에서 1개). 한 엔트리는 폭과 emoji base 속성(`\p{Emoji}`, `\p{Emoji_Modifier_Base}`)을 함께 담아, extender 판정이 앞 코드포인트의 속성을 다시 계산하지 않는다.
