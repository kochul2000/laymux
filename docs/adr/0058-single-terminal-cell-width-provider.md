...
- `\p{Mn}\p{Me}\p{Cf}` 판정은 엔진 Unicode 데이터를 쓰므로 폭 0 집합은 Chromium/Node 버전에 따라 미세하게 달라질 수 있다. 폭 2 집합은 명시 테이블이므로 새 Unicode emoji 블록은 수동으로 추가해야 한다. `charProperties` 는 출력 코드포인트마다 호출되므로 조회는 Unicode 전 범위(`U+0`–`U+10FFFF`)를 덮는 lazy `Uint8Array` 캐시로 상시 O(1) 을 유지한다. 캐시 상한을 BMP+SMP 로 두면 CJK 확장 B–D 와 tag/variation selector supplement 가 캐시를 통째로 우회해 코드포인트마다 정규식 + 문자열 할당을 돌게 되므로, 상한을 낮춰 메모리를 아끼는 선택은 하지 않는다(엔트리당 1 바이트 = 약 1.1 MB, 앱 전체에서 1개). 한 엔트리는 폭과 emoji base 속성(`\p{Emoji}`, `\p{Emoji_Modifier_Base}`)을 함께 담아, extender 판정이 앞 코드포인트의 속성을 다시 계산하지 않는다. **판정 순서는 zero-width 먼저, wide 나중이다** — wide 구간과 `p{Mn}` 은 서로소가 아니고 `U+302A`-`U+302D`, `U+3099`, `U+309A`, `U+16FE4` 7개가 교차한다(`U+3099`/`U+309A` 는 NFD 일본어의 濁点/半濁点). wide 를 먼저 보면 이 7개가 폭 2 가 되고 결합 조건(`width === 0`)도 통과하지 못해 독립 클러스터가 된다. 순서를 바꿔 얻을 성능 이득은 애초에 없다 — `computeCacheEntry` 가 폭 0 이 아닌 모든 엔트리에 대해 emoji property escape 2개를 무조건 돌므로 wide 코드포인트는 어느 순서에서도 first touch 에 정규식을 낸다. 원래 주석이 내세운 "property escape 를 first-touch 경로에서 뺀다" 는 목표는 캐시 상한과 무관하게 처음부터 달성 불가였고, 순서 교체가 없앤 것은 3개 중 1개뿐이다. 게다가 `cacheEntry` 가 전 범위를 memoize 하므로 코드포인트당 최대 1회다(issue #544).
...
remote 표면이 provider 를 받지 못한다는 위 Consequences 항목은 해소됐다. 같은 모듈을 `unicode-provider.js` asset 으로 빌드해 remote 클라이언트가 첫 write 전에 등록한다. 커밋된 생성물이 소스와 갈리는 것은 asset 을 실행해 TS 소스와 비교하는 테스트가 잡는다. 폭 테이블 복제는 하지 않았다.
