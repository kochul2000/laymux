...
- activity·sync-CWD·MCP 입력처럼 상태를 근거로 side effect를 승인하는 경로는 strict snapshot의 오류를 `Shell`, 빈 target, not-found, `null`로 축소하지 않는다. 표시용 activity detector의 degraded fallback은 유지하되 strict detector는 authoritative ring뿐 아니라 `known_claude_terminals`, `known_codex_terminals`, interactive-app grace/exit cache와 PTY registry를 detection 전후에 검증해 poison을 `Result`로 전파한다. bulk activity는 `terminals → output_buffers → ring → activity caches → pty_handles` 순서를 지키며, CWD source/target admission은 poison 시 차단한다. MCP pre-write sampling 실패는 쓰기 전에 끝나고 post-write capture 실패는 이미 수행된 write와 byte count를 tool error에 보존한다.
...
- activity bulk 조회와 sync-CWD source/target 계산은 ring뿐 아니라 activity cache·PTY registry poison에서도 부분/빈 성공이나 `Shell`을 반환하지 않는다. MCP capture는 관찰 실패와 입력 미수행을 구분하므로, 실패 응답을 받은 호출자는 `written`/`sideEffect`를 확인한 뒤 재시도 여부를 결정해야 한다.
