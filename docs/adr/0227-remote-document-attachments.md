...
5. 상한이 10 MiB인 이유는 Cloud tunnel의 HTTP 요청 상한 16 MiB다. 10 MiB의 base64 JSON은 약 13.4 MiB로 그 안에 들어간다. desktop tunnel의 stream pending 상한은 16 MiB로, socket pending 상한은 32 MiB로 올린다. relay는 ASGI body chunk 하나를 frame 하나로 보내므로 frame 개수 상한은 frame 크기를 가정하지 않고 바이트 상한에서 1 KiB floor로 유도한다(16 MiB / 1 KiB). Android E2E는 세 겹의 경계를 같은 값에서 유도한다: (a) RPC envelope body 상한은 handler가 host의 `attachmentMaxMib`에서 계산한다 — "그 크기의 첨부 JSON을 AEAD·base64url로 한 번 더 감싼 크기"이며 10 MiB에서 약 17.8 MiB, 항상 최소 2 MiB. 요청 body는 session 검증보다 먼저 버퍼되므로 host 설정보다 큰 상한을 static으로 두지 않는다. (b) 복호화한 요청 plaintext 상한은 첨부 JSON의 cap(10 MiB 기준 약 13.4 MiB)에 wrapper slack을 더한 값이며, 응답 plaintext 상한(4 MiB)과 분리한다. 이 산술은 client가 page의 첨부 JSON을 byte 그대로 plaintext에 싣는다는 전제 위에 있다. Android native는 body를 `JSONObject`로 재직렬화하지 않고 검증한 문자열을 그대로 splice한다 — AOSP `org.json`은 `/`를 `\/`로 escape하므로 base64 payload를 재직렬화하면 plaintext가 상한을 넘어 cap 근처 첨부가 세션 종료로 끝난다. (c) Android native bridge의 body 문자열 상한은 cap 기준으로 맞춘다.
...
- **Android E2E RPC envelope 상한을 cap 기준 static으로**: 코드가 짧지만 pairing이 없는 Tailscale peer도 요청마다 약 18 MiB를 host에 버퍼시킬 수 있다. session 검증은 body를 다 읽은 뒤에야 가능하므로 상한은 첨부 handler와 같이 settings에서 매 요청 계산한다.
