...
- **재접속은 탈취가 아니다 — 먼저 묻는다.** 자동 경로는 claim 전에 `session/status` 를 조회하고, 다른 누군가가 제어권을 갖고 있으면(`active`) 물러난다. 예외는 resume capability 를 들고 있을 때뿐이며, 그것은 **이 탭이 소유했던** lease 만 대체할 수 있다(ADR-0037). 조회 실패 중 확정으로 취급하는 것은 `401`(토큰)·`403`(원격 비활성)뿐이고, 그 밖의 실패는 자문일 뿐이라 claim 을 막지 않는다. **pagehide 의 release 는 로컬 `leaseId` 도 비운다** — bfcache 복원은 문서 변수를 그대로 되살리므로, 남아 있는 `leaseId` 는 "아직 제어권이 있다" 로 읽혀 복귀 자체를 건너뛰게 만든다(stash 된 resume capability 가 release drain 을 따라가는 통로다).
