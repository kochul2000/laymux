# 0219. Android system back은 PC 소유 Remote UI stack을 먼저 닫는다
...
Android wrapper는 Remote surface의 system back을 전부 가로채 첫 입력에는 이탈 경고를, 2초 안의 두 번째 입력에는 연결 종료를 적용한다. 이 정책은 terminal session을 실수로 끝내지 않지만 PC가 제공한 Remote 문서 안의 현재 UI stack을 보지 않는다. 따라서 FileViewer를 읽거나 navigation drawer를 연 상태에서도 back이 그 레이어를 닫지 않고 이탈 경고부터 표시한다.
...
범위는 Android system back이 현재 Remote 문서의 일시적 레이어를 닫는 입력 라우팅과 기존 disconnect guard의 결합이다. browser/PWA의 history navigation, native pairing·connection-settings modal, FileViewer 내부 explorer back 버튼의 의미, Remote 레이어의 시각 디자인은 비목표다.
...
- 함수가 없거나 `false`를 반환하면 혼합 버전·로딩 중 문서에서도 기존 첫 back 경고와 2초 안의 두 번째 back disconnect로 안전하게 fallback한다.
...
- FileViewer·OAuth 확인·Composer 추천·Remote navigation을 사용한 뒤 system back이 현재 맥락을 먼저 닫고, 닫을 레이어가 없을 때만 연결 이탈 경고가 나타난다.
...
- Playwright는 실제 Composer popup, OAuth/FileViewer stacking, drawer 하위 화면·Dock·drawer 순서를 고정하고, Android 단위 테스트는 레이어 소비가 pending disconnect 경고를 해제하는지 검증한다. Android compile과 Remote asset drift 검증이 native 호출과 배포 bundle의 연결을 보장한다.
