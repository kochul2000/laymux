...
- Status: Superseded by [0259](0259-remote-file-viewer-system-back-to-explorer.md) (FileViewer system back 정책만; 나머지 결정은 유지)
- Date: 2026-08-30
...
- OAuth relay modal 닫기는 진행 중인 begin·forward 작업의 UI revision을 무효화한다. 늦게 도착한 등록·전달 성공/실패는 새 relay session을 지우거나 native relay, OS browser, browser popup 또는 현재 modal 상태를 다시 쓰지 않는다.
...
- Playwright는 실제 Composer popup, OAuth/FileViewer stacking, drawer 하위 화면·Dock·drawer 순서와 OAuth begin·forward 응답 지연 중 back 취소를 고정하고, Android 단위 테스트는 native loading overlay와 문서 레이어 소비가 pending disconnect 경고를 해제하는지 검증한다. Android compile과 Remote asset drift 검증이 native 호출과 배포 bundle의 연결을 보장한다.
