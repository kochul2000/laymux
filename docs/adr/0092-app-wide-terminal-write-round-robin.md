...
범위는 데스크톱에 마운트된 `TerminalView`들의 visible xterm physical write admission과 non-stabilized byte의 enqueue/dequeue quantum이다. PTY producer/ring, Tauri event, rendererless checkpoint, logical segment 처리, 병합 allowlist, 커서/OSC/IME 상태기는 바꾸지 않는다. 활성 pane 특별 우선순위, visible/hidden 가중치, Remote xterm scheduling은 비목표다.
...
- 모든 non-stabilized `Uint8Array`는 source가 live든 replay든 pane-local FIFO에 최대 64 KiB 조각으로 enqueue한다. scheduler가 turn을 부여할 때 다른 owner가 이미 pending이면 그 turn의 fresh dequeue는 compatible live 조각을 최대 64 KiB까지만 합친다. 다른 owner가 없으면 compatible live 조각은 기존 128 part·256 KiB 상한까지 합쳐 단독 live pane 처리량을 유지한다.
- replay는 producer가 coalescing을 허용하지 않는 callback barrier다. 한 logical replay가 여러 64 KiB entry로 잘려도 entry마다 독립 physical write/callback을 가지며 서로 또는 다음 replay request와 합치지 않는다. string과 stabilized frame은 enqueue에서도 자르지 않고 기존 원자 경계를 유지한다. 단독 상태에서 이미 materialize된 64~256 KiB live batch가 backpressure로 복원된 뒤 경쟁이 시작돼도 같은 batch 객체·buffer·callback 집합을 재시도한다. retry를 64 KiB로 다시 쪼개 no-rematerialization 불변식을 깨지 않는다.
...
- byte 순서·batch allowlist·128 part/256 KiB hard 상한·stabilizer/string/replay/IME barrier·visible/checkpoint ACK 교집합은 그대로다. 모든 non-stabilized byte enqueue 조각은 64 KiB이며 경쟁 turn의 fresh live coalescing 상한도 64 KiB, 단독 live owner의 coalescing 상한은 256 KiB다. replay는 source barrier 때문에 단독이어도 64 KiB entry 하나씩만 physical parse한다. 공정성 scheduler는 성능 때문에 정확성 경계를 합치거나 건너뛰지 않는다.
...
- 64 KiB는 bytes를 폐기하는 budget이 아니라 pane 사이의 admission quantum이다. 한 live pane만 남으면 같은 네 조각을 최대 256 KiB로 다시 materialize하므로 단독 live flood의 기존 physical-write 처리량을 의도적으로 보존한다. 경쟁 시작 전 만들어진 retry가 64 KiB보다 클 수 있다는 예외는 byte/callback identity 보존 비용이다.
- replay는 정확성 barrier라 위 단독 live fast path를 사용하지 않는다. 이전에는 한 replay request를 최대 256 KiB씩 제출했지만 이제 최대 64 KiB마다 parser callback과 macrotask hand-off가 생겨 복원 처리량이 낮아질 수 있다. 대신 실기에서 807 ms였던 긴 parse가 replay에서 왔을 가능성까지 포함해 replay에도 같은 enqueue quantum을 적용함으로써 256 KiB replay 예외를 닫는다.
...
- 순수 scheduler/queue 테스트는 one-active, round-robin, contention 신호, pending dedupe, live 64/256 KiB dequeue 상한, replay 64 KiB callback barrier, restored retry identity, pending-only epoch 취소, unmount release, stale callback idempotence와 동일 `instanceId` replacement 세대 격리를 고정한다. `TerminalView` 통합 테스트는 replay의 physical 64 KiB 경계와 원자 string 후속 write를 구분하고, 두 pane queue를 unmount 전에 끝까지 drain해 pane별 exact byte stream·physical callback exactly-once·discard에 따른 replacement attach 0건·최종 parsed ACK exactly-once를 고정한다.
