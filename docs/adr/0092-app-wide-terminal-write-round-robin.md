...
- Status: Proposed
- Date: 2026-07-29
- Source: 사용자 보고(issue #661: 여러 pane 동시 대량 출력에서 UI·입력 심각한 지연) · 2026-07-29 dev baseline(1 pane 82.7 ms, 2 pane 193.1 ms, 4 pane 556.5 ms) · [architecture/data-flow.md §8.8](../architecture/data-flow.md) · [ADR-0080](0080-output-backlog-coalescing-and-out-of-band-frontend-vitals.md) · [ADR-0084](0084-desktop-terminal-output-parsed-credit.md)
- Extends: [ADR-0080](0080-output-backlog-coalescing-and-out-of-band-frontend-vitals.md)의 pane-local macrotask 양보를 앱 전역 admission 순서로 확장한다. batch allowlist·크기·byte 경계와 [ADR-0084](0084-desktop-terminal-output-parsed-credit.md)의 parsed ACK 교집합은 변경하지 않는다.
...
issue #661의 동일한 bounded flood baseline에서 관측한 상호작용 지연은 1 pane 82.7 ms, 2 pane 193.1 ms, 4 pane 556.5 ms로 pane 수보다 빠르게 증가했다. 이 수치만으로 모든 지연 원인을 확정하지는 않지만 pane-local 양보만으로 앱 전역 응답성을 보장할 수 없다는 구조적 결함과 부합한다. 동시에 sequence·ring·visible/checkpoint parse 교집합은 손실 없이 유지해야 하므로 output drop이나 ACK 선행으로 비용을 숨길 수 없다.

범위는 데스크톱에 마운트된 `TerminalView`들의 visible xterm physical write admission이다. PTY producer/ring, Tauri event, rendererless checkpoint, logical segment 처리, per-pane batch 크기와 병합 allowlist, 커서/OSC/IME 상태기는 바꾸지 않는다. 활성 pane 특별 우선순위, visible/hidden 가중치, Remote xterm scheduling은 비목표다.
