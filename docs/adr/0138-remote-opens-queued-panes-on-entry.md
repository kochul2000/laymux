...
- **focus 가 곧 "열어달라" 요청이다.** focus 는 pane 의 workspace 를 활성화하고 그 pane 을 focused pane 으로 만든다. 세션은 그 결과로 pane 이 mount·render 되면서 startup slot 을 통해 생긴다. 다만 **focus 응답은 판정 근거가 아니다** — `terminals.setFocus` 는 세션을 기다리지 않고 포커스를 적용한 즉시 성공으로 답하므로, 응답이 왔다는 사실은 PTY 존재를 전혀 보증하지 않는다. 판정은 navigation 폴링만 소유한다. 폴링 상한은 데스크톱의 terminal 준비 상한과 같은 20초다(선행 슬롯 watchdog 10초를 포함할 수 있어야 한다).
...
- **`terminals.setFocus` 가 세션 준비까지 기다리게 만들어 focus 응답을 판정으로 사용**: 대기가 bridge 요청 예산(5초, `automation_server/helpers.rs`)에 갇혀 선행 슬롯 watchdog(10초)조차 못 덮는다. 예산을 늘리면 모든 자동화 요청의 응답 상한이 같이 늘고, 이 핸들러의 응답 계약은 MCP `focus_terminal` 과 공유되어 느린 cold start 가 실패로 보고된다. Remote 가 응답에 의존하지 않으면 계약을 건드릴 이유가 없다.
...
- cold 진입 1회당 focus 요청이 하나 나가고, 열릴 때까지 0.4초 간격의 navigation 재조회가 발생한다. focus 가 실패로 끝나도 판정에는 쓰이지 않으므로 폴링은 상한까지 그대로 진행한다.
