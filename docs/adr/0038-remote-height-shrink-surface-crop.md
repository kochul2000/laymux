...
- 커서 행이 crop 창 밖에 있으면(예: `clear` 직후 상단 행) 커서 행이 보이는 만큼만 sizer를 아래로 이동시킨다. 커서·IME composition UI가 화면 밖에 남지 않게 하는 보정이며, crop이 풀리면 원위치한다.
- crop은 clipping 전용 wrapper 안에서만 일어난다. host에 직접 붙는 터치 선택 핸들은 clipping 밖에 남아 하단 행 핸들이 잘리지 않는다.
- fit 생략과 resize 전송은 별개다. attach는 resize dedup 키를 리셋하므로, crop 중의 새 attach·터미널 전환에서도 보존된 `cols/rows`를 PTY에 재전송해 renderer와 PTY geometry가 어긋나지 않게 한다.
