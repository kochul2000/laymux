...
- ADR-0072 Consequences (7) 이 실행되는 단정이 됐다. 같은 프레임 시퀀스를 무손실 경로와 gap+복구
  경로로 흘려 셀·커서까지 동일함을 비교한다. 사보타주 대조군은 **복구 경로 안**에 둔다 — 드라이버가
  복구가 적용한 범위를 두 번 쓰게 하는 노브이며, 그래야 대조군이 재는 것이 "`diffScreens` 가 중복
  페인트를 잡는다"가 아니라 "복구가 바이트를 정확히 한 번만 splice 한다"가 된다.
...
- 하니스의 `output-surface-driver.ts` 는 `TerminalView` 의 gap→복구 흐름을 재현한다.
  **이것은 중복이고 표류할 수 있다.** 표류의 대가는 "화면 스위트가 통과하는데 실제 배선은 다르다"이며,
  ...
  단계 자체가 바뀌면 이 드라이버도 같은 PR 에서 따라가야 한다 — 첫 사례가 issue #607 이고,
  이 PR 은 그 semantics(attach 창 gap 도 복구로 갚는다 · 중첩 gap 은 회차 상한까지 재시도하고
  그 회차가 이미 적용한 범위는 반드시 쓴다 · 상한 초과는 `nestedGapEscalation`)에 드라이버를
  맞춘 상태로 들어간다. 그 결과 뒤집힌 단정이 하나 있다: attach 창에서 난 gap 은 이제 화면을
  잃지 않는다.
- **드라이버의 카운터는 프로덕션 버킷의 부분집합이다.** 이름은 같지만 `malformedDelta`·
  `attachFailure`·`repairTimeout` 은 이 계층에서 발생할 수 없어(가짜 ring 이 타입된 delta 를 주고
  `attach()` 에 실패 경로가 없으며 `resume()` 이 동기다) 칸 자체를 두지 않고, 반대로 대조군이
  "재부착이 실제로 돌았다"를 단정해야 해서 프로덕션에 없는 `reattach` 칸을 하나 더 둔다. 이 비대칭은
  드라이버 주석에 이유까지 적어 둔다 — 다음 사람이 빠진 칸을 버그로 읽지 않게 하려는 것이다.
