...
- 읽기 전용 필드와 revision 제외 필드는 Rust의 단일 경로 목록으로 정의한다. backend는 같은 목록을 `revisionIgnoredPaths`로 frontend apply bridge에 전달하며, frontend는 자체 필드 목록을 갖지 않고 저장 전후 비교에 전달받은 JSON Pointer만 제외한다. 비교용 snapshot은 구조 상태를 보강하는 CWD·Claude session IPC를 생략해 쓰기마다 전체 snapshot IPC를 반복하지 않는다.
- 이 동시성 보호는 범용 `update_settings`뿐 아니라 기존 `set_app_theme`·`set_profile_defaults`·`update_profile` 및 대응 REST setter에도 적용한다. Settings UI와 경쟁하면 덮어쓰기 대신 REST `409 Conflict` 또는 MCP tool error를 반환하며, 호출자는 최신 설정을 다시 읽고 재시도한다.
...
- frontend snapshot의 Rust 미지원 필드는 top-level과 nested 모두 bridge에서 명시적으로 거부하는 계약 테스트로 보호한다. 이 loud fail은 조용한 유실을 막기 위한 의도된 CI 가드다.
