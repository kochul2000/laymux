...
8. **등록은 CSS 규칙이 아니라 `FontFace` 객체로.** src 가 한 번 실패한 `@font-face` 규칙은 error 상태로 남아 같은 규칙을 통한 재시도가 아예 재요청을 내지 않는다. 매 시도마다 새 `FontFace` 를 만들면 재요청이 되고, family 문자열을 CSS 텍스트에 끼워 넣을 일도 없어진다. 실패는 다음 navigation 갱신이나 attach 에서 재시도하되 face 당 3 회로 제한한다 — 모바일에서 한 번의 일시적 실패가 세션 전체를 폴백 폰트에 묶으면 안 되고, 정말 못 받는 폰트는 조용히 포기해야 한다.
9. **라이선스 책임.** 토글 기본 off + 설정 문서에 재배포 경고를 명시해, 폰트 바이너리를 내보내는 선택을 사용자가 명시적으로 하도록 만든다. 앱은 폰트 라이선스를 판정하지 않는다.
...
- 데스크톱 프로세스는 서빙 중인 페이스의 원본 바이트와 brotli 본을 메모리에 유지한다. 바이트 캐시는 16 파일, 광고 캐시는 8 face 로 제한하고 넘치는 만큼만 버린다 — 상주 바이트 천장은 16 × 8 MiB(+ brotli 본)이다. 통째로 비우면 face 가 상한보다 많을 때 매 요청이 전체를 다시 해석하는 thrash 가 된다.
- 두 캐시는 따로 evict 되므로 광고 캐시 히트 경로에서 토큰이 아직 서빙 가능한지 확인하고, 아니면 그 항목을 버리고 다시 해석한다. 확인 없이 돌려주면 죽은 URL 을 계속 광고하게 된다.
- 토글을 끄면 이미 광고된 URL 도 즉시 404 다 — 라우트가 게이트에서 토글을 함께 본다. 캐시에 남은 바이트는 앱 재시작이나 eviction 으로 사라진다.
