...
Native는 pending encrypted RPC의 sequence를 먼저 확정하고 그 exact 응답을 원래 document request-id callback에 전달한 뒤 Remote 문서에 foreground 복구를 알린다. 따라서 PC에서 성공한 claim·입력 같은 mutation을 실패로 오인해 재시도하지 않는다. Remote 문서는 Android 시작 URL의 auto-connect 의사를 다시 활성화하고, pagehide 때 임시 저장한 resume capability를 즉시 메모리로 회수하며, native 실행 전에 취소되거나 queue에서 폐기된 나머지 HTTP 요청의 Promise만 transient 실패로 종료한다. 이어서 stale output peer를 닫아 기존 snapshot reattach 경로를 시작하고 retained lease를 즉시 heartbeat로 검증한다. 따라서 첫 claim 도중 background로 전환된 경우도 exact 결과를 이어받거나 실행되지 않은 요청만 자동으로 다시 시도하고, 문서 메모리의 terminal 선택, xterm viewport, composer 초안은 그대로 유지한다.
...
Foreground 복구는 native가 재개한 단일 RPC의 exact 응답을 먼저 원래 Promise에 전달하고, callback이 실행되지 않은 나머지 HTTP Promise만 명시적으로 실패시켜야 한다. 이어서 stale output을 모두 닫고 즉시 heartbeat를 실행한다. 이 순서가 어긋나면 성공한 mutation을 재시도하거나 문서 reload가 가려주던 pending 상태와 half-open socket이 남는다. APK와 PC 버전이 맞지 않으면 reload fallback이 계속 동작하므로 기능 저하는 기존 수준으로 제한된다.
