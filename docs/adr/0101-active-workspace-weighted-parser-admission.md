...
- hidden 처리량은 경쟁 중 낮아진다. 활성 workspace 에 flood pane 4개(focused 1 + visible 3), 별도 background workspace 에 hidden flood pane 8개를 두고 15초를 잰 실측에서 활성 workspace 의 parsed 지분은 pane 가중치(4:2:1·`K=8`) 정책의 **33.4%**(활성 pane 500 KiB/s vs hidden pane 468 KiB/s — 사실상 균등)에서 클래스 몫 정책의 **71.3%**(focused pane 1539 KiB/s, hidden pane 207 KiB/s)로 올라갔고 focused pane 의 평균 backlog 는 510 KiB 에서 118 KiB 로 줄었다. 이론값 80%(5+3 of 10)에 못 미치는 것은 focused pane 의 producer 가 약 1.5 MiB/s 로 먼저 한계이기 때문이다. hidden 은 그만큼 늦어지며 byte 는 버리지 않고 PTY backpressure 로 나타난다.
