...
- 이번 변경의 실기 측정은 그 하네스가 아니라 **축소 측정**으로 했다. 측정 머신에 Python 이 없어 `scripts/bench/terminal_output_661.py` 를 실행할 수 없었고, 같은 Automation API 와 같은 flood 생성기(`terminal-output-flood.ps1`, barrier 정렬, `-FlushEvery 256`)로 hidden 8 pane + 활성 workspace 1 pane 을 15초 동안 재고 parsed 지분과 backlog 만 비교했다. dev 19281, 8 hidden pane 경쟁(모든 hidden backlog ≈ 510 KiB) 기준으로 활성 pane 의 parsed 지분은 10.7%(활성:hidden 비 0.96 — 균등 배분)에서 25.2~28.4%(비 2.69~3.18)로, 평균 backlog 는 511.7 KiB 에서 119~128 KiB 로 바뀌었다. 활성 pane 의 비가 16:1 까지 가지 않는 것은 그 구성에서 활성 pane 의 producer 가 약 1.5~1.7 MiB/s 로 먼저 한계이기 때문이다. **미검증:** ADR-0098 Decision 11 의 전체 acceptance(150,000 라인 catch-up, screenshot·bridge·control echo latency, repair/fail-stop 0건)는 이 변경 위에서 아직 돌리지 못했다. Python 이 있는 환경에서 병합 전 또는 후속으로 같은 하네스를 다시 돌려야 한다.
