...
- 프레임 집합의 정확성은 **"렌더는 rAF 에서만 일어난다"** 는 xterm 의 성질에 기댄다. 렌더 경로가 rAF 밖으로 나가면 이 skip 은 근거를 잃는다 — `noteTerminalRendered()` 훅이 그 경우의 안전판이자, 그 성질을 코드에 명시해 두는 자리다. 집합 등록을 `rebuild()` **앞**에 두어, 동기 paint 가 보고한 delete 가 뒤늦은 add 에 덮이지 않게 했다(콜백이 던지면 `catch` 에서 되돌린다). 정확성이 호출 순서에 걸리지 않는다.
- **위 "총 6회" 는 조용한 pane 기준이다.** `noteTerminalRendered()` 가 매 `onRender` 마다 터미널을 집합에서 빼므로, 복귀 중에도 계속 그리는 pane(`watch`, 빌드 로그, TUI 애니메이션)은 pass 사이에 다시 대상이 된다. 이건 **필요한** 재구성이다 — paint 로 모델이 다시 채워졌으니 그 다음 wipe 는 그 pane 도 무효화한다. 계측에서 6 을 넘는 것은 회귀가 아니다.
