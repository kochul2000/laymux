...
- Amended by: [ADR-0189](0189-update-release-channels.md) — 단일 stable endpoint, `x.y.z` 전용 클라이언트 계약, prerelease 번들 집합·tag 검사·게시 게이트가 채널 계약으로 정정된다. 서명 검증, `UpdateManager` 소유권, Automation/Remote 노출 범위와 lease 정책은 유효하다.
