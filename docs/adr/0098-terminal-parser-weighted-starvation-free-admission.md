...
`MessageChannel` task source가 Tauri event·input·paint를 장시간 추월하지 않도록 7개 handoff 뒤 여덟 번째 handoff는 `setTimeout(0)`으로 제출한다. 이 timer yield는 한 번의 연속 browser task-source 양보이며, 그 사이 `MessageChannel` task가 timer nesting chain을 끊으므로 모든 turn을 timer로 넘기던 처리량 회귀를 되살리지 않는다.
