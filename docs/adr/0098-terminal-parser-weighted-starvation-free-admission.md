...
- 2/4/7/8 hot-pane 150,000-line dev 측정은 Automation control echo/screenshot latency, frontend health, parser frontier service gap, 고정 frontier target의 전체 catch-up, throughput, final marker와 fail-stop/repair를 함께 기록한다. control·bridge·diagnostics는 flood와 catch-up에 계속 병행하되, 동기 main-thread 작업인 html2canvas screenshot은 독립적인 3초 renderer-checkpoint 계약에 그 실행 시간을 중복 부과하지 않도록 고정 frontier catch-up 직후 같은 flood 안에서 실행한다. bridge·diagnostics·실제 image payload가 있는 screenshot·control echo와 frontend report age는 모두 5초 미만이어야 하며 backend bridge timeout도 없어야 한다. control echo는 run마다 5회 측정해 p50/p95/max를 남긴다. 실제 OS key latency는 devinput lease를 사용한 별도 실측으로 기록한다. 3초 checkpoint catch-up 또는 5초 parsed-progress 계약을 넘으면 weight·quantum을 재검토하되 single-slot·lossless 계약을 우회하지 않는다.
