...
- 자원 상한은 streak와 React mount 밖의 WebView/window-scoped terminal-id registry가 소유한다. attach와 ACK 종류별로 bridge operation을 호출하기 **직전** actual started-but-unsettled 수를 늘리고, underlying Promise가 resolve/reject할 때 exactly-once operation lease를 정착해 그 수를 줄인다. 따라서 watchdog 전에 unmount된 영구 pending operation도 사라지지 않으며, 중간의 attach/ACK 성공은 자기 lease만 정착하고 아직 pending인 과거 operation 수를 지우지 않는다.
- 같은 terminal id의 remount는 registry entry와 attach/ACK별 상한을 공유한다. 새 bridge operation은 해당 종류의 실제 outstanding operation이 6 미만일 때만 만든다. 6이면 current epoch를 폐기하고 readiness를 닫아 backend producer를 마지막 lease의 bounded credit에서 fail-stop하며, mount는 current epoch capacity waiter만 소유한다. operation 하나가 늦게 정착해 슬롯을 돌려주면 waiter를 먼저 소비하고 current epoch가 여전히 같을 때만 bounded backoff 뒤 replacement 하나를 예약한다. 여러 operation의 연속 정착은 같은 blocked epoch의 recovery를 중복 예약하지 않는다.
...
- unmount는 sender/timer와 자기 capacity waiter를 폐기하고 epoch를 전진시키되 이미 시작한 operation lease는 해제하지 않는다. 그 뒤 도착한 attach/ACK completion은 공유 registry 계수만 내리고 stale UI recovery는 예약하지 않는다. 같은 id가 remount되어 새 waiter가 cap에서 기다리고 있다면 그 current mount만 한 번 깨운다. active mount가 없고 attach/ACK outstanding이 모두 0일 때만 terminal-id entry를 제거하므로, 삭제된 terminal의 영구 pending operation도 정착 또는 WebView process 종료까지 계수에 남고 같은 id 재사용은 그 계수를 이어받는다.
...
- control bridge가 계속 응답하지 않아도 backend output과 frontend pending delta는 ADR-0084 window/ring 경계 안에서 멈추고, 같은 terminal id의 실제 미정착 attach·ACK operation도 WebView 수명 동안 종류별 최대 6개다. timeout과 성공의 교대나 timeout 전 반복 remount도 과거 pending operation을 지우지 않으므로 상한을 우회하지 못한다.
