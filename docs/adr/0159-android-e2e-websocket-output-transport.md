...
- 장기 WebSocket stream은 누적 수명 byte 상한을 두지 않고 relay의 frame·queue·pending byte·concurrency·I/O timeout 상한을 계속 적용한다. 한 stream의 backpressure 초과는 해당 stream error와 tombstone으로 격리하고 desktop tunnel 전체를 닫지 않는다. Desktop의 HTTP와 WebSocket task는 generation-safe task id를 가지며 자연 종료 completion도 active stream slot을 즉시 회수한다.
