...
- Relay는 encrypted binary record를 양방향으로 그대로 전달한다. 64 KiB보다 큰 record는 text-only tunnel의 1 MiB frame 상한 아래에서 opaque `fragment {index,count,messageBytes}`로 나누고, public WebSocket에 내보내기 전에 최대 2,629,658 encrypted bytes를 exact 한 binary message로 재조립한다. 이 transport metadata는 terminal id, lease id, Remote v1 header, snapshot/delta bytes를 드러내지 않는다. relay가 관찰 가능한 값은 routing metadata, 연결·record 시각과 ciphertext·fragment 크기, close/error 상태다.
...
- `laymux`와 `laymux-server`가 같은 wire contract를 함께 구현해야 한다. 두 저장소의 테스트는 exact query/path, 최대 snapshot record의 bounded fragmentation/reassembly, 장기 stream의 누적 전송, task slot 회수, key direction/nonce binding, nonce reuse rejection, Remote page binary ACK를 각각 고정한다.
