...
- Relay는 encrypted binary record를 양방향으로 그대로 전달한다. public caller의 a2d ingress는 첫 encrypted `OPEN` binary message 한 개와 4 KiB 상한만 허용하고, text·초과 크기·두 번째 message는 tunnel base64 직렬화 전에 거부한다. Python connector와 native desktop sender는 64 KiB보다 큰 record를 text-only tunnel의 1 MiB frame 상한 아래에서 같은 opaque `fragment {index,count,messageBytes}` 계약으로 나누고, relay receiver는 public WebSocket에 내보내기 전에 최대 2,629,658 encrypted bytes를 exact 한 binary message로 재조립한다. 이 transport metadata는 terminal id, lease id, Remote v1 header, snapshot/delta bytes를 드러내지 않는다. relay가 관찰 가능한 값은 routing metadata, 연결·record 시각과 ciphertext·fragment 크기, close/error 상태다.
- Desktop은 고정 local Axum WebSocket `/remote/v1/e2e/output`과 native Cloud tunnel special-case를 두 개의 transport adapter로만 둔다. Python connector가 local WebSocket으로 전달한 경우와 native tunnel이 직접 전달한 경우 모두 같은 Rust output core가 session·stream nonce·lease·renderer checkpoint·AEAD record sequence를 검증하고 Remote v1 header/body pair를 만든다. connector가 local Remote 인증을 위해 덧붙이는 `token` query는 local adapter에서만 허용하는 transport 확장이며 canonical routing 세 필드의 의미나 암호 계약에는 들어가지 않는다. 이 local adapter는 loopback·exact path·Origin 없음·Remote enabled·exact token을 모두 요구하고 direct client용 IP/Tailscale/Origin 정책에서는 분리한다. public peer의 주소와 Origin은 relay의 고정 계약과 rate/concurrency gate를 통과한 뒤 loopback hop에 보존되지 않기 때문이다.
...
- Native bridge는 최대 plaintext snapshot record 2,629,633 bytes 한 건과 추가 2 MiB backlog로 queue를 제한하고 한 record in-flight ACK를 사용한다. TEXT header는 parser가 수락한 뒤, BINARY body는 대응하는 xterm write promise가 끝난 뒤 ACK한다. AEAD와 network callback은 UI thread 밖에서 실행한다. pending과 active를 합친 output open 예약은 8개이며 같은 stream id의 중복 open도 거부한다.
- 장기 WebSocket stream은 초기 `websocket.accept`/opening frame의 bounded handshake timeout은 유지하되 그 뒤에는 누적 수명 byte 상한과 HTTP용 다음-frame timeout을 두지 않는다. 단방향으로 영구 idle일 수 있으므로 방향별 read timeout도 두지 않고 relay의 frame·queue·pending byte·concurrency·write timeout과 underlying WebSocket ping/pong lifecycle은 계속 적용한다. 한 stream의 backpressure 초과는 해당 stream error와 tombstone으로 격리하고 desktop tunnel 전체를 닫지 않는다. bridge는 peer cancellation 때 반대쪽 read pump를 즉시 취소하며, Desktop의 HTTP와 WebSocket task는 generation-safe task id를 가지고 자연 종료 completion도 active stream slot을 즉시 회수한다.
...
- `laymux`와 `laymux-server`가 같은 wire contract를 함께 구현해야 한다. 두 저장소의 테스트는 exact query/path, 단일·4 KiB a2d `OPEN`, 모든 tunnel peer의 최대 snapshot record bounded fragmentation/reassembly, 장기 stream의 누적 전송, task slot 회수, key direction/nonce binding, nonce reuse rejection, Remote page binary ACK를 각각 고정한다.
- WebView feature 미지원 기기는 control UI를 열 수 있어도 terminal output 연결은 실패한다. 보안 경계를 낮추는 JS interface/base64 fallback은 제공하지 않는다.
- 개별 record가 protocol 최대를 넘거나 protocol 최대 한 건에 더해 둔 2 MiB backlog를 넘거나 ACK가 진행되지 않으면 연결을 닫고 기존 Remote reconnect/snapshot 경로로 복구한다. sustained output에서 추가 backlog 상한이 반복적으로 부족하다는 측정이 나오면 record batching 또는 credit 크기를 별도 결정으로 재검토한다.
