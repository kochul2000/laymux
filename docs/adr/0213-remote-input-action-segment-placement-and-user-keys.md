...
키 모델에도 중복과 과잉이 남아 있었다. 고정 action `ctrl-c`와 소프트키 `c-c`는 같은 `\x03`을 보내면서 라벨만 `Ctrl+C`와 `^C`로 달랐다. 반대로 `^A ^D ^E ^K ^R ^W ^Z`는 `Hidden` 목록에 상시 존재하면서 대부분의 사용자에게 한 번도 쓰이지 않는다. 그러면서 정작 자기 워크플로에 필요한 조합키(가령 `Ctrl+G`, `Alt+.`, `Ctrl+←`)는 등록할 방법이 없었다 — 기존 "Custom keys"는 미리 정의된 키를 고르는 토글이지 새 키를 정의하는 기능이 아니었다.
...
- `laymux.remote.keybar`의 `zones`는 `{ main: {left, center, right}, expanded: {left, center, right} }`이며 배치와 구역 내 순서의 단일 진실원이다. `hidden`은 저장하지 않는다 — 어느 구역에도 없는 action이 곧 숨겨진(`Hidden`) action이다.
- **배치가 곧 활성화다.** `sets`, `custom`, `usedCustom`, `order`를 모두 제거한다. 두 행 중 어딘가에 배치된 action만 렌더하고, 배치되지 않은 action은 `Hidden` 목록에 남는다. ADR-0040의 "사용자가 정한 순서를 보존한다"는 결정은 구역 내 순서가 계승하며, 별도 전역 `order` projection은 두지 않는다.
- 편집 UI는 하나의 칩 편집기로 통일한다. Main 행·Keys 행의 각 구역과 `Hidden` 목록이 모두 드롭 대상이고, 고정 action과 소프트키를 구분 없이 칩으로 다룬다. ADR-0040의 long-press Pointer Events 드래그를 그대로 쓰되 행·구역·`Hidden` 경계를 자유롭게 넘는다. **숨겨진 칩을 탭하면 곧바로 Keys 행 끝에 배치되고 선택 상태로 남아** 행·구역 select와 구역 내 이동 버튼이 바로 옆에 열린다 — "쓸지"와 "어디에 둘지"가 두 화면이 아니라 한 동작 간격이다. 배치된 칩을 탭하면 선택만 토글한다. 드래그는 enhancement이고 탭 경로가 정본이다.
- `Keys`는 여전히 구조 토글이므로 main 행의 세 구역 또는 `Hidden` 에만 놓을 수 있고, 자기가 여는 확장행에는 들어갈 수 없다. 숨기면 확장 열림 상태를 `false`로 저장한다.
...
- **`hidden` 배열도 저장**: 숨긴 것들의 순서까지 사용자가 정할 수 있지만, `Hidden` 목록은 카테고리 그룹으로 렌더하므로 저장된 순서가 화면에 드러나지 않는다. 저장하지 않으면 중복 검사가 6개 구역으로 끝난다.
...
- **`^A`~`^Z` 전체 유지 + 등록 기능만 추가**: 트리밍 없이도 요구는 충족되지만, `Hidden` 목록이 계속 실사용되지 않는 칩으로 채워져 등록 기능이 해결하려던 문제(원하는 키를 찾기 어렵다)를 그대로 남긴다.
...
- 설정에서 배울 조작이 하나로 준다. 드래그는 행·구역·`Hidden` 을 모두 넘고, 숨겨진 키는 탭 한 번으로 바로 쓸 수 있다. 반대로 기존 사용자는 익숙한 `Action placement` 목록과 세트 체크박스를 잃는다.
