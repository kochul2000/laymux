# 0113. 절전 방지는 설정이 모드를 소유하고 OS 억제는 Rust 단일 지점이 건다
...
- **"busy" 의 정의는 상태 표시(모래시계)와 같은 것 하나만 쓴다.** `activity.type === "running" || outputActive === true` 를 `isTerminalBusy()` 하나로 공유하고, `terminalActivity` 위젯과 절전 방지가 함께 참조한다. 두 곳이 서로 다른 busy 정의를 갖는 순간 "모래시계가 도는데 잠들었다"가 발생한다.
...
- **실패는 조용히 넘기지 않되 설정을 되돌리지도 않는다.** `systemd-inhibit` 부재 등으로 억제를 못 걸면 커맨드는 `Err` 를 반환하고 프론트는 로그만 남긴다. 사용자가 고른 모드는 그대로 유지된다(다른 머신에서 같은 설정을 쓸 수 있다).
...
- 절전 억제가 걸리고 풀리는 지점이 `SleepInhibitor::set` 하나로 좁혀진다. 억제가 안 풀리는 버그를 볼 곳이 한 군데다.
