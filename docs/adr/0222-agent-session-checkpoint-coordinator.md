...
- Superseded by: [ADR-0232](0232-unconsumed-resume-checkpoint.md) (Proposed), 입력 전 명시적 resume의 startup 대기 및 critical checkpoint 허용 판정에 한정한다.
- Date: 2026-08-31
...
- `Unknown`과 `NoAgent`를 같은 빈 map으로 축약하지 않는다. timeout·poison·probe unavailable은 부재의 증거가 아니므로 파괴적 삭제로 바꾸지 않는다. 다만 provider 저장소 건강 실패는 현재 live candidate/PID의 귀속에 영향을 주는 terminal ID 집합에 결부한다. native process snapshot과 WSL probe는 서로 독립인 관측 domain이므로 한쪽 실패가 다른 쪽의 정확한 귀속을 무효화하지 않는다. 한 Claude PID의 파일 손상으로 다른 Claude PID, 다른 provider, shell terminal을 전역 `Unknown`으로 만들지 않는다. 반대로 건강한 관측에서 provider/session 충돌이나 검증 실패가 확인된 `ActiveButUnidentified`는 ADR-0118·0120의 fail-closed 원칙대로 옛 ID를 사용하지 않는다.
