...
- **finalization fence**는 Rust의 공통 operation gate가 소유한다. fence 뒤에는 Local·Remote·Automation·MCP의 새 terminal input, terminal create/close와 Automation frontend action을 받지 않는다. 프론트 구조가 관측 중 바뀌면 revision 검사가 trailing checkpoint를 강제한다. 실패·timeout 또는 install 전 중단 시 fence를 해제한다.
...
- 회귀 테스트는 최소한 다음을 고정한다: `Identified`/`NoAgent`/`ActiveButUnidentified`/`Unknown` mutation·barrier 표, 권위 shell 삭제와 조회 실패 보존, 같은 PID의 새 top-level session 선택, critical 이중 안정 관측, 늦은 옛 snapshot의 역전 금지와 trailing rerun, checkpoint/cloud path merge, recovery write-block, finalization input gate, hidden eviction 실패 시 PTY 유지, trigger burst coalescing.
