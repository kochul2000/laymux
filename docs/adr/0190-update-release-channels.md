# 0190. 업데이트는 stable·beta 두 채널을 가지며 채널 매니페스트가 최신 릴리스의 단일 진실원이다
...
- 데스크톱 표면은 Settings 최상위 "일반" 그룹의 전용 **업데이트** 화면이다. 채널은 인터페이스 취향이 아니라 이 설치본이 무엇을 받을지를 정하는 앱 수명주기 설정이므로, 다른 설정 밑에 묻지 않는다. 이 화면은 현재 버전과 따라가는 채널, 채널 선택(베타 경고 포함), 수동 확인과 마지막 확인 시각, 발견된 버전의 릴리스 시각·노트·설치, 그리고 GitHub 릴리스 페이지 링크를 한자리에 모은다. 업데이트가 있을 때만 나타나는 상단 action과 Remote의 업데이트 카드는 현재 채널을 함께 보여준다 — 어느 계열을 따라가는 중인지 모르면 사용자는 자기가 받은 버전을 해석할 수 없다.
