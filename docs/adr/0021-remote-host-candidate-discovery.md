...
- `127.0.0.1` loopback 후보는 항상 포함하되 후보 목록 맨 뒤에 둔다.
...
- 감지 후보 순서는 `tailscale > lan > loopback` 이다. Remote Access 모달의 자동 폴백이 로컬 전용 loopback보다 원격에서 도달 가능한 host를 먼저 고르도록 하기 위해서다.
