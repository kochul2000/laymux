...
continuation control이 있는 envelope는 hold/close IPC를 시작한 직후 receipt도 병렬로 전송한다. Rust는 recent receipt의 immutable sequence boundary를 기준으로 늦게 도착한 hold/close를 검증한다. 단, hold 또는 close가 settle되기 전에 이미 WebView로 발행된 successor는 xterm ingress에 바로 넣지 않고 control owner가 순서대로 보류한다. 이 보류량은 backend의 4-slot 상한으로 제한되며, `heldEnvelopeId`의 envelope당 grant 하나 불변식과 hold→close sender 순서는 유지한다.
