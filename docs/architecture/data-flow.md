...
- active workspace를 숨길 때의 visible fallback은 일반 workspace 전환과 같은 `switchActiveWorkspace` 착지 경로를 사용한다. 따라서 이전 dock focus를 지우고 전역 `focusedPaneIndex`를 fallback workspace의 유효 pane으로 다시 계산한 뒤 숨김 raw state를 적용한다(issue #578 후속). 외부 상태 주입을 수선하는 coordinator도 같은 경로를 사용한다.
