...
desktop은 성공한 control/resource RPC 응답 안에 인증된 새 `expiresAt`을 넣고 Android는 이를 검증해 15분 비활성 deadline을 갱신하므로 foreground의 heartbeat가 session을 유지한다. background 전환은 output WebSocket과 새 bridge 요청을 닫고 session을 suspend해 deadline이 더 늘지 않게 하며, key와 pending exact RPC ciphertext는 현재 deadline까지 보존한다. 15분 안의 foreground 복귀는 pending response를 먼저 확정한 뒤 PC 문서를 다시 로드하며, 페이지가 자기 sessionStorage의 auto-connect 의도와 resume capability로 lease를 재검증·재확보하고 새 stream nonce와 output snapshot에 부착한다. 명시적 disconnect·pairing revision 변경·15분 비활성 만료는 session key와 output stream key를 폐기하고 Cloud dashboard로 돌아간다. 연결 실행은 네이티브 pairing sheet, 저장된 pairing·보호 정책 관리는 dashboard `…`에서 여는 별도 native dialog가 담당한다.
...
실행 중 미식별 상태의 예외는 WSL Codex의 **정확한 process 선택 + rollout FD 부재**를 provider adapter가 별도 집합으로 전달한 경우로 제한한다. 일반 `None`(후보 검증 실패·중복·모호한 PID 포함)은 예외의 증거가 아니며, native/다른 provider에는 이 증거를 합성하지 않는다. 최종 snapshot에서 pending provider/ID가 다른 pending 또는 정확한 귀속과 중복되면 pending만 거부하고 소비한다.
