...
- **229 textarea-diff는 finalize가 끝날 때까지 침묵한다**: Windows IME는 `compositionstart`보다 먼저 `keydown` 229를 보내 `_handleAnyTextareaChanges`의 `setTimeout(0)`을 예약한다. 메인 스레드가 그 창을 수 초로 늘리면 `compositionend`가 먼저 끝나 `_isComposing===false`가 되고, stock 타이머는 같은 음절을 finalizer와 따로 보낸다. 패치는 `_isSendingComposition`일 때도 이 경로를 막는다. 조합이 시작되지 않은 229 삽입은 그대로 보낸다([ADR-0164](../adr/0164-ime-textarea-diff-skip-while-sending.md)).
