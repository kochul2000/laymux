...
Pane divider의 ResizeObserver burst는 80ms trailing debounce로 한 번의 `fit()`으로 합친다. 다만 debounce가 끝났더라도 `terminal.write(data, callback)`로 전달한 PTY 청크가 xterm parser queue에 남아 있으면 reflow를 실행하지 않는다. 대기 중인 write 수가 0이 되고 마지막 PTY 청크 뒤 120ms 동안 추가 출력이 없을 때 보류된 최신 fit을 한 번 실행한다. xterm buffer reflow와 write parser가 같은 active buffer를 동시에 갱신하는 충돌을 피하고, 이전 폭을 기준으로 만들어진 ConPTY 출력이 모두 처리된 뒤 열 수를 바꾸기 위한 순서다.

Windows ConPTY는 폭 변경 뒤 현재 화면을 `ESC[?25l ESC[H ... ESC[?25h` 프레임으로 다시 출력한다. xterm이 먼저 새 폭으로 scrollback을 reflow하면 이 프레임이 viewport 경계로 이동한 과거 행을 덮어쓰므로, Windows normal buffer의 열 수가 증가하고 `baseY > 0`일 때만 500ms resize repaint filter를 활성화한다. exact frame 한 개만 xterm write에서 제외하고 앞뒤 출력은 유지한다. Rust OSC 파이프라인과 raw output ring은 그대로이며 Linux와 alternate buffer에는 적용하지 않는다([ADR-0026](../adr/0026-conpty-width-resize-repaint-filter.md)).
...
- PTY write callback이 남아 있거나 마지막 출력 뒤 120ms quiet 조건이 지나지 않은 동안 ResizeObserver reflow를 실행하지 않고, 조건 충족 뒤 보류된 fit을 정확히 한 번 실행한다.
- Windows normal buffer의 scrollback을 넓힐 때 ConPTY resize repaint 한 프레임만 제거하고, frame 밖 실제 출력은 보존한다.
