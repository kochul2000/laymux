...
Remote FileViewer는 데스크톱 overlay를 복제하거나 외부 프로세스를 실행하지 않는다([ADR-0040](../adr/0040-remote-served-file-viewer.md)). Remote drawer가 active lease로 `fileViewer.status`를 조회해 `useFileViewerStore`의 현재 `{open,path}`를 표시하고, 사용자가 현재 파일 또는 직접 입력 경로의 새 탭 열기를 누르면 child bootstrap에 메모리상의 token/lease/source를 same-origin `postMessage`로 전달한다. child의 `/remote/v1/file-viewer/render` 요청은 Rust가 lease를 확인한 뒤 frontend async bridge `fileViewer.render`로 이어진다. bridge는 `readFileForViewer(path, 8 MiB)`를 호출하고 HTML/Markdown이면 기존 `file-preview.ts` sanitizer로 완성된 preview document를 만든다. 새 탭은 text를 `textContent`, image를 제한된 data URL, preview document를 sandbox iframe `srcdoc`으로만 렌더한다. Remote 표면은 `extensionViewers` 매핑을 의도적으로 무시한다. 호스트 터미널에서 실행되는 외부 viewer는 독립 브라우저 탭으로 투영할 수 없고, remote 파일 읽기가 새 호스트 프로세스 실행 권한으로 확장되어서는 안 되기 때문이다.
