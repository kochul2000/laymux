...
- **textarea candidate와 확정 이벤트를 먼저 합친 뒤 관측을 하나씩 조정한다**: finalizer는 helper textarea가 focus를 계속 소유할 때 캡처 범위의 textarea slice와 `compositionend.data`를 기존 양방향 포함 및 suffix-prefix 최장 overlap 함수로 합쳐 accumulator를 만든다. 전체 교체로 slice가 비거나 확정값의 suffix만 남으면 이벤트의 전체 확정값이 복구되고, propagation 중 붙은 비조합 suffix도 보존된다. `compositionend.data`가 비었거나 없는 합성 호출은 기존 slice로 떨어진다. CoreBrowserTerminal의 blur handler는 textarea를 비우기 전에 현재 조합을 event-data 비허용 상태로 만들고 모든 pending generation을 취소한다. 취소 세대는 slice·event data·input/keypress 관측을 전부 폐기하며 refocus만으로 되살리지 않고, 다음 `compositionstart`만 새 세대를 허용한다. 따라서 `compositionend → input → blur → refocus → delayed flush`에서도 §8.16의 TerminalView blur fallback과 이중 전송하지 않는다. candidate에 이미 포함된 첫 input/keypress 관측이 정렬 기준을 세울 때까지 unmatched 관측은 도착 순서대로 shortest-merge prefix로 모으고, 기준이 생기면 prefix를 candidate와 병합한 뒤 나머지 관측을 하나씩 같은 함수에 fold한다. 기준 뒤에는 양방향 overlap 길이가 같은 관측을 accumulator 뒤쪽에 둔다. 끝까지 기준이 없으면 prefix 전체를 기존처럼 candidate와 병합한다. 관측끼리 무조건 먼저 합치지 않으므로 separator `" "`와 propagated syllable `"면"`의 출처 경계가 사라져 `" 면"`이 된 뒤 candidate `"면 "`에 덧붙는 일을 막으면서, candidate `"한a"` 뒤의 비중첩 단문자 관측 `"a"`, `"b"`는 `"한ab"`, anchor 없는 `"a"`, `"b"` 뒤 candidate `"한"`은 기존처럼 `"ab한"`으로 유지한다. 다만 겹치는 다문자 관측이나 의도적 반복 occurrence의 엄격한 시간 순서는 ADR-0093과 마찬가지로 보장하지 않는다. queue가 빌 때까지 `_isSendingComposition`은 유지된다.
