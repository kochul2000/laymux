...
E2E Remote 표면은 browser Remote의 **기능 코드**를 재사용하되 native transport를 사용한다. 사용자의 생체 승인으로 seed를 한 번 복호화하면 Kotlin이 challenge→establish를 수행하고 방향별 session key와 exact sequence를 단독 소유한다. APK에는 xterm·terminal 목록·navigation을 복제하지 않는다. Kotlin은 먼저 PC Laymux의 `/remote/` 문서와 정적 자산을 encrypted `kind=resource` RPC로 가져와 AEAD 검증한 뒤 synthetic HTTPS origin에 제공한다. 그 문서의 기존 `remoteFetch`와 output socket seam만 각각 async native HTTP bridge와 output bridge로 전환되므로 workspace·pane·terminal 선택, controller claim·heartbeat·release, 입력·resize와 V1 output parsing은 계속 PC가 배포한 동일 Remote page가 소유한다. native는 request id·method·allowlisted path·JSON body 및 output stream cursor만 다루고 Remote 도메인 상태를 해석하지 않는다.

모든 encrypted RPC는 Kotlin의 한 요청 in-flight FIFO에 직렬화된다. desktop은 기존 controller/lease 검사를 통과한 Remote router를 재사용한다. output open은 rendererless checkpoint snapshot과 generation/source sequence 및 V1 header를 원자적으로 잡고, 이후 120ms fixed-delay poll은 ring `resume_terminal_output` delta를 같은 암호화 FIFO로 가져와 페이지의 기존 header/binary parser에 전달한다. desktop은 성공한 RPC 응답 안에 인증된 새 `expiresAt`을 넣고 Android는 이를 검증해 15분 비활성 deadline을 갱신하므로 foreground에서 통신하는 동안 세션이 유지된다. background 전환은 output poll과 새 bridge 요청을 중지하고 session을 suspend해 deadline이 더 늘지 않게 하며, key와 pending exact ciphertext는 현재 deadline까지 보존한다. 15분 안의 foreground 복귀는 pending response를 먼저 확정한 뒤 PC 문서를 다시 로드하며, 페이지가 자기 sessionStorage의 auto-connect 의도와 resume capability로 lease를 재검증·재확보한다. 명시적 disconnect·pairing revision 변경·15분 비활성 만료는 session key를 폐기하고 Cloud dashboard로 돌아간다. pairing 관리가 필요할 때만 APK bootstrap을 표시한다. relay는 outer instance/session/sequence와 길이·타이밍만 보며 UI bytes·plaintext terminal metadata·bytes·lease는 보지 못한다.

Cloud tunnel의 접속 표면은 PC 설정 snapshot의 `cloudAccessMode`에서 파생한다([ADR-0150](../adr/0150-desktop-owned-cloud-remote-access-policy.md)). `browserAndE2e`는 기존 stream을 유지하고, `androidE2eOnly`는 고정 E2E `POST` 네 경로만 stream open과 HTTP dispatch 직전에 허용한다. 설정 저장에서 mode 변화가 관측되면 실행 중 worker를 abort해 이미 열린 browser HTTP/output stream을 함께 정리하고 같은 credential로 재연결한다. 새 worker의 첫 heartbeat가 mode를 relay registry에 게시하고, Cloud dashboard와 browser session/credential 검사는 그 live 값을 매 요청 재확인한다. Android app-mode PC 선택과 public E2E relay, Cloud account/presence, 별도 Local/Tailscale Direct 경로는 이 분기 밖이다.
