...
attach와 token별 단일 ACK IPC에는 5초 watchdog을 둔다([ADR-0086](../adr/0086-terminal-output-control-epoch-watchdog.md)). timeout은 `outputAttachEpoch`가 여전히 current일 때만 sender 폐기 → epoch 증가 → replacement 순서로 처리하고, 원 Promise의 fulfillment/rejection handler는 늦은 완료를 흡수해 새 token·confirmed prefix·화면을 변경하지 않는다. attach와 ACK timeout streak는 rate backoff 용도로 분리하며 각각 attach fulfillment와 backend가 `true`로 수락한 ACK만 초기화한다. timeout replacement는 50 ms부터 지수 backoff하되 1,000 ms를 넘지 않는다. 취소 불가능한 bridge Promise의 자원 상한은 streak가 아니라 종류별 mount-local `timed-out but unsettled` 계수가 소유한다. timeout 때 증가하고 underlying late resolve/reject 때 lease·prefix·화면을 건드리지 않은 채 감소하며, 중간의 성공은 과거 orphan 계수를 지우지 않는다. 새 operation은 실제 outstanding이 6 미만일 때만 만든다. 6이면 current epoch/readiness를 폐기한 채 producer를 ADR-0084 credit에서 fail-stop하고 capacity waiter 하나를 둔다. orphan 정착으로 슬롯이 생기면 waiter를 한 번 소비해 current epoch가 여전히 같을 때만 bounded backoff replacement 하나를 예약한다. pane remount는 새 epoch/streak/budget을 시작한다. 정상 attach reject, ACK Promise reject, backend `false` 의미는 기존대로 각각 재부착·동일 token/seq 재시도·stale lease 재부착이다. epoch 폐기/replacement 결정은 `console.warn`과 recovery counter보다 먼저 확정해 진단 throw가 liveness를 방해하지 않는다.
