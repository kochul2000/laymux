...
- DEC 2026이 활성인 동안 overlay caret/composition DOM은 숨기거나 새 buffer 좌표로 다시 그리지 않고 **마지막으로 커밋된 opacity·geometry를 그대로 동결**한다([ADR-0008](../adr/0008-shell-cursor-shadow-cursor.md)). 포커스 상실·미오픈·scrollback 이동처럼 프레임 외부에서 즉시 가시성을 잃어야 하는 상태는 동결보다 우선한다. native Windows stabilizer는 정상 transaction을 한 xterm write로 전달해 중간 paint가 보통 없지만, WSL byte pass-through는 `?2026h`와 strict park/reset 사이에 PTY write·animation frame 경계를 허용한다. 이때 sync 상태를 단순 hidden으로 계산하면 Codex `Working` 갱신마다 overlay가 사라졌다 reset 뒤 다시 나타나므로 두 host 모두 같은 "last committed frame" 규칙을 적용한다.
