...
continuation control이 있는 envelope는 hold/close가 수락된 뒤 receipt를 전송한다. 따라서 `heldEnvelopeId`의 envelope당 grant 하나 불변식과 hold→close sender 순서는 receipt pipeline과 별개로 유지된다. 다만 최대 4개 envelope가 먼저 동결될 수 있으므로 transport `grantId`는 그 envelope를 만든 시점의 grant이고, 그 안에서 나중 continuation이 열고 닫힐 때의 control grant와 다를 수 있다. frontend는 bounded closed-grant history로 이미 발행된 이전 grant successor를 source 순서대로 보류하고, 보류분을 재개할 때마다 새 opening/closing gate를 다시 적용한다. Rust는 hold/close의 envelope boundary를 `(generation, lease token, envelope id)`로 찾고 active control grant 소유권은 별도로 검증한다.
