...
- **textarea candidate를 기준으로 관측을 하나씩 조정한다**: finalizer는 캡처 범위의 textarea candidate를 accumulator로 먼저 읽는다. candidate에 이미 포함된 첫 관측이 정렬 기준을 세울 때까지 unmatched 관측은 기존 ordered prefix로 보류하고, 기준이 생기면 prefix를 candidate 앞쪽에 병합한 뒤 나머지 관측을 하나씩 양방향 포함 및 suffix-prefix 최장 overlap 함수에 fold한다. 기준 뒤에는 양방향 overlap 길이가 같은 관측을 accumulator 뒤쪽에 두어 event 순서를 유지하며, 끝까지 기준이 없으면 ordered prefix 전체를 기존처럼 candidate 앞에 병합한다. 관측끼리 무조건 먼저 합치지 않으므로 separator `" "`와 propagated syllable `"면"`의 출처 경계가 사라져 `" 면"`이 된 뒤 candidate `"면 "`에 덧붙는 일을 막으면서, candidate `"한a"` 뒤의 관측 `"a"`, `"b"`는 `"한ab"`, anchor 없는 `"a"`, `"b"` 뒤 candidate `"한"`은 기존처럼 `"ab한"`으로 유지한다. queue가 빌 때까지 `_isSendingComposition`은 유지된다.
