...
desktop v3 delivery worker는 immutable envelope를 generation당 최대 4개까지 receipt 대기 슬롯에 둔다([ADR-0098](../adr/0098-bounded-desktop-output-receipt-pipeline.md)). 각 slot은 identity·repair 횟수·deadline을 독립적으로 소유하므로 receipt는 순서 밖으로 도착해도 해당 slot만 제거하고, 같은 identity/payload retry는 bounded recent receipt history에서 idempotent하게 끝난다. worker는 이 상한이 차기 전까지 다음 bounded envelope를 emit해 WebView IPC 왕복과 고출력 전달을 겹친다. parsed ACK frontier·continuation grant·retention cap은 이 delivery receipt pipeline과 별도다. receipt deadline은 모든 slot 중 가장 이른 값에서 generation-local fail-stop을 일으키며, 기존 Automation `receiptSlot` 진단은 호환성을 위해 가장 오래된 미수령 slot만 보여 준다.

continuation control이 있는 envelope는 hold/close가 수락된 뒤 receipt를 전송한다. 따라서 `heldEnvelopeId`의 envelope당 grant 하나 불변식과 hold→close sender 순서는 receipt pipeline과 별개로 유지된다. 다만 최대 4개 envelope가 먼저 동결될 수 있으므로 transport `grantId`는 그 envelope를 만든 시점의 grant이고, 그 안에서 나중 continuation이 열고 닫힐 때의 control grant와 다를 수 있다. frontend는 앞 envelope가 control settle을 기다리는 동안 뒤따라 관측한 contiguous successor를 backend pipeline 상한과 같은 4개까지 bounded chain으로 보존하고, 단일 drain owner만 이 chain을 순차 소비한다. ingress가 기다리는 정확한 envelope를 continuation gate가 이미 소유하면 뒤 successor를 gap으로 판정하지 않고 그 owner의 settle을 기다린다. pre-hold에 동결된 null-grant envelope 뒤에 hold 반영분이 도착할 수 있으므로 null 하나만으로 closed-grant history를 지우지 않고 bounded eviction으로만 퇴출한다. 이 chain과 bounded closed-grant history로 이미 발행된 이전 grant successor를 source 순서대로 보류하고, 보류분을 재개할 때마다 새 opening/closing gate를 다시 적용한다. Rust는 hold/close의 envelope boundary를 `(generation, lease token, envelope id)`로 찾고 active control grant 소유권은 별도로 검증한다.
...
attach snapshot과 각 v3 envelope는 generation/token/source sequence를 clamp하거나 보간하지 않는다. envelope는 physical bytes 최대 64 KiB, logical delta 최대 8,192개, 직렬화 payload 1 MiB 미만을 모두 만족해야 하고 하나라도 넘거나 metadata·geometry·sequence가 모순이면 그 generation을 typed fail-stop한다. frontend ingress는 envelope backing과 logical descriptor를 bounded ownership 안으로 넘긴 뒤 receipt를 보내지만, opener/closing transition이 있으면 해당 hold/close가 먼저 수락되어야 receipt로 다음 envelope slot을 열 수 있다. receipt는 parser 완료를 뜻하지 않는다. 원래 delta 경계는 native Windows stabilizer, WSL recognizer, §8.5 cursor/OSC/alternate-buffer/activity 경로에 그대로 전달하고 rendererless checkpoint만 같은 generation·geometry revision의 contiguous segment를 최대 256 KiB까지 합친다([ADR-0080](../adr/0080-output-backlog-coalescing-and-out-of-band-frontend-vitals.md)).

Rust delivery worker는 receipt 전 envelope 전체(`data`, `deltaEnds`, `geometryRuns`, envelope/grant identity)를 최대 4개 frozen in-flight slot으로 보존한다. bounded event emit 재시도가 실패하거나 emit 성공 뒤 WebView callback이 유실돼도 해당 값을 폐기하지 않는다. frontend의 1초 low-frequency watchdog과 관측된 v3 gap/overlap은 `(generation,token,expectedEnvelopeId,grantId,expectedSeqStart)`로 `repair_terminal_output_envelope`를 호출한다. 정상 idle은 아무 작업도 하지 않는다. 특히 receipt 완료 뒤 parsed ACK가 느려 producer pending queue가 쌓인 동안 worker가 아직 다음 envelope를 동결하지 않은 상태도 정상 idle이며, 이 구간의 next frontier는 pending queue tail인 `observedSeq`가 아니라 첫 pending delta의 `seqStart`다. 반대로 emit edge가 실제로 유실됐다면 해당 frozen in-flight slot이 남으므로 같은 요청은 `exact`를 반환한다. `exact` 응답만 동일 preflight·continuation·queue·visible/checkpoint parser 경로에 정확히 한 번 적용한다. bounded ledger가 event/repair 중복을 byte·delta geometry까지 비교해 dedupe하며 payload가 다르면 identity conflict다. stale session, 현재 in-flight와의 mismatch, malformed 응답 또는 bounded repair 회수 소진은 reset/replay/replacement attach 없이 typed fail-stop한다. repair가 receipt deadline을 무한 연장할 수 없도록 generation당 회수 상한을 두며, receipt 또는 유한 expiry가 delivery/credit waiter를 깨운다. production v3 repair는 기존 raw v2 `resume_terminal_output`을 섞지 않는다. `terminal-output-v2`, `acknowledge_terminal_output`, `resume_terminal_output`과 그 gap-repair/auto-reattach 로직은 구 backend와 fixture를 위한 staged fallback일 뿐 production v3의 liveness 또는 정확성 경로가 아니다.

visible xterm 앞의 `TerminalWriteBatchQueue`는 계속 pane-local single-flight physical byte FIFO, batch materialization, callback과 discard의 유일한 소유자다([ADR-0092](../adr/0092-app-wide-terminal-write-round-robin.md)). v3 ingress는 backing을 semantic pipeline에 넘기고 queue에는 그 backing의 zero-copy `Uint8Array.subarray()` descriptor를 전달한다. queue가 materialize한 physical write가 accepted된 뒤 visible callback과 rendererless checkpoint write가 모두 같은 contiguous source prefix를 완료했을 때만 parsed ACK가 전진한다. native stabilizer가 bytes를 보류하면 parsed ACK도 보류되고, receipt나 queue enqueue만으로 ACK하지 않는다. replay/string, stabilized frame, cursor park deadline, authoritative frame end, composition-active write는 barrier이며, hard 상한 128 part·256 KiB와 경쟁 시 64 KiB fairness quantum은 유지된다.

여러 pane의 local FIFO는 앱 전역 `TerminalWriteFairScheduler`(`terminal-write-fair-scheduler.ts`)에서 physical admission turn을 얻는다([ADR-0092](../adr/0092-app-wide-terminal-write-round-robin.md)). owner key는 `instanceId`가 아니라 xterm 생성 effect마다 새로 만드는 opaque `Symbol`이다. 따라서 profile 변경으로 같은 id의 replacement effect가 생겨도 old effect의 늦은 callback·pending 취소·unmount release는 새 세대 turn을 건드리지 않는다. active lease는 앱 전체에 하나이며 한 turn은 physical batch 하나만 제출하고, parse callback·동기 거부·lifecycle 취소 중 하나로 끝난 pane은 backlog가 남아도 대기열 꼬리로 돌아간다. 같은 pane의 future turn은 하나로 dedupe하므로 먼저 기다리던 pane이 각각 한 번씩 실행되는 starvation-free round-robin이다. scheduler는 turn을 꺼낸 뒤 다른 owner가 pending인지 함께 알려 준다. 다른 owner가 기다리면 local FIFO의 fresh live dequeue는 64 KiB fairness quantum을 사용하고, 없으면 compatible live entry를 256 KiB까지 coalesce해 단독 live pane 처리량을 유지한다. replay entry는 이 contention 판정과 무관하게 항상 64 KiB 이하의 한 callback barrier다. 전역 idle 최초 request는 기존 대화형 latency를 보존하려 즉시 제출하지만 active write 뒤의 다음 pane은 새 macrotask에서 시작해 Automation·input·paint가 끼어들 기회를 준다. 동기 backpressure면 이미 만든 같은 batch/buffer를 local FIFO head에 복원하고 전역 lease를 반환한 뒤 16 ms 후 새 turn으로 재시도한다. scheduler는 byte·metadata를 읽지 않고 contention 사실만 전달하므로 v3 ingress와 attach가 정한 byte 전순서 및 local batch barrier는 그대로다. 최초 attach, 사용자 recreate와 구 v2 fallback reattach는 아직 제출하지 않은 old-epoch turn/request를 폐기하되 accepted write의 전역 lease와 parse callback은 유지하고, unmount만 active lease까지 취소한다. production v3 fail-stop은 replacement turn을 만들지 않는다. parsed/replay waiter는 `onDiscard`로 종결하고 이미 drain된 stabilizer callback 묶음도 parsed와 discard 중 하나로만 정확히 한 번 settle한다.
...
receipt·hold·close는 모두 `(terminal id,generation,lease token,envelope id,grant id)` identity와 immutable payload를 사용한다. receipt의 grant는 frozen transport envelope identity다. hold/close의 grant는 continuation control identity이며, pipeline에서 이전 grant로 동결된 envelope 안에 새 frame transition이 들어갈 수 있으므로 transport grant와 같다고 가정하지 않는다. hold는 DECSET opener를 실은 envelope와 `frameStartSeq`에 결합하고, close는 active grant와 closing envelope의 전체 `[seqStart, seqEnd]` 안 `closeSeq`·reason에 결합한다. 같은 identity/payload retry는 idempotent하며 stale completion은 현재 receipt slot, grant, parsed frontier를 바꾸지 않는다. 같은 identity의 다른 payload, 잘못된 opener/closing envelope 또는 sequence 범위는 typed identity conflict다. frontend는 호출마다 5초 watchdog을 두고 같은 identity/payload로만 retry한다. WebView/window-scoped registry가 receipt/hold/close의 공유 delivery FIFO와 terminal-local 6개·WebView-global 6개 orphan hard cap을 소유하며, attach와 parsed ACK의 기존 독립 budget은 유지한다([ADR-0086](../adr/0086-terminal-output-control-epoch-watchdog.md)). parsed ACK sender는 정상 composite capacity가 찼을 때 generation/token을 바꾸지 않는 logical waiter 하나로 FIFO에 남고, 대기 중 contiguous parsed prefix를 최신 prefix까지 coalesce한다. slot 전에는 command watchdog을 시작하지 않으며 screen reset/replay나 replacement attach를 만들지 않는다. registry는 unmount·stale owner의 waiter와 미사용 reservation을 반환하고 다음 eligible waiter를 깨운다. 실제 command timeout과 timed-out orphan hard cap은 기존 fail-stop 경계를 유지하며, cap 도달도 waiter를 깨워 정상 포화와 구분한다([ADR-0094](../adr/0094-terminal-output-control-capacity-admission.md)). cap의 orphan 수에는 watchdog이 실제 timeout으로 표시한 미정산 Promise만 포함한다. 정상 in-flight가 capacity를 채운 동안 새 호출은 fail-stop하지 않고 공유 FIFO에서 lease 반환을 기다린다. backend도 receipt deadline, active grant expiry, hard ceiling에서의 parsed-progress expiry를 각각 5초로 소유하고 expiry·retire·유효 ACK/close 때 delivery·credit waiter를 반드시 깨운다. 어떤 hard ceiling도 영구 대기가 될 수 없다.

contract violation, receipt/grant/progress expiry, control orphan cap 또는 surface unavailable은 `healthy | backpressured | failStopped` backend 상태와 reason, generation/token, ring/delivery/parsed bounds로 남고 pane readiness를 false로 만든다. mount-local lifecycle owner가 `unavailable`을 확정하면 current generation/token과 허용 reason만 `fail_stop_terminal_output_surface`로 backend SoT에 게시한다. stale identity는 상태를 바꾸지 않고 `false`, 잘못된 reason은 오류다. backend가 receipt/grant/progress/identity 실패를 먼저 확정한 경우에도 같은 delivery close owner가 `terminal-output-fail-stopped`를 camelCase `{terminalId,generation,leaseToken,reason}`로 정확히 한 번 emit한다. UI는 명시적인 “output stopped” 상태를 표시하며 자동 screen reset, snapshot replay, replacement attach로 숨기지 않는다. 복구 계약은 사용자가 해당 terminal을 close한 뒤 새 generation으로 recreate하는 것뿐이다. 이 fail-stop은 transport/credit 경계이고 authoritative protocol/ring 기록 자체가 실패한 경우에는 ADR-0088의 generation teardown을 따른다.
