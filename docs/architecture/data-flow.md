...
[ADR-0242](../adr/0242-readonly-spreadsheet-viewer.md): `.xls`, `.xlsx`, `.xlsb`, `.ods`는 외부 뷰어 매핑 다음에 전용 `SpreadsheetPreview`로 연결한다. Rust calamine이 시트 목록과 선택 시트의 sparse 셀 값을 반환하고 React가 현재 시트 검색·TSV 복사를 담당한다. [ADR-0244](../adr/0244-spreadsheet-selection-and-windowed-scroll.md)에 따라 고정 28px 행 높이·144px 열 너비와 스크롤 위치로 보이는 행·열+여유 영역만 렌더링하며 페이지 UI는 없다. 작은 시트는 창 너비로 늘리지 않는다. 행·열 머리글 클릭/드래그, 셀 직사각형 드래그와 Shift 확장으로 선택한다. 선택은 필터 결과 좌표로 유지하고 검색·시트 변경 시 초기화한다. 표 포커스의 브라우저 `copy` 이벤트는 화면 밖을 포함한 선택 범위를 TSV로 제공한다. 가장자리 드래그는 자동 스크롤한다. 긴 셀은 한 줄 생략하고 툴팁에 전체 값을 표시한다. Rust는 표시 상한을 처음 초과하면 셀 순회를 중단하며, 잘린 응답의 크기는 중단 시점까지 관측한 좌표 기준이다. 파일 변경/시트 전환의 늦은 응답은 폐기한다. 원본 16 MiB, ZIP 해제 64 MiB, 표시 10,000행·256열·100,000셀·값 4 MiB 상한을 두며 잘림은 화면에 알린다. 기존 Remote 파일 읽기와 FileViewerContent는 바꾸지 않는다.
