...
- **파일/디렉토리 경로 → viewer·cwd 전파**(#363, **선택 기반**) — URL이 아닌 "스킴 없는 파일/디렉토리 경로"(예: `ui/src/index.css`, `Cargo.toml`, `/etc/hosts`, `src/components`)는 `createPathLinkProvider`(`ui/src/lib/path-link-provider.ts`)가 별도 `ILinkProvider`로 처리한다. **사용자가 드래그로 선택한 한 덩어리만** 대상으로 한다(기존의 hover 줄 전체 토큰 stat 방식은 제거 — 느리고 Windows에서 동작 안 함).
  - 검증 흐름은 `TerminalView`의 `onSelectionChange`/드래그 종료 `pointerup` 시점에 1회 수행한다: ① 설정 `terminal.pathLinkEnabled` off면 종료 ② 선택이 비었거나 `terminal.pathLinkMaxLength` 초과면 기존 밑줄 비움 ③ `trimSelectionToPath`(`trimPathToken`+`looksLikePath` 재사용)로 경로처럼 보이는지 판별 ④ `joinCwdPath`로 cwd와 조합(없으면 종료) ⑤ **`stat_path`를 선택당 1회만** 호출 ⑥ `decidePathLinkAction`으로 분기 — 존재 안 함=밑줄 없음, 파일=`openFile`, 디렉토리=`changeDir`. 검증되면 `setVerifiedSelection({bufferLine,startCol,endCol,absPath,isDirectory})`로 범위를 저장하고 `terminal.refresh`로 밑줄을 띄운다.
  - `provideLinks`는 **저장된 검증 범위와 겹치는 줄에 대해서만** ILink를 돌려준다(provideLinks 내부에서는 stat하지 않는다). 선택 범위는 `terminal.getSelectionPosition()`(buffer 좌표, 0-based)을 provider의 1-based 버퍼라인/컬럼으로 매핑한다. 경로는 보통 한 줄이므로 단일 라인을 확실히 지원하고, 여러 줄 선택은 미지원(첫 줄만 사용)이되 깨지지 않게 처리한다. 선택이 바뀌거나 해제되면 `clear()` 후 refresh로 밑줄을 거둔다.
  - 클릭 분기: 파일이면 `useFileViewerStore.openFileViewer`로 통합 뷰어를 열고, 디렉토리면 FileExplorer와 동일한 force 1회 `sync-cwd` 메시지(`handleLxMessage`, 현재 pane의 `instanceId`/`syncGroup`)로 cwd를 전파한다.
  - 경로 판별 휴리스틱과 cwd 조합은 순수 모듈 `ui/src/lib/path-link-detect.ts`(`looksLikePath`/`trimPathToken`/`trimSelectionToPath`/`isWithinPathLengthLimit`/`joinCwdPath`/`normalizeMsysCwd`/`decidePathLinkAction`). false positive를 줄이려고 URL 스킴 제외, 절대경로 허용, 상대경로는 디렉토리 구분자 1개↑ 또는 확장자 보유 단일 파일명만 인정하고, 후행 `:line:col`(grep 스타일)·문장부호·따옴표/괄호를 토큰에서 떼어낸다.
...
해제:
  - 기준은 입력 종류(마우스/화살표)가 아니라 프로그램의 진입/포커스 동작 자체 (ADR 0010)
  - focus 기반 자동 해제의 SoT = AppLayout 의 두 effect (activeWorkspaceId/focusedPaneIndex 감지).
    마우스·화살표 모두 이 경로를 거치므로 입력 핸들러에 해제 로직을 두지 않는다.
  ...
  - 워크스페이스 셀렉터 클릭(WorkspaceSelectorView)·알림 네비게이션(useKeyboardShortcuts)은
    별도의 명시 해제 경로다.
  - 예외: manual 모드, requiresAction 알림은 명시 해제 전까지 유지
```
...
`force=true`는 소스 측 게이트(에코 루프·소스 activity·`cwd_send`)와 대상 `cwd_receive` 필터를 우회하지만, 대상이 명령 실행 중/TUI 앱이면 cd 주입을 막는 `filter_targets_not_busy` 게이트는 유지한다. 대상이 file explorer일 때의 추종은 백엔드 cd 주입이 아니라, `do_sync_cwd`가 `sync-cwd` 이벤트에 실어 보내는 `force` 플래그를 `FileExplorerView`가 (cwdReceive와 무관하게 항상 등록된) 리스너에서 받아 처리한다. 가드·이벤트 상세는 [api-contracts.md §10](api-contracts.md)의 "1회성 CWD 전파" 참조.
