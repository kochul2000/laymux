...
- **파일 경로 → viewer**(#363) — URL이 아닌 "스킴 없는 파일 경로"(예: `ui/src/index.css`, `Cargo.toml`, `/etc/hosts`)는 `createPathLinkProvider`(`ui/src/lib/path-link-provider.ts`)가 별도 `ILinkProvider`로 처리한다. hover 시 줄의 경로 후보를 추출 → 이 pane의 syncGroup CWD(`useTerminalStore` instance의 `cwd`)와 조합해 절대 경로를 만들고 → 백엔드 `stat_path`로 **실제 존재(파일)** 를 검증한 경로만 링크로 등록한다(유효하면 밑줄+클릭 가능, 무효하면 밑줄 없음). 클릭 시 `openExternal`이 아니라 `useFileViewerStore.openFileViewer`로 통합 파일 뷰어를 연다.
  - 경로 판별 휴리스틱과 cwd 조합은 순수 모듈 `ui/src/lib/path-link-detect.ts`(`looksLikePath`/`findPathCandidateAtCol`/`findPathCandidatesInLine`/`joinCwdPath`). false positive를 줄이려고 URL 스킴 제외, 절대경로 허용, 상대경로는 디렉토리 구분자 1개↑ 또는 확장자 보유 단일 파일명만 인정하고, 후행 `:line:col`(grep 스타일)·문장부호·따옴표/괄호를 토큰에서 떼어낸다.
  - `provideLinks`는 동기 콜백이라 비동기 `stat_path` 결과를 그 자리에서 기다릴 수 없다. `PathValidationCache`가 검증 결과(`valid`/`invalid`/`pending`)를 캐시하고, 검증이 끝나면 `terminal.refresh`로 해당 영역을 다시 그려 밑줄이 반영된다.
