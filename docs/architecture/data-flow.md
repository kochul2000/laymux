...
- **평문 `#123` 이슈/PR 참조**(#439) — codex는 `#123`을 OSC 8로 감싸 xterm이 네이티브로 클릭 가능하게 만들지만, Claude Code는 **평문**으로 출력해 링크가 없다. `createPrLinkProvider`(`ui/src/lib/pr-link-provider.ts`)가 `(?<!\w)#(\d+)\b` 패턴을 탐지해(색상 `#fff`·`abc#12`·`v1.2#3` 오탐 회피), 클릭 시 해당 pane의 GitHub repo를 `{repoBase}/issues/{n}`로 연다(GitHub이 issues↔pull을 번호로 리다이렉트하므로 이슈·PR 모두 열림). `repoBase`(`https://github.com/{owner}/{repo}`)는 백엔드 `resolve_git_remote`가 pane cwd의 git `origin` remote를 정규화해 구한 값으로, cwd 변경 시 effect가 `repoBaseRef`에 채운다(provider는 async 호출 없이 ref만 **동기**로 읽음). GitHub repo가 아니면 `repoBase`가 null → 링크 미생성. 항상 켜져 있고 별도 설정 토글은 없다(codex와 동일 UX). 순수 매칭·컬럼 계산은 `findPrTokens`로 분리해 단위 테스트로 덮는다.
...
Terminal surface는 [ADR-0029](../adr/0029-detached-terminal-input-composer.md)에 따라 `direct`와 `composer` 두 입력 모드를 가진다. Direct는 xterm helper textarea와 raw `terminal.onData` 경로를 유지한다. Composer는 output viewport 아래의 네이티브 textarea가 아직 전송하지 않은 초안의 진실원이며, Insert는 text만, Send는 text 뒤에 CR 제출 의도를 보낸다. Composer가 차지하는 높이는 xterm host와 겹치지 않고 flex sibling으로 배치되며 모드 전환 뒤 공통 guarded fit을 실행한다. Composer 중에는 xterm native cursor, shadow overlay caret, composition preview를 숨기고 xterm keyboard event를 차단하지만, xterm이 생성한 protocol reply용 `onData`는 유지한다.

PC의 명시적 기본 모드 선호만 `laymux.desktop.inputMode`에 저장하고 최초 기본값은 `direct`다. 현재 모드와 `ComposerDraftState { text, revision, inFlight }`는 terminal id별 runtime Map에만 둔다. Insert/Send 시작 시 `{ terminalId, revision, text, submit, token }`을 캡처하고 같은 terminal의 중복 action을 막는다. 전송 중 textarea 편집은 허용하며 성공 응답도 현재 text/revision이 캡처와 같을 때만 비운다. 실패·결과 불명확·stale token은 초안을 보존한다. Remote page는 별도 키 `laymux.remote.inputMode`를 사용하고 저장값이 없으면 coarse pointer는 composer, fine pointer는 direct를 기본으로 하며, 초안/현재 모드는 마찬가지로 runtime terminal Map에만 둔다.
...
- `WorkspaceSelectorView`의 pane 요약 행도 이 `paneNumber` 오름차순으로 렌더한다. 단, 표시 순서만 정렬하며 포커스와 `PaneMinimap.highlightIndex`는 레이아웃 조작용 원본 `paneIndex`를 계속 사용한다.
