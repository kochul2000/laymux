...
Remote 표시 설정은 이 localStorage 경계의 예외가 아니라 별도 PC 소유 설정이다([ADR-0173](../adr/0173-remote-display-settings-pc-owned-and-lease-gated.md), [ADR-0200](../adr/0200-remote-composer-opacity-state-settings.md)). `settings.remote`의 terminal/composer/menu 글자 크기와 Composer Idle·Focused·Active 불투명도, 터치 민감도를 PC Settings는 기존 settings store·저장 경로로 편집하고, Remote drawer는 이 비민감 필드들과 opaque settings revision만 노출하는 좁은 GET/PUT projection으로 읽고 쓴다. Remote PUT은 active controller lease와 GET에서 받은 `expectedRevision`을 요구한다. 공통 settings bridge의 CAS가 stale 저장을 `409`로 거절하면 Remote는 최신 projection을 다시 읽는다. lease 검증부터 frontend apply 완료까지 non-PTY owner handoff barrier를 유지해 release·reclaim·expiry가 저장과 원자적 순서를 갖는다. 저장 성공 시 글자 크기·Composer opacity CSS 변수를 갱신하고 xterm options 재적용 → guarded fit/resize로 현재 surface에 반영한다. Composer opacity 상태는 disabled → active raw 상태(초안·추천·IME·전송) → DOM focus → idle 우선순위를 한 계산 함수에서 `data-opacity-state`로 투영한다. Android E2E도 별도 상태를 두지 않고 desktop 제공 Remote page가 native encrypted HTTP bridge를 통해 같은 endpoint를 호출한다.
