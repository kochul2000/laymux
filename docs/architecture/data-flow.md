...
attach와 token별 단일 ACK IPC에는 5초 watchdog을 둔다([ADR-0086](../adr/0086-terminal-output-control-epoch-watchdog.md)). timeout은 `outputAttachEpoch`가 여전히 current일 때만 sender 폐기 → epoch 증가 → replacement 순서로 처리하고, 원 Promise의 fulfillment/rejection handler는 늦은 완료를 흡수해 새 token·confirmed prefix·화면을 변경하지 않는다. attach와 ACK timeout streak는 분리하며 각각 attach fulfillment와 backend가 `true`로 수락한 ACK만 초기화한다. timeout replacement는 50 ms부터 지수 backoff하되 1,000 ms를 넘지 않는다. bridge Promise는 취소할 수 없으므로 종류별 초기 요청 하나와 replacement 다섯 개, 합계 6개의 unsettled operation까지만 만들고, 여섯 번째 timeout 뒤에는 current epoch/readiness를 폐기한 채 producer를 ADR-0084 credit에서 fail-stop한다. pane remount는 새 mount-local epoch와 streak를 시작한다. 정상 attach reject, ACK Promise reject, backend `false` 의미는 기존대로 각각 재부착·동일 token/seq 재시도·stale lease 재부착이다. epoch 폐기/replacement 결정은 `console.warn`과 recovery counter보다 먼저 확정해 진단 throw가 liveness를 방해하지 않는다. `attachTimeout`과 `ackTimeout` counter는 backend session 수명 동안 각 watchdog 발화를 센다.
