...
해제:
  ...
  - 워크스페이스/pane 진입 시 읽음 처리:
      · 워크스페이스 셀렉터 클릭 (WorkspaceSelectorView)
      · pane 마우스 클릭 진입 (WorkspaceArea onPaneFocus)
      · 화살표/단축키 진입 (useKeyboardShortcuts) + AppLayout 자동 해제 effect
  - 예외: notificationDismiss="manual" 모드, requiresAction 알림은 명시 해제 전까지 유지
```
