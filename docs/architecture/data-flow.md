...
WSL의 Claude Chrome 호스트(`argv[1] == --chrome-native-host`)는 공통 게스트 역할 검사로 대화 후보에서 제외한다. 세션 probe V3의 P 행은 helper 여부만 추가로 반환하며 argv 원문은 보내지 않는다. helper의 PID·PPID는 깊이 계산에 계속 포함하고, 실제 대화 두 개의 동률은 여전히 귀속 불가다. 명시적인 역할 증거를 못 읽으면 후보를 유지하며, 세션 파일이 있는 PID를 우선하는 우회는 하지 않는다([ADR-0252](../adr/0252-wsl-claude-chrome-helper-role.md)).
