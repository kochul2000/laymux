...
- **carry-over 앵커는 체인 시작에서 유도한다**: §8.x 의 #546 수정(PR #548)이 넣은 `echoed` 판정에 결함이 있었다. 그 판정은 "shadow 가 움직였다" 를 "shadow 가 확정분을 다 따라잡았다" 로 취급한다. Codex 실측: `ㄱㄱㄱ` 에서 두 번째 carry-over 시점에 PTY 는 **첫 자모만** 에코했으므로 shadow 는 열 2 에 있는데 확정분은 두 자모(4 셀)였다. 그걸 채택하면서 산술로 맞춰둔 열 4 가 **뒤로 끌려가** 세 번째 자모가 두 번째 위에 겹쳐 그려졌고, 프리뷰가 `ㄱㄱ` 에 멈춘 것처럼 보였다. 이제 앵커는 `chainAnchor + width(체인 시작 이후 확정분)` 로 유도하고, 라이브 값은 **그보다 뒤처지지 않을 때만** 채택한다(단조 가드). 라이브를 완전히 버리지 않는 이유는 wrap 보정 — xterm 은 넓은 글리프를 쪼개지 않고 다음 행으로 밀어내므로 에코된 커서가 산술로 닿을 수 없는 행에 있을 수 있다.
  - 그 대가로 "뒤로 움직인 shadow 를 채택한다" 던 기존 동작은 버렸다. `cursorAbsY` 는 절대 행이라 뷰포트 스크롤로는 변하지 않고, 체인 도중 입력 위치를 진짜로 뒤로 옮기는 것(clear)은 조합 자체를 무효화한다. "움직였다" 를 "권위 있다" 로 읽는 것이 바로 위 회귀의 원인이었다.
