...
[ADR-0226](../adr/0226-composer-stars-are-host-global-persistent-state.md)의 별표는 이 비영속 history와 분리된 명시적 opt-in이다. `settings.json`의 backend-owned `terminal.composerStarredEntries` ordered set은 workspace·pane·history scope와 무관하게 호스트 전역이며 Desktop과 Remote가 공유한다. 전용 IPC/lease-gated Remote API만 원자적으로 토글하고, 일반 settings patch는 이 필드를 read-only로 본다. 자동완성은 prefix가 맞는 별표 항목을 최신 별표부터 먼저 내고 남은 자리를 현재 runtime history로 채우며 중복과 exact query를 제외한다. 별표 버튼은 행 선택과 textarea focus를 건드리지 않는다. 최대 200개·항목당 UTF-8 16 KiB이고 한도 초과는 기존 별표를 지우지 않고 거절한다. Desktop Settings의 Terminal 섹션은 이 필드를 읽어 전체 목록을 표시하고, 전용 mutation으로 항목을 즉시 추가·삭제한다. 별표 목록은 일반 Settings 초안에서 분리되어 다른 미저장 Terminal 설정과 서로 되돌리지 않는다.
