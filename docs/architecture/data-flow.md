...
Windows ConPTY는 폭 변경 뒤 현재 화면을 `ESC[?25l ESC[H ... ESC[?25h` 프레임으로 다시 출력한다. xterm이 먼저 새 폭으로 scrollback을 reflow하면 이 프레임이 viewport 경계로 이동한 과거 행을 덮어쓰므로, Windows normal buffer의 열 수가 증가하고 `baseY > 0`일 때만 500ms resize repaint 탐색 창을 활성화한다. 필터는 PTY 청크 경계와 무관하게 start/end marker를 스트리밍 탐지해 exact frame 한 개만 xterm write에서 제외하고 앞뒤 출력은 유지한다. start marker를 찾으면 청크로 분할된 end marker를 위해 별도의 500ms 완료 창을 시작한다. 불완전한 start marker 후보는 불일치하거나 탐색 창이 만료되면 정상 출력으로 방출하고, end marker 없이 완료 창이 만료된 프레임은 폐기한 뒤 필터를 해제한다. Rust OSC 파이프라인과 raw output ring은 그대로이며 Linux와 alternate buffer에는 적용하지 않는다([ADR-0026](../adr/0026-conpty-width-resize-repaint-filter.md)).
...
- PTY 출력 또는 세션 복원 write callback이 남아 있는 동안 ResizeObserver reflow를 실행하지 않고, queue drain 뒤 보류된 fit을 정확히 한 번 실행한다.
- Windows의 120ms output quiet 대기는 지속 출력에서도 최초 보류 뒤 500ms 안에 끝나며, Linux resize에는 적용하지 않는다.
- Windows normal buffer의 scrollback을 넓힐 때 청크 경계에서 분할된 marker를 인식하고, 탐색 창 끝에서 start marker가 완성된 frame에도 별도 완료 창을 적용한다. ConPTY resize repaint 한 프레임만 제거해 frame 밖 실제 출력은 보존하며, 완료되지 않은 frame은 제한 시간 뒤 필터를 해제한다.
