...
- **상태 소유자는 CompositionHelper 하나다**: exact bundle patch가 xterm 6.0.0 ESM/CJS의 CompositionHelper에 generation FIFO를 둔다. 각 delayed finalizer는 캡처 범위·다음 composition 경계·이미 전송된 길이·순서 있는 input/keypress 관측 목록·완료 상태를 별도 record로 소유한다. 새 `compositionstart`는 직전 generation의 textarea 상한을 먼저 고정하므로 다음 조합 위치가 이전 timer에 섞이지 않는다.
...
- **textarea candidate를 기준으로 관측을 하나씩 조정한다**: finalizer는 캡처 범위의 textarea candidate를 accumulator로 먼저 읽는다. candidate에 이미 포함된 첫 관측이 정렬 기준을 세울 때까지 unmatched 관측은 도착 순서대로 기존 양방향 포함 및 suffix-prefix 최장 overlap 함수에 넣어 shortest-merge prefix로 모으고, 기준이 생기면 prefix를 candidate와 병합한 뒤 나머지 관측을 하나씩 같은 함수에 fold한다. 기준 뒤에는 양방향 overlap 길이가 같은 관측을 accumulator 뒤쪽에 둔다. 끝까지 기준이 없으면 prefix 전체를 기존처럼 candidate와 병합한다. 관측끼리 무조건 먼저 합치지 않으므로 separator `" "`와 propagated syllable `"면"`의 출처 경계가 사라져 `" 면"`이 된 뒤 candidate `"면 "`에 덧붙는 일을 막으면서, candidate `"한a"` 뒤의 비중첩 단문자 관측 `"a"`, `"b"`는 `"한ab"`, anchor 없는 `"a"`, `"b"` 뒤 candidate `"한"`은 기존처럼 `"ab한"`으로 유지한다. 다만 겹치는 다문자 관측이나 의도적 반복 occurrence의 엄격한 시간 순서는 ADR-0093과 마찬가지로 보장하지 않는다. queue가 빌 때까지 `_isSendingComposition`은 유지된다.
...
- **설치가 계약 관문이다**: `ui/scripts/patch-xterm-reflow.mjs`는 pristine bundle을 선행 keypress patch 형태로 만든 뒤 generation correction과 229 textarea-diff skip을 적용한다. 이미 선행 patch 또는 최종 patch인 설치도 각각 upgrade/idempotent 성공한다. 두 번들의 helper state·input/keypress owner·consumed keypress 강제 취소·delayed/immediate flush·composition epoch·sending 중 textarea-diff skip이 모두 맞지 않으면 postinstall이 실패하고 조용한 부분 적용은 없다. Remote CJS 번들은 wheel patch와 이 textarea-diff skip·epoch만 적용한다.
- **결정적 테스트는 실제 xterm 대상이다**: 기존 6개 조합 이벤트열과 `compositionend → input → keypress`, separator 뒤 propagated keypress의 조건부 기본 삽입, input-only, timer 전 연속 두 generation, input/keyPress 뒤 일반 keydown 교차, 그리고 229가 compositionstart보다 먼저 온 뒤 compositionend가 timer보다 앞서는 stall 순서를 `Terminal.onData`까지 검증한다. pending이 없는 ordinary input·keypress와 조합 없는 229 삽입도 즉시 전송을 고정한다. 순수 mock 판정으로 이 상태 전이를 대체하지 않는다.
