...
visible xterm 앞의 `TerminalWriteBatchQueue`(`terminal-write-batch-queue.ts`)는 pane-local single-flight FIFO다. xterm parse callback이 돌아온 뒤에만 그 pane의 다음 physical write를 내고, producer가 허용한 ordinary live `Uint8Array` 중 attach epoch·geometry revision이 같은 것만 묶는다. 모든 non-stabilized live/replay byte는 64 KiB 이하의 logical entry로 들어간다. replay/string, stabilized frame, cursor park deadline, authoritative frame end, composition-active write는 항상 barrier다. replay `Uint8Array`는 64 KiB로 잘리지만 각 entry가 독립 callback barrier라 같은 logical request 안에서도 합치지 않는다. parsed/discard callback을 가진 request도 기본은 barrier지만, ordinary live producer가 동일 batch key에서 `coalesceCallbacks`를 명시하면 한 physical parse 경계를 공유할 수 있다. 이때 materialized batch는 모든 logical entry의 `onParsed`를 성공 시 각각 호출하고 어느 physical part라도 reject/clear되면 관련 `onDiscard`를 각각 정확히 한 번 호출하므로 ACK hole을 숨기지 않는다. dequeue 때 현재 composition도 다시 확인하므로 기다리는 중 IME가 시작된 write를 뒤늦게 합치지 않는다. hard 상한은 최대 128 part·256 KiB이고 경쟁 scheduler turn은 fresh live coalescing을 64 KiB로 더 제한한다. 이미 materialize된 backpressure retry는 경쟁 상태가 바뀌어도 같은 batch 객체와 buffer로 재시도한다.

여러 pane의 local FIFO는 앱 전역 `TerminalWriteFairScheduler`(`terminal-write-fair-scheduler.ts`)에서 physical admission turn을 얻는다([ADR-0092](../adr/0092-app-wide-terminal-write-round-robin.md)). owner key는 `instanceId`가 아니라 xterm 생성 effect마다 새로 만드는 opaque `Symbol`이다. 따라서 profile 변경으로 같은 id의 replacement effect가 생겨도 old effect의 늦은 callback·pending 취소·unmount release는 새 세대 turn을 건드리지 않는다. active lease는 앱 전체에 하나이며 한 turn은 physical batch 하나만 제출하고, parse callback·동기 거부·lifecycle 취소 중 하나로 끝난 pane은 backlog가 남아도 대기열 꼬리로 돌아간다. 같은 pane의 future turn은 하나로 dedupe하므로 먼저 기다리던 pane이 각각 한 번씩 실행되는 starvation-free round-robin이다. scheduler는 turn을 꺼낸 뒤 다른 owner가 pending인지 함께 알려 준다. 다른 owner가 기다리면 local FIFO의 fresh live dequeue는 64 KiB fairness quantum을 사용하고, 없으면 compatible live entry를 256 KiB까지 coalesce해 단독 live pane 처리량을 유지한다. replay entry는 이 contention 판정과 무관하게 항상 64 KiB 이하의 한 callback barrier다. 전역 idle 최초 request는 기존 대화형 latency를 보존하려 즉시 제출하지만 active write 뒤의 다음 pane은 새 macrotask에서 시작해 Automation·input·paint가 끼어들 기회를 준다. 동기 backpressure면 이미 만든 같은 batch/buffer를 local FIFO head에 복원하고 전역 lease를 반환한 뒤 16 ms 후 새 turn으로 재시도한다. scheduler는 byte·metadata를 읽지 않고 contention 사실만 전달하므로 exact repair와 attach가 정한 byte 전순서 및 local batch barrier는 그대로다. reattach는 아직 제출하지 않은 old-epoch turn/request를 폐기하되 accepted write의 전역 lease와 parse callback은 유지하고, unmount만 active lease까지 취소한다. parsed/replay waiter는 `onDiscard`로 종결하고 이미 drain된 stabilizer callback 묶음도 parsed와 discard 중 하나로만 정확히 한 번 settle한다.
...
### 8.14 조합 commit 과 pending keypress 경합 (issues #527, #660)

xterm 의 `CompositionHelper._finalizeComposition(true)` 는 확정 텍스트를 즉시 보내지 않는다 — 조합 범위를 캡처하고 `_isSendingComposition = true` 로 표시한 뒤 `setTimeout(0)` 안에서 textarea 최종값을 읽는다. 그 사이 legacy `keypress` 가 오면 같은 입력이 (1) keypress 경로와 (2) 나중에 복원된 textarea candidate 두 곳에 나타날 수 있다. 반대로 keypress 시점의 임시 candidate만 보고 한쪽을 즉시 버리면 빠른 입력을 잃는다([ADR-0093](../adr/0093-xterm-composition-keypress-reconciliation-owner.md), [ADR-0062](../adr/0062-composition-commit-keypress-race.md) 대체).

- **상태 소유자는 CompositionHelper 하나다**: exact bundle patch가 xterm 6.0.0 ESM/CJS의 `CompositionHelper.keypress(text)`를 추가한다. `_isSendingComposition`이 true일 때 CoreBrowserTerminal은 keypress를 즉시 PTY로 보내지 않고 `_pendingKeypressData`에 순서대로 보류한다. 일반 keypress는 기존 경로를 그대로 쓴다.
- **최종 candidate와 한 번 조정한다**: delayed timer와 일반 keydown의 immediate finalize가 모두 `_sendCompositionInput`으로 들어간다. candidate/keyPress가 서로를 포함하면 더 긴 쪽을 택하고, 아니면 양방향 suffix-prefix 최장 overlap을 비교해 가장 짧은 ordered merge를 만든다. overlap이 같으면 실제 이벤트 순서인 keypress-first다.
- **외부 guard는 없다**: TerminalView는 `compositionend` 시작 위치를 캡처하거나 keypress를 `preventDefault`/억제하지 않는다. `xterm-pending-composition.ts`의 private 접근은 issue #555 blur fallback이 deferred send 중복을 피하는 `_isSendingComposition` 하나로 축소됐다.
- **설치가 계약 관문이다**: `ui/scripts/patch-xterm-reflow.mjs`는 두 번들의 helper state·keypress owner·delayed/immediate send·CoreBrowserTerminal handoff exact target을 모두 요구한다. xterm 상향으로 하나라도 달라지면 postinstall이 실패하고, 조용한 부분 적용은 없다.
- **결정적 테스트는 실제 xterm 대상이다**: clear→compositionend→keypress→input candidate 복원, suffix overlap, unmatched keypress 순서, candidate-contained keypress, 여러 keypress partial overlap, 일반 keydown immediate finalize의 6개 이벤트열을 `Terminal.onData`까지 검증한다. 순수 mock 판정으로 이 상태 전이를 대체하지 않는다.
- **미검증**: Windows WebView2/한국어 IME 실기와 다중 pane flood 조건의 재현률·key-to-PTY exact byte는 dev 19281에서 확인해야 한다. 자동 테스트는 브라우저 이벤트열 이후 xterm 경로를 고정한다.
