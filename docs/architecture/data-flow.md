...
- split CSI/control string, payload 안의 fake marker, 문법 불일치, 50ms timeout, 1 MiB 초과, unterminated control string, attach generation 교체를 각각 회귀 테스트로 고정한다. fail-open 결과는 원본 바이트와 순서가 같아야 한다. transaction 밖 standalone lone ESC/partial CSI는 자체 deadline 없이 실제 final까지 보류한 뒤 원본을 방출하며 attach reset에서만 폐기한다. prefix를 이미 방출한 뒤 transaction timeout·상한 fail-open이 발생하면 재출력하지 않는 `passEscape`/`passCsi`/`passControl` lexical 상태로 이어가 실제 terminator/final까지 payload 안의 marker 모양 바이트를 다시 해석하지 않는다. CSI를 취소하는 새 ESC는 그 바이트를 다음 escape opener로 재소비해 실제 frame reset을 놓치거나 후속 프레임 밖 tail을 권위화하지 않아야 한다. `awaitingRestore`에서 이 ESC가 다음 `?2026h`를 시작하면 이전 transaction은 그 ESC 직전까지만 fail-open하고 새 frame transaction이 해당 ESC를 소유한다. 두 strict tail은 Native/WSL 모두 one-chunk와 모든 split boundary, 복수 position, interruption, position-only 거부를 고정하고, native는 position-first 성공 시 final show 보존·즉시 metadata·deadline 부재를 추가로 검증한다. WSL recognizer는 같은 청크의 legacy+strict reset metadata 범위와 lifecycle reset도 별도로 고정한다.
