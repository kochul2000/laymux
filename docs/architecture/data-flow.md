...
- **결정적 테스트는 실제 xterm 대상이다**: 기존 6개 조합 이벤트열과 `compositionend → input → keypress`, separator 뒤 propagated keypress의 조건부 기본 삽입, input-only, timer 전 연속 두 generation, input/keyPress 뒤 일반 keydown 교차, 229가 compositionstart보다 먼저 온 뒤 compositionend가 timer보다 앞서는 stall, 잔여 textarea 전체가 새 확정값으로 교체되는 순서, 연속 두 확정 뒤 blur fallback과의 FIFO 결합을 `Terminal.onData`까지 검증한다. pending이 없는 ordinary input·keypress와 조합 없는 229 삽입도 즉시 전송을 고정한다. 순수 mock 판정으로 이 상태 전이를 대체하지 않는다.
...
- **연속 세대의 blur 경계도 FIFO다.** 한 태스크에 `compositionend`가 여러 번 와도 xterm은 generation별 스냅샷을 소유한다. blur가 타이머들보다 먼저 오면 xterm blur handler가 최신 세대보다 앞선 스냅샷들을 동기적으로 FIFO flush한 뒤 최신 세대만 취소한다. 이어 실행되는 TerminalView fallback은 자신이 보관한 최신 `lastFinalizedText`만 보내므로 전체 순서는 유지되고, controller에 별도 다중 슬롯을 만들지 않는다.
