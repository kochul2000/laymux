...
- **파일/디렉토리 경로 → viewer·cwd 전파**(#363, **bounded 복수 선택·데코레이션**, [ADR-0148](../adr/0148-bounded-multi-path-selection-links.md)) — URL이 아닌 "스킴 없는 파일/디렉토리 경로"(예: `ui/src/index.css`, `Cargo.toml`, `/etc/hosts`, `laymux`, `v3`)는 `createPathLinkController`(`ui/src/lib/path-link-provider.ts`)가 처리한다. 사용자가 드래그한 최대 1,024자·8줄을 검색 범위로 삼고 최대 16개 후보를 활성화한다(기존의 hover 줄 전체 토큰 stat 방식은 제거 — 느리고 Windows에서 동작 안 함).
  ...
  - 후보는 `joinCwdPath`로 CWD와 조합하고 동일 절대경로를 deduplicate한 뒤 bounded `stat_paths` 한 번으로 검증한다. desktop은 존재하는 파일과 디렉터리를 모두 후보별 decoration으로 그리고 pointer 아래 정확한 후보를 파일=`openFile`, 디렉터리=`changeDir`로 분기한다. 드래그 중 변화는 120ms trailing debounce하고 pointer-up에서 최종 선택을 즉시 평가한다.
  ...
  - **커서·클릭은 `TerminalView`가 hit-test로 처리한다.** 데코레이션 요소는 `pointer-events: none`(순수 시각)이라 클릭·드래그가 그대로 xterm으로 전달된다 → ⓐ 포인터(손가락) 커서는 `mousemove` 시 `getHit(clientX,clientY)`로 **후보 밑줄 사각형 안일 때만** 켜고(벗어나면 원래 커서), ⓑ 클릭은 `mousedown`(capture) 시점에 그 좌표 아래의 정확한 후보를 캡처해 두고 이동 없이 `mouseup`이면 `activate`로 연다 — 클릭 시 xterm이 선택을 지워 current 목록이 비기 때문이다. **드래그면 무시**해 일반 재선택이 되게 두고, 새 선택은 `onSelectionChange`가 재평가한다.
