...
- **오라클**: `process_tree::interactive_app_in_pty(state, terminal_id)` 가 PTY `child_pid` 의 자손 트리를 BFS 로 훑어 `claude.exe`/`codex.exe`(Linux: `claude`/`codex`)를 찾는다. 가장 얕은 매치가 그 pane 의 포그라운드 앱(서브프로세스로 다른 앱을 띄워도 루트에 가까운 쪽). 전역 프로세스 스냅샷은 1초 TTL 로 캐시해 스피너 틱마다의 재열거를 막는다.
- **양성 권위**: `is_{claude,codex}_terminal_from_buffer` 는 오라클이 해당 앱을 보면 타이틀/버퍼와 무관하게 `true` + 캐시 갱신. 긴 세션에서 시작 배너가 16KB 창 밖으로 밀려나고 타이틀이 스피너뿐이어도 인식이 유지된다.
- **false-exit 억제**: PTY 콜백의 타이틀 상태머신이 "비-앱 타이틀 → 종료"로 판정해도, 오라클이 프로세스 생존을 확인하면 그 종료를 무효화한다(`claude_detected`·캐시·grace window·`claude_was_working` 보존, `interactiveAppExited` 미발행, 허위 "task completed" 미발행). 종료는 프로세스 소멸로만 확정된다.
- **fallback**: PID 없는 PTY(직렬 등)·스냅샷 실패·트리에 앱 없음(`None`)은 비확정으로 보고 기존 타이틀/캐시 신호로 폴백한다.
