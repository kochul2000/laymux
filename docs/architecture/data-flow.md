...
Android 앱의 E2E Remote 표면은 browser Remote와 다른 native transport를 사용한다([ADR-0146](../adr/0146-android-e2e-session-and-encrypted-remote-rpc.md)). 사용자의 생체 승인으로 seed를 한 번 복호화하면 Kotlin이 challenge→establish를 수행하고 방향별 session key와 exact sequence를 단독 소유한다. APK 로컬 WebView는 `connect-src 'none'`을 유지하며 xterm 입력·resize를 native bridge action으로만 보내고, Kotlin은 이를 한 요청 in-flight의 encrypted RPC FIFO에 직렬화한다. desktop은 기존 controller claim/heartbeat/lease 검사 뒤 allowlisted 내부 Remote 동작만 실행한다. 출력 open은 rendererless checkpoint snapshot과 generation/source sequence를 원자적으로 잡고, 이후 120ms fixed-delay poll은 ring `resume_terminal_output` delta를 같은 암호화 FIFO로 가져온다. desktop은 성공한 RPC 응답 안에 인증된 새 `expiresAt`을 넣고 Android는 이를 검증해 15분 비활성 deadline을 갱신하므로 foreground에서 통신하는 동안 세션이 유지된다. background 전환은 poll·heartbeat를 취소하고 session을 suspend해 deadline이 더 늘지 않게 하며, key와 pending exact ciphertext는 현재 deadline까지 보존한다. 15분 안의 foreground 복귀는 pending response를 먼저 확정한다. 기존 lease heartbeat가 성공하면 lease를 재사용하고, 만료됐다면 복귀 직후 캡처한 E2E deadline까지만 claim 충돌을 재시도해 새 lease를 받은 뒤 checkpoint부터 다시 부착한다. 명시적 disconnect·pairing revision 변경·15분 비활성 만료는 연결 세대와 session key를 폐기한다. relay는 outer instance/session/sequence와 길이·타이밍만 보며 plaintext terminal metadata·bytes·lease는 보지 못한다.
