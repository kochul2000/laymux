...
Remote xterm의 선택 파일 링크도 같은 FileViewer 권한 경계와 명시적 host path action을 사용한다([ADR-0148](../adr/0148-bounded-multi-path-selection-links.md)). 브라우저는 선택 원문과 active terminal id만 `/remote/v1/file-viewer/path-link`로 보내며 CWD나 조합된 path를 제안하지 않는다. `fileViewer.pathLink` frontend bridge가 desktop `useTerminalStore`의 최신 CWD와 `useSettingsStore.terminal` 설정을 읽고, 위 §8.6과 같은 bounded maximal-munch 후보 추출 → `joinCwdPath` → 중복 제거 → `statPaths` 흐름을 실행한다. 존재하는 일반 파일만 token·절대 path·선택 상대 범위 목록으로 반환하고 디렉터리는 Remote 링크로 활성화하지 않는다. 요청 본문은 `{terminalId, mode, lines, caret?}`이며 `mode`가 트리거를 고른다([ADR-0188](../adr/0188-path-link-ambient-detection-triggers.md)): `selection`(드래그, 8줄·4,096자), `point`(탭·클릭 — `caret`의 `{lineIndex,index}`가 UTF-16 offset으로 토큰을 지목, 1줄), `screen`(유휴 화면 스캔, 64줄·8,192자). 서버(`viewer_routes.rs`)가 mode별 상한과 caret 범위를 검사하고 텍스트는 trim하지 않는다. Remote page는 드래그 중 selection 변화를 trailing debounce하고 pointer-up에서 최종 선택을 즉시 검증하며, 탭은 URL 링크 활성화가 실패하고 기존 밑줄 위도 아니며 선택이 없을 때만 `point`로 평가하고(밑줄 위 탭은 viewer 열기), 출력이 500ms 멈추면 보이는 화면을 `screen`으로 한 번 평가한다. 직전 스캔과 화면이 같아도 그 스캔의 밑줄이 남아 있지 않으면 다시 평가한다(예약이 먼저 밑줄을 거두므로 무조건 생략하면 표시가 사라진다). 새 선택·terminal/lease 전환·xterm reset·resize/reflow에서는 진행 중 요청과 모든 marker를 폐기하고, write·스크롤은 `screen` scope만 폐기한 뒤 유휴 타이머를 다시 잡고, write 는 남아 있는 `selection`·`point` 밑줄의 원문을 `revalidatePathLinkScopes`로 재확인해 어긋난 것만 거둔다(desktop `revalidate()`와 같은 계약). `point`·`screen` 응답은 줄 전체 offset을 쓰므로 `mapRemoteLinePathRange`가 그 셀에 같은 원문이 남아 있는지 확인하고 어긋나면 표시하지 않는다(문자열 폴백 없음). 각 응답 범위는 최신 선택 원문과 exact match인지 다시 확인한 뒤 실제 xterm cell width(CJK·전각 포함)를 사용해 surface-local 좌표에 매핑하고 `registerMarker`+`registerDecoration`으로 밑줄을 그리며, pointer 아래 정확한 사각형의 primary click에서 `openFileViewerTab(path)`를 호출한다. selection revision·terminal·lease·capability가 바뀐 늦은 bridge 응답은 표시되지 않는다. 실제 파일 내용은 클릭 뒤 child의 기존 render 요청에서만 읽는다.
...
- **파일/디렉토리 경로 → viewer·cwd 전파**(#363, **bounded 복수 선택·데코레이션**, [ADR-0148](../adr/0148-bounded-multi-path-selection-links.md), [ADR-0165](../adr/0165-desktop-path-link-validates-on-pointer-release.md), [ADR-0188](../adr/0188-path-link-ambient-detection-triggers.md)) — URL이 아닌 "스킴 없는 파일/디렉토리 경로"(예: `ui/src/index.css`, `Cargo.toml`, `/etc/hosts`, `laymux`, `v3`)는 `createPathLinkController`(`ui/src/lib/path-link-provider.ts`)가 처리한다.
  - **발견 트리거 3종과 비용 상한**(ADR-0188). 각 트리거는 자기 **scope**(`selection`/`point`/`screen`)의 데코레이션만 교체·해제하고, hit-test 만 전체를 함께 본다. 트리거당 filesystem 조회는 배치 1회로 고정된다.
    ...
    - `screen` (Remote 전용) — 출력이 500ms 멈추면 보이는 화면 1장(최대 64줄·8,192자)에서 strong candidate 와 공백 확장 후보(ADR-0191)를 합쳐 최대 64개. 상한을 넘으면 뒤쪽을 버린 **부분 결과**를 낸다(선택의 all-or-nothing과 의도적으로 다르다). write·resize·buffer 전환·스크롤은 이 scope 를 즉시 폐기하고 타이머를 다시 잡으며, 직전 스캔과 화면 텍스트가 같으면 요청하지 않는다. 선택이 살아 있으면 스캔하지 않는다.
  - 데스크톱 hover dwell/클릭 평가는 `createPathLinkPointEvaluator`(`ui/src/lib/path-link-point.ts`)가 소유하고 `TerminalView`는 설정·버퍼·좌표 변환(`getClickCellCoords`)·IPC 만 주입한다. 밑줄 아래 원문이 재출력으로 바뀌면 `onWriteParsed` 의 `pathLink.revalidate()` 가 그 항목만 폐기한다(`VerifiedPathSelection.token` 이 판정 기준, 마커가 살아 있으면 마커의 현재 라인을 읽는다). 같은 write 에서 point 재조회 memo 도 잊되 진행 중 조회는 살린다. 재조회 방지는 끝난 지점과 조회 중인 지점 둘 다 대상이며, click slop 안의 포인터 이동은 dwell 타이머를 다시 잡지 않는다.
