...
- **Remote 브라우저 URL**(#516) — self-hosted `WebLinksAddon`이 평문 HTTP(S) URL을, xterm `linkHandler`가 OSC 8 HTTP(S) URL을 활성화하며 둘 다 `openRemoteUrl`로 라우팅한다. 이 함수는 URL을 다시 파싱해 `http:`/`https:`만 `_blank` + `noopener,noreferrer`로 열고, 비-HTTP 스킴과 잘못된 URL은 무시한다. 새 API나 controller 권한은 만들지 않는다. 이번 Remote 대응은 #345/#351의 평문·OSC 8 범위이며, 들여쓰기 하드랩(#145)과 TUI 마우스 트래킹 우회(#352)는 기존 데스크톱 전용 동작을 복제하지 않는다.
