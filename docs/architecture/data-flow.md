...
- **파일/디렉토리 경로 → viewer·cwd 전파**(#363, **bounded 복수 선택·데코레이션**, [ADR-0148](../adr/0148-bounded-multi-path-selection-links.md)) — URL이 아닌 "스킴 없는 파일/디렉토리 경로"(예: `ui/src/index.css`, `Cargo.toml`, `/etc/hosts`, `laymux`, `v3`)는 `createPathLinkController`(`ui/src/lib/path-link-provider.ts`)가 처리한다. 사용자가 드래그한 최대 1,024자·8줄을 검색 범위로 삼고 최대 16개 후보를 활성화한다(기존의 hover 줄 전체 토큰 stat 방식은 제거 — 느리고 Windows에서 동작 안 함).
  - `extractPathCandidatesFromSelection`은 왼쪽부터 maximal token을 소비하고 `:line:col`·문장부호를 정리한다. 확정하거나 버린 긴 token 내부의 suffix/basename은 다시 후보로 만들지 않으며 긴 후보의 stat 실패도 내부 fallback을 일으키지 않는다. 단일 token 선택은 `laymux` 같은 맨이름을 계속 받지만, 넓은 선택은 절대경로·구분자·확장자가 있는 strong candidate만 받는다. `terminal.pathLinkMaxLength`(기본 256)는 후보별 상한이다. 전체 길이·줄·후보 상한을 넘으면 부분 링크 없이 검사를 생략한다.
