...
- hover로 자동 열린 portal은 pane과 toolbar를 하나의 pointer 소유 영역으로 취급한다. DOM상 pane 밖으로 이동해도 toolbar로 건너가는 동안 유지되며, 둘 사이 placement gap은 짧은 exit grace로 건넌다. hover 해제로 `⋯` anchor가 언마운트돼도 열린 menu 수명 동안 마지막 anchor의 pane-relative 위치를 유지해 포인터 아래에서 점프하지 않는다. `hoverIdleSeconds` 만료처럼 추가 pointer event 없이 hover 소유권이 끝난 경우도 같은 grace 뒤 닫되, 그 사이 toolbar에 진입하면 유지한다. 소유 workspace나 `persistState`로 DOM에 남은 dock이 비활성화되면 portal과 열린 상태를 즉시 폐기한다. 명시적으로 연 toolbar는 첫 액션으로 focus를 옮기며, 바깥 클릭으로 닫는다. Escape로 닫을 때는 살아 있는 trigger로 focus를 돌리고, hover 해제로 trigger가 이미 언마운트됐다면 프로그램적으로 focus 가능한 소유 pane으로 복원한다.
