...
| `compositionend` → `input("a")` → blur | 확정값과 관측을 함께 동기 flush | `onData ["가a"]`, suffix 유실 없음 |
| `compositionend` → timer → blur | 이미 pending이 없음 | 기존 `onData ["가"]`만 유지 |
| 조합 없는 잔여물 | — | blur에서 **무조건** 비움 |

- **xterm은 끝난 세대와 진행 중 조합을 나눈다.** CoreBrowserTerminal의 blur listener는 먼저 완료된 generation FIFO를 flush한 뒤 textarea를 비운다. 아직 `compositionend`가 없는 조합은 generation이 없으므로 xterm이 보내지 않고, 뒤따르는 TerminalView listener가 프리뷰를 확정한다. blur 뒤 늦은 `compositionend`는 취소 세대로 소비되어 다시 보내지 않는다.
