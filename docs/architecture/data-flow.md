...
control/resource encrypted RPC는 Kotlin의 한 요청 in-flight FIFO에 직렬화되고 desktop의 기존 controller/lease 검사를 통과한 Remote router를 재사용한다. terminal output은 별도 장기 WSS다. Android가 32-byte `streamNonce`를 만들고 session 방향 key에서 stream key를 파생해 encrypted `OPEN {terminalId,leaseId}`를 보내면 desktop이 renderer checkpoint를 attach하고 browser와 동일한 V1 JSON text header와 binary snapshot/delta를 각각 AEAD record로 즉시 보낸다. 64 KiB보다 큰 record는 opaque tunnel fragment로 나뉘고 relay가 protocol 최대 encrypted record 2,629,658 bytes 안에서 exact binary message로 재조립하므로 1 MiB tunnel frame 상한을 넘지 않는다. relay는 outer instance/session/stream nonce와 ciphertext·fragment 크기·타이밍만 보며 plaintext terminal metadata·bytes·lease는 보지 못한다. Android native는 복호화한 TEXT/BINARY만 exact local origin의 `WebMessage` ArrayBuffer로 넘기고, 페이지는 표준 output socket parser를 그대로 사용한다. header 수락과 xterm binary write 완료 ACK 사이에는 native가 한 record만 bridge에 두며 queue는 protocol 최대 plaintext record 2,629,633 bytes 한 건과 추가 2 MiB backlog로 제한한다. pending/active output open은 합계 8개로 제한한다. output bytes에는 base64/JSON/`evaluateJavascript`를 쓰지 않는다([ADR-0158](../adr/0158-android-e2e-websocket-output-transport.md)).
