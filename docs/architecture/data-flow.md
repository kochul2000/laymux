...
- **파일/디렉토리 경로 → viewer·cwd 전파**(#363, **bounded 복수 선택·데코레이션**, [ADR-0148](../adr/0148-bounded-multi-path-selection-links.md), [ADR-0165](../adr/0165-desktop-path-link-validates-on-pointer-release.md), [ADR-0188](../adr/0188-path-link-ambient-detection-triggers.md)) — URL이 아닌 "스킴 없는 파일/디렉토리 경로"(예: `ui/src/index.css`, `Cargo.toml`, `/etc/hosts`, `laymux`, `v3`)는 `createPathLinkController`(`ui/src/lib/path-link-provider.ts`)가 처리한다.
  - **발견 트리거 3종과 비용 상한**(ADR-0188). 각 트리거는 자기 **scope**(`selection`/`point`/`screen`)의 데코레이션만 교체·해제하고, hit-test 만 전체를 함께 본다. 트리거당 filesystem 조회는 배치 1회로 고정된다.
    - `selection` (desktop·Remote) — 드래그한 최대 1,024자·8줄에서 최대 16개 후보. 상한을 넘으면 부분 결과 없이 검사를 생략한다.
    - `point` (desktop hover dwell 300ms·이동 없는 클릭, Remote 단일 탭·데스크톱 모드 클릭) — 포인터 아래 **maximal token 하나**만 후보로 만들어 `stat_paths` 1건. 지목된 토큰이므로 맨이름(`laymux`)도 받는다. 이미 밑줄이 있는 지점, 버튼이 눌린 동안(드래그), 같은 (라인·토큰·cwd) 재방문에서는 조회하지 않는다. **클릭은 발견이지 실행이 아니다** — 미검증 문구의 클릭은 밑줄만 켜고, 열기는 밑줄 위 클릭에서만 일어난다.
    - `screen` (Remote 전용) — 출력이 500ms 멈추면 보이는 화면 1장(최대 64줄·8,192자)에서 strong candidate 최대 64개. 상한을 넘으면 뒤쪽을 버린 **부분 결과**를 낸다(선택의 all-or-nothing과 의도적으로 다르다). write·resize·buffer 전환·스크롤은 이 scope 를 즉시 폐기하고 타이머를 다시 잡으며, 직전 스캔과 화면 텍스트가 같으면 요청하지 않는다. 선택이 살아 있으면 스캔하지 않는다.
  ...
  - **수정자 클릭 → 호스트 OS 위임**(#687, [ADR-0100](../adr/0100-path-link-host-os-open-modifier-contract.md)) — 밑줄 사각형 안에서 **Ctrl+좌클릭**은 대상을 호스트 OS로 넘기고(파일=연결 프로그램, 디렉토리=파일 관리자), **Ctrl+Shift+좌클릭**은 파일 관리자에서 부모 폴더를 열고 대상을 선택 표시한다(부모가 없는 드라이브/공유 루트는 열기로 낮춘다). WSL 경로도 호스트 맥락으로 열린다 — 백엔드 `open_in_os`가 `stat_path`와 같은 `resolve_address_path_following_symlinks`로 호스트 경로를 만든다.
    ...
    - 순수 로직은 두 곳으로 나뉜다([ADR-0191](../adr/0191-viewer-os-handoff-buttons.md)) — **트리거 고유 입력 계약**(`decidePathLinkClickAction`/`isOsHandoffAction`/`osHandoffModeForAction`/`pathLinkHintKey`)은 `ui/src/lib/path-link-os-open.ts`, **트리거와 무관한 확인 정책**(`HARD_CONFIRM_EXTENSIONS`/`requiresHardConfirm`/`needsOsHandoffConfirm`/`osHandoffConfirmKey`)은 `ui/src/lib/os-handoff.ts`가 소유한다(FileViewer 버튼이 같은 정책을 공유한다). mousedown·mouseup 상태 기계는 주입형 `createPathLinkClickHandlers`(`path-link-click.ts`), 라우팅은 `createPathLinkController.activate(sel, action)`, 실행은 데스크톱 전용 커맨드 `open_in_os`다(Automation/MCP/Remote 미노출). `TerminalView`는 스토어·i18n·포커스만 주입해 배선한다.
