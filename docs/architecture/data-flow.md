...
Remote 브라우저 xterm은 display mirror이고 PC xterm이 terminal protocol query의 단일 응답자다. snapshot·replay 중 생긴 `onData`는 전부 폐기하고, live output의 CSI DSR·DA·DECRQM·XTVERSION과 query slot을 하나라도 포함한 OSC 4/10/11/12 색상 payload는 parser handler가 원천 차단한다. OSC parser handler는 payload 전체를 claim해야 하므로 혼합 payload의 setter slot은 query를 제거하고 원래 색상 index·순서를 보존한 setter-only OSC로 xterm에 다시 적용한다. setter-only payload는 xterm 기본 handler로 넘긴다.
