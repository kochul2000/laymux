...
- 신규 설정은 `controlBar.defaultMode = "pinned"`로 pane 컨트롤을 바로 보인다. 저장된 `hover`·`minimized` 기본 모드와 pane별 오버라이드는 우선 적용한다. Dock 신규 기본값은 왼쪽 선택기와 오른쪽 Memo / Files / GitHub의 3등분이며 저장된 Dock 구성은 바꾸지 않는다([ADR-0265](../adr/0265-pc-remote-first-use-defaults.md)).
- 메뉴 진입은 `⋯`, 컨트롤바를 최소화하는 내부 액션은 `−` 아이콘과 “컨트롤바 숨기기” 툴팁으로 구분한다.
...
- 충돌한 컨트롤은 hover/pinned 모드 모두 작은 `⋯` anchor로 접는다. pane이나 anchor에 마우스를 올리는 것만으로는 메뉴를 열지 않고, `⋯`를 클릭하거나 키보드로 활성화할 때만 `document.body`의 `position: fixed` toolbar portal을 연다. 헤더 높이와 View 작업영역의 rect·스크롤 위치는 변하지 않는다.
- floating toolbar의 첫 가로 경계는 viewport가 아니라 소유 pane의 실제 폭이다. 메뉴는 콘텐츠에 필요한 폭만 차지하며 pane 전체 폭으로 늘리지 않는다. 불투명한 overlay 배경과 각진 테두리, 내부 여백으로 헤더와 구분한다. 전체 액션은 가로 flex-wrap으로 1~여러 줄에 유지한다. wrapped flex는 `safe flex-end`로 정렬해 pane보다 넓은 단일 컨트롤의 overflow가 측정·스크롤 가능한 inline-end 방향으로 나가게 하고, 그 컨트롤만 필요한 만큼 viewport 안에서 가로로 빠져나온다. viewport 자체가 그 최소 폭보다 작으면 가로 스크롤을 최종 fallback으로 둬 액션을 버리거나 화면 밖에 고정하지 않는다. 기본 margin 두 배 이하인 양의 viewport 축에서는 margin을 0으로 양보해 0px surface를 만들지 않고 해당 축 전체를 최종 스크롤 영역으로 쓴다. 세로는 먼저 anchor 바로 아래에서 **pane 안에 전부 들어가는지** 보고, 부족하면 viewport의 위쪽으로 flip한 뒤, 위도 부족할 때만 pane 경계를 넘어 아래쪽을 사용한다. 좌우는 가용 viewport margin 안으로 clamp한다. 어느 방향도 물리적으로 전체 높이를 제공하지 못하는 경우에만 더 넓은 쪽에 최대 높이와 내부 스크롤을 둬 모든 액션을 계속 접근 가능하게 한다. 이때 제약된 border box 높이를 다음 배치의 자연 높이로 되먹이지 않는다. 자연 바깥 높이는 `scrollHeight + (offsetHeight - clientHeight)`로 보존해 테두리와 가로 스크롤바까지 포함하므로, ResizeObserver가 정확한 1~2px 경계를 다시 측정해도 스크롤 배치가 진동하지 않는다. placement 계산의 SoT는 `ui/src/lib/floating-toolbar-placement.ts`다.
- 열린 portal은 포인터 이동이나 hover 만료로 닫지 않는다. `⋯` 재클릭·바깥 클릭·Escape로 닫는다. hover 해제로 anchor가 언마운트되어도 메뉴 수명 동안 마지막 anchor의 pane-relative 위치를 유지해 점프를 막는다. 소유 workspace나 `persistState`로 DOM에 남은 dock이 비활성화되면 portal·열린 상태·hover 원시 상태를 즉시 폐기해 재활성화만으로 되살아나지 않게 한다. 메뉴를 열면 첫 액션으로 focus를 옮긴다. Escape로 닫을 때는 살아 있는 trigger로 focus를 돌리고, trigger가 이미 언마운트됐다면 프로그램적으로 focus 가능한 소유 pane으로 복원한다.
...
`FileViewer`는 두 축으로 렌더러를 고른다([ADR-0109](../adr/0109-file-viewer-typed-preview-renderers.md)). Rust `read_file_for_viewer`가 바이트에서 정한 `kind`(`text | image | pdf | archive | binary`)와, 프론트엔드가 경로 확장자에서 정한 preview 종류다. preview 종류는 다시 두 계열로 갈린다 — document(`html`/`markdown`)는 sanitizer+CSP+sandbox iframe 을 타고, structured(`json`/`jsonl`/`diff`/`csv`/`log`/`code`)는 `ui/src/lib/preview/`의 순수 파서가 만든 값을 `ui/src/components/ui/preview/`의 컴포넌트가 React DOM 으로 그린다. structured 경로는 HTML 문자열을 만들지 않으므로 sanitizer 가 없다. 아래 확장자 외부 뷰어 매핑은 두 축 모두보다 우선한다. HTML은 sanitizer 처리 후 DOM과 인라인 CSS에서 표시할 콘텐츠가 남는지 검사한다. 크기·여백만 지정한 빈 컨테이너와 `display:none`·`opacity:0`인 하위 트리는 제외하고, `content-visibility:hidden`은 요소 자신의 배경·테두리를 보존한 채 자손만 제외한다. 상속된 `visibility:hidden/collapse` 및 0 글꼴 크기를 텍스트 판정에 반영한다. 자손의 명시적 visibility·글꼴 복원은 허용한다. 텍스트, 이미지·체크박스·기본 장식 요소, 배경·테두리·그림자 등 CSS 도형의 단서가 없으면 PC의 브라우저 등 외부 프로그램으로 열라는 안내를 문서에 넣는다. 데스크톱과 Remote가 같은 판정을 사용하며 Markdown에는 적용하지 않는다. 로딩 문구는 콘텐츠로 보존한다. 픽셀·렌더 크기를 측정하지 않는 보수적 휴리스틱이므로 클리핑·가림·배경과 같은 색 등 모든 빈 화면을 판별하지는 않는다. Remote의 빈 sandbox 경계를 유지하기 위해 부모의 iframe DOM 접근이나 측정용 추가 iframe은 도입하지 않는다. [ADR-0245](../adr/0245-empty-html-preview-os-handoff.md)에 따라 같은 함수가 빈 판정과 안전한 문서를 함께 반환하며, 데스크톱은 빈 결과를 iframe 대신 React 안내와 `OsHandoffActions` 본문 버튼으로 표시한다. HTML 원본 경로를 기존 확인 정책을 거쳐 PC 연결 프로그램에 전달하고 Source 전환은 유지한다. Remote는 안내 문서와 기존 Download를 유지하며 호스트 실행 권한을 얻지 않는다.

### 데스크톱 스프레드시트 뷰어

ODS는 calamine 호출 전에 전체 통합 문서의 반복 셀 영역(100,000셀)과 공백·문자열·문단·문자 참조의 반복 확장량(8 MiB)을 검사한다. ZIP 해제 바이트 상한만으로는 XML의 반복 지시가 만드는 할당을 제한할 수 없기 때문이다.

[ADR-0242](../adr/0242-readonly-spreadsheet-viewer.md): `.xls`, `.xlsx`, `.xlsb`, `.ods`는 외부 뷰어 매핑 다음에 전용 `SpreadsheetPreview`로 연결한다. Rust calamine이 시트 목록과 선택 시트의 sparse 셀 값을 반환하고 React가 현재 시트 검색·TSV 복사를 담당한다. [ADR-0244](../adr/0244-spreadsheet-selection-and-windowed-scroll.md)에 따라 고정 28px 행 높이·144px 열 너비와 스크롤 위치로 보이는 행·열+여유 영역만 렌더링하며 페이지 UI는 없다. 작은 시트는 창 너비로 늘리지 않는다. 행·열 머리글 클릭/드래그, 셀 직사각형 드래그와 Shift 확장으로 선택한다. 선택은 필터 결과 좌표로 유지하고 검색·시트 변경 시 초기화한다. 표 포커스의 브라우저 `copy` 이벤트는 화면 밖을 포함한 선택 범위를 TSV로 제공한다. 가장자리 드래그는 자동 스크롤한다. 긴 셀은 한 줄 생략하고 툴팁에 전체 값을 표시한다. Rust는 표시 상한을 처음 초과하면 셀 순회를 중단하며, 잘린 응답의 크기는 중단 시점까지 관측한 좌표 기준이다. 파일 변경/시트 전환의 늦은 응답은 폐기한다. XLSX/XLSB 원본 16 MiB, ZIP 해제 64 MiB(XLS/ODS 제한은 아래), 표시 10,000행·256열·100,000셀·값 4 MiB 상한을 두며 잘림은 화면에 알린다. 기존 Remote 파일 읽기와 FileViewerContent는 바꾸지 않는다.

[ADR-0246](../adr/0246-spreadsheet-demand-driven-reading.md): `SpreadsheetStreams`는 Tauri managed state로 최대 4개 reader 작업자를 소유한다. open은 첫 100행을, next는 같은 reader에서 다음 100행을 반환하며 구간 사이에는 채널에서 대기한다. XLSX/XLSB 셀은 look-ahead 하나 외에 요청 전 파싱하지 않는다. 파일 ZIP 안전 검사와 공유 문자열 준비는 선행한다. XLS/ODS는 calamine 선행 파싱 때문에 원본 2 MiB까지만 허용하며 ODS는 압축 해제/반복 문자열 8 MiB·반복 셀 100,000셀까지다. UI는 하단 접근에서 단일 next 요청을 보내고 기존 총량 상한 안에서 읽은 구간만 누적한다. 검색·복사는 읽은 범위에 한정된다고 표시한다. 파일·시트 전환/닫기와 늦은 open 응답에서 close로 reader를 해제한다. 스크롤 뷰포트 padding은 0으로 하여 sticky 헤더 위 틈을 막는다.
...
Remote FileViewer는 데스크톱 overlay를 복제하거나 외부 프로세스를 실행하지 않고 Remote 셸 문서 안의 Explorer 오버레이로 표시한다([ADR-0042](../adr/0042-remote-file-viewer-secret-capability.md), [ADR-0044](../adr/0044-remote-file-viewer-explicit-host-path.md), [ADR-0045](../adr/0045-remote-path-link-reuses-desktop-parser.md), [ADR-0184](../adr/0184-remote-file-viewer-in-page-overlay.md), [ADR-0198](../adr/0198-remote-file-explorer-overlay.md), [ADR-0208](../adr/0208-android-e2e-file-viewer-typed-capability.md)). 직접 경로 입력도 workspace drawer가 아니라 Explorer의 디렉터리 목록 위에서 수행한다. 연결·heartbeat는 FileViewer status를 자동 조회하지 않는다. 사용자가 명시적으로 `From host`를 누를 때만 active lease와 claim 성공 때 발급된 `fileViewerToken`으로 `fileViewer.status`를 조회해 `useFileViewerStore`의 현재 path를 host file path 입력에 반영한다. 요청 시작 때 입력 revision을 스냅샷하고 응답 시점에 다시 비교하므로, 조회 중 사용자가 입력을 편집했다면 늦은 host path를 적용하지 않는다. 사용자가 `Open` action을 실행하면 클릭 시점의 trim된 경로를 스냅샷으로 고정한다. 성공하면 path controls를 숨기고 파일 본문으로 전환하며 Back은 직전 Explorer 디렉터리를 다시 요청한다. 실패하면 controls와 Back을 유지해 경로를 고치거나 목록으로 돌아갈 수 있다. browser는 lease/capability 전용 헤더를 사용하고, Android E2E는 caller 헤더를 주입하지 않은 채 business body에 `fileViewerAuthorization: {leaseId,fileViewerToken}`을 더한다. E2E dispatcher가 이를 exact FileViewer POST에서만 제거·타입화하고, 현재 session 객체와 claim의 `{leaseId,ownerEpoch,instanceId,sessionId}`에 결합된 내부 proof로 Rust route에 전달한다. claim은 pairing lifecycle이 E2E registry의 현재 session 확인부터 `remote_access → remote_control`의 lease/binding 설치까지 유지되는 짧은 authorization commit이므로 pairing revision·session replace/revoke가 중간에 끼지 못한다. challenge/establish도 lifecycle 재획득 뒤 material revision을 다시 확인하고 registry를 변경한다. route는 frontend async bridge 전과 완료 직후 같은 receipt를 검증한다. owner/session transition이나 새 claim으로 capability가 폐기·회전되면 이미 계산한 payload도 버리고 `403`으로 fail closed하며 bridge 이후 응답은 `Cache-Control: no-store`를 사용한다. 성공 응답의 exact AEAD retry도 저장된 비밀 payload를 바로 반환하지 않고 원래 요청을 다시 복호화해 현재 receipt와 replay guard를 검증하며, 불일치하면 session을 revoke하고 outer `401`로 끝낸다. bridge는 browser에서 `readFileForViewer(path, 8 MiB)`, Android E2E에서 `readFileForViewer(path, 2 MiB)`를 호출한다. Android FileViewer inner JSON 상한은 3 MiB이고 일반 inner HTTP의 1 MiB 상한은 그대로다. frontend가 automation IPC envelope 전체의 escape 포함 JSON UTF-8 크기를 비할당 방식으로 먼저 검사한 뒤 Rust dispatcher가 같은 상한을 다시 적용한다. archive는 compressed source와 tar.gz inflate를 각각 source 상한으로 제한하고 ZIP entry scan은 5,000개에서 거부한다. directory list는 Rust가 표시 상한 1,000개에 look-ahead 한 건만 더 열거·metadata 조회한 bounded sample을 정렬한다. HTML/Markdown이면 기존 `file-preview.ts` sanitizer로 완성된 preview document를 만들고, Markdown은 데스크톱과 Remote 모두 같은 `marked` 동기 GFM 렌더러와 내장된 `github-markdown-css`를 사용한다. Preview 응답은 원문 `content`를 중복하지 않으며, 오버레이는 일반 text를 `textContent`, image를 제한된 data URL, preview document를 sandbox iframe `srcdoc`으로만 렌더하고 잘린 preview에는 경고를 함께 표시한다. 문서 제목은 host path를 포함하지 않는 일반 이름으로 유지하고, 직접 경로 입력은 모바일 자동 대문자화를 끈다. Remote 표면은 `extensionViewers` 매핑을 의도적으로 무시한다. 호스트 터미널에서 실행되는 외부 viewer는 Remote 오버레이로 투영할 수 없고, remote 파일 읽기가 새 호스트 프로세스 실행 권한으로 확장되어서는 안 되기 때문이다.

Remote xterm의 세 경로 발견 트리거도 같은 FileViewer 권한 경계와 명시적 host path action을 사용한다([ADR-0148](../adr/0148-bounded-multi-path-selection-links.md), [ADR-0188](../adr/0188-path-link-ambient-detection-triggers.md), [ADR-0220](../adr/0220-path-link-stable-frame-lifetime.md)). 브라우저는 `{terminalId, mode, lines, caret?}`만 `/remote/v1/file-viewer/path-link`로 보내며 CWD나 조합된 path를 제안하지 않는다. `mode`는 `selection`(드래그, route 상한 8줄·4,096자), `point`(탭·클릭, 1줄과 UTF-16 `{lineIndex,index}` caret), `screen`(유휴 viewport, 64줄·8,192자)이다. 서버(`viewer_routes.rs`)가 mode별 상한과 caret 범위를 검사하고 원문을 trim하지 않으며, `fileViewer.pathLink` frontend bridge가 desktop `useTerminalStore`의 최신 CWD와 `useSettingsStore.terminal` 설정을 읽어 위 §8.6과 같은 bounded 후보 추출 → `joinCwdPath` → 중복 제거 → `statPaths` 흐름을 실행한다. 존재하는 파일과 디렉터리를 `{token,path,kind,lineIndex,startIndex,endIndex}`로 반환하고 Remote는 파일을 viewer render, 디렉터리를 explorer list로 연다([ADR-0198](../adr/0198-remote-file-explorer-overlay.md)). page는 드래그 selection을 trailing debounce하고 pointer-up에서 최종 검증하며, URL이나 기존 밑줄이 아닌 단일 탭만 `point`로 평가한다. 출력이 500ms 멈추면 `screen`을 한 번 평가한다. write 완료 시 모든 scope의 저장 token을 안정된 xterm 셀 원문과 비교해 바뀐 항목만 폐기하며, DEC 2026 mode 중에는 아직 표시되지 않은 중간 버퍼 판정을 reset 또는 안전 timeout 뒤까지 보류한다. 살아남은 `screen` decoration은 요청 중에도 유지하고 동일한 live marker·좌표·token·path·kind 결과는 DOM identity까지 재사용한다. 성공 적용된 decoration 집합만 화면 signature를 소유한다. 모든 physical write는 화면에 없는 서버 CWD가 바뀌었을 가능성 때문에 `screen` 검증 context를 dirty로 만들고, 다음 안정 프레임 유휴 스캔은 같은 화면도 최신 CWD로 한 번 재검증한다. 취소된 요청은 새 signature를 얻지 않으며 기존 성공 집합이 살아 있으면 그 signature를 유지하고, malformed·부분 매핑 응답·decoration 일부 생성 실패나 기존 집합의 항목 탈락은 scope와 signature를 무효화한다. live selection 때문에 dirty screen을 검증할 수 없으면 기존 screen scope를 fail closed로 폐기하고 selection change가 유휴 스캔을 다시 잡는다. resize/reflow·buffer 전환·terminal/lease/capability 전환·xterm reset은 좌표나 권한이 바뀌므로 전면 폐기하지만, 일반 write와 scroll은 marker를 유지한 채 유휴 스캔만 다시 잡는다. `point`·`screen` 응답은 `mapPathLinkParts`와 `pathLinkPartsCurrent`가 최신 줄의 같은 셀에 같은 token이 남았는지 확인하고, 각 범위는 실제 xterm cell width(CJK·전각 포함)로 surface-local 좌표에 매핑한다. revision·terminal·lease·capability가 바뀐 늦은 응답은 버리고, 실제 파일 내용은 검증된 밑줄 클릭 뒤 기존 render 요청에서만 읽는다.
...
- **Codex 0.150+ position-first in-frame park**: 열린 frame의 하나 이상의 연속 CUP/HVP/CHA 뒤에 singleton `?25h`가 오고 `?2026l`로 즉시 닫힌다. final position과 show를 모두 보존한 채 위와 같은 atomic write와 metadata를 보내며, position/show/reset 사이에 다른 byte·control·CSI가 끼면 인정하지 않는다([ADR-0221](../adr/0221-codex-position-first-in-frame-cursor-park.md)).

세 정상 문법은 이미 1 MiB 상한으로 제한되므로 일반 tracked writer의 청크 분할을 적용하지 않고 `terminal.write` 한 번으로 전달한다. legacy는 frame end와 restore 사이, 두 in-frame park 순서는 final show와 position 사이에 parser callback 경계가 생기지 않는다. cache·snapshot·복원 구분선·backend mode 합성은 stabilizer를 통과하지 않는다.

WSL의 terminal별 `WslInFrameCursorParkRecognizer`는 두 strict in-frame 문법을 읽기 전용으로 판정한다. 청크 사이에는 CSI/control-string lexical state만 유지하고, 바이트를 보류·삭제·치환·재정렬하지 않으며 받은 모든 바이트를 같은 호출에서 반환한다. strict `?2026l`이 완결되면 그 token만 별도 tracked write가 되도록 현재 청크를 나누고 해당 write에 `frameEndCursorAuthoritative`를 붙인다. 따라서 같은 PTY 청크 안의 앞선 legacy reset까지 metadata가 번지지 않으며, split CSI는 다음 write의 parser 연속 상태에서 정확한 reset handler가 metadata를 소비한다. 불완전 문법은 metadata 없이 즉시 fail-open하고 legacy out-of-frame restore는 원자화하지 않는다. 최초 attach, 사용자 terminal recreate, unmount와 구 v2 fallback 재부착은 partial token과 열린 frame을 폐기한다. production v3 transport/credit failure는 stream 교체 없이 fail-stop한다.
...
- Native Windows live delta만 DEC 2026 transaction으로 안정화한다. WSL은 byte-for-byte pass-through를 유지하면서 Codex 0.145의 `?25h` → position → `?2026l`과 Codex 0.150+의 position → `?25h` → `?2026l` strict tail reset에만 metadata를 붙이고 legacy frame은 보류하지 않는다. 직접 SSH/Linux/unknown, cache/snapshot replay는 metadata 없는 pass-through다. native legacy exact restore 성공 시 transient in-frame singleton `?25h`만 빠지고, 두 strict tail은 final show를 보존한 채 다음 청크 대기 없이 즉시 방출한다. native의 정상 transaction은 일반 write 청크보다 큰 frame도 `terminal.write` 한 번으로 전달되며 write callback 뒤 public refresh가 정확히 두 번 실행된다. 활성 IME preedit의 focus/value는 유지되고 composition commit은 human-input 경로로 정확히 한 번만 전달되며, in-frame park의 final buffer cursor가 IME 앵커로 채택된다. WSL 통합 테스트는 두 strict reset 모두 legacy settle timeout을 만들지 않고, frame-open과 reset 사이에 animation frame이 끼어도 조합이 없을 때 이미 그린 overlay caret의 opacity·geometry를 유지하는지 고정한다. 같은 열린 frame 안의 활성 조합은 public buffer가 footer로 이동해도 컨트롤러 앵커를 기준으로 preview·composition caret·helper 앵커를 다음 음절까지 갱신하고, 조합 종료는 preview를 즉시 지우고 helper를 원복하면서 non-composition caret 동결을 유지해야 한다.
- split CSI/control string, payload 안의 fake marker, 문법 불일치, 50ms timeout, 1 MiB 초과, unterminated control string, attach generation 교체를 각각 회귀 테스트로 고정한다. fail-open 결과는 원본 바이트와 순서가 같아야 한다. transaction 밖 standalone lone ESC/partial CSI는 자체 deadline 없이 실제 final까지 보류한 뒤 원본을 방출하며 attach reset에서만 폐기한다. prefix를 이미 방출한 뒤 transaction timeout·상한 fail-open이 발생하면 재출력하지 않는 `passEscape`/`passCsi`/`passControl` lexical 상태로 이어가 실제 terminator/final까지 payload 안의 marker 모양 바이트를 다시 해석하지 않는다. CSI를 취소하는 새 ESC는 timeout·상한 뒤 pass 상태에서도 현재 바이트를 아직 방출하지 않은 다음 escape opener로 재소비해 실제 frame reset을 놓치거나 후속 프레임 밖 tail을 권위화하지 않아야 한다. `awaitingRestore`에서 이 ESC가 다음 `?2026h`를 시작하면 이전 transaction은 그 ESC 직전까지만 fail-open하고 새 frame transaction이 해당 ESC를 소유한다. 2026을 포함한 결합 private-mode reset은 실제 frame latch를 닫되 singleton strict reset metadata를 받지 않는다. 두 strict tail과 결합 reset은 Native/WSL 모두 one-chunk와 모든 split boundary를 고정하고, strict tail은 복수 position, interruption, position-only 거부를 추가로 검증하며 native는 position-first 성공 시 final show 보존·즉시 metadata·deadline 부재를 검증한다. WSL recognizer는 같은 청크의 legacy+strict reset metadata 범위와 lifecycle reset도 별도로 고정한다.
...
- DEC 2026이 활성이고 IME composition이 없을 때는 **non-composition overlay caret의 마지막 opacity·geometry를 그대로 동결**한다([ADR-0008](../adr/0008-shell-cursor-shadow-cursor.md)). 포커스 상실·미오픈·scrollback 이동처럼 즉시 가시성을 잃어야 하는 상태는 동결보다 우선한다. native Windows stabilizer는 정상 transaction을 한 xterm write로 전달해 중간 paint가 보통 없지만, WSL byte pass-through는 `?2026h`와 strict park/reset 사이에 PTY write·animation frame 경계를 허용한다. 이때 sync 상태를 단순 hidden으로 계산하면 Codex `Working` 갱신마다 overlay가 사라졌다 reset 뒤 다시 나타나므로 두 host 모두 같은 "last committed frame" 규칙을 적용한다.
- **활성 IME composition은 DEC 2026 동결보다 우선한다.** xterm native composition view를 항상 숨기므로 Laymux preview가 preedit의 유일한 renderer다. 열린 frame에서도 컨트롤러가 소유한 조합 text·anchor·caret offset으로 preview와 composition caret을 갱신하고 같은 caret cell에 helper textarea를 유지한다. frame body가 옮긴 public buffer cursor는 footer일 수 있으므로 이 좌표 계산에 채택하지 않는다. `active=false` 전이는 동결 조기 반환보다 먼저 preview opacity/text를 지우고 helper의 원래 inline 위치를 복원하며, 이미 그린 non-composition caret의 opacity·geometry는 건드리지 않는다.
...
- native Windows stabilizer 또는 WSL metadata-only recognizer가 strict in-frame park를 확인한 write에는 `frameEndCursorAuthoritative`가 붙는다. 이 경우 `?2026l` handler 시점의 buffer cursor는 strict tail의 final position을 이미 적용했으므로 pre-frame snapshot보다 우선하고, `parkPending`을 세우지 않으며 남은 settle timer를 해제한다. metadata가 없는 legacy frame은 pre-frame snapshot을 fallback으로 쓰고 out-of-frame park 또는 timeout을 기다린다.
...
- 열린 DEC 2026 frame이나 `?2026l` 뒤 `parkPending`이 overlay 좌표 repaint를 동결하더라도, IME composition이 종료되어 `active=false`가 되면 프리뷰 DOM의 opacity와 text를 동결 검사 전에 즉시 정리하고 helper 앵커를 원복한다. 완료된 조합 문자열이나 후보창 앵커가 frame reset·settle timeout까지 남아서는 안 된다.
...
**줄바꿈 경로**([ADR-0235](../adr/0235-wrapped-path-link-logical-lines.md)): `path-link-lines.ts`는 desktop selection·point와 Remote 3 mode가 공유한다. 실제 셀에서 논리 줄을 복원하며 `isWrapped`는 출력된 공백을 보존해 결합하고, hard wrap은 오른쪽 끝 경로 토큰·같은 들여쓰기·후속 경로 토큰 조건으로 제한한다. 완성된 확장자·새 절대경로·URL·빈 줄은 hard 결합하지 않는다. 물리 줄 64·UTF-16 8192자 이내에서 읽고 경계에서 잘린 그룹은 제외한다. point는 앞뒤 8줄, Remote screen은 viewport 앞뒤 8줄 문맥을 포함한다. 선택은 복원 결과와 실제 선택 원문의 비공백 문자 순서를 대조하고, 직사각형 선택처럼 범위가 모호하면 제외한다. 실제 선택 셀만 읽으며 8개 물리 줄·1024자 상한을 먼저 적용한다. 파싱·stat·논리 offset 겹침 해소 후 `mapPathLinkParts`가 각 물리 줄의 셀 범위로 나누며 모든 조각은 동일한 전체 경로를 연다. `pathLinkPartsCurrent`가 조각 하나의 원문·폭·wrap 또는 hard-wrap 연결 조건 변경에도 전체 밑줄과 액션 칩을 폐기한다. selection 응답은 최신 선택 좌표로 재매핑한다.
...
- **파일/디렉토리 경로 → viewer·cwd 전파**(#363, **bounded 복수 선택·데코레이션**, [ADR-0148](../adr/0148-bounded-multi-path-selection-links.md), [ADR-0165](../adr/0165-desktop-path-link-validates-on-pointer-release.md), [ADR-0188](../adr/0188-path-link-ambient-detection-triggers.md)) — URL이 아닌 "스킴 없는 파일/디렉토리 경로"(예: `ui/src/index.css`, `Cargo.toml`, `/etc/hosts`, `laymux`, `v3`)는 `createPathLinkController`(`ui/src/lib/path-link-provider.ts`)가 처리한다.
  - **발견 트리거 3종과 비용 상한**(ADR-0188). 각 트리거는 자기 **scope**(`selection`/`point`/`screen`)의 데코레이션만 교체·해제하고, hit-test 만 전체를 함께 본다. 트리거당 filesystem 조회는 배치 1회로 고정된다.
    - `selection` (desktop·Remote) — 드래그한 최대 1,024자·8줄에서 기본 후보 최대 16개. 기본 후보가 상한을 넘으면 부분 결과 없이 검사를 생략한다. 공백 확장 후보(ADR-0191, 아래)는 이 all-or-nothing 대상이 아니라 기본 후보 뒤에 배치 총량 64까지 best-effort 로 덧붙는다.
    - `point` (desktop hover dwell 300ms·이동 없는 클릭, Remote 단일 탭·데스크톱 모드 클릭) — 포인터 아래 **maximal token 하나 + 그 지점을 덮는 공백 확장 접두 ≤ 8**(ADR-0191)을 후보로 만들어 `stat_paths` 배치 1건. 지목된 토큰이므로 맨이름(`laymux`)도 받는다. 이미 밑줄이 있는 지점, 버튼이 눌린 동안(드래그), 같은 (라인·후보 집합·cwd) 재방문에서는 조회하지 않는다. **클릭은 발견이지 실행이 아니다** — 미검증 문구의 클릭은 밑줄만 켜고, 열기는 밑줄 위 클릭에서만 일어난다.
    - `screen` (Remote 전용, [ADR-0220](../adr/0220-path-link-stable-frame-lifetime.md)) — 출력이 500ms 멈추면 보이는 화면 1장(최대 64줄·8,192자)에서 strong candidate 와 공백 확장 후보(ADR-0191)를 합쳐 최대 64개. 상한을 넘으면 뒤쪽을 버린 **부분 결과**를 낸다(선택의 all-or-nothing과 의도적으로 다르다). write는 기존 밑줄의 셀 원문을 재검증해 바뀐 항목만 폐기하고 타이머를 다시 잡되, DEC 2026 synchronized-output mode 중에는 표시되지 않은 중간 버퍼이므로 재검증과 스캔을 안정 프레임까지 보류한다. 화면 signature는 요청 시작이 아니라 성공 적용된 decoration 집합에만 결속한다. physical write가 없고 그 signature와 화면 텍스트가 같으며 해당 밑줄이 온전히 남아 있을 때만 요청을 생략한다. write는 OSC 7의 비가시 CWD 변경을 포함할 수 있으므로 매번 context를 dirty로 만들고, 안정 프레임에서 같은 화면도 서버의 최신 CWD로 한 번 재검증한다. 취소된 요청은 새 signature를 얻지 않고 살아 있는 기존 성공 집합의 signature를 유지하며, 일부 원문 탈락이나 불완전 응답은 scope와 signature를 무효화한다. 달라져 재검증하더라도 좌표·token·path가 같은 live marker/decoration은 재사용한다. resize·buffer 전환은 셀 좌표계가 바뀌므로 scope를 즉시 폐기하고, 스크롤은 marker를 유지한 채 새 viewport를 스캔한다. 선택이 살아 있으면 dirty screen scope를 fail closed로 폐기하고, selection change가 유휴 스캔을 다시 예약한다.
  - 데스크톱 hover dwell/클릭 평가는 `createPathLinkPointEvaluator`(`ui/src/lib/path-link-point.ts`)가 소유하고 `TerminalView`는 설정·버퍼·좌표 변환(`getClickCellCoords`)·IPC 만 주입한다. 밑줄 아래 원문이 재출력으로 바뀌면 `onWriteParsed` 의 `pathLink.revalidate()` 가 그 항목만 폐기한다(`VerifiedPathSelection.token` 이 판정 기준, 마커가 살아 있으면 마커의 현재 라인을 읽는다). 단, DEC 2026 synchronized-output 중에는 아직 렌더되지 않은 중간 버퍼이므로 재검증을 보류하고, 정상 reset 뒤 write 완료 또는 xterm 안전 timeout을 감지한 mode monitor에서 안정 프레임을 한 번 판정한다([ADR-0220](../adr/0220-path-link-stable-frame-lifetime.md)). 같은 write 에서 point 재조회 memo 도 잊되 진행 중 조회는 살린다. 재조회 방지는 끝난 지점과 조회 중인 지점 둘 다 대상이며, click slop 안의 포인터 이동은 dwell 타이머를 다시 잡지 않는다.
  - `stat_paths` 배치 상한은 64다(한 배치가 만들 수 있는 최대 fs 조회 수 — 선택 후보 상한 16과 별개).
  - `extractPathCandidatesFromSelection`은 왼쪽부터 maximal token을 소비하고 `:line:col`·문장부호를 정리한다. Markdown/file-link 계열이 Windows 드라이브 절대경로 앞에 슬래시를 하나 붙인 `/D:/...` 형태도 `D:/...`로 정규화하며, 제거한 슬래시는 원문 후보 범위의 시작 offset에 반영한다. 확정하거나 버린 긴 token 내부의 suffix/basename은 다시 후보로 만들지 않으며 긴 후보의 stat 실패도 내부 fallback을 일으키지 않는다. 단일 token 선택은 `laymux` 같은 맨이름을 계속 받지만, 넓은 선택은 절대경로·구분자·확장자가 있는 strong candidate만 받는다. `terminal.pathLinkMaxLength`(기본 256)는 후보별 상한이다. 전체 길이·줄·후보 상한을 넘으면 부분 링크 없이 검사를 생략한다.
  - **공백이 든 경로**([ADR-0191](../adr/0191-path-link-space-extended-candidates.md)) — 공백은 토큰 경계이므로 `G:/내 드라이브/...` 같은 경로는 토큰만으로는 잡히지 않는다. 줄을 공백 단위 raw chunk 로 나눠, 앞 장식을 뗀 텍스트가 절대경로로 시작하는 chunk 를 앵커로 삼고 이어지는 chunk 끝(cut)마다 접두 후보를 만든다(앵커당 ≤ `PATH_LINK_MAX_SPACE_EXTENSIONS`=8, 스페이스 간격만 잇고 탭은 끊음, 상대경로는 앵커 불가). chunk 는 괄호를 경계로 삼지 않아 `C:\Program Files (x86)\...` 도 이어진다. 경로의 끝은 문법으로 판정하지 않는다 — 문장 꼬리가 붙은 cut 도 후보로 나가고 `stat_paths` 가 게이트다. 존재하는 후보 중 같은 줄의 겹치는 범위는 `resolveOverlappingRanges` 가 **긴 것부터 greedy** 로 남긴다(longest-existing-wins — 접두 디렉토리보다 전체 파일 경로가 이긴다). 이 겹침 해소는 세 트리거의 stat 이후 공통이다(desktop selection = `TerminalView`, point = `path-link-point.ts`, Remote 3 mode = `remote-file-viewer.ts`).
  - 후보는 `joinCwdPath`로 CWD와 조합하고 동일 절대경로를 deduplicate한 뒤 bounded `stat_paths` 한 번으로 검증한다. desktop은 존재하는 파일과 디렉터리를 모두 후보별 decoration으로 그리고 pointer 아래 정확한 후보를 파일=`openFile`, 디렉터리=`changeDir`로 분기한다. 새 pointerdown은 이전 검증 revision을 즉시 무효화하되 기존 링크 click의 mousedown 캡처가 끝나도록 decoration은 유지하고, click slop을 넘는 첫 pointermove가 이를 지운다. xterm의 document mouseup이 최종 선택을 확정하고 `onSelectionChange`를 낸 다음 window mouseup이 현재 선택을 정확히 한 번 평가한다. 호환 mouseup이 없는 release는 pointerup의 다음-task fallback이 같은 최종화를 수행하고, pointercancel은 검증 없이 gesture를 폐기한다. 이 중간 경로들은 후보 파싱·filesystem stat을 하지 않는다. Remote는 자체 trailing debounce와 pointer-up 최종 평가를 유지한다.
  - **밑줄은 xterm 데코레이션으로 직접 그린다**(linkifier hover 의존 안 함). xterm `ILinkProvider`/Linkifier는 mousemove 시점에만, 같은 셀이면 재질의를 건너뛰어, 비동기 stat 검증이 끝난 뒤 마우스가 정지해 있으면 밑줄/클릭이 안 켜졌다("나갔다 돌아와야 동작"). 그래서 검증되면 `setVerifiedSelections`가 후보마다 `registerMarker`+`registerDecoration`(둘 다 xterm **proposed API** → `Terminal` 생성 시 `allowProposedApi: true` 필수)을 만들고, 선택이 바뀌거나 해제되면 `clear()`가 모두 dispose한다. `readPathLinkLines`와 `mapPathLinkParts`는 논리 줄·UTF-16 offset과 `getSelectionPosition()`의 **0-based·end exclusive** 모델 좌표를 데코레이션의 **1-based 절대 버퍼 셀 좌표**로 보정한다.
  - **커서·클릭은 `TerminalView`가 hit-test로 처리한다.** 데코레이션 요소는 `pointer-events: none`(순수 시각)이라 클릭·드래그가 그대로 xterm으로 전달된다 → ⓐ 포인터(손가락) 커서는 `mousemove` 시 `getHit(clientX,clientY)`로 **후보 밑줄 사각형 안일 때만** 켜고(벗어나면 원래 커서), ⓑ 클릭은 `mousedown`(capture) 시점에 그 좌표 아래의 정확한 후보를 캡처해 두고 이동 없이 `mouseup`이면 `activate`로 연다 — 클릭 시 xterm이 선택을 지워 current 목록이 비기 때문이다. **드래그면 무시**해 일반 재선택이 되게 두고, 첫 drag 이동은 이전 링크를 지우며 release 최종화가 새 선택을 평가한다.
...
Android entry는 Cloud 계정 표면과 E2E Remote 표면을 서로 다른 WebView/bridge로 분리한다([ADR-0149](../adr/0149-android-thin-wrapper-runs-desktop-owned-remote-ui.md)). Cloud WebView는 설정된 public HTTPS origin의 `/app/android`와 기존 landing/dashboard만 표시하고, native Google Credential Manager login, instance UUID 선택과 PC별 연결 설정 진입만 요청한다. Cloud가 anonymous session에 저장한 32-byte nonce를 Google ID token에도 넣고 Cloud가 서명·issuer·audience·expiry와 nonce를 검증한 뒤 기존 HttpOnly session cookie를 발급한다. 선택한 instance와 confirmed pairing 또는 새로 스캔/붙여넣은 초대의 instance가 정확히 일치해야 secure 표면으로 넘어간다. Cloud WebView에는 pairing metadata·key·E2E transport bridge가 없다.

Cloud main document의 `loading`·`ready`·`unavailable`은 Kotlin이 원시 상태로 소유한다. native가 시작하는 모든 top-level navigation은 WebView `loadUrl`보다 먼저 `loading`을 발행하고 불투명 진행 표면을 띄운다. 재시도·dashboard 복귀·Google login 완료는 fresh Cloud WebView generation을 설치한 뒤 적재하므로 이전 WebView가 같은 URL의 늦은 `onPageFinished`나 오류를 보내도 현재 generation과 active URL을 모두 만족하지 못해 표시를 바꾸지 못한다. 현재 main-frame의 DNS/TLS/HTTP 실패만 `unavailable`로 바꿔 Chromium 오류 문서를 가리고 `다시 시도`를 제공하며, 하위 자원 실패는 무시한다. 현재 generation과 URL의 성공한 `onPageFinished`만 `ready`로 전환하며, Cloud bridge action은 `Cloud 전면 surface ∧ ready`에서만 허용한다. 따라서 Cloud가 오프라인이어도 앱 프로세스를 재시작하지 않고 native layer에서 복구할 수 있다.

PC 선택 뒤 pairing 표면은 네이티브 Material bottom sheet다([ADR-0178](../adr/0178-android-pairing-native-material-bottom-sheet.md), [ADR-0179](../adr/0179-android-connection-settings-from-dashboard-menu.md), [ADR-0212](../adr/0212-android-pairing-invitation-copy-paste.md)). Cloud WebView는 실제 dashboard 문서를 아래에서 계속 렌더링하지만 native layer policy가 입력·focus·접근성 descendants와 Cloud bridge action을 비활성화한다. Kotlin은 vault metadata·선택 instance·생체 가용성·연결 진행 상태를 raw state로 모으고 단일 presentation 계산을 거쳐 sheet의 상태·QR scan/clipboard 붙여넣기/재pairing·session open/취소만 갱신한다. 진행 상태는 action 재진입을 막는 판정(scan·ACK·생체 unwrap·session open)을 그대로 쓰므로 "다른 작업이 진행 중이라 거부됨"과 "진행 중으로 보임"이 어긋나지 않고, sheet 는 안내 문구 줄에 indeterminate spinner 를 함께 띄운다. 안내 문구 없이 새로고침된 진행 중 상태에서도 그 줄은 유지된다. QR과 clipboard 결과는 같은 parser·instance gate·Keystore·ACK 경로로 수렴하고 Cloud WebView에는 전달하지 않는다. 등록 지문 추가·삭제 등으로 정책별 공유 biometric alias가 영구 무효화되면 vault는 그 alias로 감싼 모든 PC envelope와 alias를 함께 폐기한다. 기존 seed 복호화 중이면 전체 재pairing 필요를 반환하고, 새 초대 암호화 준비 중이면 같은 QR/붙여넣기 입력을 보존한 채 fresh alias를 한 번 생성해 저장을 계속한다. Material dialog의 바깥 터치·swipe·system back dismiss는 Cloud 복귀 경로로 수렴하고, session 성공은 dialog를 닫은 뒤 Cloud를 숨기고 secure WebView에 PC Remote 문서를 적재한다.

Remote를 떠나는 전환은 현재 secure WebView 문서 세대의 권한을 먼저 회수하고 WebView를 폐기한 뒤 빈 새 세대를 설치한다. 다음 보안 session이 열릴 때 그 새 세대만 승인하며, Remote bridge·encrypted resource loader·binary output listener는 `Remote 전면 surface ∧ 현재 설치 세대 ∧ 승인 세대`가 모두 일치해야 동작한다. 이전 PC 문서에서 늦게 도착한 callback과 resource/output 요청은 새 PC session을 사용할 수 없다.

Android app-mode dashboard의 각 PC `…` 메뉴는 `openConnectionSettings(instanceId)` capability가 있을 때만 연결 설정 action을 노출한다([ADR-0179](../adr/0179-android-connection-settings-from-dashboard-menu.md)). native는 canonical instance UUID와 현재 Cloud foreground gate를 확인한 뒤 별도 Material alert dialog를 열고, 해당 PC의 confirmed/pending metadata·ACK 재시도·key 확인/삭제와 전역 생체 보호 정책을 표시한다. dialog 동안 Cloud WebView는 pairing sheet와 같은 비활성 layer policy를 적용하고, dismiss하면 문서를 다시 적재하지 않고 기존 dashboard 입력·접근성과 bridge만 복구한다. 생체 문제로 pairing이 차단됐을 때의 `보호 설정 열기`도 sheet 안에 설정을 펼치지 않고 이 dialog로 전환한다. 모든 설정 action은 native callback으로 직접 이어지며 Cloud JavaScript에는 instance UUID 이외의 vault 상태를 반환하지 않는다.

Android PC 선택 action은 선택적 `tailscaleUrl`도 native에 routing hint로 넘긴다([ADR-0160](../adr/0160-android-e2e-tailscale-direct-transport.md)). native는 Tailscale literal CIDR·고정 port·exact `/remote/`를 검증하고, 빈 hint은 Cloud-only로 보되 비어 있지 않은 invalid hint는 선택을 fail closed한다. 후보가 있으면 Cloud public route 대신 PC의 `/remote/v1/e2e/*`에서 새 challenge/establish를 먼저 수행한다. Direct 네트워크 실패만 새 client session nonce를 만든 Cloud handshake로 이어지고, cancel/background generation이 바뀌면 Cloud 요청 전에 중단하며 HTTP/proof/AEAD 실패는 경로를 바꾸지 않는다. 열린 `RemoteSession`은 선택 transport 하나를 고정하며 RPC와 output WebSocket은 같은 transport adapter의 route mapping을 사용한다. Direct session의 RPC 또는 output socket network failure는 중복 요청이 하나의 UI-thread coordinator로 수렴하게 하고, 기존 session·stream을 닫은 뒤 vault 보호 정책을 다시 적용해 Cloud-only handshake와 PC 문서·snapshot reload를 수행한다.

E2E Remote 표면은 browser Remote의 **기능 코드**를 재사용하되 native transport를 사용한다. 사용자의 생체 승인으로 seed를 한 번 복호화하면 Kotlin이 challenge→establish를 수행하고 방향별 session key와 exact sequence를 단독 소유한다. APK에는 xterm·terminal 목록·navigation을 복제하지 않는다. Kotlin은 먼저 PC Laymux의 `/remote/` 문서와 정적 자산을 encrypted `kind=resource` RPC로 가져와 AEAD 검증한 뒤 synthetic HTTPS origin에 제공한다. 그 문서의 기존 `remoteFetch`와 output socket seam만 각각 async native HTTP bridge와 binary output adapter로 전환되므로 workspace·pane·terminal 선택, controller claim·heartbeat·release, 입력·resize와 V1 output parsing은 계속 PC가 배포한 동일 Remote page가 소유한다. native는 request id·method·allowlisted path·JSON body와 stream별 crypto/queue만 다루고 Remote 도메인 상태를 해석하지 않는다.

Android Remote의 확인된 시스템 뒤로가기는 PC 문서의 기존 Exit 버튼을 실행한다. 이 경로가 자동 재연결 의도와 입력 작업을 취소하고 controller lease 반납 요청을 마친 뒤 `disconnectRemote` bridge로 완료를 알릴 때만 native가 session key를 폐기하고 대시보드로 이동한다. PC 문서가 진행 중인 Exit의 claim revision으로 중복 종료를 합치고 작업 완료 시 해제한다. 수동 Connect는 성공 여부와 무관하게 이전 Exit를 대체하므로 실패한 재연결 뒤에도 다시 뒤로가기로 종료할 수 있다. 아직 Remote 문서가 준비되지 않은 경우는 기존 native 세션 정리로 돌아간다. 반납 통신 실패는 기존 Exit의 best-effort 정책을 따르며 서버 lease 만료가 최종 회수를 보장한다([ADR-0149](../adr/0149-android-thin-wrapper-runs-desktop-owned-remote-ui.md), [ADR-0155](../adr/0155-android-background-remote-lease-grace.md)).

control/resource encrypted RPC는 Kotlin의 한 요청 in-flight FIFO에 직렬화되고 desktop의 기존 controller/lease 검사를 통과한 Remote router를 재사용한다. terminal output은 별도 장기 WSS다. Android가 32-byte `streamNonce`를 만들고 session 방향 key에서 stream key를 파생해 encrypted `OPEN {terminalId,leaseId}`를 보내면 relay가 단 한 개의 binary message와 4 KiB 상한을 base64 직렬화 전에 강제하고 desktop이 renderer checkpoint를 attach한다. Python connector 경로는 guarded local Axum `/remote/v1/e2e/output` WebSocket으로 전달하고 native tunnel 경로는 transport adapter에서 직접 전달하지만, 둘 다 같은 Rust output core가 session·nonce·lease·checkpoint·AEAD sequence를 검증하고 browser와 동일한 V1 JSON text header와 binary snapshot/delta를 각각 AEAD record로 즉시 만든다. connector의 내부 hop은 loopback·exact path·Origin 없음·Remote enabled·exact token을 요구하되 public direct client용 IP/Tailscale/Origin 정책에서는 분리한다. Python connector와 native desktop sender는 64 KiB보다 큰 record를 같은 bounded opaque tunnel fragment 계약으로 나누고 relay receiver가 protocol 최대 encrypted record 2,629,658 bytes 안에서 exact binary message로 복원하므로 1 MiB tunnel frame 상한을 넘지 않는다. WebSocket은 초기 opening acknowledgement timeout을 유지하지만 그 뒤에는 단방향으로 영구 idle일 수 있어 HTTP next-frame/방향별 read timeout을 적용하지 않고 cancellation·bounded queue·write timeout·ping/pong으로 종료와 자원을 관리한다. relay는 outer instance/session/stream nonce와 ciphertext·fragment 크기·타이밍만 보며 plaintext terminal metadata·bytes·lease는 보지 못한다. Android native는 최대 encrypted record 크기를 복사·AEAD 전에 검사하고 복호화한 TEXT/BINARY만 exact local origin의 `WebMessage` ArrayBuffer로 넘기며, 페이지는 표준 output socket parser를 그대로 사용한다. header 수락과 xterm binary write 완료 ACK 사이에는 native가 한 record만 bridge에 두며 queue는 protocol 최대 plaintext record 2,629,633 bytes 한 건과 추가 2 MiB backlog로 제한한다. pending/active output open은 합계 8개로 제한한다. output bytes에는 base64/JSON/`evaluateJavascript`를 쓰지 않는다([ADR-0159](../adr/0159-android-e2e-websocket-output-transport.md)).

desktop은 성공한 control/resource RPC 응답 안에 인증된 새 `expiresAt`을 넣고 Android는 이를 검증해 15분 비활성 deadline을 갱신하므로 foreground의 heartbeat가 session을 유지한다. background 전환은 output WebSocket과 새 bridge 요청을 닫고 session을 suspend해 deadline이 더 늘지 않게 하며, key와 pending exact RPC ciphertext는 현재 deadline까지 보존한다. 15분 안의 foreground 복귀는 pending response를 먼저 확정한 뒤 같은 PC 문서를 유지한 채 native transport만 복원하고, 페이지가 자기 sessionStorage의 auto-connect 의도와 resume capability로 lease를 재검증·재확보해 새 stream nonce와 output snapshot에 부착한다. 문서가 foreground callback을 지원하지 않을 때만 다시 로드한다([ADR-0163](../adr/0163-android-foreground-preserves-remote-document.md)). 명시적 disconnect와 pairing revision 변경은 session key와 output stream key를 폐기하고 Cloud dashboard로 돌아간다. 15분 비활성 만료는 key를 폐기한 뒤 선택한 PC와 Remote surface에서 보호 정책에 따른 재인증을 시작하며, 진행 중인 재인증을 foreground의 session 부재 복구가 dashboard 전환으로 취소하지 않는다. 재인증이 불가능하거나 사용자가 취소하거나 새 session 열기가 실패한 때만 Cloud dashboard로 돌아간다. 연결 실행은 네이티브 pairing sheet, 저장된 pairing·보호 정책 관리는 dashboard `…`에서 여는 별도 native dialog가 담당한다.

Cloud tunnel의 접속 표면은 PC 설정 snapshot의 `cloudAccessMode`에서 파생한다([ADR-0150](../adr/0150-desktop-owned-cloud-remote-access-policy.md)). `browserAndE2e`는 기존 stream을 유지하고, `androidE2eOnly`는 고정 E2E `POST` 네 경로와 exact `/remote/v1/e2e/output` encrypted WebSocket만 stream open에서 허용하며 HTTP는 dispatch 직전에 다시 검사한다. 설정 저장에서 mode 변화가 관측되면 실행 중 worker를 abort해 이미 열린 browser HTTP/output stream을 함께 정리하고 같은 credential로 재연결한다. 새 worker의 첫 heartbeat가 mode를 relay registry에 게시하고, Cloud dashboard와 browser session/credential 검사는 그 live 값을 매 요청 재확인한다. Android app-mode PC 선택과 public E2E relay, Cloud account/presence, 별도 Local/Tailscale Direct 경로는 이 분기 밖이다.

Terminal surface는 [ADR-0029](../adr/0029-detached-terminal-input-composer.md)와 이를 대체한 [ADR-0034](../adr/0034-single-send-terminal-composer.md)에 따라 `direct`와 `composer` 두 입력 모드를 가진다. Direct는 xterm helper textarea와 raw `terminal.onData` 경로를 유지한다. Composer의 입력 editor가 아직 전송하지 않은 초안의 진실원이며, Desktop은 textarea, Remote는 [ADR-0236](../adr/0236-remote-composer-inline-attachments.md)의 contenteditable을 사용한다. 사용자 action은 text 뒤에 CR 제출 의도를 보내는 Send 하나만 제공한다. 전송 gesture는 [ADR-0036](../adr/0036-remote-composer-layout-rule.md)의 layout 규칙 `mobileLayout = (pointer: coarse) || localApp=1`을 따른다. desktop layout(PC WebView, fine-pointer 웹 Remote)에서는 일반 Enter가 Send이고 Shift+Enter가 editor 줄바꿈이다. mobile layout(터치 기기, PC 앱 임베드 모바일 뷰)에서는 Enter가 줄바꿈이며 footer 우측의 Send 버튼만 제출하고, keybinding 시스템 밖의 키보드 전송 단축키는 두지 않는다. 모든 surface에서 IME 조합 중 Enter는 제출하지 않는다. Composer가 차지하는 높이는 xterm host와 겹치지 않고 flex sibling으로 배치되며 모드 전환 뒤 공통 guarded fit을 실행한다. Composer 중에는 xterm native cursor, shadow overlay caret, composition preview를 숨기고 xterm keyboard event를 차단하지만, xterm이 생성한 protocol reply용 `onData`는 유지한다.

Remote direct 모드의 IME 조합 텍스트(preedit)는 확정 후 점유할 셀 위에 놓인다([ADR-0171](../adr/0171-remote-preedit-laid-out-on-committed-cells.md)). 위치는 xterm 소유 그대로다 — `updateCompositionElements`가 `.composition-view`를 커서 셀에 앵커하고 helper textarea를 그 rect로 미러링한다([ADR-0061](../adr/0061-native-ime-candidate-anchor.md)). 내용만 remote client가 다시 깐다: `remote-app.js`의 `installCompositionCellLayout`이 xterm 다음 순서로 textarea의 `compositionstart`/`update`/`end`를 구독해, view를 클러스터당 `셀 수 × cellWidth` 폭의 inline-block 박스 하나로 교체한다. 클러스터와 셀 수는 `unicode-provider.js`가 노출하는 `splitCellClusters`에서 오며 desktop과 같은 표를 쓴다([ADR-0058](../adr/0058-single-terminal-cell-width-provider.md)). 이렇게 하지 않으면 view는 폰트의 자연 advance로 흘러(한글 ≈1셀) 확정 시 2셀로 벌어진다. 오른쪽 끝 wrap은 여전히 xterm의 `white-space: nowrap` 동작이다.

PC의 명시적 기본 모드 선호만 `laymux.desktop.inputMode`에 저장하고 최초 기본값은 `direct`다. 현재 모드와 `ComposerDraftState { text, revision, inFlight }`는 terminal id별 runtime Map에만 둔다. Send 시작 시 `{ terminalId, revision, text, token }`을 캡처하고 같은 terminal의 중복 action을 막는다. 전송 중 editor 편집은 허용하며 성공 응답도 현재 text/revision이 캡처와 같을 때만 비운다. 실패·결과 불명확·stale token은 초안을 보존한다. 데스크톱 surface는 이 runtime Map 을 React state 로 복제하지 않고 `useSyncExternalStore`로 직접 구독한다(현재 모드는 `subscribeRuntimeInputMode`, 초안은 `subscribeRuntimeComposerDraft`). 따라서 `instanceId` 가 바뀌면 재구독 한 번으로 모드·초안이 새 terminal 것으로 갈아끼워지고, 별도의 seeding effect 가 없다. `laymux.desktop.inputMode` 기본값은 terminal 별 **최초 읽기 시점에 고정**된다 — 다른 pane 에서 기본값을 바꿔도 이미 살아 있는 pane 의 모드를 소급해서 뒤집지 않으며, 외부 store snapshot 이 render 마다 흔들리지 않는다. output protocol readiness 도 boolean 이 아니라 "준비를 알린 terminal id" 로 들고 있어 terminal 을 갈아끼우면 파생으로 곧바로 미준비가 된다(issue #567). Remote page는 별도 키 `laymux.remote.inputMode`를 사용하고 저장값이 없으면 포인터 종류와 관계없이 composer를 기본으로 하며([ADR-0265](../adr/0265-pc-remote-first-use-defaults.md)), 초안/현재 모드는 마찬가지로 runtime terminal Map에만 둔다.

전송한 초안은 `pushComposerHistory`로 **scope 버킷별** runtime Composer history(oldest→newest, runtime Map, 버킷당 최대 200개)에 쌓인다. 버킷 키는 `composerHistoryScopeKey(scope, { terminalId, workspaceId })` 한 곳에서만 도출하며([ADR-0055](../adr/0055-composer-history-scope-setting.md)), `settings.json` 의 `terminal.composerHistoryScope`(`global`(기본)/`workspace`/`pane`)가 스코프를 고른다 — `global` → 고정 키 `global`, `workspace` → `ws:{workspaceId}`(데스크톱은 `resolveWorkspaceId` 로 해석), `pane` → `pane:{terminalId}`. workspace 를 해석할 수 없으면(dock 등) 공유 버킷으로 승격하지 않고 `pane:` 키로 좁게 fallback 한다. 읽기 세 경로와 쓰기가 모두 이 함수를 거치므로 쓰기와 읽기가 다른 버킷을 보는 일이 없다. 스코프를 바꿔도 병합·이관은 없고 다른 버킷을 읽기 시작할 뿐이며, 열려 있던 recall 목록은 버킷 키에 태깅된 상태 파생으로 자동 닫힌다(`historyScopeKey` prop). 수명은 pane 종료 시 그 pane 버킷만(`clearRuntimeComposerState(terminalId)`), 워크스페이스 삭제 시 그 워크스페이스 버킷만(`clearComposerHistoryForWorkspace`), reload 시 전부다. `global` 은 앱 전체가 200 cap 을 공유하므로 오래된 항목이 더 빨리 밀린다. cap 은 스코프와 무관하게 버킷당 200 이다. **별표를 누르지 않은 입력 내용(초안 텍스트·history 항목·자동완성 후보)은 비밀번호 등 민감정보 누출 방지를 위해 엄격히 in-memory only 다.** 이 history 는 draft 와 마찬가지로 `settings.json`·`localStorage`·`sessionStorage`·서버·로그 어디에도 영속·전송하지 않으며, WebView reload(= `clearRuntimeComposerState`)로 흔적 없이 사라진다([ADR-0029](../adr/0029-detached-terminal-input-composer.md)의 미전송 문자열 비영속 경계 유지). 이 비영속 불변식은 시크릿 문자열이 어떤 web storage/설정 스냅샷에도 새지 않음을 단언하는 회귀 테스트(`terminal-input-composer-state.test.ts`)로 고정했다. 리모트 대응판도 같은 in-memory only 원칙을 따른다. 세 가지 recall 경로가 이 history 를 읽는다. (1) shell prompt 에서 초안 edge 의 ↑/↓ 는 항목을 하나씩 draft 로 불러온다. (2) issue #504: 비어 있고 포커스된 초안에서 **Tab** 을 누르면 `selectComposerHistoryEntries`(최신순·중복 제거·최대 N개, 기본 8)로 만든 목록을 composer 위에 뜨는 listbox 로 보여준다. 각 항목은 한 줄 ellipsis 이고 목록은 `max-h` 로 제한해 화면을 과하게 가리지 않는다. 목록이 열린 동안 ↑/↓·Tab 은 선택 이동, Enter/클릭은 선택 항목을 draft 로 채우기, Escape·편집·blur 는 닫기이며, 이 동안 edge ↑/↓ recall·passthrough·Send 로 키가 새지 않는다. history 가 없으면 Tab 은 기존대로 통과한다. 이 팝업은 `settings.json`의 `terminal.composerHistoryPopup`(기본 on, 설정 UI 토글 제공)로 끌 수 있다. (3) issue #505: 비어 있지 **않은** 초안을 타이핑하는 동안 `selectComposerAutocompleteSuggestions`(prefix 대소문자 무시·최신순·중복 제거·`value` 완전 일치 제외이되 `send:true` 별표는 유지·최대 N개, 기본 8)가 만든 자동완성 dropdown 을 같은 자리에 띄운다. 초기에는 강조 항목이 없어(activeIndex=−1) 일반 Enter 는 계속 Send 로 동작하고, **Tab** 은 강조 항목(없으면 첫 항목)을 draft 로 채운다. ↑/↓ 로 항목을 오가며 강조가 생기면 그때만 Enter 가 선택으로 바뀌고, 강조가 없는 ↑ 는 소비하지 않아 edge recall 로 넘어간다. Escape·편집·blur 는 닫기이며, 짧은 클릭/pointerup 은 채우기(또는 `send`면 Send)다. 자동완성 행 롱클릭은 별표 편집기를 열고 그 pointer의 수락/Send를 소비한다. (2)의 Tab 팝업(빈 초안)과 (3)의 자동완성(비어 있지 않은 초안)은 초안 길이로 상호 배타이므로 키/포커스가 충돌하지 않는다. 자동완성은 `settings.json`의 `terminal.composerAutocomplete`(기본 on, 설정 UI 토글 제공)로 끌 수 있다. history 데이터 자체는 설정이 아니므로 두 키 모두 기능 on/off 만 담고 기록은 runtime 에 남긴다. Remote 정적 페이지도 (2)·(3) 두 recall 경로를 동일 설계로 이식했다. 추가로 Remote 는 소프트 키보드에 Tab 키가 없는 터치 기기를 위해, 비어 있는 초안 editor 를 탭/클릭해도 (2)와 같은 recall 팝업을 연다(포인터 제스처라 키바인딩 규칙([api-contracts §15.5](./api-contracts.md)) 밖이고, history 가 없거나 토글 off 면 no-op). 다만 Remote 는 일반 `settings.json` 을 노출하지 않으므로 두 토글과 공유 스코프는 surface-local `localStorage`(`laymux.remote.composerHistoryPopup`/`laymux.remote.composerAutocomplete`, 기본 on / `laymux.remote.composerHistoryScope`, 기본 `global`)에 두고, history 는 페이지 runtime Map 에만 유지해 데스크톱과 동일하게 in-memory only 로 어떤 storage 에도 영속·전송하지 않는다([api-contracts §13.4](./api-contracts.md)).

[ADR-0226](../adr/0226-composer-stars-are-host-global-persistent-state.md)·[ADR-0229](../adr/0229-composer-starred-entries-have-label-and-send.md)의 별표는 이 비영속 history와 분리된 명시적 opt-in이다. `settings.json`의 backend-owned `terminal.composerStarredEntries` ordered set은 workspace·pane·history scope와 무관하게 호스트 전역이며 Desktop과 Remote가 공유한다. 원소는 `{ value, label, send }`이고 정체성은 `value`다. 레거시 `string[]`은 `{ value, label: "", send: false }`로 읽는다. 전용 IPC/lease-gated Remote API만 원자적으로 바꾸고, 일반 settings patch는 이 필드를 read-only로 본다. 자동완성은 value 또는 label prefix가 맞는 별표 항목을 최신 별표부터 먼저 내고 남은 자리를 현재 runtime history로 채운다. exact-query 제외는 `value` 완전 일치에만 적용하며 `send: true`인 별표는 초안이 이미 그 값이어도 남아 고르면 Send 한다. 수락(클릭·Tab·강조 후 Enter)은 초안을 `value`로 채운 뒤 `send`가 켜져 있고 비활성·전송 중이 아니면 이어서 Composer Send를 호출한다. 별표 버튼은 행 선택과 Composer editor focus를 건드리지 않는다. 최대 200개, `value` UTF-8 16 KiB, `label` 256바이트이고 한도 초과는 기존 별표를 지우지 않고 거절한다. Desktop Settings의 Terminal 섹션 행 클릭과 자동완성 롱클릭(500ms)은 같은 편집기를 열고 전용 mutation으로 즉시 저장한다. 롱클릭 문턱을 넘긴 pointer는 그 제스처의 pick/Send를 소비한다. 아직 별표가 아닌 행을 저장하면 별표가 된다. 별표 목록은 일반 Settings 초안에서 분리되어 다른 미저장 Terminal 설정과 서로 되돌리지 않는다.

호스트는 전용 별표 mutation마다 runtime revision을 증가시킨다. Remote는 claim 응답 이후 heartbeat 성공 시 현재 revision으로 조건부 GET을 보내고, revision이 달라 전체 목록이 온 경우에만 메모리 목록을 교체한다. 더 낮은 revision 응답은 무시하므로 늦은 초기 GET이 이후 토글 결과를 되돌리지 않으며, lease 종료 시 목록과 revision을 함께 버린다.

Remote의 editor 탭 recall에는 **탭 시작 전에 소프트 키보드가 이미 열려 있어야 한다**는 전제가 추가된다. 키보드를 올리는 첫 탭은 DOM focus가 남아 있더라도 목록을 열지 않는다. 판정은 `VirtualKeyboard.boundingRect`를 우선하고, 이를 지원하지 않는 브라우저에서는 현재 폭별 keyboard-closed `VisualViewport` 최대 높이와 현재 높이의 유의미한 차이를 사용한다. 열린 history/자동완성 목록 상태에서 editor의 빈 영역을 탭하면 초안은 유지한 채 목록만 닫는다([ADR-0207](../adr/0207-remote-composer-recall-observes-soft-keyboard-geometry.md)).

Remote 표시 설정도 같은 surface-local 원칙을 따른다([ADR-0209](../adr/0209-remote-display-preferences-are-device-local.md)). Remote page는 `localStorage["laymux.remote.displaySettings"]`의 terminal/composer/menu 글자 크기, Composer Idle·Focused·Active 불투명도, checkpoint 예산, wheel/fast wheel/한 손가락/두 손가락 민감도를 연결·lease와 무관하게 읽고 쓴다. 저장 성공 시 CSS 변수와 xterm options·터치 지역 상태를 즉시 갱신하고 guarded fit/resize로 현재 surface에 반영한다. 다음 최초 attach에는 로컬 checkpoint 예산을 `historyKib`로 보낸다. 이 경로는 PC settings store, settings bridge/CAS, `/remote/v1/display-settings`, Android E2E HTTP allowlist를 사용하지 않는다. host appearance는 font family/assets·cursor·theme만 보내고, font binary 및 widget 데이터 공개 여부는 PC의 `settings.remote.serveTerminalFont`·`settings.remote.widgets`가 계속 결정한다. Composer opacity 상태는 disabled → active raw 상태(초안·추천·IME·전송) → DOM focus → idle 우선순위를 한 계산 함수에서 `data-opacity-state`로 투영한다.
...
새 Remote 문서는 포인터 종류와 관계없이 Composer, Main/Keys 버튼 100%, Composer 불투명도 50/70/100%, 탐색 너비 300px, 최초 checkpoint 예산 8 KiB를 기기 로컬 기본값으로 사용한다([ADR-0265](../adr/0265-pc-remote-first-use-defaults.md)). Keybar의 기본 Main은 `^C · Q · Esc · /clr`와 `Keyboard · Keys · Send`, Expanded는 왼쪽 `Composer · NavPad · Notifications oldest · Tab`와 오른쪽 `^U · ^L · ^T · DPad · PgUp · PgDn · Attachment`다. Q는 내장 raw key이고 `/clr`는 `submit:true`인 편집 가능 사용자 키다. 두 플로팅 패드는 기본 활성으로 왼쪽 탐색·알림/오른쪽 방향에 놓고, 일반 플로팅 버튼은 비워 둔다. 초기화는 유효한 저장 `direct`·꺼진 패드·빈 사용자 키/완전한 빈 행·커스텀 순서/좌표를 보존한다. 행 구조가 잘못되어 기본 배치로 돌아갈 때는 현재 사용자 키 목록으로 `knownIds`를 계산해 존재하지 않는 기본 사용자 키 액션을 제거한다. 설정 로드는 키 입력을 실행하지 않으며 PC Direct·위젯 빈 배치/꺼진 status line·에이전트 세션 복원 기본값과 호스트 연결·보안/공개 정책을 변경하지 않는다.
...
tracked xterm write는 각 요청에 `replay` 또는 `live` source를 붙이고 xterm callback FIFO와 같은 순서로 parse context와 output generation을 유지한다. cache·snapshot·복원용 합성 write 중 xterm이 만든 terminal reply는 폐기하고, contiguous live PTY delta를 parse하며 만든 reply만 Tauri `write_terminal_protocol_reply(id, generation, data)`로 같은 generation의 PTY FIFO에 전달한다. backend는 현재 `PtyHandle` generation과 다른 요청을 write 전에 거절한다. 이 전용 경로는 human-control owner permit을 요구하지 않으므로 Remote lease가 활성 또는 Local owner 상태가 아직 unknown이어도 OSC 10/11 같은 emulator response가 유실되지 않는다. Rust가 같은 query에 중복 응답하거나 response byte allowlist로 출처를 추정하지 않는다. 단, [ADR-0141](../adr/0141-conpty-bootstrap-da-reply.md)의 Windows PTY bootstrap 예외는 raw current-generation output에서 exact `ESC[c`를 최근 관측한 one-shot guard가 있을 때, generation을 보존한 attach replay가 만든 고정 xterm의 exact `ESC[?1;2c`만 `write_terminal_bootstrap_protocol_reply`로 허용한다. guard는 관측 query 수와 claim한 reply 수를 결속해 live가 먼저 오면 bootstrap을 거절하고, bootstrap이 먼저 오면 새 query 없는 뒤늦은 exact live reply를 폐기한다. 다른 replay reply와 Windows 외 handle은 계속 폐기하며 Rust는 DA 응답을 합성하지 않는다. 또한 ADR-0131의 backend-validated native Windows Codex 복귀 guard는 이 origin 분류가 끝난 전용 live 경로에서만 선행 exact startup query와 대응하는 최초 OSC 10/11 RGB reply를 제거한다. 두 예외는 출처 추정이나 대체 응답 생성이 아니라 generation-local query/lifecycle 증명을 추가하는 경계다.
...
keyboard·IME·paste·mouse·focus에서 생기는 human input은 xterm data emission 전에 막는다. remote owner snapshot을 알기 전과 Remote 활성 중에는 xterm `disableStdin`을 켠다. 고정 버전 xterm 6.0.0의 ESM·CommonJS 번들은 postinstall에서 gate를 `disableStdin && wasUserInput`으로 좁혀 human data만 차단하고 parser-generated reply는 유지하며, exact pattern이 바뀌면 설치를 실패시킨다. custom key handler와 structured paste/composer도 같은 fail-closed 상태를 확인한다. 공개 `onData`가 버리는 `wasUserInput` origin은 고정 버전 private `CoreService.onUserInput` 동기 신호로 복원해 timer로 지연된 IME commit도 human으로 분류하고, wrapper capture event는 xterm이 user bit 없이 만드는 focus/mouse report를 보완한다. private origin adapter가 없으면 ambiguous live data도 human으로 fail-closed한다. replay reply는 원칙적으로 폐기하고 신뢰 가능한 live parser reply만 전용 `write_terminal_protocol_reply`를 사용한다. 유일한 replay 예외인 Windows bootstrap Primary DA도 frontend allowlist만으로 쓰지 않고 recent exact query·current generation·one-shot을 backend가 다시 증명한다([ADR-0141](../adr/0141-conpty-bootstrap-da-reply.md)). human route는 전송 직전에 Local owner 상태를 다시 확인한 뒤 owner-gated `write_to_terminal`을 사용하며 backend permit이 최종 권한 경계다([ADR-0054](../adr/0054-xterm-human-and-protocol-data-origin.md)).

xterm DEFAULT mouse encoding의 고정 6바이트 `CSI M Pb Px Py`는 `onData`가 아니라 `onBinary`로 나온다([ADR-0216](../adr/0216-xterm-legacy-mouse-binary-input-boundary.md)). TerminalView는 current output generation을 캡처하고 각 binary-string code unit을 한 바이트 값으로 직렬화해 desktop 내부 `write_terminal_binary_input(id, generation, data)`에 정확히 한 번 제출한다. backend는 exact report 형식·generation·Local owner를 검증한 뒤 기존 human-input FIFO에 `Vec<u8>` 그대로 enqueue한다. Linux PTY는 high-bit payload도 보존한다. Windows ConPTY는 UTF-8 채널이므로 payload 중 `0x80..0xff`가 있으면 잘못된 좌표로 변환하지 않고 enqueue 전에 거부하며, `0x00..0x7f`만 그대로 전달한다. SGR mouse의 `onData` 경로는 변하지 않는다. binary mouse attempt는 human 입력 전달 counter·coalesced 실패 알림에 포함하지만 parser reply/replay와 Direct 최근 입력 line model에는 포함하지 않는다.

Composer와 clipboard paste는 Tauri `write_terminal_input(id, text, submit)` 또는 Remote `/remote/v1/terminals/{id}/input`으로 intent만 보낸다. Composer Send는 항상 `submit=true`, Direct clipboard paste는 `submit=false`다. 비동기 smart-paste 결과를 PTY로 보내기 직전에도 현재 모드가 `direct`인지 다시 확인하므로, clipboard 조회 중 Composer로 전환된 입력은 draft 경계를 우회하지 않는다. Rust가 줄바꿈을 CR로 정규화하고 authoritative bracketed-paste 상태가 켜져 있으면 text 부분만 `CSI 200~`/`CSI 201~`로 감싼 뒤 선택적 submit CR을 붙인다. 사용자 키·SGR/SGR-pixels mouse·focus reporting·Remote soft key는 기존 raw write 경로를 사용한다. 단, `Send Enter`를 켠 Remote 사용자 키는 파싱된 sequence를 text로 `/input`의 `submit:true`에 보내 기존 백엔드 제출 처리를 재사용한다. raw 큐를 먼저 flush하고 동일한 클라이언트 write chain에서 직렬화한다([ADR-0234](../adr/0234-remote-user-key-explicit-submit.md)). Local/Remote raw write, structured write, resize는 모두 backend human-control owner permit을 등록한 뒤 PTY table lock을 놓고 실제 I/O를 수행하므로 frontend lease status는 UX fail-closed 장벽이고 backend permit이 최종 권한 경계다. live parser가 생성한 terminal protocol reply만 위 전용 non-human 경로를 사용한다.

Remote 파일 첨부는 browser `File`/긴 clipboard text를 곧바로 PTY에 넣지 않는다([ADR-0181](../adr/0181-remote-terminal-file-attachments.md)). page가 파일 하나씩 base64 JSON으로 `/remote/v1/terminals/{id}/attachments`에 보내면 Rust는 active controller non-PTY mutation permit을 먼저 등록하고 terminal profile을 복사한 뒤 AppState lock 없이 blocking worker에서 종류·`remote.attachmentMaxMib` 상한(1~10 MiB)·그 64배의 cache quota·1,024-file 상한을 검증해 사용자별 private cache에 UUID `create_new` 파일로 저장한다. Android System WebView가 native chooser 복귀 때 `FileList`를 채우고도 `change`를 누락하는 호환 경로는 window focus 복귀 직후의 bounded retry가 같은 선택을 처리한다. retry identity는 chooser open 시점의 generation·lease·terminal snapshot을 소유하고 disconnect·새 chooser·terminal 변경 때 timer와 input을 함께 폐기하므로 이전 선택이 새 lease로 넘어가지 않으며, upload in-flight guard와 완료 시 input clear가 정상 `change` 경로와의 중복을 막는다. 응답 path는 WSL profile이면 guest path로 변환된다. page는 Composer의 캡처한 selection/revision에 path를 삽입하거나 Direct의 기존 structured `submit=false` 경로로 path를 전달한다. 업로드 중 editor/Send/attach를 잠그고 terminal/lease가 바뀐 응답은 입력에 반영하지 않는다. 하나의 attempt와 `AbortSignal`은 upload, Direct path 입력, 긴 paste 실패 fallback 입력을 함께 감싼다. disconnect와 non-Android BFCache `pagehide` release는 lease release를 기다리기 전에 chooser와 이 attempt의 controller/identity를 폐기하며 각 await 뒤 identity를 재검증하므로 이전 attempt의 catch/finally나 늦은 입력 completion이 새 lease의 UI를 다시 잠그거나 입력 fallback을 만들지 않는다. release drain 중 새 claim이 시작되면 이전 Exit/Desktop 전환 continuation도 claim revision 불일치로 폐기한다. UTF-8 5 KiB 초과 paste는 `pasted-text.txt`로 이 흐름에 들어가며 upload busy이면 raw input 대신 거절하고 실제 업로드 실패 시에만 원문 paste로 되돌아간다. 파일 저장과 PTY 입력은 분리되어 전자만 성공한 orphan은 startup age cleanup이 회수한다.
...
### 8.14 조합 commit 과 세대별 input·keypress 경합 (issues #527, #660, xterm.js #6049)

xterm 의 `CompositionHelper._finalizeComposition(true)` 는 확정 텍스트를 즉시 보내지 않는다 — 조합 범위를 캡처하고 `_isSendingComposition = true` 로 표시한 뒤 `setTimeout(0)` 안에서 textarea 최종값을 읽는다. Windows WebView2에서는 그 사이 `input(insertText)`가 legacy `keypress`보다 먼저 오거나 단독으로 올 수 있다. input을 즉시 보내면 finalizer가 같은 candidate를 다시 보내고, 단일 pending 상태를 공유하면 timer 전에 연속된 조합 세대가 서로를 덮어 문자 유실·순서 변경을 만든다([ADR-0093](../adr/0093-xterm-composition-keypress-reconciliation-owner.md), [ADR-0062](../adr/0062-composition-commit-keypress-race.md) 대체). 또한 Windows TSF IME가 이미 전송된 문자열이 남은 helper textarea 전체를 새 조합값으로 교체하면 캡처한 시작 위치가 낡아, 확정 문자열의 앞부분 또는 전체를 substring 단계에서 잃는다([ADR-0230](../adr/0230-xterm-compositionend-data-recovers-replaced-textarea.md)).

- **상태 소유자는 CompositionHelper 하나다**: exact bundle patch가 xterm 6.0.0 ESM/CJS의 CompositionHelper에 generation FIFO를 둔다. 각 delayed finalizer는 캡처 범위·다음 composition 직전의 textarea 문자열 스냅샷·이미 전송된 길이·`compositionend.data` 확정 후보·순서 있는 input/keypress 관측 목록·취소/완료 상태를 별도 record로 소유한다. 새 `compositionstart`는 직전 generation의 candidate 문자열 자체를 먼저 고정하므로 다음 조합의 전체 textarea 교체가 이전 timer에 섞이지 않는다.
- **input·keypress·finalizer가 같은 generation에 합류한다**: CoreBrowserTerminal의 `_inputEvent`와 `_keyPress`는 계산한 텍스트를 helper에 먼저 제안한다. pending generation이 있으면 즉시 PTY로 보내지 않고 최신 미완료 record에 보류한다. 이때 소비한 legacy `keypress`는 `cancel(event, true)`로 기본 textarea 삽입까지 막는다 — 관측만 보류하고 DOM 기본 동작을 남기면 WebView2가 같은 확정 음절을 textarea candidate에 다시 넣어 finalizer가 `면 면`처럼 오염된 전체 문자열을 보낸다. pending generation이 없으면 ordinary input/keypress를 기존처럼 즉시 보낸다. timer는 자기 record만 완료하고 FIFO 선두의 완료 세대부터 한 번씩 flush한다. 일반 keydown immediate finalize도 pending FIFO를 먼저 비운 뒤 자기 키를 보낸다.
- **textarea candidate와 확정 이벤트를 먼저 합친 뒤 관측을 하나씩 조정한다**: finalizer는 취소되지 않은 generation의 textarea slice와 `compositionend.data`를 기존 양방향 포함 및 suffix-prefix 최장 overlap 함수로 합쳐 accumulator를 만든다. 시작점이 스냅샷 범위를 넘고 스냅샷이 확정값을 포함하면 전체 스냅샷을 교체 candidate로 사용한다. 따라서 전체 교체나 비어 있는 스냅샷으로 slice가 비거나 확정값의 suffix만 남아도 이벤트의 전체 확정값이 복구되고, propagation 중 붙은 비조합 suffix는 확정값 뒤에 보존된다. `compositionend.data`가 비었거나 없는 합성 호출은 기존 slice로 떨어진다. CoreBrowserTerminal의 blur handler는 textarea를 비우기 전에 현재 조합을 event-data 비허용 상태로 만들고 이미 끝난 pending generation을 모두 FIFO flush한다. 각 세대의 slice·event data·input/keypress 관측이 clear 전에 정확히 한 번 전송되므로 §8.16의 TerminalView fallback은 pending이 해소된 것을 보고 다시 보내지 않는다. 아직 `compositionend`가 없는 진행 중 조합만 fallback이 맡는다. blur 뒤 늦게 생성된 generation은 취소되고 새 input·keypress 관측을 받지 않으므로 refocus 뒤 일반 입력은 즉시 경로로 전달된다. candidate에 이미 포함된 첫 input/keypress 관측이 정렬 기준을 세울 때까지 unmatched 관측은 도착 순서대로 shortest-merge prefix로 모으고, 기준이 생기면 prefix를 candidate와 병합한 뒤 나머지 관측을 하나씩 같은 함수에 fold한다. 기준 뒤에는 양방향 overlap 길이가 같은 관측을 accumulator 뒤쪽에 둔다. 끝까지 기준이 없으면 prefix 전체를 기존처럼 candidate와 병합한다. 관측끼리 무조건 먼저 합치지 않으므로 separator `" "`와 propagated syllable `"면"`의 출처 경계가 사라져 `" 면"`이 된 뒤 candidate `"면 "`에 덧붙는 일을 막으면서, candidate `"한a"` 뒤의 비중첩 단문자 관측 `"a"`, `"b"`는 `"한ab"`, anchor 없는 `"a"`, `"b"` 뒤 candidate `"한"`은 기존처럼 `"ab한"`으로 유지한다. 다만 겹치는 다문자 관측이나 의도적 반복 occurrence의 엄격한 시간 순서는 ADR-0093과 마찬가지로 보장하지 않는다. queue가 빌 때까지 `_isSendingComposition`은 유지된다.
- **229 textarea-diff는 스냅샷 이후 조합이 시작되면 침묵한다**: Windows IME는 `compositionstart`보다 먼저 `keydown` 229를 보내 `_handleAnyTextareaChanges`의 `setTimeout(0)`을 예약한다. 메인 스레드가 그 창을 수 초로 늘리면 `compositionend`가 먼저 끝나고, stock 타이머는 `!_isComposing`만 보고 같은 음절을 finalizer와 따로 보낸다. `_isSendingComposition`만 막으면 이후 일반 keydown의 immediate finalize가 플래그를 내린 뒤 타이머가 `가`를 다시 보낸다. 패치는 `compositionstart` epoch가 예약 시점과 다르거나 `_isComposing`/`_isSendingComposition`이면 보내지 않는다. 조합이 시작되지 않은 229 삽입은 그대로 보낸다([ADR-0164](../adr/0164-ime-textarea-diff-skip-while-sending.md)).
- **외부 guard는 없다**: TerminalView는 `compositionend` 시작 위치를 캡처하거나 input/keypress를 억제하지 않는다. 조합 관측 보류와 consumed keypress의 기본 동작 취소는 같은 candidate를 소유한 patched CompositionHelper/CoreBrowserTerminal 경계에서 함께 결정한다. `xterm-pending-composition.ts`의 private 접근은 issue #555 blur fallback이 deferred send 중복을 피하는 `_isSendingComposition` 하나로 축소됐다.
- **설치가 계약 관문이다**: `ui/scripts/patch-xterm-reflow.mjs`는 pristine bundle을 선행 keypress patch 형태로 만든 뒤 generation correction과 229 textarea-diff skip을 적용한다. 이미 선행 patch 또는 최종 patch인 설치도 각각 upgrade/idempotent 성공한다. 두 번들의 helper state·`compositionend.data` handoff·input/keypress owner·consumed keypress 강제 취소·delayed/immediate flush·composition epoch·sending 중 textarea-diff skip이 모두 맞지 않으면 postinstall이 실패하고 조용한 부분 적용은 없다. Remote CJS 번들은 wheel patch와 이 textarea-diff skip·epoch만 적용한다.
- **결정적 테스트는 실제 xterm 대상이다**: 기존 6개 조합 이벤트열과 `compositionend → input → keypress`, separator 뒤 propagated keypress의 조건부 기본 삽입, input-only, timer 전 연속 두 generation, input/keyPress 뒤 일반 keydown 교차, 229가 compositionstart보다 먼저 온 뒤 compositionend가 timer보다 앞서는 stall, 잔여 textarea 전체가 새 확정값으로 교체되는 순서, 연속 두 확정 및 확정 뒤 일반 입력이 blur와 만나는 FIFO 결합을 `Terminal.onData`까지 검증한다. pending이 없는 ordinary input·keypress와 조합 없는 229 삽입도 즉시 전송을 고정한다. 순수 mock 판정으로 이 상태 전이를 대체하지 않는다.
- **인간 입력 전달 실패**: local owner의 xterm `onData`, DEFAULT mouse `onBinary`와 IME blur fallback은 각 raw 입력을 정확히 한 번만 해당 human-input command로 제출한다. IPC 거절은 Rust 미수락의 증명이 아니므로 재전송하지 않으며, terminal별 `attempts`/`succeeded`/`failed` 및 byte 합계를 `frontend.inputDelivery`로 내보낸다. `onData`는 UTF-8 길이, `onBinary`는 code unit 수가 전송 byte 수다. completion은 제출 session token과 one-shot attempt token이 현 entry와 같을 때만 한 번 정산한다. close 요청은 backend close IPC를 기다리지 않고 즉시 entry를 fence해 close/replacement 뒤 late completion을 버리며, 반복 실패의 수치는 모두 누적하되 action-required 오류 알림은 session당 하나로 coalesce해 사용자가 새 입력을 명시적으로 만들게 한다. protocol reply와 owner permit에 의해 막힌 입력은 이 카운터에 포함하지 않는다([ADR-0096](../adr/0096-terminal-human-input-write-failure-observability.md), [ADR-0216](../adr/0216-xterm-legacy-mouse-binary-input-boundary.md)).
...
- **경계를 종류별로 나눈다**: `opened`/`focused`/`viewportScrolledUp`은 실제 가시성·뷰포트 조건이므로 composition보다 우선한다. 반면 `syncOutputActive`와 `isAltBufferActive`는 앱/non-composition caret 좌표의 신뢰·소유권 조건이므로 composition 아래에 둔다. 활성 조합은 컨트롤러가 캡처한 앵커를 소유해 sync frame 중간의 raw buffer 좌표를 읽지 않고도 정확히 그릴 수 있고, 이것이 xterm native composition view가 꺼진 동안의 유일한 preedit 표면이다.
...
- **carry-over 앵커는 체인 시작에서 유도하고, 행이 바뀌면 원점을 다시 잡는다**: #546 수정(PR #548)이 넣은 `echoed` 판정에 결함이 있었다. 그 판정은 "shadow 가 움직였다" 를 "shadow 가 확정분을 다 따라잡았다" 로 취급한다. Codex 실측: `ㄱㄱㄱ` 에서 두 번째 carry-over 시점에 PTY 는 **첫 자모만** 에코했으므로 shadow 는 열 2 인데 확정분은 두 자모(4 셀)였다. 그걸 채택하며 산술로 맞춰둔 4 가 **뒤로 끌려가** 세 번째 자모가 두 번째 위에 겹쳐 그려졌고, 프리뷰가 `ㄱㄱ` 에 멈춘 것처럼 보였다.
  ...
  - **앱이 자기 입력상자를 소유하면 산술은 틀린다 — 정착한 커서를 채택한다 (issue #569)**: 앵커 산술은 **터미널이** 텍스트를 어디 놓을지를 예측한다. 자기 입력상자를 소유하는 앱은 그럴 의무가 없다 — Codex 는 상자 안에서 자기 폭으로 랩하고 연속행을 **2칸 들여쓴다**. 실측(cols 150): `도` 가 줄 끝을 채우면 `레미` 가 다음 행에 `"  레미"` 로 온다. 산술은 열 0 을 가리키므로 프리뷰가 2칸 왼쪽에 그려지고, 다음 음절은 앞 음절 **위에 겹쳐** 그려져 지운다(`도시라` 는 `도`⏎`라시` 가 된다).
    ...
    - **정착하지 않은 커서는 아예 주지 않는다**: DEC 2026 프레임 열림, save/restore repaint 진행 중, **`parkPending`**(legacy Codex 는 `?2026l` 을 cursor가 footer에 있는 채로 닫고 진짜 park를 ~15ms 뒤 다음 청크에 보낸다), alt 버퍼(행 좌표계가 다르다), sync-output. Codex 0.145의 strict `?25h` → position → `?2026l`과 Codex 0.150+의 position → `?25h` → `?2026l` tail은 reset buffer cursor 자체가 final park이므로 `parkPending`을 세우지 않고 이 정착 경로를 즉시 연다([ADR-0076](../adr/0076-codex-in-frame-cursor-park.md), [ADR-0221](../adr/0221-codex-position-first-in-frame-cursor-park.md)).
    ...
    - **DEC 2026은 가시 프리뷰 수명을 연장하지 않는다.** `pending-finalize`의 논리 상태가 남아 있어도 `active=false`가 되면 열린 sync frame이나 `parkPending`의 caret 동결보다 먼저 preview를 지우고 helper 앵커를 원복한다. 다음 carry-over를 받을 내부 체인은 예정된 deferred reset까지 유지한다.
...
실제 `Terminal`과 controller를 jsdom에 함께 띄워 측정한 현재 계약:

| 순서 | xterm blur 처리 | 최종 결과 |
| --- | --- | --- |
| blur만 (`compositionend` 없음) | 완료 generation이 없어 **onData 없음** | TerminalView가 보이던 조합을 확정 |
| `compositionend` → blur → timer | pending FIFO를 clear 전에 동기 flush | `onData ["가"]`, fallback 없음 |
| stale start → `compositionend` → `input("a")` → blur | 전체 교체값과 관측을 함께 동기 flush | `onData ["가a"]`, suffix 역전·유실 없음 |
| `compositionend` → timer → blur | 이미 pending이 없음 | 기존 `onData ["가"]`만 유지 |
| 조합 없는 잔여물 | — | blur에서 **무조건** 비움 |

- **xterm은 끝난 세대와 진행 중 조합을 나눈다.** CoreBrowserTerminal의 blur listener는 먼저 완료된 generation FIFO를 flush한 뒤 textarea를 비운다. 아직 `compositionend`가 없는 조합은 generation이 없으므로 xterm이 보내지 않고, 뒤따르는 TerminalView listener가 프리뷰를 확정한다. blur 뒤 늦은 `compositionend`는 취소 세대로 소비하되 그 세대가 새 관측을 받지 않게 하므로 refocus 뒤 일반 입력은 즉시 전달된다.
- **phase는 판별자가 아니다.** WebView2 + Windows IME가 blur 전에 `compositionend`를 보내면 controller phase는 `pending-finalize`지만, 그 사이 xterm의 앞선 blur listener가 전송을 끝낼 수 있다. controller는 listener 실행 뒤의 xterm pending 플래그가 거짓이면 `lastFinalizedText`를 다시 보내지 않는다. 참으로 남는 구형/호환 경로에서만 그 값을 fallback한다.
- **carry-over도 소유권을 나눈다.** 같은 tick에 끝난 세대들은 xterm이 FIFO로 먼저 보내고, 아직 진행 중인 최신 조합은 controller가 `state.text || latestCompositionDisplayText`로 보낸다. xterm pending이 남는 호환 경로에서는 `lastFinalizedText`와 진행 중 조합을 이어 보내 기존 유실 방어를 유지한다.
- **controller의 텍스트 출처는 textarea가 아니다.** xterm이 자기 blur handler에서 값을 비우므로 controller는 캡처한 확정값과 동기 프리뷰만 사용한다. 완료 세대의 textarea snapshot·event data·input/keypress 관측은 xterm generation record가 소유한다.
- **리셋은 유지한다.** 진행 중 blur는 xterm의 `_isComposing`을 true로 남기므로 프리뷰가 다음 포커스 사이클까지 살아 있는 것을 막아야 한다.
- **리더가 `null`이면 "pending 아님"으로 떨어진다.** private shape가 바뀐 빌드에서 포커스 이동마다 중복하는 것보다 fail-closed 유실이 낫고, xterm 설치 계약 테스트가 같은 필드를 읽어 배포 전에 실패한다.
- **결정적 테스트는 양쪽 소유자를 함께 구동한다.** 진행 중 blur, end 뒤 blur·refocus, 연속 generation, 확정 뒤 보류된 일반 입력을 실제 `Terminal.onData`와 controller `onCommit` 결합 결과로 고정한다.
...
- **연속 세대의 blur 경계도 FIFO다.** 한 태스크에 `compositionend`가 여러 번 와도 xterm은 generation별 스냅샷을 소유한다. blur가 타이머들보다 먼저 오면 xterm blur handler가 모든 완료 세대를 동기적으로 FIFO flush하고 pending을 내린다. 이어 실행되는 TerminalView fallback은 확정 세대를 다시 보내지 않으며, controller에 별도 다중 슬롯을 만들지 않는다.
...
- **네 분기 판정 (이 이슈 종료 시점)**:
  ...
  | `syncOutputActive`   | DEC 2026 프레임 단위, 입력 중 누적 가능 | **고쳤다** (조합이 우선, non-composition caret만 동결)            |
  | `!focused`           | 다른 곳을 보는 중                       | 숨김 유지 — **#556 이 blur 를 확정으로 처리하므로 유실이 아니다** |

  구조 진단(CSS 무조건 hide × 렌더러 하나)은 참이지만 결론은 "일괄 차단" 이 아니라 **분기마다 의미를 나눠 판정**하는 것이다. 실제 가시성 조건인 scrollback·focus는 숨김을 유지하고, caret 소유권 조건인 alt buffer·sync output은 활성 조합보다 아래에 둔다. sync frame은 WSL PTY 경계에서 여러 IME event에 걸칠 수 있으므로 단순한 한-frame 깜빡임으로 취급하지 않는다(§8.5).
...
- **sync raw reason이 xterm native cursor만 보호한다.** `?2026h` parser handler가 `syncOutputCursorGateActive=true`를 먼저 적용하므로 직접 DOM paint는 새 내용을 그릴 수 있어도 `isCursorHidden`을 읽어 미확정 native cursor는 만들지 못한다. Laymux가 소유한 활성 composition preview·composition caret·helper 앵커의 의미 상태를 동결하는 gate는 아니며, 이들은 §8.5의 컨트롤러 앵커 계약을 따른다. 이 계약은 전체 frame content atomicity를 보장하지 않는다.
...
- workspace 행의 quick-hide 버튼은 항상 DOM에 존재하고 hover 또는 `:focus-within`에서 시각화된다. Workspace selector의 quick-hide·workspace 터미널 지우기·layout 삭제·workspace 닫기는 같은 파괴적 action 규칙을 따른다([ADR-0211](../adr/0211-workspace-selector-destructive-actions-require-two-activations.md)). `workspaceSelector.confirmDestructiveActions`는 기본 `true`이며, 켜져 있으면 첫 활성화가 버튼 로컬 confirmation 상태와 danger 표시/“다시 클릭” 접근성 이름만 만들고 같은 버튼의 두 번째 활성화에서 기존 domain action을 실행한다. 포인터 이탈·blur·Escape·메뉴/컴포넌트 닫힘은 confirmation을 취소한다. 설정을 끄면 첫 활성화가 기존 action을 바로 실행한다. 숨김 실행 뒤에는 최근 action을 5초 Undo snackbar로 되돌릴 수 있다. Workspace 키바인딩과 Automation·MCP·Remote 호출은 클릭 컨트롤이 아니므로 이 UI gate를 거치지 않는다.
...
WSL Claude 복원은 live `✳ <대화 제목>`이 프로세스 식별보다 먼저 도착할 수 있다([ADR-0252](../adr/0252-claude-idle-before-app-identification.md)). `useSyncEvents`는 아직 이름이 확인되지 않은 pane의 유효한 live 유휴 타이틀 하나만 generation·appSession·lastUserInputAt과 함께 보류한다. 실제 수신 시 activity 순서를 검사하고 attach 대기 중에도 보존한다. 큐 재생은 관측을 다시 수집하거나 보류를 폐기하지 않는다. generation과 준비 상태가 확인되고 같은 범위의 Claude가 식별되었으며 다른 작업 관측이 없으면 `observeTaskTitle`로 한 번 반영한다. appSession 미확정 시에는 보류한 live 이벤트의 값을 먼저 바인딩한다. 그 사이 새 유효 live 타이틀·명령·사용자 타이틀 변경·제출 입력, generation/appSession 변경·준비된 세션의 준비 해제, 다른 앱 식별, pane 삭제·구독 해제가 있으면 폐기한다. 최초 attach 준비 대기와 시간 경과만으로는 유휴를 무효화하지 않는다. xterm의 캐시·표시 타이틀은 이 경로에 들어오지 않는다. 최초 유휴는 `idle/confirmed`가 되어 Alt+L을 허용하지만 성공이나 완료 알림을 합성하지 않으며, 아이콘은 기존 ADR-0251대로 `—`다.
...
`outputActive`는 PTY 바이트 볼륨과 DEC 2026 프레임 검출만 소비한다. OSC 133 명령 실행과 앱 작업 타이틀은 작업 관측에만 사용하며 출력 활동 타이머를 켜거나 끄지 않는다. 종료 뒤 잔여 출력 신호는 기존 2초 해제로 끝나며 종료 아이콘을 덮거나 별도 배지를 만들지 않는다.
...
#### Codex 턴 기록과 출력 활동의 분리

[ADR-0248](../adr/0248-codex-turn-lifecycle-activity.md)에 따라 `get_codex_turn_states`는 기존 pane→프로세스→TUI 대화 식별 경로를 재사용한다. PTY generation을 조회 전후 검사하고, 해당 프로세스의 현재 대화 선택 로그 ID·session ID에 귀속된 rollout만 읽는다. 복원 전용 fallback과 중복 귀속은 작업 상태 근거로 사용하지 않는다. 새 빈 세션은 idle, 파일 경로·읽기·형식을 확인하지 못하면 unknown이다.
...
- distribution 해석·검증(`is_safe_distro_name`)·실행 plumbing 은 `wsl_probe` 가 소유하고 세션 귀속 프로브(`commands/wsl_agent_session`)와 공유한다. 두 프로브는 `wsl_probe/agent-role.sh`의 역할 판정도 공유한다. Claude의 첫 실행 인자가 정확히 `--chrome-native-host`인 경우만 대화 후보에서 제외하며, argv를 읽지 못하면 후보를 유지한다. helper만 남으면 대화 실행 중 판정을 유지하지 않는다([ADR-0253](../adr/0253-wsl-claude-chrome-helper-role.md)).
...
- **두 생산자의 판정은 파생 시점 stamp 로 순서화**한다([ADR-0136](../adr/0136-activity-derivation-stamp-and-scoped-exit.md)). PTY 콜백은 타이틀 판정 직전에, 워커는 스냅샷 직전에 `activity_order::next_activity_sequence()` 를 읽어 `activitySequence` 로 싣는다. 프론트는 pane 마다 적용한 stamp 를 기억하고 그보다 오래된 **activity** 만 무시한다 — 같은 이벤트의 title·메시지·출력 신호는 항상 적용된다. watermark 는 수락한 모든 판정에서 전진한다(값이 그대로여도).
- **PTY 세대가 교체된 pane 의 판정은 백엔드에서 폐기**: 워커가 스냅샷 전후로 세대를 읽어, 달라진 pane 은 diff 포함 여부와 무관하게 발행에서 빼고 발행 기록에서도 지운다(같은 앱으로 재시작하면 diff 가 침묵하므로 기록만 남으면 새 instance 가 60초를 기다린다). 같은 pane 의 종료 전이도 건너뛴다 — teardown 이 이미 정리한다.
- **세대 확인 → 종료 정리 → 발행은 terminal catalog 락 아래 한 덩어리**: create(중복 검사·세대 예약)와 close(모든 id-keyed 정리)가 쓰는 그 락이 lifecycle 직렬화 지점이다. 확인 직후 시작된 close 와 교차하면 close 가 지운 exit marker 를 되살린다. 감지 sweep 은 이 구간 밖이다.
- **종료 전이 전체를 소유하되 `(terminal, app)` + 감지 epoch 단위**: `InteractiveApp{app}` → 그 앱 아님 으로 바뀐 pane 마다 exit marker 기록 + 그 앱을 가리키는 grace window 제거 + 그 앱의 known 캐시·PTY 콜백 감지 플래그 해제, Claude 가 종료했을 때만 `claude_was_working`/`claude_last_working_title`/`claude_message` 정리 + 메시지 소거 이벤트 발행. 앱 간 handover 도 첫 앱의 종료로 취급하지만 후임 세션의 상태는 건드리지 않는다. 같은 앱 재실행(Claude→Claude)은 이름으로 구분되지 않으므로 `PtyCallbackState.detection_epoch`(entry 마다 증가)를 판정과 함께 읽고, 정리 시점에 달라졌으면 정리 전체를 포기한다. 사라진 pane 은 제외(teardown 이 처리). 두 종료 경로(PTY 콜백 / 워커)는 `activity::apply_interactive_app_exit` 하나를 공유하며, 콜백은 epoch 가드 없이(`None`) 호출한다.
...
- **로그인 후 복구**: 기동·조회 실패 시 워커는 유지하고 실패한 CLI·PTY는 종료한다. 다음 자동 재조회 또는 수동 refresh에서 같은 프로필·config dir로 새로 기동하므로 WSL pane 등에서 로그인한 정보를 다시 읽는다. 성공한 CLI는 유지하며, 재기동으로 연속 실패 횟수를 초기화하지 않는다. 위젯 재구독은 필요 없다([ADR-0247](../adr/0247-usage-profile-login-recovery.md)).
...
`usage.codex.profile`은 글꼴뿐 아니라 실행 환경도 선택한다. Windows에서 WSL 프로필이면 배포판·사용자·작업 경로 옵션을 보존하고 WSL Bash 안에서 app-server를 실행한다. `CODEX_HOME`은 Bash 초기화 후 위치 인자로 받아 설정하며 셸 코드에 보간하지 않는다. 잘못된 프로필·지원하지 않는 WSL 실행/관리 옵션은 실패로 반환한다. 인증 실패 후에도 공유 폴러가 다음 조회를 수행하며 새 프로세스가 로그인 정보를 다시 읽는다([ADR-0247](../adr/0247-usage-profile-login-recovery.md)).

Codex는 `/status` TUI를 파싱하지 않는다. `CodexUsageView`가 마운트된 동안 Rust command가 짧게 실행한 로컬 `codex app-server --stdio`에 `initialize`/`initialized` 뒤 `account/rateLimits/read`를 요청한다. 응답의 `rateLimitsByLimitId` primary window를 원시 usage row로 만들고, 프론트가 reset 시각과 window duration에서 elapsed bar를 계산한다. 비-Spark 행은 `Weekly limit`(좁으면 `Weekly`), Spark 행은 `Spark Weekly limit`(좁으면 `Spark`)으로 표시한다. `settings.usage.codex`가 실행 환경·font profile·600~3600초 갱신 간격·두 행의 표시 선택(하나 이상)·추가 `CODEX_HOME` 계정을 소유한다. 추가 계정은 해당 경로에서 사용자가 `codex login`을 끝낸 뒤 pane control bar에서 선택하며, Rust는 그 경로를 app-server 자식의 `CODEX_HOME` 환경으로만 전달한다. app-server listener·Codex thread·사용자 terminal에는 접근하지 않는다([ADR-0104](../adr/0104-codex-usage-app-server-probe.md)).

### 10.5.2 GrokUsageView

기동·조회 실패 후 복구는 Claude와 같은 워커 재시도 정책을 쓴다. 실패한 CLI·PTY를 종료하고 다음 조회 때 같은 프로필·config dir로 재기동하며, 최초 3회 실패는 60초 간격, 이후는 정상 600~3600초 간격이다. 로그인 후 자동 복구에 refresh나 재구독은 필요 없다. 성공한 프로브는 정상 간격 사이에 유지한다([ADR-0247](../adr/0247-usage-profile-login-recovery.md)).

Grok은 Claude와 같이 레지스트리 밖 headless PTY probe가 `grok --trust`를 띄워 `/usage`를 읽는다. 폴더 trust 프롬프트(`Do you trust the contents of this directory?` / `Yes, proceed`)도 `Grok Build`를 포함하므로, 그 화면은 ready가 아니다 — 오인하면 이어지는 ESC가 `No, quit`로 grok를 종료하고 `/usage`가 셸에 들어가 `parseFailed`가 된다. 현재 세션 크롬은 `Grok Build`를 다시 그리지 않고 `Shift+Tab:mode` / `Ctrl+x:shortcuts` 푸터만 보여 준다. 이 화면을 셸로 보면 프로브가 살아 있는 TUI에 `grok --trust`를 다시 치고 startup timeout으로 죽는다. ready는 옛 배너와 이 푸터를 함께 인정한다. 드라이빙은 Claude와 같이 `/usage`와 ENTER를 분리하고, 첫 조회 앞 ESC는 쓰지 않으며, 모달을 닫을 때도 ESC는 한 번만 보낸다. 1.0.1+ `/usage`는 `Context usage` / `Usage limit` / `Session info` 탭 모달이다. 계정 한도는 `Usage limit`에 있으므로 그 탭에서 멈추고, 옛 스트립 UI만 Right/Tab으로 `weekly`/`credits`/`payg`를 합친다. 레거시 `Monthly limit` 블록은 섹션 경계로만 쓰고 스냅샷 행을 만들지 않는다([ADR-0167](../adr/0167-grok-usage-drops-legacy-monthly.md)). 상류 `allowance_lines`는 한도 헤더·퍼센트 막대·`Credits: $x.xx`·`Pay as you go: Enabled` + `Usage: $used / $cap`를 서로 다른 행에 그리므로, 파서는 라벨 행과 다음 비어 있지 않은 값 행을 결합한다. `Resets:` 줄의 박스 문자는 버린다. weekly 행의 elapsed 막대는 Claude 주간과 같이 `lib/usage-pace.ts`가 `Resets:` / `Next reset:` 원문(`August 20, 16:13`, `Mon 9am`)과 현재 시각에서 7일 창으로 도출한다. credits·payg는 창이 없어 elapsed를 두지 않는다. 값이 있는 행이 있어야 `ready`이며, 라벨만 있고 수치가 없으면 UI `--`로 남는 조용한 오답을 내지 않는다([ADR-0156](../adr/0156-grok-first-class-agent.md)). grok가 셸로 떨어지면 같은 조회 안에서 `--trust`로 다시 띄운다. leftover `Command 'usage' not found`는 grok 미설치가 아니다 — 같은 줄에 `grok`와 missing 마커가 있을 때만 `grokMissing`이다. 조회 중 재기동 실패는 워커를 죽이지 않고 다음 cadence에서 재시도한다. 첫 기동이 `grokMissing`/`startupTimeout`/PTY 오류로 끝나도 워커 스레드는 재시도를 예약한다. 비정상 종료로 handle이 끊긴 경우에는 구독이 남은 채 refresh하거나 같은 id로 재구독하면 워커를 다시 만든다. 구독이 없으면 refresh는 워커를 기동하지 않는다.
...
업데이트는 `download → 서명 검증 → finalization fence → 승인 mutation drain → critical checkpoint → updater on_before_exit 자식 정리/file-lock wait → install/restart` 순서다. mutation admission permit은 Local·Remote·Automation·MCP 입력, Automation frontend action과 terminal 생성/종료의 전체 수명을 덮으며 lx group command·sync-CWD 주입·xterm protocol reply도 같은 gate를 통과한다. sync-CWD permit은 PTY write 이후의 propagation marker·authoritative CWD·event까지 유지된다. Automation action은 backend HTTP waiter가 5초에 timeout되어도 frontend의 늦은 `automation_response`까지 detached permit을 유지한다. fence 이후 새 permit은 거부하고 기존 permit, controller operation, timeout/fault 뒤 아직 끝나지 않은 live PTY worker completion과 handle 제거 시 별도 격리한 retired completion을 최대 16초 기다린다. close mutation permit을 보유한 채 fault 여부와 무관한 worker completion을 live registry에서 격리 목록으로 옮기므로, 제거 뒤 fault가 발생해도 drain이 둘 사이의 공백을 관측할 수 없다. Windows process-tree helper도 1초 deadline으로 제한해 정지한 `taskkill`이 hidden eviction의 대상 fence를 무기한 붙잡지 못한다. 여러 hidden target의 유계 teardown은 병렬로 실행하고 scope guard가 모든 command 반환에서 fence를 해제하므로 fence-held 시간이 pane 수에 선형으로 늘어나지 않는다. fence 중 들어온 terminal create와 일반 TerminalView close는 admission 밖에서 취소를 기다린 뒤 normal permit으로 생성 또는 정리돼, 실패한 checkpoint 뒤 생성 요청이 유실되거나 pane 삭제와 겹친 backend PTY가 고아로 남지 않는다. critical checkpoint는 같은 terminal generation과 귀속 결과를 150ms 간격으로 두 번 관측하고 모든 live terminal이 `Identified`, `NoAgent` 또는 ADR-0232의 `RestorePending`이며 CWD 조회도 성공할 때만 ACK한다. StrictMode에서 정리된 listener가 이미 받은 request도 오류 ACK해 backend timeout까지 침묵하지 않는다. drain·checkpoint 실패 또는 timeout이면 fence를 풀고 installer를 시작하지 않는다. 숨김 terminal eviction은 ADR-0231에 따라 backend command가 대상 terminal별 입력 fence부터 대상 critical checkpoint와 PTY close까지 한 트랜잭션으로 소유하며, 프론트는 ACK 직전 숨김 eligibility를 다시 검사한다. backend가 실제로 닫았다고 반환한 pane만 unmount하고, 동시에 visibility·timeout이 바뀐 닫힌 pane은 eviction set 전이를 현재 hook이 즉시 재평가해 remount하며, 실패한 대상은 재시도한다.
