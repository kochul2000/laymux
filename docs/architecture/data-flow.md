...
Remote 브라우저 xterm은 display mirror이고 PC xterm이 terminal protocol query의 단일 응답자다. snapshot·replay 중 생긴 `onData`는 전부 폐기하고, live output의 CSI DSR·DA·DECRQM·XTVERSION과 순수 OSC 4/10/11/12 색상 query는 parser handler가 원천 차단한다. OSC 색상 설정이나 query와 설정이 섞인 payload는 표시 부작용을 보존하기 위해 xterm 기본 handler로 넘긴다.
