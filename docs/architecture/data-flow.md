...
Cloud main document의 `loading`·`ready`·`unavailable`은 Kotlin이 원시 상태로 소유한다. 허용된 top-level navigation 시작은 `loading`으로 바꾸고 불투명 네이티브 진행 표면을 띄우며, 같은 active URL의 main-frame DNS/TLS/HTTP 실패는 `unavailable`로 바꿔 Chromium 오류 문서를 가리고 `다시 시도`를 제공한다. 실패 뒤 따라오는 `onPageFinished`, 하위 자원 실패와 이전 navigation의 늦은 callback은 현재 표시를 바꾸지 않는다. 같은 URL을 다시 적재해 성공한 `onPageFinished`만 `ready`로 전환하며, Cloud bridge action은 `Cloud 전면 surface ∧ ready`에서만 허용한다. 따라서 Cloud가 오프라인이어도 앱 프로세스를 재시작하지 않고 native layer에서 복구할 수 있다.
