...
- **상태 소유자는 CompositionHelper 하나다**: exact bundle patch가 xterm 6.0.0 ESM/CJS의 CompositionHelper에 generation FIFO를 둔다. 각 delayed finalizer는 캡처 범위·다음 composition 경계·이미 전송된 길이·`compositionend.data` 확정 후보·순서 있는 input/keypress 관측 목록·완료 상태를 별도 record로 소유한다. 새 `compositionstart`는 직전 generation의 textarea 상한을 먼저 고정하므로 다음 조합 위치가 이전 timer에 섞이지 않는다.
