...
- carry-over 앵커는 **확정 텍스트의 셀 폭만큼 직접 전진**시킨다. 같은 틱에는 PTY 왕복이 없어 shadow cursor 가 아직 확정 전 셀을 보고하므로 그것을 그대로 쓰면 새 음절을 앞 음절 위에 그린다. `getAnchor()` 가 이미 전진해 있으면(다른 행이거나 확정 폭 이상 이동) 그 값을 권위로 쓰고, 아니면 폭 산술로 전진한다 — 왕복 없이 결정된다.
