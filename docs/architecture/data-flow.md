...
Android entry는 Cloud 계정 표면과 E2E Remote 표면을 서로 다른 WebView/bridge로 분리한다([ADR-0149](../adr/0149-android-thin-wrapper-runs-desktop-owned-remote-ui.md)). Cloud WebView는 설정된 public HTTPS origin의 `/app/android`와 기존 landing/dashboard만 표시하고, native Google Credential Manager login과 instance UUID 선택 두 동작만 요청한다. Cloud가 anonymous session에 저장한 32-byte nonce를 Google ID token에도 넣고 Cloud가 서명·issuer·audience·expiry와 nonce를 검증한 뒤 기존 HttpOnly session cookie를 발급한다. 선택한 instance와 confirmed pairing 또는 새 QR의 instance가 정확히 일치해야 secure 표면으로 넘어간다. Cloud WebView에는 pairing/E2E bridge가 없다.
