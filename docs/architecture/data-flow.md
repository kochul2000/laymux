...
attach와 token별 단일 ACK IPC에는 5초 watchdog을 둔다([ADR-0086](../adr/0086-terminal-output-control-epoch-watchdog.md)). timeout은 `outputAttachEpoch`가 여전히 current일 때만 sender 폐기 → epoch 증가 → replacement 순서로 처리하고, 원 Promise의 fulfillment/rejection handler는 늦은 완료를 흡수해 새 token·confirmed prefix·화면을 변경하지 않는다. attach와 ACK timeout streak는 rate backoff 용도로 분리하며 각각 attach fulfillment와 backend가 `true`로 수락한 ACK만 초기화한다. timeout replacement는 50 ms부터 지수 backoff하되 1,000 ms를 넘지 않는다. 취소 불가능한 bridge Promise의 자원 상한은 streak나 mount가 아니라 WebView/window-scoped terminal-id registry가 소유한다. attach/ACK 종류별 bridge 호출 직전에 actual started-but-unsettled operation lease를 증가시키고 underlying resolve/reject에서 exactly once 감소시키므로, watchdog 전에 unmount된 영구 pending operation과 과거 timeout orphan을 같은 상한으로 센다. 같은 id remount는 계수를 공유하고 새 operation은 실제 outstanding이 6 미만일 때만 만든다. 6이면 current epoch/readiness를 폐기한 채 producer를 ADR-0084 credit에서 fail-stop하고 mount-local current capacity waiter 하나만 둔다. operation 정착으로 슬롯이 생기면 waiter를 한 번 소비해 current epoch가 여전히 같을 때만 bounded backoff replacement 하나를 예약한다. unmount는 waiter만 제거하고 operation lease는 유지한다. active mount가 없고 attach/ACK outstanding이 모두 0일 때만 entry를 제거하므로, 삭제 후 영구 pending과 같은 terminal id 재사용도 정착 또는 WebView 종료까지 공유한다. 정상 attach reject, ACK Promise reject, backend `false` 의미는 기존대로 각각 재부착·동일 token/seq 재시도·stale lease 재부착이다. epoch 폐기/replacement 결정은 `console.warn`과 recovery counter보다 먼저 확정해 진단 throw가 liveness를 방해하지 않는다.
