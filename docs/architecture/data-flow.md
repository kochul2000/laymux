...
**워크스페이스 전환은 remount 가 아니다.** `WorkspaceArea` 는 한 번 활성화된 워크스페이스를 계속 마운트해 둔 채 `display:none` 으로만 감추고(`PaneGrid` 도 pane 박스에 같은 처리), `TerminalView` 의 xterm/PTY 수명은 `[instanceId, profile, …]` 에 달려 있으며 `resizePane` 은 pane id 를 보존한다. 따라서 전환도 pane resize 도 `TerminalView` 를 unmount 하지 않고, `TERMINAL_ATTACH_SNAPSHOT_MAX_BYTES` snapshot replay 는 두 경로 어디에도 없다 — 남는 것은 위 hide→show 복귀의 atlas 재생성과 공통 스케줄러를 통과하는 fit 하나다. 실제로 remount 하는 경로는 profile 변경, hidden pane 회수(#269), `movePaneToWorkspace` 뿐이다. 폭주 중 레이아웃 변경이 프론트를 수십 초 무응답으로 만드는 비용은 이 replay 가 아니라 §8.8 의 출력 백로그이며, `attaches`/`attachReplayBytes` 카운터가 이 판정을 실기에서도 반증 가능하게 한다([ADR-0079](../adr/0079-output-backlog-coalescing-and-out-of-band-frontend-vitals.md)).

Windows output quiet window 는 **delta 도착 시각**으로 잰다. "최근에 PTY 출력이 도착하지 않았다"는 뜻이므로 write 시점이 아니라 `applyOutputSegments` 진입에서 기록한다 — write 시점에 기록하면 §8.8 의 apply 큐에 붙들린 세그먼트가 침묵으로 읽혀, 아직 옛 격자를 향한 출력이 큐에 남은 상태에서 fit 이 풀린다.
...
PTY output callback은 `TerminalProtocolState`로 전체 output의 `CSI ? 2004 h/l`과 reset을 chunk 경계에 걸쳐 추적한다. callback은 terminal별 protocol gate를 잡은 뒤 generation-scoped sequenced output ring에 같은 bytes를 기록하고 `terminal-output-v2-{id}`에 `{ generation, seqStart, seqEnd, data, geometry }`를 발행한다. resize는 뒤따르는 PTY bytes보다 먼저 `{revision,cols,rows}`를 갱신하므로 각 delta는 그 bytes를 해석할 authoritative geometry를 가진다. `attach_terminal_output`은 같은 protocol gate → output buffer 순서로 raw `{ state: { version, generation, snapshotStartSeq, snapshotSeq, sourceStartSeq, sourceSeq, snapshotKind:"raw", protocolRevision, modes, geometry }, snapshot }`을 원자 캡처한다. recent ring이 오래된 DECSET을 버려도 backend protocol state는 유지된다.
...
**백로그는 세그먼트가 아니라 바이트로 값을 치른다**([ADR-0079](../adr/0079-output-backlog-coalescing-and-out-of-band-frontend-vitals.md)). sequence 가 확정된 세그먼트는 `applyOutputSegments` → `TerminalOutputApplyQueue`(`terminal-output-apply-queue.ts`) 를 지나 표면에 닿는다. 세그먼트 하나를 적용하는 비용은 크기와 무관한 상수다 — `terminal.write` + parse 콜백, checkpoint xterm 의 별도 write(§8.8 위), stabilizer push, `TextDecoder` 라운드, 그리고 §8.5 감지기들의 1 KiB·16 KiB 롤링 윈도 전체 스캔. 폭주는 같은 바이트를 수천 개의 작은 delta 로 전달하므로 그 상수가 메인 스레드를 채우고, 채워진 메인 스레드 뒤에서 `automation-request` 가 굶어 `Frontend response timeout` 이 된다(issue #606). 그래서 큐는 **표면이 한가하면 즉시 적용**(대화형 지연 불변)하고, **표면이 밀려 있으면 붙들었다가 진행 중인 write 가 끝나는 시점에 병합해서** 넣는다 — 트리거는 타이머가 아니라 backpressure 이므로 지연 상한은 진행 중인 parse 이고 배치는 밀린 만큼만 자란다. 병합은 `coalesceTerminalOutputSegments` 가 하며 generation 변경·geometry revision 변경·sequence 불연속·1 MiB 에서 거부한다 — geometry 경계는 checkpoint 모델이 그 사이에서 resize 해야 하는 지점이고 위 복구가 가로지르기를 거부하는 지점과 같다. 도착 순서 큐 하나뿐이므로 복구 범위는 자신이 앞에 splice 하는 delta 보다 먼저 표면에 닿고, 전체 재부착은 `reset()` + replay 가 화면을 새로 만들기 때문에 큐를 비운다. write 완료 시점에는 **보류된 fit 을 먼저** 흘려보낸 뒤 병합 배치를 넣는다 — 반대 순서면 지속 폭주가 fit 을 무한히 굶긴다. 병합의 셀 격자 동일성은 `terminal-output-coalesce.screen.test.ts` 가 실제 xterm 으로 고정한다([ADR-0074](../adr/0074-xterm-cell-grid-screen-test-tier.md)).

파이프라인 비용은 `terminal-output-pipeline-metrics.ts` 의 terminal 별 카운터로 센다(`deltaEvents`·`deltaBytes`·`segmentsIn`·`segmentsOut`·`applyQueueMaxDepth`·`xtermWrites`·`xtermWriteBytes`·`writeQueueMaxDepth`·`writeBackpressure`·`checkpointApplies`·`fits`·`fitDeferredMaxMs`·`atlasRebuilds`·`attaches`·`attachReplayBytes`). 아래 복구 카운터와 같은 규율이다 — 진단 전용이고 수명은 백엔드 세션 하나다. 읽는 곳은 `GET /api/v1/diagnostics/frontend` 이며, 그 경로는 프론트를 경유하지 않으므로 **메인 스레드가 멈춰 있는 동안에도 답한다**([api-contracts.md §12](api-contracts.md)). `segmentsIn / segmentsOut` 이 병합률, `attachReplayBytes`/`attaches` 가 "레이아웃 변경이 1 MiB replay 를 유발하는가"의 직접 반증, `fitDeferredMaxMs` 가 fit 이 write FIFO 뒤에서 굶은 시간이다.

복구 사건은 `terminal-output-recovery-metrics.ts` 의 terminal 별 누적 카운터 10종(`gap`·`repair`·`ringEscalation`·`geometryEscalation`·`nestedGap`·`nestedGapEscalation`·`repairTimeout`·`repairFailure`·`malformedDelta`·`attachFailure`)으로 세고 모든 경고에 그 스냅샷을 실어 출력한다. **`ringEscalation` 은 backend 가 `null` 을 답한 경우만 센다** — ADR-0072 의 재검토 조건이 그 칸 하나에 걸려 있으므로 다른 사건을 섞으면 안 된다. 복구 구간 뒤의 또 다른 구멍은 `nestedGap`(재부착 단위가 아니라 회차마다 하나씩 세는 순수 관측치), 그 루프가 회차 상한을 넘겨 재부착으로 끝난 경우만 `nestedGapEscalation`(`TERMINAL_OUTPUT_REPAIR_MAX_ROUNDS` 를 조정할 유일한 근거이므로 같은 이유로 단독 칸이다), watchdog 창을 넘긴 왕복은 `repairTimeout`, resize 교차는 `geometryEscalation`, 서브 payload 검증 실패는 `malformedDelta`, 그 외 거절·예외는 `repairFailure` 다. 분류는 오류 메시지 매칭이 아니라 round-trip 의 실패 지점과 `TerminalOutputRepairError.reason` 으로 결정한다. 카운터 수명은 백엔드 세션 하나다 — `close_terminal_session` 경로가 그 id 의 엔트리를 지우고, 그 호출처가 `TerminalView` 의 xterm 생성 effect cleanup 이므로 unmount(hidden pane 회수·profile 변경 포함) 도 카운터를 0 으로 되돌린다. "이 pane 이 계속 놓친다"는 한 mount 안에서 읽는다. PTY callback 도 `emit` 실패를 삼키지 않고 폐기한 sequence 구간과 누적 폐기 수를 로그로 남긴다.
...
- 판정 로직은 `ui/src/lib/terminal-focus-ownership.ts` 의 `createTerminalFocusOwnership` 이 전부 소유한다(DOM 이벤트 등록 없음, 순수 컨트롤러). `TerminalView` 는 window `blur`/`focus`/capture `pointerdown` + surface `focusout` 배선과 helper 재바인딩·pane focus 해제 통지만 담당하며, IME 조합 컨트롤러(`ime-composition-controller.ts`)는 focus 소유권을 알지 않는다.
...
- **복원(window focus)**: focus 시점에 다른 요소가 focus 를 쥐고 있으면 기록을 버리고 아무것도 하지 않는다. 주인 없는 상태(`null`/`body`/`documentElement`)거나 아직 helper 자신이면 다음 프레임에 재확인 후 `helper.focus()` 를 한 번 호출하고 기록을 소비한다 — `activeElement === helper` 를 동기 시점에 조기 종료하지 않는 것은 window `focus` 직후 webview 가 focus 를 떨어뜨리는 순서를 덮기 위해서다. 프레임 사이에 모달·검색창·설정 입력·다른 pane helper 가 focus 를 얻었으면 복원을 취소한다 — 앱 활성화 시 전역 `terminal.focus()` 는 어디에서도 호출하지 않는다.
