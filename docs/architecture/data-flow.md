...
해제:
  ...
  - 해제 단위는 notificationDismiss 모드를 따른다:
...
| `workspace.next` / `prev` | `Ctrl+Alt+↓` / `Ctrl+Alt+↑` | 다음 / 이전 워크스페이스. 전환 후 Pane 포커스 자동 이동(기본) — `convenience.dockArrowFocusPane=false` 면 Dock 포커스 유지 (#311) |
...
**1회성 CWD 전파 (issue #293).** 위 흐름은 셸의 `cd`가 자동 전파되는 경로다. 이와 별개로, 컨트롤 패널(`PaneControlBar`)의 "Propagate CWD once" 버튼은 현재 view의 CWD를 sync group에 한 번만 밀어넣는다. 소스에 따라 경로가 갈린다:
