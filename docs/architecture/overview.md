...
| `MemoView` | 자유 | 간단한 텍스트 메모장. 내용은 `cache/memo.json`에 pane별로 저장. `memo.copyOnSelect` 는 **lazy** 다 — 선택은 pending 으로만 잡아두고, 포커스 아웃(다른 pane mousedown·window blur)·선택 해제·pane 바깥에서 끝난 드래그 릴리즈에서만 클립보드로 flush 한다. pane 안에서 끝난 드래그는 flush 하지 않는다: 곧이어 오는 Ctrl+V 가 선택 영역을 클립보드 내용으로 치환할 수 있어야 하고, paste 이벤트는 pending 을 폐기한다 (#307, #710) |
