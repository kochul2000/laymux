...
#### Pane staggered reveal (흰 화면 방지)

Pane 이 많은 워크스페이스를 활성화하면 모든 `TerminalView` 가 단일 React 커밋에서 동시에 마운트되어(수십 개 `terminal.open()` + canvas/WebGL 컨텍스트 할당) 메인 스레드가 잠시 막히고, 각 pane 콘텐츠가 자기 배경을 칠하기 전 위치 지정 `<div>` 가 브라우저 기본 흰색으로 번쩍인다. 이를 막기 위해 pane 을 백그라운드에서 순차 마운트한다.
