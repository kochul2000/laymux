...
Remote는 PC 메모와 같은 키를 사용하는 Memo overlay를 제공한다. 저장된 메모와 navigation의 비어 있는 MemoView pane을 선택할 수 있다. Settings의 `Panels` 탭에서 Files/GitHub/Memo/탐색 제외/PC 모드 헤더 버튼을 각각 표시하거나 숨기고 우측 가장자리 스와이프 대상을 선택한다. 숨김은 가용성·권한·제스처 대상에 영향을 주지 않으며 설정은 기기 로컬이다([ADR-0260](../adr/0260-remote-shared-memo-and-panel-preferences.md)).
