...
원격 클라이언트는 이 두 표면을 header 아래 한 줄 스트립으로 접어 미러한다([ADR-0123](../adr/0123-remote-widget-strip-mirrors-desktop.md)). 데스크톱이 그리고 있는 위젯만 나타나고 배치·값은 모두 데스크톱이 소유하며, 원격은 `/remote/v1/widgets` 가 준 표시 모델을 그리기만 한다.
