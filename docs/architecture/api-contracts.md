...
Cloud dashboard에서 instance를 선택해 pairing 표면에 진입하면 실제 Cloud WebView는 하위 레이어에서 계속 `VISIBLE`로 렌더링되고, APK local secure WebView가 full-container 투명 overlay와 불투명 bottom sheet를 위에 표시한다([ADR-0157](../adr/0157-android-pairing-sheet-over-live-cloud-webview.md)). 정적 dashboard 복제나 snapshot을 사용하지 않는다. overlay 동안 Cloud WebView는 disabled·focus 제외 상태이고 접근성 descendants도 숨겨 touch·keyboard·TalkBack이 secure WebView만 대상으로 하며, secure 문서의 full-screen dismiss layer가 sheet 밖 입력까지 모두 받는다. 뒤 Cloud JavaScript가 계속 실행되더라도 native는 현재 전면 surface가 Cloud일 때만 `signInWithGoogle`·`selectInstance` bridge 호출을 받는다. 보안 상태와 QR·생체·E2E action은 불투명 secure sheet 안에서만 렌더링하고 Cloud 배경은 비신뢰 문맥으로 취급한다. 닫기 애니메이션 뒤 secure WebView를 숨기고 Cloud의 입력·접근성을 복구한다. PC Remote 표면에서는 기존처럼 Cloud WebView를 숨긴다.

`apps/android`는 [ADR-0149](../adr/0149-android-thin-wrapper-runs-desktop-owned-remote-ui.md)에 따라 Cloud account 표면과 pairing/E2E 표면을 서로 다른 WebView로 분리한다. Cloud WebView entry는 빌드에 설정된 public HTTPS origin의 `/app/android`다. 이 WebView는 그 exact origin의 top-level navigation만 허용하고 기존 Cloud landing/dashboard와 HttpOnly `laymux_session` cookie를 사용한다. JavaScript interface는 `LaymuxCloud.signInWithGoogle(nonce)`와 `LaymuxCloud.selectInstance(instanceId)` 두 메서드뿐이며 pairing metadata·QR·seed·session key·Remote HTTP/output bridge를 제공하지 않는다. Android app-mode HTML은 `frame-src 'none'`·`frame-ancestors 'none'` CSP와 `X-Frame-Options: DENY`를 보내 all-frame JavaScript interface가 nested document에 노출되지 않게 한다. dashboard의 Android app mode는 기존 instance 목록을 그대로 렌더하되 online Connect form만 canonical instance UUID 선택 호출로 바꾸고 browser `/app/connect`와 Tailscale action은 표시하지 않는다.
...
pairing manager main document·script·style은 APK `assets`와 `WebViewAssetLoader`의 `https://appassets.androidplatform.net/assets/`에서 적재한다. 이 secure WebView는 APK local origin과 아래 AEAD resource용 synthetic origin만 허용하고 CSP도 `default-src 'none'`, `connect-src 'none'`을 사용한다. ACK 네트워크 요청은 JavaScript가 아니라 Kotlin native 계층만 수행한다. pairing manager의 `LaymuxNative` bridge는 scan·instance별 ACK 재시도 요청, 비밀이 아닌 pairing metadata 목록 조회, 생체 보호 설정, instance별 key 확인/삭제를 제공하며 secret byte·pairing ID·client nonce를 반환하지 않는다. PC 제공 Remote 문서로 navigation하기 전에는 같은 이름의 interface를 E2E HTTP/output transport, 현재 session disconnect, 외부 링크 열기만 가진 별도 객체로 교체하고, pairing 상태 callback도 Remote surface에서는 실행하지 않는다. secure WebView는 다중 창을 켜지 않고 off-origin navigation을 거부하므로 Remote 문서의 터미널 링크는 `openExternalUrl(url)` 하나로 native에 넘긴다. native는 공백·제어문자 부재, `http`/`https` scheme, 비어 있지 않고 userinfo 없는 authority를 다시 검증하고 scheme만 소문자로 정규화한 뒤, 현재 local surface가 Remote일 때만 `ACTION_VIEW`+`CATEGORY_BROWSABLE`+`FLAG_ACTIVITY_NEW_TASK`로 OS 브라우저를 시작한다. path·query·fragment는 검증하지 않고 그대로 넘긴다. `java.net.URI`는 WHATWG URL이 이스케이프하지 않는 문자를 거부해 정상 링크를 삼키므로 사용하지 않는다. 검증 실패는 무시하고 처리할 activity가 없으면 toast로 실패 닫힘하며, 앱은 `http`/`https` intent filter를 선언하지 않는다([ADR-0162](../adr/0162-android-remote-link-opens-os-browser.md)). pairing manager는 저장된 PC 목록과 confirmed/pending 상태를 표시하고 각 레코드를 독립적으로 확인·삭제한다. Cloud dashboard가 선택한 instance가 있으면 그 instance와 정확히 일치하는 저장된 confirmed pairing 또는 새로 스캔한 QR만 ACK와 session open에 사용한다. 선택한 PC의 confirmed 레코드가 없으면 다른 저장 pairing을 대체 선택하지 않고 QR을 요구하며, 기존 pending 레코드의 ACK 재시도는 pairing manager의 명시적 동작으로만 제공한다. 선택 변경은 기존 native E2E session을 먼저 닫는다([ADR-0154](../adr/0154-android-multi-instance-pairing-vault.md)).
