...
Remote 입력 action은 기본행(`main`)과 `Keys` 확장행(`expanded`) 각각의 `left`·`center`·`right` 정렬 구역에 배치하며, 어느 구역에도 없으면 팔레트(숨김)에 있다([ADR-0213](../adr/0213-remote-input-action-segment-placement-and-user-keys.md), [ADR-0186](../adr/0186-remote-input-action-three-zone-layout.md)). 구역은 폭이 남는 기기에서 순서만으로 표현할 수 없는 좌·우 정렬을 표현하기 위한 것이며, 폭이 모자라면 행 내부 가로 스크롤로 자연히 되돌아간다. 기본값은 기본행 `left: ^C` / `right: Keyboard · Keys · Send`, 확장행 `left: Composer 전환 · P↕N↔ · Esc · Tab · ⇧Tab · ↕↔` / `right: ^J · ^U · ^T · ^L`이고 `Attach file`은 팔레트에 있다. Send는 저장된 배치와 무관하게 Composer mode일 때만 렌더한다. `Keys`는 기본행의 세 구역 또는 팔레트만 가능하고 자기가 여는 확장행에는 둘 수 없는 구조 토글이다. Keys를 팔레트로 보내면 열린 확장행을 즉시 닫아 `expanded=false`로 저장하되 확장행 구성과 순서는 보존한다. 그 밖의 고정 action(`Keyboard`, `Send`, `Composer`, `Attach file`)과 모든 소프트키는 두 행의 여섯 구역과 팔레트 사이를 자유롭게 오간다. 배치 편집은 key bar 안의 팝오버가 아니라 항상 접근 가능한 Remote drawer의 Settings → Input bar에 둔다.

`laymux.remote.keybar`의 `zones`(`{main:{left,center,right}, expanded:{left,center,right}}`)가 배치와 구역 내 순서의 SoT다. **배치가 곧 활성화다** — 두 행 중 어딘가에 배치된 action만 렌더하고, 배치되지 않은 것이 팔레트다. 팔레트 목록은 저장하지 않고 미배치에서 파생하며, 세트/커스텀 선택(`sets`/`custom`/`usedCustom`)과 전역 `order` projection은 없다(ADR-0040의 사용자 순서는 구역 내 순서가 계승한다). 저장값이 이 shape이 아니면 — ADR-0186의 평탄 배열 포함 — 마이그레이션 없이 기본 배치로 되돌리고, 정상 shape 안의 미지·중복 ID만 항목 단위로 버린다. 미배치 action을 기본값으로 채워 넣지 않으며 버전 필드도 두지 않는다. 배치·확장 상태와 사용자 등록 키는 `localStorage`에만 저장하는 surface-local UI 상태이며 `settings.json`, Remote API와 PTY 계약은 바꾸지 않는다([ADR-0004](../adr/0004-settings-vs-ui-state-separation.md), [ADR-0028](../adr/0028-remote-soft-key-toolbar.md), [ADR-0213](../adr/0213-remote-input-action-segment-placement-and-user-keys.md)).
...
Remote drawer Settings의 `Input bar` 편집기는 배치와 순서를 하나의 칩 모델로 다룬다. Main 행·Keys 행의 각 구역과 팔레트가 모두 드롭 대상이고, 고정 action과 소프트키를 구분 없이 칩으로 보여 준다. long-press Pointer Events drag(HTML native DnD 아님)와 삽입 표시선으로 구역·행·팔레트 경계를 넘어 옮기며, `Keys` 칩만은 확장행 구역을 drag target으로 받지 않는다. 칩을 탭하면 행·구역 select(`Main · Left` … `Keys · Right`, `Palette`)와 구역 내 `First`·`←`·`→`·`Last`가 나오는 접근성 경로가 열리고, 사용자 등록 키에는 `Delete`가 함께 붙는다 — drag는 enhancement이고 이 탭 경로가 정본이다. 팔레트는 미배치 action을 카테고리(Actions / Pane·Alert nav / Navigation / Editing / Ctrl keys / Function / Custom keys)로 묶어 보여 준다. `Reset`은 배치와 확장 상태만 기본값으로 되돌리고 등록된 사용자 키는 지우지 않는다. 두 행은 CSS grid 3열 + `justify-content: space-between`으로 그리며, 구역 컨테이너에는 `min-width: 0`을 주지 않는다(트랙의 자동 최소 크기가 0이 되면 구역이 내용보다 좁아져 이웃과 겹친다). 화면 폭보다 길면 각 행 내부에서만 좌우 스크롤하며 한 줄을 유지하고 scrollbar track은 노출하지 않는다. app/header/main/footer/key-bar 경계는 `min-width: 0`을 유지해 문서 outer overflow를 만들지 않는다.
