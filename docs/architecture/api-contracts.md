...
`do_sync_cwd`는 `force: bool` 인자를 받는다. 컨트롤 패널(`PaneControlBar`)의 "Propagate CWD once" 버튼이 `propagate_cwd_once` 커맨드를 통해 `force=true`로 호출한다. 평소 동기화를 꺼둔 file explorer/viewer 등을 *지금 이 순간의* CWD로 한 번만 따라오게 만드는 것이 목적이다.
...
`propagate_cwd_once`는 소스 터미널의 `session.cwd`가 비어 있으면(OSC 7 미발행) no-op으로 `Ok`를 돌려준다. 백엔드 `terminal_id`는 프론트의 `ViewRenderer` instanceId 규칙(`terminal-${paneId}` / `file-explorer-${paneId}`)과 동일하게 구성한다.
