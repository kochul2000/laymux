...
`remote.touchScrollSensitivity`(한 손가락, 기본 1)와 `remote.twoFingerScrollSensitivity`(두 손가락, 기본 5)는 **xterm 옵션이 아니다**([ADR-0177](../adr/0177-remote-two-finger-touch-scroll-sensitivity.md)). Remote 페이지가 소유한 손가락 드래그의 픽셀→행 환산에서 입력 델타에 한 번 곱하며(`consumeTouchScrollLines(term, deltaY, sensitivity)`), 하위 셀 나머지(`scrollRemainderPx`)에는 다시 곱하지 않는다. 옵션 번들 대신 페이지 지역 상태(`adoptTouchScrollSensitivity` 가 두 값을 흡수)가 들고 있으며 appearance 가 적용되는 두 지점(터미널 생성·appearance 갱신)에서 갱신된다 — xterm 은 모르는 옵션 키를 거부한다. 정상 스크롤백 모드에서는 계산한 행 수를 xterm viewport에 적용한다. 마우스 트래킹이 꺼진 alternate buffer(Codex transcript overlay 등)에서는 같은 계산 결과를 위/아래 커서키로 바꾸고, ConPTY의 반복 축약을 피하도록 각 행을 별도 Remote write로 보낸다. 두 경로 모두 한 손가락은 `touchScrollSensitivity`, 두 손가락(`twoFingerScrolling` 모드)은 `twoFingerScrollSensitivity` 를 쓴다. **마우스 트래킹을 켠 전체화면 TUI 에서는 드래그가 합성 wheel 이벤트로 앱에 전달되므로 한 손가락·두 손가락 모두 `remote.scrollSensitivity` 가 적용된다**(두 배율을 겹쳐 곱하지 않는다).
...
```jsonc
{
  "remote": {
    ...
    "snapshotMaxKib": 4,                // 원격 screen checkpoint의 scrollback 기본 예산(KiB). 1~1024로 clamp; 현재 화면·alt buffer·mode 복원 bytes는 예산을 넘을 수 있으나 절대 상한은 1 MiB. client의 scroll-top history 요청은 이 값을 같은 clamp 범위 안에서 올릴 수만 있다(ADR-0182)
    ...
    "menuFontSize": 13,                  // Remote 내비게이션 드로어(메뉴) 기준 글자 크기(px). 6~72, 드로어의 작은 글씨 tier가 비례 스케일
...
- **채널을 바꾸는 표면은 데스크톱 Settings ▸ 일반 ▸ 업데이트(`nav-update` → `update-channel-select`)와 Automation 일반 설정 patch 뿐이다.** Remote 는 범용 host 설정 쓰기 표면이 아니므로(`/remote/v1/display-settings` 는 PC 소유 표시·스크롤 필드만 다루는 좁은 projection) 채널을 바꾸지 않고, `/remote/v1/update` snapshot 의 `channel` 을 읽어 표시만 한다. beta 를 고르면 그 자리에서 경고를 띄운다(`update-channel-beta-warning`).
...
`/remote/asset/<stem>-<sha256 앞 16 hex>.<js|css>`는 서빙 페이지가 실제로 참조하는 클라이언트 자산 route다([ADR-0169](../adr/0169-remote-client-hashed-immutable-assets-and-gzip.md)). main document는 마크업 셸만 남아 `no-store`를 유지하고, minified 앱 번들(`remote-app.min.{js,css}` — 소스는 같은 디렉터리의 `remote-app.{js,css}`, 재생성은 `cd ui && npm run build:remote-page`, 드리프트는 ui 테스트가 검증)과 vendor xterm 파일을 이 route의 해시 URL로 참조한다. URL이 곧 내용이므로 `Cache-Control: private, max-age=31536000, immutable`(폰트와 같은 근거)이고, gate는 vendor asset과 동일하다. 셸·해시 자산은 기동 시 사전 압축한 gzip 본을 `Accept-Encoding` 협상으로 서빙하며 `Vary: Accept-Encoding`을 보낸다. 기존 `/remote/vendor/*` 고정 route는 호환용으로 유지되지만 서빙 페이지는 더 이상 참조하지 않는다.
...
Remote 표시 글자 크기의 SoT는 PC의 `settings.remote.terminalFontSize`(기본 14)·`settings.remote.composerFontSize`(기본 16)·`settings.remote.menuFontSize`(기본 13)이며 모두 6~72 정수다([ADR-0173](../adr/0173-remote-display-settings-pc-owned-and-lease-gated.md), [ADR-0199](../adr/0199-remote-menu-font-size-pc-owned.md)). terminal 값은 Remote xterm cell에만 적용하고 desktop profile 크기는 바꾸지 않으며, composer 값은 composer textarea와 history/autocomplete 목록에 함께 적용한다. menu 값은 내비게이션 드로어의 기준 글자 크기로, 드로어 스코프에서 `--fs-xs/--fs-sm/--fs-md` 토큰을 10/13·11/13 비율로 재유도하고 workspace 목록의 em 기반 텍스트·배지가 비례 스케일한다(CSS 변수 `--remote-menu-font-size`). 같은 projection·PUT 계약에 Composer 불투명도 세 값도 실린다: `composerIdleOpacity`(55)·`composerFocusedOpacity`(80)·`composerActiveOpacity`(100)는 20~100 정수 백분율이며 5 단위 입력과 `Idle ≤ Focused ≤ Active` 불변식을 사용한다([ADR-0200](../adr/0200-remote-composer-opacity-state-settings.md)). 터치 드래그 민감도 두 값도 같은 계약에 실린다: `settings.remote.touchScrollSensitivity`(한 손가락, 기본 1)와 `settings.remote.twoFingerScrollSensitivity`(두 손가락, 기본 5), 각 0.1~20([ADR-0177](../adr/0177-remote-two-finger-touch-scroll-sensitivity.md)). 이 둘은 xterm 옵션이 아니라 페이지 지역 상태(`adoptTouchScrollSensitivity`)로 흡수되며 저장 즉시 현재 문서에 반영된다(별도 appearance 재푸시 없음). Remote Settings drawer는 연결 시와 drawer 진입 시 이 projection을 다시 읽고, 저장 성공 시 현재 xterm appearance를 다시 적용해 fit하며 composer CSS 변수와 상태도 즉시 갱신한다. revision 충돌은 `409`로 받고 최신 projection을 재조회한다.
...
|---|---|---|
| `/remote/v1/display-settings` | GET | 기존 Remote enabled + Direct bearer/IP/Origin 또는 Android E2E session/AEAD gate. lease 불필요. `{terminalFontSize,composerFontSize,menuFontSize,composerIdleOpacity,composerFocusedOpacity,composerActiveOpacity,touchScrollSensitivity,twoFingerScrollSensitivity,revision}` 반환(불투명도는 20~100 clamp와 단조 순서 정규화, 민감도는 0.1~20 clamp) |
| `/remote/v1/display-settings` | PUT | 같은 인증 gate + active controller `leaseId`. body `{leaseId,expectedRevision,terminalFontSize,composerFontSize,menuFontSize,composerIdleOpacity,composerFocusedOpacity,composerActiveOpacity,touchScrollSensitivity,twoFingerScrollSensitivity}`; 공통 settings snapshot/apply bridge의 CAS로 검증·Zustand 갱신·영속 저장. stale revision은 `409` |
...
scroll-top history expansion([ADR-0182](../adr/0182-remote-scroll-top-history-expansion.md)): output attach는 선택적 history 예산을 함께 보낼 수 있다. Direct/Cloud는 output WebSocket 쿼리 `historyKib`, Android E2E는 암호화 open record의 선택 필드 `historyKib`다. 서버가 쓰는 실제 예산은 `max(effective(snapshotMaxKib), clamp(historyKib, 1..=1024))`이며, 요청은 owner 설정 예산을 올릴 수만 있고 내리지 못한다. 파싱 불가능한 값은 세 transport 모두 fail-open으로 owner 예산 attach가 된다(값 때문에 attach를 거절하지 않는다). 1 MiB 절대 상한과 checkpoint 검증 경로는 그대로다. Remote page는 normal buffer이면서 mouse tracking이 아닐 때, 스크롤 제스처(위쪽 wheel·위쪽 드래그·Shift+PageUp)가 viewport를 최상단으로 *도달*시키거나 그 상태로 위쪽 드래그/wheel이 오면 재attach하며(제스처 없이 출력이 밀어낸 최상단 도달과 포인터 누름은 무시, 연결 중단 알림 중에는 요청 안 함), 요청 예산은 지금 들고 있는 화면에서 유도한다 — `clamp(현재 snapshot bytes × 4, 64 KiB, 1024 KiB)`. page는 owner 설정을 모르므로 고정 사다리는 `snapshotMaxKib`가 그 단계 이상일 때 무효 요청이 된다. 재attach는 재접속 경로와 동일하게 live tail 기준 행 오프셋을 복원해 읽던 줄을 유지한다. 각 요청은 요청 id로 자기 attach의 snapshot만 정산하고, 끼어든 attach(재접속·pane 전환·조기 실패·소켓 종료·타임아웃)는 요청을 취소하면서 예산을 요청 직전 값으로 되돌린다. 전달된 snapshot의 binary 길이가 직전보다 크지 않으면 그 terminal에서는 더 이상 요청하지 않는다(행 수는 replay geometry와 이후 fit reflow 때문에 attach 간 비교 불가). 이미 client 상한(1024 KiB)으로 요청한 뒤 다음 요청이 다시 상한을 넘게 계산되고 직전 checkpoint가 그 예산을 실제로 채웠다면, attach 없이 "더 큰 checkpoint를 요구할 수 없다"는 별도 문구로 알리고 같은 상태로 들어간다(예산을 못 채운 checkpoint는 데스크톱이 먼저 바닥난 것이므로 "이전 출력 없음"으로 알린다). 예산·소진 상태는 attach 대상 terminal에 귀속되어 pane 전환·연결 해제 시 초기화되고, 사용자가 직접 다시 attach하는 경우(같은 pane 재선택 포함)도 owner 예산에서 시작한다. 자동 재접속만 확장 예산을 유지하며, 소켓이 연속으로 열리지 않으면 owner 예산으로 내려가면서 소진 상태도 함께 푼다. `LaymuxNative.supportsOutputHistoryBudget()`이 없는 구버전 Android 커넥터에는 요청 자체를 보내지 않는다(커넥터가 알 수 없는 open record 필드를 거부하므로). 커넥터는 범위 밖 `historyKib`를 stream 실패가 아니라 "확장 없음"(0)으로 떨어뜨린다. `snapshotKind`가 없는 legacy 무순번 host는 attach 전에 알 수 없어 요청 1회를 쓴 뒤 확장 대상에서 제외된다.
