...
`/remote/asset/<stem>-<sha256 앞 16 hex>.<js|css>`는 서빙 페이지가 실제로 참조하는 클라이언트 자산 route다([ADR-0169](../adr/0169-remote-client-hashed-immutable-assets-and-gzip.md)). main document는 마크업 셸만 남아 `no-store`를 유지하고, minified 앱 번들(`remote-app.min.{js,css}` — 앱/CSS 소스는 같은 디렉터리의 `remote-app.{js,css}`, Lucide DOM 경계는 `ui/src/remote/remote-icons.js`, 재생성은 `cd ui && npm run build:remote-page`, 드리프트는 세 소스의 해시를 포함한 ui 테스트가 검증)과 vendor xterm 파일을 이 route의 해시 URL로 참조한다. URL이 곧 내용이므로 `Cache-Control: private, max-age=31536000, immutable`(폰트와 같은 근거)이고, gate는 vendor asset과 동일하다. 셸·해시 자산은 기동 시 사전 압축한 gzip 본을 `Accept-Encoding` 협상으로 서빙하며 `Vary: Accept-Encoding`을 보낸다. 기존 `/remote/vendor/*` 고정 route는 호환용으로 유지되지만 서빙 페이지는 더 이상 참조하지 않는다.

Remote 범용 픽토그램은 [ADR-0210](../adr/0210-remote-lucide-icon-boundary.md)에 따라 vanilla `lucide` named icon을 `ui/src/remote/remote-icons.js` 한 경계에서 DOM SVG로 만든다. 셸의 정적 아이콘은 `data-remote-icon` placeholder를 앱 시작 시 hydration하고 동적 파일·가시성·입력 모드·이동·widget 아이콘은 같은 경계의 `setRemoteIcon`을 사용한다. 기본 `size=14`, `strokeWidth=2`, `currentColor`, 장식 아이콘 접근성 속성은 데스크톱 공용 래퍼와 같다. pane 명령 상태의 `⏳`·`✓`·`✗`·`—` projection은 호환 데이터로 유지하되 Remote 렌더링 경계에서 Lucide로 바꾼다. 터미널 keycap label, 단위·수학 기호, 앱 로고와 제품 고유 데이터 시각화는 이 아이콘 경계의 대상이 아니다.
