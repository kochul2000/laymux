...
모바일 remote의 app 높이는 `visualViewport.height`를 CSS 변수 `--remote-viewport-height`로 반영한다. 브라우저가 지원하면 viewport meta의 `interactive-widget=resizes-content`도 함께 사용한다. 같은 관측값은 터치 Composer의 keyboard-visible 판정에도 쓰되, DOM focus를 proxy로 삼지 않는다. `VirtualKeyboard.boundingRect`가 있으면 이를 우선하고, 없으면 폭별 keyboard-closed 최대 높이와 현재 높이의 차이가 80px 및 기준 높이의 15% 이상일 때만 열린 것으로 본다. 폭 변경은 기준 높이를 재설정해 회전을 키보드로 오인하지 않는다([ADR-0207](../adr/0207-remote-composer-recall-observes-soft-keyboard-geometry.md)). 폭 520px 이하에서는 footer를 한 줄로 유지하기 위해 header와 중복되는 terminal 상태 문구를 숨긴다. 이 값들은 모두 surface-local DOM geometry이며 Remote API/PTY 소유권 계약을 바꾸지 않는다.
