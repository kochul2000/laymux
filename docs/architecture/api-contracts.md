...
terminal output의 public WSS는 `/api/android/e2e/output` 하나다([ADR-0159](../adr/0159-android-e2e-websocket-output-transport.md)). browser session·relay cookie 없이 public origin에서 받고 query는 exact canonical `instanceId`(1..128 identifier), `sessionId`(16-byte base64url), `streamNonce`(32-byte base64url)만 허용한다. relay는 이를 빈 header와 고정 desktop path `/remote/v1/e2e/output`으로 열고 encrypted binary record를 그대로 중계한다. record wire는 `version:u8 || sequence:u64-be || AES-256-GCM ciphertext+tag`다. protocol 최대 plaintext record는 type byte와 1 MiB renderer checkpoint 및 bounded output-ring suffix를 합친 2,629,633 bytes이고 encrypted record는 25-byte wire overhead를 더한 2,629,658 bytes다. 64 KiB보다 큰 WebSocket message는 `fragment {index,count,messageBytes}` tunnel frame들로 나누며 relay는 순서·총 길이·최대 64 fragments를 검증해 public edge에 exact 한 binary message로만 내보낸다. session 방향 key에서 output domain·instance/session/stream nonce를 HKDF salt에 묶어 stream별 a2d/d2a key를 파생하며 desktop은 session 안에서 stream nonce 재사용을 거부한다. 첫 a2d plaintext는 `OPEN(1) || {terminalId,leaseId}`이고 d2a plaintext는 `TEXT(2) || TerminalOutputFrameHeaderV1 JSON` 또는 `BINARY(3) || exact bytes`다. Android native는 `WebViewCompat.addWebMessageListener`를 `https://remote.laymux.invalid` main frame에만 설치하고 `JavaScriptReplyProxy.postMessage(ByteArray)` ArrayBuffer로 TEXT/BINARY를 전달한다. queue는 protocol 최대 plaintext record 한 건과 추가 2 MiB backlog로 제한하고 header parse와 xterm write callback ACK로 한 record씩 진행하며 pending/active output open은 합계 8개로 제한한다. output byte 경로에는 base64·JSON envelope·`evaluateJavascript`가 없다. relay는 routing metadata와 ciphertext·fragment 크기·타이밍 외에 terminal id·lease·header·output bytes를 해석하거나 저장하지 않는다. 장기 WebSocket은 누적 수명 byte 상한 대신 기존 frame·queue·pending byte·concurrency·I/O timeout 상한을 적용하고 한 stream의 backpressure 실패를 tunnel 전체가 아닌 해당 stream에 격리하며, desktop은 자연 종료 task completion을 generation-safe id로 active stream map에서 회수한다. APK WebView는 `connect-src 'none'`이고 network/key/sequence는 Kotlin native 계층이 소유한다. xterm.js·fit addon·CSS는 PC Laymux가 제공하고 AEAD 검증된 resource bridge를 통해서만 WebView에 들어온다.
