...
- **채널을 바꾸는 표면은 데스크톱 Settings ▸ 일반 ▸ 업데이트(`nav-update` → `update-channel-select`)와 Automation 일반 설정 patch 뿐이다.** Remote 는 host 설정 쓰기 표면이 아니므로(`/remote/v1/display-settings` 는 기기-로컬 5필드 전용) 채널을 바꾸지 않고, `/remote/v1/update` snapshot 의 `channel` 을 읽어 표시만 한다. beta 를 고르면 그 자리에서 경고를 띄운다(`update-channel-beta-warning`).
...
Remote 표시 글자 크기의 SoT는 PC의 `settings.remote.terminalFontSize`(기본 14)·`settings.remote.composerFontSize`(기본 16)·`settings.remote.menuFontSize`(기본 13)이며 모두 6~72 정수다([ADR-0173](../adr/0173-remote-display-settings-pc-owned-and-lease-gated.md), [ADR-0199](../adr/0199-remote-menu-font-size-pc-owned.md)). terminal 값은 Remote xterm cell에만 적용하고 desktop profile 크기는 바꾸지 않으며, composer 값은 composer textarea와 history/autocomplete 목록에 함께 적용한다. menu 값은 내비게이션 드로어의 기준 글자 크기로, 드로어 스코프에서 `--fs-xs/--fs-sm/--fs-md` 토큰을 10/13·11/13 비율로 재유도하고 workspace 목록의 em 기반 텍스트·배지가 비례 스케일한다(CSS 변수 `--remote-menu-font-size`). 같은 projection·PUT 계약에 터치 드래그 민감도 두 값도 실린다: `settings.remote.touchScrollSensitivity`(한 손가락, 기본 1)와 `settings.remote.twoFingerScrollSensitivity`(두 손가락, 기본 5), 각 0.1~20([ADR-0177](../adr/0177-remote-two-finger-touch-scroll-sensitivity.md)). 이 둘은 xterm 옵션이 아니라 페이지 지역 상태(`adoptTouchScrollSensitivity`)로 흡수되며 저장 즉시 현재 문서에 반영된다(별도 appearance 재푸시 없음). Remote Settings drawer는 연결 시와 drawer 진입 시 이 projection을 다시 읽고, 저장 성공 시 현재 xterm appearance를 다시 적용해 fit하며 composer CSS 변수도 즉시 갱신한다. revision 충돌은 `409`로 받고 최신 projection을 재조회한다.

| Endpoint | Method | 권한·응답 |
|---|---|---|
| `/remote/v1/display-settings` | GET | 기존 Remote enabled + Direct bearer/IP/Origin 또는 Android E2E session/AEAD gate. lease 불필요. `{terminalFontSize,composerFontSize,menuFontSize,touchScrollSensitivity,twoFingerScrollSensitivity,revision}` 반환(민감도는 0.1~20 clamp) |
| `/remote/v1/display-settings` | PUT | 같은 인증 gate + active controller `leaseId`. body `{leaseId,expectedRevision,terminalFontSize,composerFontSize,menuFontSize,touchScrollSensitivity,twoFingerScrollSensitivity}`; 공통 settings snapshot/apply bridge의 CAS로 검증·Zustand 갱신·영속 저장. stale revision은 `409` |
