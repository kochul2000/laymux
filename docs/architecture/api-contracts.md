...
[ADR-0226](../adr/0226-composer-stars-are-host-global-persistent-state.md)에 따라 사용자가 recall/자동완성 행의 별표 버튼을 명시적으로 누르거나 Desktop Settings의 Terminal 섹션에서 직접 추가한 문자열만 비영속 경계의 예외다. SoT는 `settings.json`의 backend-owned `terminal.composerStarredEntries: string[]`이며 workspace·pane·history scope와 무관한 호스트 전역 ordered set이다. 최대 200개, 항목당 UTF-8 16 KiB이고 빈 문자열·중복·한도 초과는 거절한다. 일반 settings patch에서는 `/terminal/composerStarredEntries`가 sensitive read-only이고 frontend checkpoint는 디스크 최신값을 보존한다. Desktop Settings는 전체 목록을 표시하며 추가·삭제를 일반 Save 초안과 분리해 즉시 전용 IPC로 반영한다. Desktop IPC `set_composer_starred_entry(text,starred)`와 bearer/IP/origin guard + active controller lease를 모두 요구하는 `GET /remote/v1/composer/starred?leaseId=...`, `POST /remote/v1/composer/starred { leaseId,text,starred }`만 진입점이다. 응답은 `{revision:number,entries?:string[]}`이고 mutation은 atomic replace 뒤 `composer-starred-entries-changed` event를 내보낸다. Remote는 claim 직후 목록을 읽고 `localStorage` 사본을 만들지 않는다. 자동완성은 별표 최신순을 runtime history보다 먼저 합성하고 중복·exact query·기존 표시 cap을 유지한다. 별표 버튼 pointer action은 행 선택을 막고 editor focus를 유지한다. 아래의 “Remote는 host settings를 읽지 않는다”는 설명은 이 좁은 별표 전용 endpoint를 제외한 범용 settings surface가 없다는 뜻이다.

Remote 별표 GET/POST 응답은 runtime 단조 revision과 선택적 전체 목록 `{revision:number, entries?:string[]}`을 반환한다. GET의 `revision` query가 현재값과 같으면 `entries`를 생략한다. Remote는 claim 직후 전체 목록을 읽고 heartbeat 성공 뒤 조건부 GET으로 Desktop 변경을 수렴시키며, 더 낮은 revision의 늦은 응답은 적용하지 않는다. lease를 잃으면 surface revision과 목록을 함께 폐기한다.
