...
클라이언트는 이 `403` 을 파일 수준 에러로 취급하지 않는다. FileViewer/Explorer 요청이 `401`·`403`·`409` 로 실패하면 `/remote/v1/session/status` 로 소유권을 한 번 되묻고, 응답의 `leaseId` 가 자기 lease 와 다르면 heartbeat `409` 와 같은 등급(`hostTookOver: false`)으로 제어권 상실을 처리해 재연결 의사를 유지한 채 오버레이를 닫는다. 프로브가 실패하거나 lease 가 살아 있으면 원래 에러를 그대로 보여준다. 또한 `visibilitychange`(visible)·`pageshow`·`online` 은 lease 를 쥔 채 돌아온 경우 `maybeAutoConnect()`(없는 lease 를 claim 하는 경로)로 빠지지 않고 즉시 heartbeat 로 그 lease 를 프로브한다 — 얼린 탭에서는 heartbeat 인터벌 자체가 멈추므로 복귀 직후의 첫 조작이 죽은 lease 를 만나지 않게 한다 ([ADR-0204](../adr/0204-remote-lease-recovery-on-control-failure.md)).
