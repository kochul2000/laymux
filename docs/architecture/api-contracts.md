...
Android system back은 Remote 문서를 벗어나기 전에 고정된 `window.laymuxRemoteUi.dismissTopLayer()`를 평가한다([ADR-0219](../adr/0219-android-back-defers-to-remote-ui-stack.md)). 이 함수는 OAuth relay modal → FileViewer overlay → Composer recall/autocomplete → drawer 하위 화면 → 열린 Dock disclosure → drawer 순서로 현재 보이는 최상위 레이어 하나만 닫고 소비 여부만 반환한다. Android native는 DOM·drawer view·viewer mode를 해석하지 않으며, 문서 세대와 Remote 전면 surface가 callback 시점에도 유효할 때만 그 결과를 받는다. 레이어가 닫히면 이전 disconnect 경고를 해제하고, 닫을 레이어가 없을 때만 첫 back 경고·2초 안의 두 번째 back disconnect 정책을 적용한다. 연속 back 평가가 진행 중인 동안 중복 평가는 시작하지 않는다.
