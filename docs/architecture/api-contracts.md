...
Remote 입력 action은 기본행(`main`)·`Keys` 확장행(`expanded`)·숨김(`hidden`)의 3-zone 배치를 사용한다([ADR-0186](../adr/0186-remote-input-action-three-zone-layout.md)). 권장 기본값은 기본행 `Ctrl+C · Keyboard · Keys · Send`, 확장행 `Composer 전환 · 기존 기본 특수키 배열(step+nav)`, 숨김 `Attach file`이다. Send는 저장된 zone과 무관하게 Composer mode일 때만 렌더한다. `Keys`는 기본행 또는 숨김만 가능하고 확장행에는 둘 수 없는 구조 토글이다. Keys를 숨기면 열린 확장행을 즉시 닫아 `expanded=false`로 저장하되 확장행 구성과 순서는 보존한다. 그 밖의 고정 action(`Ctrl+C`, `Keyboard`, `Send`, `Composer`, `Attach file`)과 모든 일반 소프트키는 세 zone 사이에서 이동하고 각 행 안에서 재정렬할 수 있다. 배치 편집은 key bar 안의 팝오버가 아니라 항상 접근 가능한 Remote drawer의 Settings → Input bar에 둔다.

`laymux.remote.keybar`의 `zones`가 action 배치와 행내 순서의 SoT이고, `sets`/`custom`은 활성 일반 소프트키 집합만 결정한다. 비활성 소프트키의 zone과 행내 순서는 보존되어 다시 활성화할 때 복구된다. 기존 `order`는 `zones`에서 매번 재계산하는 ADR-0040 호환 projection이며 zone이나 행내 순서의 독립 소유권을 갖지 않는다. 새 custom 키를 최초 선택하면 ADR-0040대로 활성 Keys 행 끝에 붙이되, 이미 main/hidden으로 배치한 key의 zone은 바꾸지 않고 비활성화 후 재활성화할 때도 보존된 zone과 순서를 유지한다. 저장값의 구조·ID·중복이나 Keys 위치가 비정상이면 안전한 기본값으로 정규화하고 별도 버전/마이그레이션은 두지 않는다. 배치·확장 상태는 `localStorage`에만 저장하는 surface-local UI 상태이며 `settings.json`, Remote API와 PTY 계약은 바꾸지 않는다([ADR-0004](../adr/0004-settings-vs-ui-state-separation.md), [ADR-0028](../adr/0028-remote-soft-key-toolbar.md), [ADR-0040](../adr/0040-remote-soft-key-user-order.md)).

확장행의 방향키·Tab·Esc·PgUp/PgDn·Ctrl 조합·F 키 등은 기존 `Ctrl+C`와 동일하게 `enqueueInput` → `/remote/v1/terminals/{id}/write` 로 escape 시퀀스를 보낸다(새 endpoint 없음). 방향키·Home·End는 `terminal.modes.applicationCursorKeysMode`를 반영해 SS3(`\x1bO`)/CSI(`\x1b[`)를 고른다. 키 버튼과 `Ctrl+C`·`Keys`·`Keyboard`, header pane 복사 버튼은 pointer activation의 기본 포커스 이동을 `mousedown`·`pointerdown` 양쪽에서 막아 현재 포커스된 입력 표면(composer 에디터 또는 xterm helper textarea)과 열린 모바일 소프트 키보드를 유지하되 실제 전송은 `click`으로 처리한다. WebKit/iOS는 `pointerdown` preventDefault만으로 포커스 이동을 막지 못하므로 `mousedown`도 함께 막는다(공유 `preventFocusSteal`/`keepInputSurfaceFocus`, [#482](https://github.com/kochul2000/laymux/issues/482)). Navigation 세트의 `↕↔` 방향 패드는 누르는 동안 상·우·하·좌 힌트를 표시하고 18px 이상 flick한 우세 방향을 기존 입력 경로로 보낸다. 임계거리 미만 탭과 취소 pointer는 입력을 보내지 않는다.

Remote drawer Settings의 `Key order` 섹션은 활성 키를 main/Keys 행 경계가 보이는 컴팩트 칩 그리드로 보여 주며 long-press Pointer Events drag와 삽입 표시선으로 각 행 안 순서를 바꾼다. 칩을 탭하면 현재 행 안에서만 동작하는 `맨 앞`·`왼쪽`·`오른쪽`·`맨 뒤` 보조 동작이 나타나고, 행 경계를 넘는 대상은 disabled이며 drag target으로도 받지 않는다. `Reset`은 zone을 바꾸지 않고 각 zone의 소프트키를 `KEY_ORDER` 순서로 복원한다. 비활성 커스텀 키를 최초 선택하면 현재 활성 Keys 행의 맨 뒤에 추가하고, 비활성화 후 재활성화에는 보존된 zone과 순서를 쓴다. `order`가 없는 저장 값은 정본 `KEY_ORDER`로 보완하고 알 수 없는 ID·중복 ID는 제거한다. 기본행과 확장행은 화면 폭보다 길면 각 행 내부에서만 좌우 스크롤하며 한 줄을 유지하고 scrollbar track은 노출하지 않는다. app/header/main/footer/key-bar 경계는 `min-width: 0`을 유지해 문서 outer overflow를 만들지 않는다.
