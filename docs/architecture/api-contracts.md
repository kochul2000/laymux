...
delivery worker는 generation당 receipt 전 immutable v3 envelope를 최대 4개 보유한다([ADR-0098](../adr/0098-bounded-desktop-output-receipt-pipeline.md)). slot마다 identity·deadline·repair 회수를 가지며 receipt는 순서와 무관하게 정확한 slot 하나를 release한다. 같은 identity/payload의 receipt retry는 bounded recent history에서 idempotent하고, repair/hold/close도 target identity의 slot 또는 recent receipt boundary를 찾는다. 모든 slot이 찬 경우에만 다음 envelope emit을 멈추며, 가장 이른 receipt deadline 만료는 기존 typed fail-stop이다. API의 `receiptSlot` 진단 필드는 호환성을 위해 가장 오래된 미수령 slot을 뜻한다.
