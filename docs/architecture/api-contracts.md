...
Remote drawer는 PC selector의 `display.minimap/environment/activity/path/result`와 `pathEllipsis`를 직접 gate로 사용한다. 각 workspace는 pane 목록 아래에 PC와 같은 마지막 명령(상태 아이콘·계산된 상태 문구 또는 명령·상대 시간) 또는 최신 unread 알림 한 줄을 항상 확보한다. 열린 drawer의 정보는 2초 주기로 다시 읽으며 drawer close, disconnect, document hidden에서 폴을 멈춘다. 선택 revision이나 lease가 달라진 늦은 응답은 적용하지 않는다.
