...
Remote drawer 본문은 workspace 기본 화면과 Hidden workspaces·Notifications·Connection·Settings 하위 화면을 가진다. 연결된 상태에서 drawer를 열면 매번 workspace 기본 화면으로 시작하며 workspace 목록과 Dock terminals를 렌더한다. 파일 탐색과 직접 경로 열기는 drawer가 아니라 메인 헤더 폴더 버튼이 여는 Remote Explorer가 소유한다. Hidden workspaces·Notifications·Connection은 헤더의 각 버튼으로, 기기-로컬 Display와 설치 권유는 Settings 버튼으로 진입하고 뒤로가기는 workspace 기본 화면으로 돌아가 진입 버튼에 focus를 복원한다. Hidden workspaces와 Notifications의 상태는 Settings 업데이트 표시와 같은 5px 점으로만 나타내며 숫자 badge를 그리지 않는다. exact hidden/unread 개수는 각 버튼의 접근성 이름과 title에 유지한다([ADR-0187](../adr/0187-remote-drawer-status-dots-and-hidden-subview.md)). 기본은 기존 floating drawer이며 상단 메뉴 버튼이나 바깥 scrim으로 닫는다. drawer 설정과 상단 핀 버튼은 같은 `navigationPinned` 요청값을 토글하며, `window.innerWidth > navigationPinCutoff`일 때만 drawer를 terminal 옆 열에 고정하고 scrim·메뉴 버튼·close 동작을 비활성화한다. 컷오프 이하에서는 요청값을 보존한 채 floating drawer로 폴백한다. `navigationWidth`는 두 모드에 공통 적용하며 이 세 값은 연결·lease와 무관한 기기 로컬 표시 설정이다([ADR-0233](../adr/0233-remote-workspace-menu-pinning.md)). 아직 lease가 없거나 확정적으로 제어권을 잃어 drawer가 열릴 때만 Connection을 바로 표시한다. 하위 화면 선택은 ADR-0015의 surface-local drawer 상태이며 Remote API나 host 설정에 저장하지 않는다.
