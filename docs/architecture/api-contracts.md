...
`laymux.remote.keybar`의 `zones`가 action 배치와 행내 순서의 SoT이고, `sets`/`custom`은 활성 일반 소프트키 집합만 결정한다. 비활성 소프트키의 zone은 보존되어 다시 활성화할 때 복구된다. 기존 `order`는 ADR-0040의 전체 소프트키 정렬 조작과 호환되는 projection이며 zone 소유권을 갖지 않는다. 새 custom 키를 선택하면 ADR-0040대로 활성 Keys 행 끝에 붙이되, 이미 main/hidden으로 배치한 key의 zone은 바꾸지 않는다. 저장값의 구조·ID·중복이나 Keys 위치가 비정상이면 안전한 기본값으로 정규화하고 별도 버전/마이그레이션은 두지 않는다. 배치·확장 상태는 `localStorage`에만 저장하는 surface-local UI 상태이며 `settings.json`, Remote API와 PTY 계약은 바꾸지 않는다([ADR-0004](../adr/0004-settings-vs-ui-state-separation.md), [ADR-0028](../adr/0028-remote-soft-key-toolbar.md), [ADR-0040](../adr/0040-remote-soft-key-user-order.md)).
...
Remote drawer Settings의 `Key order` 섹션은 활성 키를 커스텀 팔레트와 같은 컴팩트 칩 그리드로 보여 주며 long-press Pointer Events drag와 삽입 표시선으로 순서를 바꾼다. 칩을 탭하면 `맨 앞`·`왼쪽`·`오른쪽`·`맨 뒤` 보조 동작이 나타나고 `Reset`은 `KEY_ORDER`로 복원한다. 비활성 커스텀 키를 새로 선택하면 현재 활성 키의 맨 뒤에 추가한다. `order`가 없는 저장 값은 정본 `KEY_ORDER`로 보완하고 알 수 없는 ID·중복 ID는 제거한다. 기본행과 확장행은 화면 폭보다 길면 각 행 내부에서만 좌우 스크롤하며 한 줄을 유지하고 scrollbar track은 노출하지 않는다. app/header/main/footer/key-bar 경계는 `min-width: 0`을 유지해 문서 outer overflow를 만들지 않는다.
