...
Remote Settings의 `Floating` 탭은 커서·패널/알림 플릭 패드를 각각 켜고 크기·X/Y 위치를 조절하며, 기존 입력 액션에서 일반 탭 버튼을 여러 개 추가·삭제한다([ADR-0237](../adr/0237-remote-floating-input-controls.md)). 배치는 `laymux.remote.keybar.floating = { pads: { dpad, navPad }, buttons: [{ id, actionId, enabled, size, x, y }] }`에 기기 로컬로 저장한다. 각 패드도 `enabled, size, x, y`를 가지며 기본 비활성이다. 크기는 44–128px, X/Y는 가용 화면 안의 0–1 좌표다. 일반 버튼은 직접 드래그, 패드는 별도 이동 손잡이로 움직이고 드래그·취소는 입력을 보내지 않는다. 일반 플로팅 방향키도 탭만 실행하며 패드의 기존 플릭/반복은 유지한다. visual viewport 변화 시 화면 안으로 재배치한다. 메뉴·pane 식별자 복사·파일 탐색·스킵·PC 모드는 Hidden 목록과 플로팅 선택지에 제공하며 기존 원본 버튼의 활성 조건과 실행을 공유한다. API 추가 없이 브라우저 DOM 조작과 스크린샷으로 설정·이동을 검증한다.
