...
`force=true`는 **소스 측 게이트를 모두 우회**한다 — 위 가드 목록의 1(에코 루프), 2(소스 activity=Shell), 3(`cwd_send`), 그리고 대상 측 4의 `cwd_receive` 필터까지. 즉 사용자가 직접 누른 명시적 의도이므로 자동 전파의 노이즈 차단 게이트를 적용하지 않는다.

단, 대상 측 `filter_targets_not_busy`(`Running`/`InteractiveApp` 제외)와 `filter_targets_needing_cd`(동일 CWD 중복 제외)는 `force` 여부와 무관하게 유지한다 — 명령 실행 중이거나 TUI 앱이 떠 있는 대상에 `cd`를 주입하면 그 입력 버퍼를 오염시키기 때문이다.

**대상이 file explorer일 때의 추종(프론트 경로).** file explorer의 CWD 추종은 백엔드 `cd` 주입이 아니라 순수 프론트 경로다. `FileExplorerView`는 이벤트 리스너를 **cwdReceive와 무관하게 항상 등록**하고, 두 이벤트를 구분 처리한다: ① `terminal-cwd-changed`(일반 OSC 변경)는 `cwdReceive on` + 소스 `cwdSend !== false`일 때만 추종, ② `sync-cwd` 페이로드의 `force === true`이고 `groupId`가 자신의 syncGroup과 같으면 cwdReceive가 off여도 추종. `do_sync_cwd`는 `EVENT_SYNC_CWD` 페이로드에 `force`를 실어 보낸다.
...
**상태 갱신·이벤트 대상 = `idle_targets` (2차 리뷰 P1, issue #293).** `do_sync_cwd`가 backend `session.cwd`를 갱신하고 `EVENT_SYNC_CWD.targets`에 싣는 집합은 `receiving_targets`(busy 포함)가 아니라 `idle_targets`다. busy(`Running`/`InteractiveApp`)로 `filter_targets_not_busy`에서 제외된 대상은 실제 `cd`를 받지 못해 목적 CWD에 있지 않은데, 이를 도착으로 표기하면 그 대상이 idle이 된 뒤 같은 one-shot을 다시 눌러도 `filter_targets_needing_cd`가 "이미 동일 CWD"로 보고 `cd`를 영구히 건너뛴다. `idle_targets`는 실제 `cd`된 `target_terminals` + 이미 같은 CWD라 skip된 것들의 합집합이라 둘 다 목적 CWD에 있음이 보장된다. force 경로는 `cwd_receive=off` 대상까지 `receiving_targets`에 넣으므로 이 버그가 바로 재현됐다.
...
> **전체·정본 엔드포인트 목록은 `REGISTERED_ROUTES`(`automation_server/types.rs`)와 `GET /api/v1/docs`(JSON 자기설명)가 SoT** 다. e2e 테스트가 `build_router()` ↔ `/docs` 일치를 강제한다(현재 등록 라우트 52개). 아래 표는 대표 엔드포인트 요약이며 전수 목록이 아니다.
