...
PC WebView의 `attach_terminal_output(id)` 응답은 raw attach state/snapshot과 `flowControl:{token:string,windowBytes:number}`를 함께 반환한다. token은 generation-local desktop lease의 불투명 문자열이며 JavaScript 안전 정수로 해석하지 않는다. generation당 활성 desktop lease는 하나이고 새 attach는 같은 generation의 이전 token을 즉시 무효화한다. visible xterm과 rendererless checkpoint xterm이 같은 contiguous prefix를 모두 parse하면 `acknowledge_terminal_output(id,generation,token,seq)`를 호출한다. 현재 generation/token이면 `true`, stale generation/token이면 `false`이며, 같은 seq 재전송은 idempotent 성공이다. 이미 ACK한 값보다 작은 seq 또는 backend `write_seq`보다 큰 seq는 계약 오류다. ACK IPC 일시 실패는 같은 token/seq로 재시도하되 reattach/unmount된 sender는 폐기하고, `false` 응답은 stale lease로 판정해 재시도 없이 현재 surface를 재부착한다. attach와 token-scoped ACK Promise는 각각 5초 watchdog을 가진다. timeout이면 캡처한 epoch/token sender를 먼저 폐기하고 current epoch 하나만 50 ms부터 최대 1,000 ms의 backoff로 교체하며, orphan late resolve/reject는 흡수해 새 lease·ACK prefix에 반영하지 않는다. attach fulfillment와 backend가 수락한 ACK만 각 rate-backoff streak를 초기화한다. 자원 상한은 React effect 밖의 WebView/window-scoped registry가 소유한다. attach/ACK bridge 호출 직전에 terminal-local lease와 WebView-global lease를 함께 얻고 underlying resolve/reject에서 exactly once 둘 다 내린다. global lease 실패는 local lease와 빈 entry를 즉시 rollback하므로 IPC나 entry를 남기지 않는다. 상한은 종류별 terminal id당 6개이면서 WebView 전체 6개, 즉 한 WebView에서 attach 최대 6개 + ACK 최대 6개 = 합계 최대 12개다. 같은 id remount와 서로 다른 id의 반복 생성·제거 모두 이 global 계수를 공유한다. local/global cap이면 readiness를 닫아 producer를 bounded fail-stop하고 종류별 global FIFO waiter에 current mount를 둔다. late settle은 반환 슬롯마다 stale owner를 건너뛰고 eligible current mount 하나만 깨우며, local cap에 막힌 terminal은 큐 위치를 유지한 채 다음 eligible terminal이 global slot을 사용하게 한다. unmount는 자기 waiter와 epoch만 폐기하고 composite lease는 정착까지 유지한다. terminal entry는 actual outstanding 동안만 존재하므로 cap에서 거절된 새 id는 entry를 남기지 않고, 삭제된 terminal의 영구 pending operation은 정착 또는 WebView 종료까지 기존 entry/global 계수에 남는다. recovery 상태 변경은 warning/counter보다 먼저 확정하며 진단 실패가 이를 막지 않는다([ADR-0086](../adr/0086-terminal-output-control-epoch-watchdog.md)). 이 credit 계약은 desktop Tauri surface에만 적용하고 Remote/Cloud output frame과 subscriber overflow 계약은 바꾸지 않는다([ADR-0084](../adr/0084-desktop-terminal-output-parsed-credit.md)).
...
현재 `resize_terminal` Tauri IPC, `/remote/v1/terminals/{id}/resize`의 one-shot HTTP request/response와 Remote V1 output WebSocket에는 prepare/apply/status token, browser source-prefix ACK, adoption ACK가 없고 exact request entry point도 노출하지 않는다. Automation/MCP/HTTP caller도 parser surface가 아니므로 스스로 ACK할 수 없다. 따라서 현재 runtime은 `exactGeometryCutover` capability를 광고하지 않는다. #632가 Local·Remote owner coordinator protocol과 platform provenance capability를 함께 구현해 exact entry point를 추가할 때 ACK protocol이 없는 surface의 요청은 physical resize 전에 거절하며, current owner coordinator에 위임하지 않는 Automation/Remote 요청을 one-shot resize로 fallback하지 않는다.
