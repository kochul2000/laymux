...
RPC plaintext `kind=http`는 session status/claim/heartbeat/release, navigation/widgets/notification/workspace/terminal Remote 동작의 exact method/path만 desktop 내부 Remote router에 재주입한다. PC page가 async request id와 JSON body를 native bridge에 넘기며 Kotlin은 terminal·workspace·lease 의미를 해석하지 않는다. `terminalOutputOpen`은 render checkpoint snapshot과 generation/source sequence, wire sequence offset, 기존 V1 snapshot header와 binary body를 반환한다. `terminalOutputPoll`은 terminal output ring의 exact resume 결과인 V1 delta header/body·idle·reattach를 반환한다. Android는 active output stream마다 120ms fixed-delay로 poll하고, PC page의 기존 V1 parser와 claim/heartbeat/lease 로직이 browser transport와 동일하게 이를 소비한다.

RPC plaintext의 field 이름은 tag와 마찬가지로 camelCase가 정본이다. `resource`는 `path`, `http`는 `method`·`path`·`body`, `terminalOutputOpen`은 `terminalId`·`leaseId`, `terminalOutputPoll`은 `terminalId`·`leaseId`·`generation`·`sourceSeq`·`wireSeqOffset`을 보낸다. desktop은 알 수 없는 field를 거부하므로 Rust field 철자(`terminal_id` 등)는 계약이 아니며 받지 않는다. 이 계약을 벗어난 요청은 plaintext 파싱 단계에서 `500`이 되고, Android에는 terminal 본문이 비어 있고 무한 재연결하는 증상으로만 보인다.

cloud server의 세 public route는 public origin에서만 받고 challenge/establish는 4 KiB request·8 KiB response, RPC는 2 MiB request·6 MiB response로 제한한다. RPC response 상한은 1 MiB renderer checkpoint와 전체 bounded output-ring suffix가 내부·외부 base64 확장을 거쳐도 들어가는 값이며 desktop 테스트가 이론상 최대 조합을 고정한다. exact Pydantic schema, IP·instance별 rate limit, 기존 user/IP concurrency limit을 적용하고 caller가 desktop path/header/query를 고르지 못하게 하며 모든 desktop E2E 응답은 `Cache-Control: no-store`다. challenge/establish는 body parsing 전부터 기존 인증 시도 제한(기본 60초당 30회)을 쓰고, 정상 120ms polling을 막지 않도록 RPC는 별도 bounded limiter `ANDROID_E2E_RPC_RATE_LIMIT_ATTEMPTS`(기본 같은 60초 window당 1200회)를 IP와 instance 각각에 적용한다. relay는 ciphertext 내부 UI resource·Remote operation·terminal bytes·lease를 해석하거나 저장하지 않는다. APK WebView는 `connect-src 'none'`이고 network/key/sequence는 Kotlin native 계층이 소유한다. xterm.js·fit addon·CSS는 PC Laymux가 제공하고 AEAD 검증된 resource bridge를 통해서만 WebView에 들어온다.
