...
`path-link` body는 `{ "terminalId": "...", "selection": "..." }`이며 client CWD·path·좌표는 받지 않는다. Rust route는 빈 필드, Unicode scalar 256자 초과 terminal id, 4096자 초과 transport selection을 `400`으로 거르고 `fileViewer.pathLink` async bridge에 원문을 전달한다. frontend는 [ADR-0148](../adr/0148-bounded-multi-path-selection-links.md)에 따라 최대 1,024자·8줄에서 maximal-munch 비중첩 후보를 최대 16개 추출하고, 해당 terminal의 최신 store CWD와 `terminal.pathLinkEnabled`/후보별 `pathLinkMaxLength`를 적용해 `joinCwdPath` 뒤 중복 절대경로를 제거한 `statPaths` 한 번으로 검증한다. 설정 off, 부적합/초과 선택, CWD 없음, 존재하는 경로 없음은 `{valid:false}`이고 성공은 `{valid:true,matches:[{token,path,kind,lineIndex,startIndex,endIndex}]}`다. `kind`는 `"file" | "directory"`이며 클라이언트는 file 클릭을 render로, directory 클릭을 explorer 모드 list로 라우팅한다([ADR-0198](../adr/0198-remote-file-explorer-overlay.md) — 디렉터리 후보를 비활성으로 두던 이전 결정을 개정). 각 범위는 선택 원문 기준 UTF-16 offset이며 브라우저가 최신 선택과 exact match인지 재검증한 뒤 밑줄 좌표로 바꾼다. 브라우저는 드래그 중 selection 변화를 trailing debounce하고 새 선택에서 진행 중 요청을 취소한다. 요청 당시 selection revision·terminal·lease·capability가 하나라도 바뀐 응답은 버리며, 응답 시점의 최신 xterm selection 좌표로 각 밑줄 범위를 다시 계산한다. 좌표/decoration은 ADR-0015의 surface-local 상태로 유지한다. 검증은 stat만 수행하며 파일 내용은 prefetch하지 않는다.
