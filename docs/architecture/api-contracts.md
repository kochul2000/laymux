...
**손실 정책.** 드롭이 한 건이라도 있으면 `recovered` 상태가 되고, `useSessionPersistence` 가 `setBlockPersist(true)` 로 공유 쓰기 차단 상태를 켠 채 기동한다. `saveSettings` 경계가 세션 영속과 Automation `settings.applySnapshot`을 포함한 모든 일반 `settings.json` 쓰기에 이 상태를 강제하며, 원본을 의도적으로 대체하는 `resetSettings`만 우회한다. 사용자가 `SettingsRecoveryModal` 에서 확인하면 backend가 그 시점의 파일을 다시 lenient-load해 atomic replace하고, 그 정확한 snapshot의 Remote access·cloud tunnel·update channel을 backend runtime에도 재조정한 뒤 반환한다. 프론트는 이 반환 snapshot을 구조 상태까지 전부 store에 적용한 뒤에만 쓰기 차단을 푼다. 따라서 모달을 연 채 사용자가 파일을 고친 경우에도 startup 때 읽은 오래된 복구본이 다음 checkpoint에서 수동 편집을 덮거나 backend가 과거 Remote 권한을 계속 적용하지 않는다. 그전까지는 레이아웃 변경도 저장되지 않으며 원본 파일은 손대지 않는다. 로더 자체는 어떤 경우에도 복구 결과를 디스크에 되쓰지 않는다. 모달은 드롭된 경로를 전부 나열한다(개수 요약만으로는 무엇을 잃었는지 알 수 없다).
...
Remote 기기는 같은 네 값을 PC 설정과 분리해 `localStorage["laymux.remote.displaySettings"]`에 둔다([ADR-0209](../adr/0209-remote-display-preferences-are-device-local.md)). `scrollSensitivity`(기본 1), `fastScrollSensitivity`(기본 5), `touchScrollSensitivity`(기본 1), `twoFingerScrollSensitivity`(기본 5)이며 Remote drawer에서 연결 전에도 편집할 수 있다. 따라서 데스크톱과 각 Remote 기기는 서로 상속·동기화하지 않는다([ADR-0142](../adr/0142-wheel-scroll-sensitivity-per-surface.md)).

유효 범위는 모두 `0.1..=20`이다. 데스크톱 상수는 Rust `constants.rs`와 프론트 `lib/scroll-sensitivity.ts`, Remote 상수와 기본값은 정적 Remote client가 각각 소유한다. **비양수·비수치는 clamp 대상이 아니라 기본값 fallback 대상**이다 — xterm 이 비양수 sensitivity 에 throw 하기 때문이며, 양수인데 범위를 벗어난 값만 경계로 clamp 한다. `validate_settings` 는 PC 설정인 `/terminal/scrollSensitivity`·`/terminal/fastScrollSensitivity`만 보고하고, Remote client는 localStorage를 읽고 저장할 때 자체 정규화한다.
...
`touchScrollSensitivity`와 `twoFingerScrollSensitivity`는 **xterm 옵션이 아니다**([ADR-0177](../adr/0177-remote-two-finger-touch-scroll-sensitivity.md)). Remote 페이지가 소유한 손가락 드래그의 픽셀→행 환산에서 입력 델타에 한 번 곱하며(`consumeTouchScrollLines(term, deltaY, sensitivity)`), 하위 셀 나머지(`scrollRemainderPx`)에는 다시 곱하지 않는다. 옵션 번들 대신 페이지 지역 상태(`adoptTouchScrollSensitivity`)가 두 값을 들고 저장 즉시 갱신한다. 정상 스크롤백 모드에서는 계산한 행 수를 xterm viewport에 적용한다. 마우스 트래킹이 꺼진 alternate buffer(Codex transcript overlay 등)에서는 같은 계산 결과를 위/아래 커서키로 바꾸고, ConPTY의 반복 축약을 피하도록 각 행을 별도 Remote write로 보낸다. **마우스 트래킹을 켠 전체화면 TUI에서는 한 손가락·두 손가락 모두 기기 로컬 `scrollSensitivity`가 적용된다**(두 배율을 겹쳐 곱하지 않는다).
...
`cwd` 위젯은 옵션을 갖지 않는다. 표시 대상은 키보드 입력과 같은 포커스 SoT 로 계산한다 — dock 포커스가 있으면 해당 dock pane, 아니면 활성 workspace 의 `focusedPaneIndex`가 가리키는 pane이며, 그 pane 이 `TerminalView`일 때만 live `terminalStore` CWD를 표시한다. workspace마다 남아 있는 `TerminalInstance.isFocused` 메타데이터는 chrome의 현재 포커스 판정에 사용하지 않는다. 데스크톱 위젯과 Remote snapshot은 이 계산 함수를 공유한다.
...
위젯 설정은 실제 컴포넌트 미리보기 → 네 위치의 개수 표시·선택 → 선택한 위치의 순서 목록·추가 → 선택한 위젯의 이동·제거·옵션 순서로 편집한다. 미리보기에서 위젯을 고르거나 다른 위치로 옮기면 해당 위치의 목록을 보여 준다. 공통 글꼴·글자 크기·status line 표시는 기본 접힌 공통 모양 영역에 둔다. 위치 선택과 펼침 상태는 편집기 로컬 UI 상태이며 설정 스키마에 저장하지 않는다. 꺼진 status line도 편집할 수 있고, 켜는 위치를 안내한다(ADR-0105·0107 직접 적용).
...
Remote Access 모달은 런타임 성격의 조작만 담당한다: 이번 실행 동안 허용, URL/token 복사, 데스크톱 앱 내부 모바일 모드 열기, remote controller reclaim. PC Settings에는 `Remote Connection` 섹션 하나만 두고 시작 시 자동 허용, 인증·IP allowlist, 자동 모바일 폭, host·Cloud 연결 정책과 호스트 데이터 공개 정책(`serveTerminalFont`, `widgets`)을 편집한다. 별도 `Remote Display` 섹션은 없다. terminal/composer/menu 글자 크기, Composer 상태별 불투명도, checkpoint 예산, 네 가지 스크롤 민감도, workspace 메뉴 고정·너비·고정 컷오프는 각 Remote 기기 UI가 `localStorage["laymux.remote.displaySettings"]`에 저장한다([ADR-0209](../adr/0209-remote-display-preferences-are-device-local.md), [ADR-0233](../adr/0233-remote-workspace-menu-pinning.md)). 메뉴 고정은 drawer 설정과 메뉴 도구 줄 맨 왼쪽의 핀 버튼이 같은 `navigationPinned` 값을 편집한다. 데스크톱 앱 내부의 모바일 모드는 기존 `/remote/` Direct Remote UI를 `localApp=1&autoConnect=1` iframe으로 여는 로컬 전용 표시 모드이며, 이 iframe도 자기 WebView storage를 사용하는 하나의 Remote 기기다. 해당 iframe은 remote lease를 잡을 수 있으므로 PC WebView의 remote-control overlay는 로컬 모바일 모드가 활성인 동안 숨긴다. 오버레이의 종료 경로는 호스트가 소유한다([ADR-0215](../adr/0215-remote-page-frames-only-the-desktop-app.md)): iframe이 `localApp=1`로 부팅하면 `laymux:mobile-mode-ready`를 부모에게 보내고, 그 인사가 오지 않으면 호스트가 직접 "PC 모드로 돌아가기" 카드를 그린다. ESC는 인사가 오기 전까지 오버레이를 닫는다 — 인사한 뒤의 ESC는 살아 있는 모바일 뷰가 갖는다. 오버레이는 iframe origin에서 온 `laymux:mobile-mode-ready`·`laymux:desktop-mode`만 받고, 비교할 origin이 없으면 fail-closed로 무시한다. 인사·타임아웃 판정은 URL이 아니라 store의 진입 회차(`session`)로 키잉한다 — 같은 port·token은 매번 같은 URL을 만들고 오버레이는 언마운트되지 않으므로, URL로 키잉하면 재진입이 이전 회차의 판정을 물려받는다.
...
```jsonc
{
  "remote": {
    ...
    "cloudAccessMode": "browserAndE2e", // browserAndE2e(기본) | androidE2eOnly. Cloud WSS data plane만 제어
    ...
    "attachmentMaxMib": 1,              // Remote 첨부 최대 크기(MiB, 1~10). Cloud relay 경로는 relay 상한이 더 작으면 그 값으로 제한(ADR-0227)
    "attachmentAllowAllExtensions": false, // 모든 파일 형식을 내용 검사 없이 opaque binary 로 받음
    "attachmentExtraExtensions": []     // 기본 종류 외에 그대로 저장할 확장자(소문자, 점 없이)
  }
}
```

Remote 기기 로컬 표시 설정의 저장 형태는 다음과 같다. 이 객체는 `settings.json`, Settings IPC/MCP, Cloud tunnel로 동기화하지 않는다. 누락·타입 오류·범위 밖 값은 Remote client가 필드별 기본값 또는 경계값으로 정규화하며 별도 마이그레이션은 하지 않는다.

```jsonc
// localStorage["laymux.remote.displaySettings"]
{
  ...
  "snapshotMaxKib": 4,             // 최초 checkpoint 요청 예산, 1~1024 KiB
  "scrollSensitivity": 1,          // 0.1~20
...
Pairing 성공 후와 앱 시작 시(원격 제어가 실효적으로 켜져 있고 `cloudAutoReconnect=true`이며 keyring token이 존재할 때) desktop은 `cloudTunnelUrl` 로 native Rust WSS outbound tunnel을 연다. WSS 접속에는 `Authorization: Bearer <device-token>` 를 사용한다. 터널 연결 수명주기는 원격 제어 게이트(`settings.remote.enabled || runtimeRemoteAccess.enabled`)에 종속된다([ADR-0030](../adr/0030-cloud-tunnel-follows-remote-control-gate.md)): 게이트가 꺼져 있으면 터널을 열지 않아 relay에 online으로 보고하지 않고 인스턴스가 대시보드에 노출되지 않는다. `set_remote_runtime_access` 로 게이트가 꺼지면 살아 있는 터널을 즉시 종료하고(`connected=false`, instance id는 유지) 다시 켜지면 재접속한다. `cloud_connect_start` 도 게이트가 꺼져 있으면 브라우저를 열거나 relay에 접속하기 전에 거절하고 `lastError` 를 남긴다 — 원격 제어가 꺼진 상태에서는 클라우드에 어떤 액션도 하지 않는다. relay가 첫 `ready` frame 또는 이후 `heartbeat.ack` frame으로 `{ instanceId }` 를 보내면 `AppState.cloud.connected=true` 와 instance id를 갱신하고, socket이 끊기면 `connected=false` 로 전환한 뒤 지수 backoff로 재접속한다. 단, relay가 4001 또는 401 계열 인증 실패 close를 보내거나 WSS handshake가 401로 실패하면 자동 재접속을 중단하고 재-pair가 필요한 `lastError` 를 남긴다. `cloud_disconnect` 는 tunnel worker를 중지하고 device token과 Android pairing record를 keyring에서 삭제하며, cloud 저장 필드 정리와 `AppState.cloud` 리셋을 best-effort 로 수행한다.

Cloud WSS data plane의 PC별 정책 정본은 `settings.remote.cloudAccessMode`다([ADR-0150](../adr/0150-desktop-owned-cloud-remote-access-policy.md)). 기본 `browserAndE2e`는 기존 Cloud browser HTTP/WebSocket과 Android E2E를 함께 허용한다. `androidE2eOnly`는 빈 query의 exact `POST /remote/v1/e2e/{pair/ack,session/challenge,session/establish,rpc}`와 exact encrypted output WebSocket `/remote/v1/e2e/output?instanceId=...&sessionId=...&streamNonce=...`만 허용한다. output query는 세 canonical field만, headers는 빈 목록만 허용한다. 일반 Remote 문서·자산·API와 다른 WebSocket은 desktop tunnel 입구에서 non-retryable `cloud_remote_policy_denied`로 거부한다. stream open 뒤 mode가 바뀌는 경계도 HTTP dispatch 직전에 다시 검사한다. mode 저장 시 실행 중인 tunnel만 즉시 중단·재연결해 기존 stream과 heartbeat를 닫고 새 첫 heartbeat에 정책을 싣는다. 연결 자원 guard는 상위 worker가 취소돼도 pending request body를 폐기하고 responding HTTP·WebSocket·writer·presence task를 모두 abort하며, 정책 재시작은 이 worker의 종료 완료 뒤에만 새 연결을 연다. tunnel이 없으면 설정 저장만으로 Cloud presence를 만들지 않는다. 이 설정은 Local/Tailscale Direct Remote, Cloud 로그인·등록·presence와 Android public E2E relay route를 끄지 않는다.
...
Cloud tunnel heartbeat payload는 필수 `cloudAccessMode`와 선택 `tailscaleUrl`을 보낸다. 예: `{ "cloudAccessMode": "androidE2eOnly", "tailscaleUrl": "http://<tailscale-ip>:<19280|19281>/remote/" }`([ADR-0139](../adr/0139-cloud-tunnel-tailscale-route-advertisement-and-direct-gate.md), [ADR-0150](../adr/0150-desktop-owned-cloud-remote-access-policy.md)). `tailscaleUrl`은 `get_remote_host_candidates`의 Tailscale IP에서만 만들고 token·allowlist 같은 비밀은 싣지 않는다. Tailscale CLI probe는 family별 2초로 제한하고 tunnel I/O와 독립된 task에서 heartbeat 주기마다 다시 계산하며, 값이 바뀌면 즉시 갱신 frame을 보낸다. relay는 두 값을 현재 tunnel connection에만 보관한다. mode가 오기 전과 명시적으로 알 수 없는 값은 browser를 fail-closed하고, 필드가 없는 구버전 desktop heartbeat만 `browserAndE2e`로 해석한다. strict Tailscale IP/port/path 검증을 통과한 URL은 인스턴스 카드의 별도 Tailscale 버튼으로 노출하며 `androidE2eOnly`에서도 유지한다. 이는 주소 가용성 광고이지 브라우저에서의 reachability 보장이 아니며 relay 자동 fallback을 만들지 않는다.
...
```jsonc
{
  "codex": {
    ...
    "sessionMaxAgeHours": 24,            // lifecycle 없는 구버전 Codex 후보에만 적용 (0 = 나이 필터 해제, 기본 24)
...
체크포인트용 통합 IPC `get_terminal_session_attributions(claudeSessionMaxAgeHours,codexSessionMaxAgeHours,grokSessionMaxAgeHours)`는 live terminal ID별 `{generation,state,provider?,sessionId?}`를 반환한다([ADR-0222](../adr/0222-agent-session-checkpoint-coordinator.md)). `state`는 `identified | fresh | restorePending | noAgent | activeButUnidentified | unknown`이다. provider별 command 결과와 fresh process-tree liveness를 Rust에서 합치므로 프론트 activity는 판정 입력이 아니다. exact ID·ADR-0238의 검증된 fresh·권위 있는 부재·ADR-0232의 입력 전 명시적 복원점은 conclusive이며, 조회 실패는 빈 map/`noAgent`로 축약하지 않는다. WSL의 명시 distro 파싱이나 default distro 결정부터 실패한 경우도 probe 대상 없음으로 축약하지 않고 조회 건강 실패로 전달해 `unknown`으로 만든다. provider 저장소 오류는 영향을 받은 terminal/live candidate/PID 집합에만 건강 실패가 되며, native process snapshot과 WSL probe 실패 범위도 host domain별로 분리한다. 따라서 native snapshot 실패가 정확한 WSL 귀속을 덮지 않고, 한 Claude PID의 손상이 다른 Claude PID·provider·shell terminal을 `unknown`으로 만들지 않는다. Claude는 live descendant PID 이름의 session file만 읽으므로 무관한 stale JSON도 판정에 참여하지 않는다. Claude·Codex·Grok provider lookup은 동시에 시작해 provider마다 독립인 최대 3초 WSL adapter budget을 직렬 합산하지 않는다. 저장된 session의 startup override를 선택하면 PTY 생성 완료(`sessionReady`)를 기다리지 않고 15초 attribution grace를 시작하며, 그 기간의 backend `noAgent`와 `activeButUnidentified`를 `unknown`으로 승격한다.
...
- **채널을 바꾸는 표면은 데스크톱 Settings ▸ 일반 ▸ 업데이트(`nav-update` → `update-channel-select`)와 Automation 일반 설정 patch 뿐이다.** Remote drawer는 기기 로컬 표시 설정만 쓰므로 host 업데이트 채널을 바꾸지 않고, `/remote/v1/update` snapshot 의 `channel` 을 읽어 표시만 한다. beta 를 고르면 그 자리에서 경고를 띄운다(`update-channel-beta-warning`).
- **배포 대상은 당분간 Windows 설치·자동 업데이트와 Android APK다**([ADR-0240](../adr/0240-windows-android-release-targets.md)). 데스크톱 build job은 Windows x86_64 하나이며 Linux 데스크톱 빌드를 실행하지 않는다. Windows의 WSL 정적 동반 바이너리는 유지한다. 채널 검증은 `windows-x86_64`만 필수로 요구하고, 제공된 모든 플랫폼의 URL·서명·tag 검증은 유지한다. 새 채널에 과거 Linux 바이너리를 섞지 않으므로 중단 기간의 Linux 설치본에는 새 자동 업데이트가 제공되지 않는다. Android 선택 발행·채널 전진 조건은 기존대로다.
...
이 전체 경로는 하나의 session mutation permit을 보유한다. 대상 PTY의 `cd` write가 반환된 시점이 끝이 아니라, `mark_propagated`, `session.cwd` 갱신과 `EVENT_SYNC_CWD` 발행까지 완료돼야 permit을 놓는다. 따라서 update finalization은 승인된 CWD 전파의 후반 상태 갱신을 drain하기 전에 checkpoint하지 않는다([ADR-0222](../adr/0222-agent-session-checkpoint-coordinator.md)).
...
`settings.json`은 Remote 설정의 영속 정본이다. Rust는 앱 시작 시 `settings.remote`를 `AppState.remote_access`의 메모리 snapshot으로 적재하고, `save_settings`/`reset_settings` 및 cloud pairing/disconnect 설정 저장이 디스크에 성공한 뒤 같은 snapshot을 갱신한다. `save_settings`/`reset_settings`가 실효 enabled 상태를 바꾸면 snapshot 교체와 owner gate 전환을 한 트랜잭션으로 시작하고 `remote-control-changed`를 발행하며, 전환 결과에 맞춰 cloud tunnel lifecycle도 reconcile한다. 실효 enabled는 그대로지만 `cloudAccessMode`가 바뀌면 실행 중인 Cloud tunnel만 재시작한다. cloud pairing/disconnect는 access gate와 access mode를 건드리지 않고 pairing 소유 필드만 snapshot에 반영한다. human-control permit 생성과 resize/write 입력 핫패스는 이 snapshot에 runtime access override만 합성하며 설정 파일을 다시 읽거나 JSON migration·validation을 수행하지 않는다. loader 결과가 `Recovered`이면 frontend save와 backend patch는 모두 원본을 보존하며 실패한다. loader가 원문에 결부한 `recoveryRevision`과 현재 원문의 revision이 일치하는 복구 모달 승인만 `acknowledge_settings_recovery(expectedRecoveryRevision)`로 lenient recovery 결과를 atomic replace할 수 있다. 오류는 `{kind:"recoveryDocumentRejected",message}`와 `{kind:"runtimeReconcileFailed",message}`로 구분한다. document 거절만 최신 dropped path를 다시 로드하고, commit 뒤 runtime 실패는 모달과 write-block을 유지해 같은 command로 재조정을 재시도한다. 프론트는 성공 반환본을 store에 적용한 뒤 persistence를 재개한다. `reset_settings`는 사용자가 선택한 별도 초기화 경로다.

현재 브라우저 entry는 Rust remote server가 self-hosted xterm.js 자산을 콘텐츠 해시 `/remote/asset/*`(호환용 `/remote/vendor/*` 병행, [ADR-0169](../adr/0169-remote-client-hashed-immutable-assets-and-gzip.md))에서 제공하는 중간 구현이다. CDN이나 Vite dev server에 의존하지 않으며, 출력 WebSocket의 PTY byte stream을 xterm에 그대로 기록하고 xterm 입력/resize 이벤트를 Remote UI API로 다시 보낸다. ADR-0013의 최종 목표인 같은 React bundle 기반 Full UI/Focused UI 전환과 `RemoteHttpWsClient` adapter 추출은 후속 리팩터링 대상이다.

#### Android Cloud entry, native Google login과 QR v2

Cloud dashboard에서 instance를 선택해 pairing 표면에 진입하면 실제 Cloud WebView는 하위 레이어에서 계속 `VISIBLE`로 렌더링되고, 네이티브 Material bottom sheet가 그 위의 modal pairing UI를 소유한다([ADR-0178](../adr/0178-android-pairing-native-material-bottom-sheet.md), [ADR-0179](../adr/0179-android-connection-settings-from-dashboard-menu.md), [ADR-0212](../adr/0212-android-pairing-invitation-copy-paste.md)). sheet는 현재 선택의 상태·session open/취소·QR scan/clipboard 붙여넣기/재pairing·dismiss만 표시하고 저장된 pairing/보호 설정을 포함하지 않는다. 정적 dashboard 복제나 snapshot을 사용하지 않는다. sheet 동안 Cloud WebView는 disabled·focus 제외 상태이고 접근성 descendants도 숨기며 Cloud bridge action도 거부한다. native dialog가 scrim·drag·swipe/system back dismiss·inset·modal 접근성을 담당하고, pairing·생체·E2E action은 JavaScript bridge 없이 Kotlin 함수에 직접 연결한다. 뒤 Cloud JavaScript가 계속 실행되더라도 native는 현재 전면 surface가 Cloud일 때만 account bridge 호출을 받는다. Cloud 배경은 비신뢰 문맥이며 dialog의 modal 차단과 Cloud WebView 비활성화를 함께 강제한다. sheet를 닫으면 Cloud 입력·접근성을 복구하고, PC Remote 표면으로 전환하면 sheet와 Cloud WebView를 숨긴 뒤 secure WebView를 표시한다.

`apps/android`는 [ADR-0149](../adr/0149-android-thin-wrapper-runs-desktop-owned-remote-ui.md)에 따라 Cloud account WebView와 E2E Remote secure WebView를 분리하고, 그 사이의 pairing 단계는 [ADR-0178](../adr/0178-android-pairing-native-material-bottom-sheet.md)의 native UI로 둔다. Cloud WebView entry는 빌드에 설정된 public HTTPS origin의 `/app/android`다. 이 WebView는 그 exact origin의 top-level navigation만 허용하고 기존 Cloud landing/dashboard와 HttpOnly `laymux_session` cookie를 사용한다. JavaScript interface는 `LaymuxCloud.signInWithGoogle(nonce)`, `selectInstance(instanceId)`, `selectInstanceRoute(instanceId,tailscaleUrl)`와 `openConnectionSettings(instanceId)`만 제공하며 pairing metadata·QR·seed·session key·Remote HTTP/output bridge를 제공하지 않는다. 모든 instance ID는 canonical UUID validation을 거치며, settings 호출도 현재 전면 surface가 Cloud일 때만 처리한다. Android app-mode HTML은 `frame-src 'none'`·`frame-ancestors 'none'` CSP와 `X-Frame-Options: DENY`를 보내 all-frame JavaScript interface가 nested document에 노출되지 않게 한다. dashboard의 Android app mode는 기존 instance 목록을 그대로 렌더하되 online Connect form만 canonical instance UUID 선택 호출로 바꾸고 browser `/app/connect`와 Tailscale action은 표시하지 않는다. 각 PC 카드의 `…`에는 Android app-mode에서만 `연결 설정`을 추가하고 bridge capability가 없으면 JavaScript가 이 항목을 숨긴다([ADR-0179](../adr/0179-android-connection-settings-from-dashboard-menu.md)).

Google authorization endpoint는 embedded WebView에서 열지 않는다. `/app/android`는 anonymous signed session에 32-byte CSPRNG nonce를 저장하고 정확히 43자 base64url 값으로 native bridge에 제공한다. Android Credential Manager는 이 값을 `nonce`로, server Web client ID를 audience로 사용해 명시적 Sign in with Google ID token을 받는다. native는 token을 JavaScript 또는 WebView request에 반환하지 않는다. Android cookie store에서 Cloud WebView의 anonymous HttpOnly session cookie를 읽어 별도 native HTTPS stack의 같은 exact Cloud origin 고정 `POST /api/android/auth/google`에 결합하고, `application/x-www-form-urlencoded`의 단일 `id_token` 필드로 제출한다. redirect를 자동 추적하지 않으며 성공은 정확한 `303 /dashboard?client=android`와 CR/LF 없는 bounded `Set-Cookie`가 함께 있을 때뿐이다. 그 cookie를 Android cookie store에 비동기로 설치하고 성공 callback을 모두 받은 뒤 동기 flush를 마쳐야 dashboard를 GET하며, 설치 거부 시 navigation 없이 실패 닫힘한다. Cloud는 body를 16 KiB, token을 12 KiB로 제한하고 IP별 기본 60초당 10회 rate limit을 적용한다. Google 서명·issuer·audience·expiry와 session-bound single-use nonce를 모두 검증한 뒤 기존 user를 upsert하고 기존 HttpOnly session cookie를 발급한다. nonce mismatch와 재사용은 거부한다.

pairing manager는 연결용 native Material bottom sheet와 관리용 native Material alert dialog로 나뉘며 각 typed Kotlin raw state에서 단일 presentation을 계산한다([ADR-0179](../adr/0179-android-connection-settings-from-dashboard-menu.md), [ADR-0212](../adr/0212-android-pairing-invitation-copy-paste.md)). sheet의 scan/clipboard 붙여넣기/재pairing과 session open, 설정 dialog의 instance별 ACK 재시도·생체 보호 설정·key 확인/삭제는 기존 native 함수에 직접 연결하고 pairing 전용 HTML·JavaScript interface를 두지 않는다. 설정 dialog는 dashboard `…`에서 받은 instance 하나만 표시하고 dismiss 시 기존 dashboard 문서의 입력·접근성과 Cloud bridge를 복구한다. secure WebView는 아래 AEAD resource용 synthetic origin의 PC Remote 문서만 표시한다. `LaymuxNative` interface는 E2E HTTP/output transport, 현재 session disconnect, 외부 링크 열기만 제공하며 secret byte·pairing ID·client nonce를 반환하지 않는다. native는 숨겨진 이전 문서가 권한을 유지하지 않도록 현재 전면 surface가 Remote일 때만 이 interface의 transport·output·disconnect·외부 링크 동작을 처리한다. secure WebView는 다중 창을 켜지 않고 off-origin navigation을 거부하므로 Remote 문서의 터미널 링크는 `openExternalUrl(url)` 하나로 native에 넘긴다. native는 공백·제어문자 부재, `http`/`https` scheme, 비어 있지 않고 userinfo 없는 authority를 다시 검증하고 scheme만 소문자로 정규화한 뒤, `ACTION_VIEW`+`CATEGORY_BROWSABLE`+`FLAG_ACTIVITY_NEW_TASK`로 OS 브라우저를 시작한다. path·query·fragment는 검증하지 않고 그대로 넘긴다. `java.net.URI`는 WHATWG URL이 이스케이프하지 않는 문자를 거부해 정상 링크를 삼키므로 사용하지 않는다. 검증 실패는 무시하고 처리할 activity가 없으면 toast로 실패 닫힘하며, 앱은 `http`/`https` intent filter를 선언하지 않는다([ADR-0162](../adr/0162-android-remote-link-opens-os-browser.md)). Cloud dashboard가 선택한 instance가 있으면 그 instance와 정확히 일치하는 저장된 confirmed pairing 또는 새로 스캔/붙여넣은 초대만 ACK와 session open에 사용한다. 선택한 PC의 confirmed 레코드가 없으면 다른 저장 pairing을 대체 선택하지 않고 새 초대를 요구하며, 기존 pending 레코드의 ACK 재시도는 해당 PC의 연결 설정에서만 제공한다. 선택 변경은 기존 native E2E session을 먼저 닫는다([ADR-0154](../adr/0154-android-multi-instance-pairing-vault.md)).
...
Android secure WebView는 인증된 Remote 문서 로드가 끝나 로딩 overlay를 내린 다음 UI turn에 touch 기반 view focus를 복원한다. 이 지연 callback은 실행 직전에 Remote 전면 surface·secure WebView identity·문서 세대를 다시 검증한다. native view focus만 준비하고 DOM editor를 자동 focus하거나 IME를 열지는 않으며, 첫 `Keyboard` 사용자 gesture가 Remote Composer editor의 유효한 InputConnection을 만들 수 있게 한다. APK가 띄우는 Remote 문서도 coarse pointer이므로 §13의 pointer 축 focus 계약([ADR-0196](../adr/0196-remote-coarse-pointer-attach-defers-input-focus.md))이 그대로 적용된다 — 페이지 역시 attach 완료 시 입력 surface를 focus하지 않으므로 native와 DOM 양쪽이 같은 첫 gesture를 기다린다.

Android system back은 secure WebView보다 위에 표시된 native Remote loading/cancel overlay를 먼저 `cancelRemoteConnection()`으로 취소하고, 없으면 Remote 문서를 벗어나기 전에 고정된 `window.laymuxRemoteUi.dismissTopLayer()`를 평가한다([ADR-0219](../adr/0219-android-back-defers-to-remote-ui-stack.md)). 이 함수는 OAuth relay modal → FileViewer overlay → Composer 별표 편집기 → drawer 하위 화면 → 열린 Dock disclosure → drawer → Composer recall/autocomplete 순서로 현재 보이는 최상위 레이어 하나만 닫고 소비 여부만 반환한다. Composer popup의 자체 z-index는 부모 stacking context 안에 있으므로 drawer·scrim보다 아래이며, focus를 유지한 채 drawer를 여는 widget 경로에서도 실제 적층 순서를 따른다. Android native는 DOM·drawer view·viewer mode를 해석하지 않으며, 문서 세대와 Remote 전면 surface가 callback 시점에도 유효할 때만 그 결과를 받는다. OAuth modal을 닫으면 진행 중인 begin·forward revision도 무효화해 늦은 응답이 새 relay session을 지우거나 native relay·OS browser·browser popup·현재 modal 상태를 다시 만들지 못하게 한다. native overlay나 문서 레이어가 닫히면 이전 disconnect 경고와 표시 중인 경고 Toast를 해제하고, 닫을 레이어가 없을 때만 첫 back 경고·2초 안의 두 번째 back disconnect 정책을 적용한다. Remote surface 이탈과 실제 disconnect에서도 과거 Toast를 취소한다. 연속 back 평가가 진행 중인 동안 중복 평가는 시작하지 않는다.

Android가 QR scan 또는 명시적 clipboard 붙여넣기로 입력받는 초기 초대 계약은 다음 한 줄 URI다.
...
scheme=`laymux`, authority=`pair`, path=`/v2`가 정확해야 하며 필수 필드 누락, 중복 key, 알 수 없는 key, fragment, 잘못된 길이·문자·만료 시각은 전부 거부한다. clipboard 입력은 사용자가 native sheet의 action을 누를 때만 첫 text item을 읽고 trim하며, 빈 값과 4 KiB 초과 값을 URI parse 전에 거부한다. 앱은 clipboard를 자동 감시·읽기·삭제하지 않는다. QR/clipboard seed는 Android private preferences의 `pairing:<instanceId>`별 storage version 4 envelope에 AES-256-GCM ciphertext로만 저장한다. 같은 instance를 다시 스캔하거나 붙여넣으면 그 envelope만 교체하고 다른 PC 레코드는 보존한다. 표시용 endpoint·instance·pairing·expires·client nonce·confirmed-at·label은 비밀이 아닌 metadata로 분리해 `loadMetadata()` 목록 조회가 wrapping key 초기화나 seed decrypt를 유발하지 않으며 앱 backup은 비활성화한다. native pairing sheet는 표시하는 endpoint·instance·confirmed-at·label만 사용하고 protocol 재시도 값인 pairing id와 client nonce는 UI 계층에 전달하지 않는다. 이전 version 3 `pairing` singleton은 migration·fallback read 없이 vault 초기화 때 폐기한다([ADR-0212](../adr/0212-android-pairing-invitation-copy-paste.md)).

보호 정책의 저장값은 `biometric`(미설정 시 기본)과 명시적 opt-out인 `keystoreOnly`뿐이다. `biometric`은 alias `com.laymux.android.pairing-wrap.v2.biometric`의 auth-per-use AES key와 `BIOMETRIC_STRONG`만 허용하는 `BiometricPrompt.CryptoObject`를 사용한다. PIN·패턴 fallback은 없고 생체 hardware/등록이 없으면 QR 저장과 seed 사용을 막는다. 이 차단 상태의 pairing 화면은 `보호 설정 열기`로 별도 연결 설정 dialog에 이동해 `keystoreOnly` opt-out 방법을 표시하되 정책을 자동 변경하지 않는다. `keystoreOnly`는 alias `com.laymux.android.pairing-wrap.v2.keystore-only`의 사용자 인증 없는 AES key를 사용한다. wrapping key는 PC별이 아니라 보호 정책별 하나이며 해당 정책의 모든 envelope가 공유한다. 개별 pairing 삭제는 envelope만 지우고 alias를 유지한다. 정책을 바꾸면 모든 pairing과 두 alias를 삭제하고 모든 PC의 재pairing을 요구한다. 생체 등록 집합 변경도 biometric alias를 무효화해 그 alias를 공유하는 pairing을 사용할 수 없게 한다. 앱 시작·metadata 목록 조회에는 인증을 요구하지 않고 QR seed 저장, instance별 키 보호 확인, 실제 seed 사용에만 요구한다.

생체 프롬프트는 activity가 resume 상태일 때만 띄운다. API 28의 `androidx.biometric`은 내부적으로 AppCompat dialog를 사용하므로 host Activity는 AppCompat에서 파생된 `Theme.Material3`를 사용한다. QR 스캐너는 별도 activity라 결과 callback이 host resume보다 먼저 도착할 수 있고, 그 시점의 `BiometricPrompt`는 표시도 callback도 없이 사라져 pairing 상태가 영구히 pending으로 남는다. paused 상태에서 요청한 프롬프트는 host의 FragmentManager가 resume을 dispatch한 뒤(`onPostResume`) 실행한다. 대기 슬롯은 하나뿐이라 이미 대기 중이면 새 요청을 거부하고 그 요청이 준비한 비밀을 즉시 닫으며, 앱이 백그라운드로 가면 `onStop`에서 대기 요청을 폐기하고 보관 중인 seed·decryption을 닫는다. 스캔 결과 Task는 listener에서 즉시 버려 QR 문자열의 seed가 activity 수명 동안 heap에 남지 않게 하고, 이미 파괴된 activity로 도착한 스캔 결과는 seed를 닫고 버린다. QR scan을 시작하면 `ModuleInstallClient.installModules`로 Google Code Scanner module을 요청하고 already-installed 응답 또는 install completed 상태 뒤에만 기존 scanner를 연다. 설치 실패·취소·일시 중지는 현재 시도를 끝내고 인터넷 연결 후 재시도하라고 안내하며, 설치 중 앱이 background로 가면 자동 scanner 실행을 취소한다. Google Play services 구버전과 카메라 권한 거부는 각각 업데이트와 권한 허용으로 구분해 안내한다. 진행 중인 작업 때문에 스캔·확인·세션 열기·보호 설정 변경을 거부할 때는 반드시 사유 문구를 함께 보내 현재 native pairing surface가 비활성화한 action을 다음 render에서 복구한다. Cloud/Remote WebView container는 system bar와 display cutout inset을 padding으로 흡수하고 native Material dialog는 자체 window inset을 적용한다. targetSdk 36은 창을 항상 edge-to-edge로 그리므로 이 처리가 없으면 PC 소유 Remote page의 상단 메뉴나 pairing action이 system bar에 가려진다.

데스크톱 Tauri IPC는 `get_android_pairing_status() -> { paired, phase, endpoint, instanceId, expiresAt, confirmedAt }`, `create_android_pairing_qr() -> { status, qrSvg, pairingPayload }`, `revoke_android_pairing() -> AndroidPairingStatus` 세 개다. `phase`는 `none | pending | confirmed`이고 기존 `paired`는 record 존재 여부와 호환되는 필드라 pending에서도 `true`다. create command는 UI 인자를 받지 않고 현재 `cloudServerBaseUrl`과 `cloudInstanceId`만 사용한다. Rust가 OS CSPRNG로 32바이트 seed와 독립된 16바이트 pairing id를 생성하고 QR SVG를 만든 뒤, v2 pending record를 OS keyring service `laymux`(debug는 `laymux-dev`), account `android-pairing-v1`에 저장한다. 저장이 성공해야 SVG와 동일 초대 URI를 반환하며, 새 발급은 기존 record를 회전한다. payload는 zeroizing Rust field에 두고 `Debug`에서 redaction하지만 Tauri IPC를 지난 JavaScript 문자열과 system clipboard는 zeroize할 수 없다. status는 계속 비밀이 아닌 metadata만 포함한다. Remote Access 모달은 cloud identity가 있을 때만 생성 버튼을 활성화하고, payload를 DOM 텍스트·접근성 이름·로그에 넣지 않은 채 pending 초대의 명시적 copy action에만 사용한다. 새 발급·폐기 요청을 시작한 즉시 이전 QR SVG와 payload를 숨기고, pending 동안 2초 간격으로 status를 갱신해 confirmed·expired에서도 메모리의 QR SVG와 payload를 함께 폐기하며, 모달을 닫을 때는 component lifetime과 함께 폐기한다. 명시적 revoke와 `cloud_disconnect`, 다른 instance 또는 server origin으로 cloud pairing이 성공한 경우는 기존 keyring record를 삭제한다. 초대 생성의 identity snapshot 조회부터 record 저장까지, ACK 확정, 상태 조회의 만료 삭제와 이 세 폐기 경로는 pairing lifecycle mutex로 직렬화한다. 이 mutex는 `AppState.remote_access`보다 먼저 잡아 폐기 완료 뒤 옛 identity의 생성 작업이 record를 되살리지 못하게 한다([ADR-0212](../adr/0212-android-pairing-invitation-copy-paste.md)).
...
#### Android E2E data session과 encrypted RPC

확정된 pairing은 [ADR-0146](../adr/0146-android-e2e-session-and-encrypted-remote-rpc.md)의 challenge→establish handshake로 15분 비활성 timeout을 가진 메모리 전용 session을 연다. challenge 응답에는 proof에 실리지 않는 표시 전용 `compatVersion`이 붙는다([ADR-0172](../adr/0172-android-e2e-compat-version-gate.md)) — desktop과 앱이 공유하는 호환 번호로, 비호환 계약 변경이 실릴 때만 1씩 올린다(현재 1, 정본은 `android_e2e/mod.rs`와 `E2eProtocol.COMPAT_VERSION`). 앱은 자기 번호와 다르면 "PC를 업데이트하세요"/"앱을 업데이트하세요" 안내로 연결을 거부하고, 필드 부재는 버저닝 도입 전 desktop = PC 구버전으로 취급한다. 앱의 strict 응답 검증에서 이 필드만 optional이다. `POST /api/android/e2e/session/challenge`는 `{version:1,instanceId,pairingId,clientNonce,clientSessionNonce}`를 고정 desktop path `/remote/v1/e2e/session/challenge`로 전달한다. desktop은 OS CSPRNG 16바이트 `challengeId`, 32바이트 `serverNonce`, 정확히 60초 뒤 `challengeExpiresAt`을 만들고 `laymux.android-e2e.challenge.response.v1` domain HMAC으로 모든 echo와 challenge를 인증한다. `now >= challengeExpiresAt`은 만료다.

`POST /api/android/e2e/session/establish`는 위 필드에 `challengeId`와 `clientProof`를 더한다. client proof의 domain은 `laymux.android-e2e.establish.request.v1`이고 pairing id·instance id·client nonce·client session nonce·challenge id·server nonce·decimal challenge expiry를 기존 `u32-be length || UTF-8 bytes` framing한다. desktop 성공 응답은 같은 echo와 server nonce, OS CSPRNG 16바이트 `sessionId`, 최초 비활성 deadline인 900초 뒤 `expiresAt`, `laymux.android-e2e.establish.response.v1` HMAC이다. Android의 로컬 key deadline은 인증된 desktop expiry와 응답 수신 시점+900초 중 이른 값이다. 같은 challenge·같은 proof의 응답 유실 재시도만 60초 challenge 수명 안에서 재현하며 Android도 transport 실패 시 exact body를 한 번 재전송한다. 예외: Tailscale Direct의 첫 challenge는 도달성 프로브라 단일 시도·짧은 connect timeout으로 제한하고, 실패 시 새 nonce의 Cloud handshake로 폴백한다(ADR-0160 적용 세부 — establish의 유실 재전송은 Direct에서도 유지). 다른 proof와 소비된 challenge는 거부한다.

HKDF-SHA-256 IKM은 pairing seed다. salt는 `SHA-256("laymux.android-e2e.hkdf.salt.v1" || frame(pairingId,instanceId,clientNonce,clientSessionNonce,serverNonce,sessionId))`, info는 요청 방향 `laymux.android-e2e.a2d.v1`, 응답 방향 `laymux.android-e2e.d2a.v1`이며 각각 32바이트 AES key를 만든다. Android와 desktop은 handshake 직후 seed 평문을 지우고 session key만 메모리에 둔다. Android는 app background에서 session을 suspend해 RPC와 deadline 갱신을 멈추고 현재 expiry를 고정 상한으로 삼아 최대 15분간 key를 보존한다. 전환 직전 Android는 인증된 `backgroundTransition` RPC 하나로 PC의 현재 `remote.androidBackgroundLeaseSeconds`를 읽고 같은 트랜잭션에서 controller lease를 유지하거나 반납한다. 기본·상한은 900초이며 `0`은 즉시 voluntary release다. pagehide는 이 설정을 독자적으로 판단하지 않고 native 전환 결과만 따른다. 전환 중 보낸 pending 응답이 나중에 성공해도 그 상한을 늘리지 않으며 각 transport retry 직전에 suspend를 다시 검사한다. foreground 복귀·새 요청은 exact expiry를 먼저 검사하며, 명시적 disconnect와 pairing 변경·삭제에서는 즉시 session을 닫는다. desktop은 pairing revision이 바뀌거나 session이 만료되면 challenge·key·cached response를 버리고, 같은 pairing revision의 새 establish가 성공하면 기존 session을 revoke해 단일 활성 session으로 교체한다.

`POST /api/android/e2e/rpc`의 relay-visible exact envelope는 `{version:1,instanceId,sessionId,sequence,ciphertext}`다. sequence는 0부터 시작하며 `0..9007199254740991`만 허용한다. 방향마다 nonce는 `0x00000000 || u64-be(sequence)`, tag는 128비트다. AAD는 방향별 `laymux.android-e2e.{a2d|d2a}.aad.v1`, version byte, framed instance/session, u64-be sequence다. Android는 한 요청만 in-flight로 두고 desktop은 exact next sequence만 실행한다. 요청을 복호화하기 직전 `now >= expiresAt`이면 만료시키고, 처리가 성공하면 desktop deadline을 인증 시각+900초로 갱신해 plaintext `{version:1,expiresAt,response}`를 암호화한다. Android는 AEAD 검증 뒤 이 expiry와 로컬 수신 시각+900초 중 이른 값으로 key deadline을 갱신하되, background 전환이 한 번이라도 pending 요청 사이에 끼면 캡처한 전환 시점 deadline을 추가 상한으로 사용한다. 바로 직전 sequence의 동일 ciphertext digest는 cached encrypted response와 당시 expiry를 재현하되 deadline을 다시 갱신하지 않는다. Android는 retryable transport 실패 때 같은 pending envelope만 로컬 deadline까지 재전송하고 새 plaintext/sequence를 만들지 않는다. background는 retry와 새 RPC를 suspend하되 pending envelope와 key를 deadline까지 보존한다. foreground 복귀는 pending을 exact 재전송해 sequence를 확정하고 원래 document request-id에 응답한 뒤, 현재 WebView main document가 정확한 synthetic Remote origin이고 foreground callback을 지원하면 문서를 유지한 채 native transport 복구를 통지한다. callback은 pagehide stash를 즉시 consume하고 native 실행 전에 취소되거나 queue에서 폐기된 나머지 HTTP Promise를 transient 실패로 종료하며 stale output peer를 닫은 뒤 retained lease를 heartbeat로 검증한다. origin·surface가 다르거나 callback이 없거나 실패하면 PC 소유 Remote 시작 문서를 다시 적재한다. 두 경로 모두 기존 browser Remote와 같은 auto-connect/status/claim 계약으로 lease를 재검증·재확보하고 새 output snapshot에 부착한다([ADR-0163](../adr/0163-android-foreground-preserves-remote-document.md)). 다른 ciphertext, 과거 sequence, gap, invalid tag는 `409` 또는 `401`로 fail closed한다.

RPC plaintext `kind=resource`는 PC Laymux가 소유한 `/remote/` main document와 그 문서가 참조하는 자산(콘텐츠 해시 `/remote/asset/*`, 고정 vendor/PWA/viewer/font)만 읽는다. `/remote/asset/*`의 allowlist는 desktop의 해시 자산 레지스트리 자체다 — 서빙 페이지가 참조하는 정확한 파일 집합이며 호출자가 이름을 지어낼 수 없다([ADR-0169](../adr/0169-remote-client-hashed-immutable-assets-and-gzip.md)). desktop은 기존 Remote resource route를 내부 호출하되 `Accept-Encoding: gzip`을 붙여 정적 자산을 압축본으로 받고, status, 제한된 security/cache/encoding/content-type header, 최대 2 MiB body를 base64url로 암호화한다. Android는 AEAD 검증 뒤 `content-encoding: gzip`이면 네이티브에서 bounded 해제(평문 2 MiB 상한, 미지원 인코딩은 fail closed)한 뒤에만 이를 `https://remote.laymux.invalid/remote/` synthetic origin의 `WebResourceResponse`로 제공한다. redirect, 외부 origin, caller header·cookie·query, path traversal와 E2E route 재귀는 허용하지 않는다. APK 자체에는 xterm이나 terminal UI를 포함하지 않는다.

resource 응답 중 desktop이 `Cache-Control: max-age>0`으로 명시하고 `no-store`/`no-cache`가 없는 200만 Android가 `instanceId + path` 키의 메모리 전용 LRU 캐시(총량 상한, TTL은 max-age를 1년으로 상한)에 보관해 재연결 전송을 줄인다([ADR-0168](../adr/0168-android-remote-resource-memory-cache-and-transfer-overlay.md)). 대상은 콘텐츠 해시 폰트(ADR-0077)·PWA 아이콘·해시 클라이언트 자산(ADR-0169)이며, `no-store` 셸 문서는 휴리스틱 없이 매번 다시 받는다. 캐시는 디스크에 쓰지 않고 프로세스 종료로 소멸하며 해당 pairing 해제 시 그 인스턴스 항목을 비운다. Remote 문서 적재 동안에는 네이티브 오버레이가 수신 개수·바이트·현재 파일·캐시 적중을 표시하고 main document `onPageFinished` 또는 표면 이탈에서 사라진다.
...
FileViewer는 이 일반 HTTP 재주입 위에 목적 제한 capability 경계를 하나 더 둔다([ADR-0208](../adr/0208-android-e2e-file-viewer-typed-capability.md)). Android Remote page는 caller header를 만들지 않고 exact FileViewer POST의 body에 `fileViewerAuthorization: {leaseId,fileViewerToken}`을 포함한다. desktop dispatcher가 strict parse 후 이를 body에서 제거하고, 현재 E2E session 객체와 claim owner epoch에 결합된 crate-private proof extension으로 바꾼다. `/status`는 browser GET 계약과 별도로 Android E2E POST alias를 쓰며, 내부 proof 없는 public POST는 거부한다. 성공한 FileViewer 응답의 exact-retry cache는 non-secret replay guard를 보관하고 재전송 직전 원래 요청과 현재 session/lease/capability를 다시 검증한다. 권한이 바뀌었으면 cached ciphertext를 내보내지 않고 session을 revoke한다. Android FileViewer source read는 2 MiB, inner JSON 응답은 3 MiB로 제한하며 일반 inner HTTP의 1 MiB 상한은 그대로다. outer wire와 4인자 native bridge는 변하지 않아 `compatVersion=1`을 유지한다.

E2E 경로의 claim 은 lease 를 그 세션에 결합한다([ADR-0170](../adr/0170-android-e2e-lease-dies-with-its-session.md), [ADR-0208](../adr/0208-android-e2e-file-viewer-typed-capability.md)). rpc 핸들러가 AEAD 검증을 마친 exact session 객체를 opaque crate 내부 request-context extension으로 싣고, claim 핸들러는 context가 registry의 현재 객체인지 확인한 뒤 lease 설치와 같은 owner lock 구간에서 `{leaseId,ownerEpoch,instanceId,sessionId}` 태그를 기록한다. 이후 claim 전처리에서 태그된 세션이 registry 에 살아있지 않으면(새 establish 의 revoke, 15분 만료, pairing revision 변경) 그 lease 를 heartbeat 만료와 같은 owner transition 으로 즉시 해제해, 재접속이 자기 자신의 죽은 lease 와 heartbeat timeout 까지 409 로 싸우지 않게 한다. browser claim(태그 없음)과 살아있는 세션의 lease 의미론은 불변이다.
...
terminal output의 public WSS는 `/api/android/e2e/output` 하나다([ADR-0159](../adr/0159-android-e2e-websocket-output-transport.md)). browser session·relay cookie 없이 public origin에서 받고 query는 exact canonical `instanceId`(1..128 identifier), `sessionId`(16-byte base64url), `streamNonce`(32-byte base64url)만 허용한다. relay는 이를 빈 header와 고정 desktop path `/remote/v1/e2e/output`으로 열고 encrypted binary record를 그대로 중계한다. Python connector는 local Remote 인증을 위한 `token` query만 이 canonical query 뒤에 덧붙여 guarded local Axum WebSocket에 연결하고, native tunnel은 local HTTP upgrade 없이 직접 transport adapter를 호출한다. connector 내부 hop은 loopback·exact path·Origin 없음·Remote enabled·exact token을 모두 요구하지만 public direct client의 IP/Tailscale/Origin 정책에서는 분리한다. 두 adapter는 같은 Rust output core를 호출하므로 session·stream nonce·lease·renderer checkpoint·AEAD sequence와 Remote v1 header/body pair 계약은 전달 방식과 무관하게 하나다. public caller의 input은 첫 encrypted `OPEN` binary message 한 개와 4 KiB 상한만 허용하며 text·초과 크기·두 번째 message는 tunnel base64 직렬화 전에 거부하고, production ASGI WebSocket parser도 모든 ingress message를 protocol 최대 2,629,658 bytes로 제한한다. record wire는 `version:u8 || sequence:u64-be || AES-256-GCM ciphertext+tag`다. protocol 최대 plaintext record는 type byte와 1 MiB renderer checkpoint 및 bounded output-ring suffix를 합친 2,629,633 bytes이고 encrypted record는 25-byte wire overhead를 더한 2,629,658 bytes다. Python connector와 native desktop sender는 64 KiB보다 큰 WebSocket message를 같은 `fragment {index,count,messageBytes}` tunnel frame 계약으로 나누며 relay receiver는 순서·총 길이·최대 64 fragments를 검증해 public WebSocket에 exact 한 binary message로만 내보낸다. session 방향 key에서 output domain·instance/session/stream nonce를 HKDF salt에 묶어 stream별 a2d/d2a key를 파생하며 desktop은 session 안에서 stream nonce 재사용을 거부한다. 한 session이 기억하는 single-use output nonce 1,024개를 모두 쓰면 이미 복호화한 nonce를 회수하지 않고 session 자체를 폐기해 다음 control RPC가 새 session 수립을 요구하게 한다. 첫 a2d plaintext는 `OPEN(1) || {terminalId,leaseId}`이고 d2a plaintext는 `TEXT(2) || TerminalOutputFrameHeaderV1 JSON` 또는 `BINARY(3) || exact bytes`다. Android native는 encrypted record를 최대 wire 크기보다 큰지 복사·복호화 전에 검사하고, `WebViewCompat.addWebMessageListener`를 `https://remote.laymux.invalid` main frame에만 설치해 `JavaScriptReplyProxy.postMessage(ByteArray)` ArrayBuffer로 TEXT/BINARY를 전달한다. queue는 protocol 최대 plaintext record 한 건과 추가 2 MiB backlog로 제한하고 header parse와 xterm write callback ACK로 한 record씩 진행하며 pending/active output open은 합계 8개로 제한한다. output byte 경로에는 base64·JSON envelope·`evaluateJavascript`가 없다. relay는 routing metadata와 ciphertext·fragment 크기·타이밍 외에 terminal id·lease·header·output bytes를 해석하거나 저장하지 않는다. 장기 WebSocket은 초기 `websocket.accept`/opening frame에 bounded handshake timeout을 적용한 뒤 누적 수명 byte·HTTP next-frame·방향별 read timeout을 적용하지 않고 frame·queue·pending byte·concurrency·write timeout·underlying ping/pong 상한을 적용한다. 한 stream의 backpressure 실패는 tunnel 전체가 아닌 해당 stream에 격리하며, bridge는 stream cancellation을 즉시 관찰하고 fragment 검증 실패를 peer에 stream error로 알린다. desktop은 malformed·초과 크기·queue-full input으로 active WebSocket을 제거할 때 해당 task를 즉시 취소하고, renderer checkpoint attach 대기 중에도 peer close와 adapter shutdown을 함께 관찰하며, 자연 종료 task completion을 generation-safe id로 active stream map에서 회수한다. APK WebView는 `connect-src 'none'`이고 network/key/sequence는 Kotlin native 계층이 소유한다. xterm.js·fit addon·CSS는 PC Laymux가 제공하고 AEAD 검증된 resource bridge를 통해서만 WebView에 들어온다.

기존 browser `/remote/v1/*`와 terminal WebSocket은 호환을 위해 평문 relay 경로를 유지하므로 E2E로 표시하지 않는다. 기존 PC Cloud login/registration도 유지한다. Android의 Cloud login cookie, instance별 QR pairing과 현재 선택 PC의 열린 E2E session은 독립 수명이며 Cloud logout은 pairing 목록을 지우지 않고 E2E disconnect/expiry도 Cloud session을 지우지 않는다. Android도 보안 session이 닫혀 있으면 pairing confirmation만 가진 상태이며 terminal E2E 연결 완료로 표시하지 않는다.
...
`/remote/asset/<stem>-<sha256 앞 16 hex>.<js|css>`는 서빙 페이지가 실제로 참조하는 클라이언트 자산 route다([ADR-0169](../adr/0169-remote-client-hashed-immutable-assets-and-gzip.md)). main document는 마크업 셸만 남아 `no-store`를 유지하고, minified 앱 번들(`remote-app.min.{js,css}` — 앱/CSS 소스는 같은 디렉터리의 `remote-app.{js,css}`, Lucide DOM 경계는 `ui/src/remote/remote-icons.js`, 재생성은 `cd ui && npm run build:remote-page`, 드리프트는 세 소스의 해시와 `lucide` lockfile identity를 포함한 ui 테스트가 검증)과 vendor xterm 파일을 이 route의 해시 URL로 참조한다. URL이 곧 내용이므로 `Cache-Control: private, max-age=31536000, immutable`(폰트와 같은 근거)이고, gate는 vendor asset과 동일하다. 셸·해시 자산은 기동 시 사전 압축한 gzip 본을 `Accept-Encoding` 협상으로 서빙하며 `Vary: Accept-Encoding`을 보낸다. 기존 `/remote/vendor/*` 고정 route는 호환용으로 유지되지만 서빙 페이지는 더 이상 참조하지 않는다.

Remote 범용 픽토그램은 [ADR-0210](../adr/0210-remote-lucide-icon-boundary.md)에 따라 vanilla `lucide` named icon을 `ui/src/remote/remote-icons.js` 한 경계에서 DOM SVG로 만든다. 셸의 정적 아이콘은 `data-remote-icon` placeholder를 앱 시작 시 hydration하고 동적 파일·가시성·입력 모드·이동·widget 아이콘은 같은 경계의 `setRemoteIcon`을 사용한다. 기본 `size=14`, `strokeWidth=2`, `currentColor`, 장식 아이콘 접근성 속성은 데스크톱 공용 래퍼와 같다. pane 명령 상태의 `⏳`·`✓`·`✗`·`—` projection은 호환 데이터로 유지하되 Remote 렌더링 경계에서 Lucide로 바꾸고, 상태 wrapper의 `role=img`·`aria-label`로 의미를 접근성 트리에 보존한다. 터미널 keycap label, 단위·수학 기호, 앱 로고와 제품 고유 데이터 시각화는 이 아이콘 경계의 대상이 아니다.

`/remote/` 셸 문서는 `Cache-Control: no-store`·`Vary: Accept-Encoding`과 함께 CSP, `Referrer-Policy: no-referrer`, `X-Content-Type-Options: nosniff`를 보낸다([ADR-0183](../adr/0183-remote-page-content-security-policy.md)). 정책 본문의 SoT는 `src-tauri/src/remote_server/page-csp.txt` 한 파일이며 Rust route가 `include_str!`로 내장하고 `ui/e2e/remote-client-assets.ts`가 같은 파일을 읽어 mock 응답에 실으므로 Playwright Remote 스위트는 서빙되는 정책 아래에서 돈다. 기본은 `default-src 'none'`이고 `script-src 'self'`에는 `'unsafe-inline'`/`'unsafe-eval'`을 넣지 않는다 — ADR-0169 이후 셸에는 inline script·inline handler·`eval`이 없다. `style-src`만 `'self' 'unsafe-inline'`인데 xterm DOM 렌더러가 셀 크기·테마·decoration용 `<style>` 요소를 주입하기 때문이다. 설치형 클라이언트(ADR-0091)를 위해 `manifest-src 'self'`와 `img-src 'self' data:`를 명시한다. `connect-src`는 `'self'`에 요청 `Host`로 만든 `ws://<authority>`·`wss://<authority>`를 더하며, `Host`는 클라이언트가 정하므로 bare authority 문법(`host[:port]`, IPv6는 대괄호 유지, port는 최대 5자리 숫자)을 통과할 때만 반영하고 실패하면 WebSocket source를 생략한다. Android 앱 경로는 resource RPC가 이 세 헤더를 전달하므로 같은 정책을 받고, `Host` 없는 RPC 요청에서는 WebSocket source가 생략된다(앱은 네이티브 transport를 쓴다). Cloud relay 경유 시에는 relay 정책과 교집합으로 적용된다. `frame-ancestors`는 데스크톱 앱 WebView origin만 담은 닫힌 허용목록이며(`tauri://localhost`·`http://tauri.localhost`, debug 빌드에 한해 `http://localhost:1420`), `page-csp.txt`의 `__APP_FRAME_ANCESTORS__` 자리를 `page.rs`의 빌드 고정 상수가 채운다 — 모바일 모드가 같은 셸을 앱 WebView 안 iframe으로 띄우므로 `'none'`은 앱 자신의 뷰를 막고 `'self'`는 임베더가 remote origin이 아니라 성립하지 않는다([ADR-0215](../adr/0215-remote-page-frames-only-the-desktop-app.md)). 그 밖의 모든 origin은 계속 거부된다.

`/remote/font/{token}.{ttf|otf}`는 vendor asset과 같은 gate를 쓰는 폰트 route다([ADR-0077](../adr/0077-remote-terminal-font-serving.md)). `settings.remote.serveTerminalFont`가 켜져 있을 때만 appearance payload가 이 URL을 광고하며, route 자체는 등록되지 않은 token에 `404`를 돌려준다. `token`은 폰트 콘텐츠 sha256의 앞 16 hex이므로 URL이 곧 내용이며 `Cache-Control: public, max-age=31536000, immutable`과 `Vary: Accept-Encoding`을 보낸다. `Accept-Encoding`에 `br`이 있으면 한 번 만들어 캐시한 brotli 본을 `Content-Encoding: br`로, `br` 없이 `gzip`만 있으면 같은 규칙의 gzip 본을 보낸다 — Android E2E resource dispatch는 gzip만 협상하며, MB급 sfnt 원본은 2 MiB resource wire 상한을 넘어 압축본이 그 경로의 유일한 통로다. 그 외에는 원본 sfnt 바이트를 그대로 보낸다. Android의 gzip 해제 상한은 4 MiB(wire 상한 2 MiB와 별개인 로컬 압축폭탄 가드)다. woff2 컨테이너 변환은 하지 않는다.
...
FileViewer는 별도 문서가 아니라 Remote 셸 문서 안 오버레이(`#fileViewerOverlay`)에서 렌더한다([ADR-0184](../adr/0184-remote-file-viewer-in-page-overlay.md)). `/remote/viewer/`·`/remote/viewer/viewer.js` route와 자격 증명을 두 번째 문서로 넘기던 `window.open` + `laymux:file-viewer-ready`/`-session` 핸드셰이크는 없다 — 래퍼 WebView는 두 번째 창이 없고 설치형 PWA는 opener 관계를 잃으므로 새 탭 전제가 주 표면에서 성립하지 않았다. 셸 문서의 정책은 §13 앞부분의 CSP([ADR-0183](../adr/0183-remote-page-content-security-policy.md))가 담당한다.
...
`claim` 성공 응답의 `leaseId`가 이후 제어 요청의 권한이다. 기존 Local input permit이 아직 남아 있으면 server는 `409 { code:"input_busy", claimReservationId, retryAfterMs, reservationTtlMs }`를 반환하고 one-shot reservation을 설치한다. 예약은 짧은 bounded TTL을 가지며, active Local 작업이 남은 동안 인증된 client가 같은 `claimReservationId`로 재시도할 때만 TTL을 다시 시작한다. 따라서 긴 PTY 작업은 연속 재시도로 기다릴 수 있지만 탭 종료·네트워크 단절로 claimant가 사라지면 새 Local 입력 차단은 마지막 재시도 뒤 한 TTL 이내에 끝난다. reservation이 살아 있는 동안 새 Local permit과 다른 claim은 앞지르지 못한다. Remote page는 이 `input_busy` 응답만 동일 token을 사용하는 claim 요청 내부에서 자동 재시도하고 서버가 갱신해 준 만료 시각을 사용한다. 자발적 owner handoff는 서버의 bounded wait 뒤에도 drain이 남으면 `active`/`transitioning: true` 상태 409를 반환하고 상위 auto-connect backoff가 새 claim으로 재시도하며, 그 외 409는 자동 재 claim하지 않는다.
...
claim 성공 응답은 FileViewer 전용 비밀 `fileViewerToken`도 포함한다([ADR-0042](../adr/0042-remote-file-viewer-secret-capability.md)). 이 값도 원문 대신 process-random keyed digest만 현재 lease id와 결합해 저장하고 status·충돌 응답에는 노출하지 않는다. 새 claim과 모든 owner transition은 이를 즉시 revoke하며, `resumeToken`과 달리 자발적 release handoff 중에도 보존하지 않는다. claim 성공 응답과 `/remote/v1/session/status` 응답은 host 첨부 정책 `attachments:{maxBytes,hostMaxBytes,relayMaxBytes,allowAllExtensions,extraExtensions}`도 포함한다([ADR-0227](../adr/0227-remote-document-attachments.md)). `maxBytes`는 요청이 들어온 transport의 실효 상한 `min(host, relay)`이고 `relayMaxBytes`는 Cloud relay 경로일 때만 값이 있다(브라우저 경로 약 11 MiB, Android E2E 경로 1 MiB — laymux-server 상한의 desktop 미러). page는 이 값으로 사전 크기 검사·오류 문구·`input[type=file]` `accept`를 맞추고, relay가 구속하면 Tailscale(direct Remote) 접속을 안내한다. 서버 판정은 항상 현재 settings와 요청 transport를 따른다.

Remote page에서 `resumeToken`은 문서가 살아 있는 동안 메모리에만 존재한다. `pagehide` 시점에만 탭 단위 `sessionStorage`(`laymux.remote.resumeToken`)에 stash하고 문서 load·bfcache 복원(`pageshow`)에서 즉시 consume(get+remove)한다. Android가 같은 문서를 foreground에서 유지하는 경우에도 native 복구 callback이 즉시 consume한다. 따라서 Duplicate Tab/`window.open`이 복제하는 살아 있는 원본의 저장소는 항상 비어 있어 복제 탭이 capability를 제시할 수 없다. 서버가 lease 상실을 확정하면(`401`/`403`/`409`) 메모리와 저장소의 capability를 모두 폐기한다. 또한 일반 browser의 `pagehide`에서는 `navigator.sendBeacon`(불가 시 keepalive fetch)으로 `/remote/v1/session/release`를 호출해 lease를 즉시 반납한다. Android pagehide는 ADR-0155의 native background 전환에 lease 판단을 맡긴다. 명시적 시스템 뒤로가기로 대시보드에 복귀할 때는 native가 PC 문서의 Exit를 호출해 release를 먼저 마치고, 문서의 완료 bridge 뒤에 E2E 세션을 닫는다. 따라서 백그라운드 lease 유예를 대시보드 복귀에 적용하지 않는다. beacon은 헤더를 실을 수 없으므로 WebSocket과 동일한 `token` query parameter 인증을 사용한다.
...
Remote drawer 본문은 workspace 기본 화면과 Hidden workspaces·Notifications·Connection·Settings 하위 화면을 가진다. 연결된 상태에서 drawer를 열면 매번 workspace 기본 화면으로 시작하며 workspace 목록과 Dock terminals를 렌더한다. 파일 탐색과 직접 경로 열기는 drawer가 아니라 메인 헤더 폴더 버튼이 여는 Remote Explorer가 소유한다. Hidden workspaces·Notifications는 헤더의 각 버튼으로, Connection은 Settings → App 세부 설정의 Connection 버튼으로, 기기-로컬 Display와 설치 권유는 Settings 버튼으로 진입하고 뒤로가기는 workspace 기본 화면으로 돌아가 진입 버튼에 focus를 복원한다. Hidden workspaces와 Notifications의 상태는 Settings 업데이트 표시와 같은 5px 점으로만 나타내며 숫자 badge를 그리지 않는다. exact hidden/unread 개수는 각 버튼의 접근성 이름과 title에 유지한다([ADR-0187](../adr/0187-remote-drawer-status-dots-and-hidden-subview.md)). 기본은 기존 floating drawer이며 상단 메뉴 버튼이나 바깥 scrim으로 닫는다. drawer 설정과 메뉴 도구 줄 맨 왼쪽의 핀 버튼은 같은 `navigationPinned` 요청값을 토글하며, `window.innerWidth > navigationPinCutoff`일 때만 drawer를 terminal 옆 열에 고정하고 scrim·메뉴 버튼·close 동작을 비활성화한다. 컷오프 이하에서는 요청값을 보존한 채 floating drawer로 폴백한다. `navigationWidth`는 두 모드에 공통 적용하며 이 세 값은 연결·lease와 무관한 기기 로컬 표시 설정이다([ADR-0233](../adr/0233-remote-workspace-menu-pinning.md)). 아직 lease가 없거나 확정적으로 제어권을 잃어 drawer가 열릴 때만 Connection을 바로 표시한다. 하위 화면 선택은 ADR-0015의 surface-local drawer 상태이며 Remote API나 host 설정에 저장하지 않는다.

| Endpoint | Method | 용도 |
|---|---|---|
| `/remote/v1/navigation` | GET | workspace 목록과 모든 workspace pane 표시 요약, dock pane 요약, terminal 표시 메타데이터 |
| `/remote/v1/workspaces/{id}/visibility` | POST | workspace의 목표 숨김 상태 설정 (`hidden`, active controller lease 필요) |
| `/remote/v1/panes/{id}/visibility` | POST | workspace pane의 목표 숨김 상태 설정 (`hidden`, active controller lease 필요) |
...
Remote drawer는 PC selector의 `display.minimap/environment/activity/path/result`, `pathEllipsis`, `lastInputMode`를 직접 gate로 사용한다. 기본 `perPane`은 각 terminal pane의 첫째 줄에 환경·activity·경로·결과를, 둘째 줄에 `selectorDisplay.lastInput`을 표시한다. `workspaceLatest`는 pane을 한 줄로 줄이고 non-hidden terminal pane의 `selectorDisplay.lastInputAt` 중 최신 입력 하나를 workspace pane 목록 아래 한 줄에 표시한다. 과거의 마지막 명령/최신 unread 알림 통합 상태 행은 빈 placeholder까지 어느 모드에서도 복원하지 않는다([ADR-0194](../adr/0194-workspace-pane-last-input-second-line.md)). 열린 drawer의 정보는 2초 주기로 다시 읽으며 drawer close, disconnect, document hidden에서 폴을 멈춘다. 선택 revision이나 lease가 달라진 늦은 응답은 적용하지 않는다.

Remote page는 같은 HTML 문서 생명주기에서 사용자가 마지막으로 선택해 attach한 `terminalId`를 메모리 hint로 유지한다. release, disconnect, 또는 서버가 확정한 lease 상실 뒤 사용자가 다시 연결하면 이 hint를 focused pane fallback보다 먼저 검토하되, 최신 navigation snapshot에서 active workspace의 live terminal 또는 visible dock의 live terminal인 경우에만 복원한다. terminal이 종료·삭제되었거나 inactive workspace/hidden dock으로 이동했으면 위 폴백 순서를 그대로 사용한다. 기억된 hint에만 live 요구가 남는 이유는, 재접속 시 이미 종료된 terminal을 사용자가 요청하지 않았는데 다시 시작해버리지 않기 위해서다 — 사용자가 방금 탭해서 진입한 pane은 반대로 열리는 동안 pinned로 고정해, 대기 중 실행되는 navigation 재조회가 다른 pane으로 표류하지 않게 한다([ADR-0138](../adr/0138-remote-opens-queued-panes-on-entry.md)). 이 hint는 surface-local 편의 상태이므로 `localStorage`/`sessionStorage`에 저장하지 않는다. Android의 in-place foreground transport 복구는 같은 문서 생명주기이므로 hint를 보존하고, 혼합 버전·origin 불일치 fallback reload에서만 host navigation 상태로 다시 선택한다.
...
`/remote/v1/workspaces/active` body는 `{ "id": "...", "leaseId": "..." }`, `/remote/v1/terminals/{id}/focus` body는 `{ "leaseId": "..." }`다. 둘 다 `X-Laymux-Remote-Lease` 헤더도 허용하며, 성공 시 `workspace-state-changed` event를 발행해 MCP resource 구독자와 Automation resource cache가 stale 상태에 머물지 않게 한다. Remote workspace 전환은 해당 workspace의 unread notification을 읽음 처리하고, remote terminal focus는 해당 terminal의 unread notification을 읽음 처리한다.

`/remote/v1/workspaces/{id}/visibility`와 `/remote/v1/panes/{id}/visibility` body는 `{ "hidden": true|false, "leaseId": "..." }`이며 같은 lease header 대체를 허용한다([ADR-0153](../adr/0153-remote-hidden-item-visibility-controls.md), [ADR-0187](../adr/0187-remote-drawer-status-dots-and-hidden-subview.md)). 재시도에도 상태가 뒤집히지 않도록 toggle이 아니라 목표 상태를 받는다. workspace 요청은 frontend bridge `ui.setWorkspaceHidden`을 통해 PC의 `setWorkspaceHiddenWithFallback`을 호출하므로 active workspace를 숨기기 전 다음 visible workspace로 착지하고 마지막 visible workspace 숨김을 거부한다. pane 요청은 존재하는 workspace pane만 `ui.setPaneHidden`으로 바꾸며 dock pane은 대상이 아니다. 복원 시 기존 store가 관련 auto-close eviction을 함께 지운다. 둘 다 성공 시 `workspace-state-changed`를 발행하고 Android E2E 내부 HTTP exact allowlist에도 포함된다. Remote drawer는 visible workspace quick-hide eye, 드로어 최상단의 작은 상태 점을 가진 crossed-eye action으로 진입하는 workspace-only 하위 화면, pane별 eye를 그리며 성공 뒤 navigation snapshot을 다시 읽는다. 일부 hidden workspace만 복원하면 다음 또는 이전 hidden eye로 focus를 옮기고, 마지막 hidden workspace를 복원하면 workspace 기본 화면으로 돌아가 방금 복원된 workspace eye에 focus를 둔다. PC/Automation 호환용 `ui.toggle*Hidden`은 유지하지만 Remote API는 사용하지 않는다.
...
두 granularity는 승격/강등 규칙으로 일관성을 유지한다(issue #507): 최신 navigation snapshot의 모든 workspace에 대해 **그 workspace ID가 workspace 제외 집합에 있음 ⟺ 그 workspace의 모든 terminal pane이 pane 제외 집합에 있음**을 불변식으로 둔다([ADR-0151](../adr/0151-remote-workspace-selector-information-parity.md)). 상단 pane 토글로 마지막 남은 pane까지 제외하면 workspace가 자동 승격되고, 한 pane이라도 다시 포함하면 자동 강등된다. drawer의 workspace 토글은 active 여부와 무관하게 그 workspace의 모든 pane ID를 pane 집합에 함께 추가/제거한다. 과거 snapshot에서 workspace 단위로만 저장된 제외 상태는 다음 전체 snapshot에서 pane 집합으로 add-only 확장한다. stale ID, 삭제된 pane/workspace ID는 순회에 영향이 없고 별도 호스트 정리 수명주기가 필요 없다.
...
| Endpoint | Method | 용도 |
|---|---|---|
| `/remote/v1/file-viewer/status` | GET (browser) / POST (Android E2E) | active lease + FileViewer capability로 데스크톱 `useFileViewerStore`의 `{open,path}` 조회. Android POST는 내부 E2E proof extension이 없는 public 호출을 거부 |
...
| `/remote/v1/file-viewer/download` | POST | active lease + FileViewer capability로 명시한 호스트 경로의 원본 바이트를 bounded payload로 반환 (ADR-0185) |
| `/remote/v1/file-viewer/path-link` | POST | active lease + FileViewer capability로 terminal의 `selection`·`point`·`screen` 원문을 desktop path-link parser/CWD/stat 경로에서 검증 |
| `/remote/v1/file-viewer/list` | POST | active lease + FileViewer capability로 호스트 디렉터리 하나를 bounded 엔트리 목록으로 조회 (ADR-0198) |

이 file-viewer endpoint 들은 모두 Remote bearer token/IP/Origin gate와 active controller lease에 더해 claim 성공자 전용 FileViewer capability를 요구한다. browser/Cloud Remote는 lease를 `X-Laymux-Remote-Lease`, capability를 `X-Laymux-Remote-File-Viewer` 헤더로 전달하며 둘 다 현재 lease에 결합돼 일치해야 한다. Android E2E Remote는 caller header를 주입하지 않고 각 FileViewer POST body에 `fileViewerAuthorization: { leaseId, fileViewerToken }`을 넣는다. E2E dispatcher는 exact FileViewer method/path에서만 이 객체를 strict parse해 business body에서 제거하고, 인증된 exact session 객체로 만든 crate-private proof extension으로 변환한다. body proof를 다른 route에 싣거나, proof를 빠뜨리거나, unknown field를 넣거나, 내부 proof와 browser 헤더를 섞으면 browser 방식으로 fallback하지 않고 동일한 `403`으로 실패한다.

Android claim은 현재 pairing revision/session 확인부터 lease 설치까지 `pairing lifecycle → E2E registry → remote_access → remote_control` 순서로 진행하는 짧은 authorization commit이다. 따라서 pairing create/revoke나 새 establish가 확인과 설치 사이에 끼지 못하며, owner lock 안에서 `{leaseId, ownerEpoch, instanceId, sessionId}` binding까지 함께 기록한다. pairing create/revoke는 revision 변경과 registry clear를 같은 lifecycle 구간에 두고, challenge/establish도 미리 읽은 material의 revision이 lifecycle 재획득 시점에 여전히 current일 때만 registry를 변경한다. FileViewer 호출은 active/non-transitioning lease, capability digest와 generation, 현재 E2E registry의 exact session 객체, claim binding이 모두 일치해야 통과한다. 서버는 frontend bridge 전과 응답 직전에 같은 권한 receipt를 검증하며, 그 사이 expiry·release·reclaim·disable·새 claim/session으로 하나라도 바뀌면 bridge 결과를 버리고 `403`으로 fail closed한다. 성공한 Android FileViewer 응답의 AEAD cache에는 raw token이 아닌 이 receipt의 replay guard를 함께 저장한다. 직전 exact ciphertext를 재시도할 때도 원래 요청을 다시 복호화해 현재 권한과 guard를 검증하고, 실패하면 session을 revoke한 뒤 outer `401`로 끝낸다. 같은 nonce/sequence로 새 오류 평문을 암호화하지 않는다. browser와 Android 모두 bridge 이후 응답에 `Cache-Control: no-store`를 보낸다. 이 변경은 기존 4인자 Android bridge와 outer `PlainRequest::Http` wire를 유지하므로 E2E `compatVersion`은 1이다.

render body의 business field는 `{ "source": "current" }` 또는 `{ "source": "path", "path": "..." }`다. `current`는 client가 보낸 path를 무시하고 desktop `useFileViewerStore`의 현재 path만 사용한다. 닫힌 current viewer, 빈 path, 알 수 없는 source는 실패한다.

클라이언트는 이 `403` 을 파일 수준 에러로 취급하지 않는다. FileViewer/Explorer 요청이 `401`·`403`·`409` 로 실패하면 `/remote/v1/session/status` 로 소유권을 한 번 되묻고, 응답의 `leaseId` 가 자기 lease 와 다르면 heartbeat `409` 와 같은 등급(`hostTookOver: false`)으로 제어권 상실을 처리해 재연결 의사를 유지한 채 오버레이를 닫는다. 프로브가 실패하거나 lease 가 살아 있으면 원래 에러를 그대로 보여준다. 또한 `visibilitychange`(visible)·`pageshow`·`online` 은 lease 를 쥔 채 돌아온 경우 `maybeAutoConnect()`(없는 lease 를 claim 하는 경로)로 빠지지 않고 즉시 heartbeat 로 그 lease 를 프로브하며, 이때 진행 중이던 heartbeat 는 은퇴시킨다 — `heartbeat()` 의 self-dedup 이 그대로면 얼어 있는 동안 매달려 있던 요청의 abort timeout 뒤로 프로브가 밀린다 — 얼린 탭에서는 heartbeat 인터벌 자체가 멈추므로 복귀 직후의 첫 조작이 죽은 lease 를 만나지 않게 한다 ([ADR-0204](../adr/0204-remote-lease-recovery-on-control-failure.md)).

`path-link` body는 `{ "terminalId": "...", "mode": "selection" | "point" | "screen", "lines": ["..."], "caret"?: { "lineIndex": 0, "index": 0 } }`이며 client CWD·조합된 path는 받지 않는다([ADR-0188](../adr/0188-path-link-ambient-detection-triggers.md)). Rust route는 빈 필드와 Unicode scalar 256자 초과 terminal id를 거르고, mode별로 `selection` 8줄·4,096 scalar, `point` 1줄·4,096 scalar, `screen` 64줄·8,192 scalar를 상한으로 둔다. `caret`은 `point`에만 필수이고 브라우저 원문의 UTF-16 offset으로 해당 줄 안을 가리켜야 한다. `lines`는 trim하지 않고 `fileViewer.pathLink` async bridge에 전달한다. frontend는 mode별 bounded parser로 selection의 maximal-munch 후보, point의 caret을 덮는 후보, screen의 strong 후보를 만들고 해당 terminal의 최신 store CWD와 `terminal.pathLinkEnabled`/후보별 `pathLinkMaxLength`를 적용해 `joinCwdPath` 뒤 중복 절대경로를 제거한 `statPaths` 한 번으로 검증한다. 설정 off, 부적합/초과 입력, CWD 없음, 존재하는 경로 없음은 `{valid:false}`이고 성공은 `{valid:true,matches:[{token,path,kind,lineIndex,startIndex,endIndex}]}`다. `kind`는 `"file" | "directory"`이며 클라이언트는 file 클릭을 render로, directory 클릭을 explorer mode list로 라우팅한다([ADR-0198](../adr/0198-remote-file-explorer-overlay.md)). 각 범위는 요청 원문 기준 UTF-16 offset이고 page는 최신 xterm 셀 원문과 exact match인지 다시 확인한 뒤 실제 셀 폭으로 밑줄 좌표를 만든다. revision·terminal·lease·capability가 하나라도 바뀐 응답은 버린다. `screen` decoration은 [ADR-0220](../adr/0220-path-link-stable-frame-lifetime.md)에 따라 write 자체가 아니라 안정 프레임에서 검증된 셀 원문으로 수명을 정한다. DEC 2026 중간 프레임에서는 판정을 보류하고, 같은 live marker·좌표·token·path·kind 결과는 DOM identity를 재사용하며, 성공 적용된 decoration 집합만 화면 signature를 소유하며 일부 decoration 생성 실패는 scope와 signature를 함께 폐기한다. OSC 7 CWD는 화면 문자열에 없고 서버가 소유하므로 모든 physical write 뒤 안정 프레임의 유휴 스캔은 같은 화면도 최신 CWD로 한 번 재검증하며, signature는 그 사이 write가 없었던 중복 평가만 생략한다. live selection 때문에 dirty screen을 검증할 수 없으면 기존 screen scope를 fail closed로 폐기하고 selection change가 유휴 스캔을 다시 예약한다. resize/reflow와 buffer 전환은 전면 폐기하지만 일반 write와 scroll은 살아 있는 marker를 유지한다. 좌표/decoration은 ADR-0015의 surface-local 상태이며 검증은 stat만 수행하고 파일 내용은 prefetch하지 않는다.

줄바꿈 경로의 후보 입력·표시 좌표는 [ADR-0235](../adr/0235-wrapped-path-link-logical-lines.md)를 따른다. Remote `path-link`의 `lines`와 응답 offset은 surface 공통 복원기가 만든 **논리 줄** 기준이다. 요청·응답 shape와 서버 CWD·stat 책임은 유지하며, 물리 줄별 셀 매핑은 클라이언트에만 남긴다. 여러 줄 조각은 같은 path와 수명을 공유한다. 어느 조각이 바뀌어도 밑줄·액션 칩 전체를 폐기한다.

`render`는 Rust route가 transport에 맞춰 고정한 `maxBytes`를 frontend async bridge에 전달한다. browser는 기존 8 MiB, Android E2E는 2 MiB source read 상한을 사용한다. Android E2E route는 추가로 `maxResponseBytes=3 MiB`를 내부 bridge params에만 주입한다. frontend는 JSON escape와 UTF-8 직렬화 바이트 수를 큰 JSON 문자열로 만들지 않고 먼저 계산해 초과 결과를 작은 viewer-limit 오류로 치환하므로, Tauri `automation_response` IPC가 2 MiB NUL text를 약 12 MiB 문자열로 팽창시키지 않는다. Android E2E dispatcher는 FileViewer inner JSON에 같은 3 MiB 상한을 다시 적용하고 일반 inner HTTP는 기존 1 MiB를 유지한다. 초과 응답은 작은 inner `413`으로 치환해 4 MiB AEAD 응답 plaintext와 6 MiB public RPC envelope 안에 머문다. 요청 plaintext 상한은 이와 별도로 Remote 첨부 cap 기준(10 MiB 첨부의 base64 JSON, 약 13.4 MiB)이며 envelope body 상한은 host의 `attachmentMaxMib`에서 매 요청 계산한다([ADR-0227](../adr/0227-remote-document-attachments.md)). `readFileForViewer`는 image·PDF에도 상한을 적용한 bounded read를 수행한다. archive는 확장자 분류 뒤 parser보다 먼저 compressed source를 같은 `maxBytes`로 bounded read하고 tar.gz inflate도 같은 상한으로 제한한다. bounded ZIP은 최종 EOCD가 선언한 중앙 디렉터리를 CDFH 고정·가변 길이로 먼저 순회해 entry가 5,000개를 넘으면 entry allocation 전에 거부한다. 이후에도 전체 source를 복구형 ZIP parser에 넘기지 않고 그 exact 중앙 디렉터리만 직접 읽으며, raw 파일명은 UTF-8 flag 또는 CP437 표를 직접 적용해 선형 시간으로 디코딩한다. 따라서 더 앞의 미검증 EOCD로 backtrack할 입력이 없고, 파일명·extra·payload에 들어 있는 EOCD signature도 변조하지 않는다. Android native save는 decoded 2 MiB에 대응하는 padded base64 문자 수를 decode 전에 검사하고 디코딩 뒤 byte 상한도 다시 확인한다. 일반 text 응답은 `{path,kind:"text",content,truncated}`, HTML/Markdown preview 응답은 원문 중복을 제거한 `{path,kind:"text",truncated,previewKind,previewDocument}`, 그 밖에는 `{path,kind:"image",dataUrl}`, `{path,kind:"pdf",dataUrl}`, `{path,kind:"archive",format,entries,totalEntries,totalBytes,truncated}`, `{path,kind:"binary",size}`다. HTML/Markdown `previewDocument`는 데스크톱 FileViewer와 같은 sanitizer/CSP builder의 결과이며 오버레이는 sandbox iframe `srcdoc`으로만 표시하고 `truncated=true`이면 잘림 경고를 함께 표시한다. 일반 text는 `textContent`, image는 `data:image/*`만 사용한다. Remote에서는 settings의 `extensionViewers` shell 매핑을 실행하지 않고 항상 이 built-in web renderer를 사용한다.

`list`는 호스트 디렉터리 하나를 오버레이의 explorer 모드로 내려준다([ADR-0198](../adr/0198-remote-file-explorer-overlay.md)). body는 `{ "path": "/abs/dir" }` 또는 `{ "source": "terminalCwd", "terminalId": "..." }`이며 후자는 헤더 폴더 버튼의 진입점이다 — `terminalId`는 선택 사항이고(256자 상한), 없거나 해당 터미널이 cwd를 보고하지 않았으면 bridge가 홈 디렉터리로 fallback한다. Rust route는 형식 검증만 하고 `maxEntries`(1,000)를 고정 주입해 `fileViewer.list` bridge에 위임한다. bridge는 `list_directory(path, maxEntries+1)`를 호출하므로 Rust의 `read_dir` 열거와 entry metadata 조회 자체가 1,001개에서 멈춘다. 그 bounded sample만 디렉터리 우선 + 이름순으로 정렬한 뒤 데스크톱 explorer와 같은 순수 함수(`joinPath`/`parentPath`)로 엔트리 절대경로와 부모 경로를 완성해 `{path,parent,entries:[{name,path,isDirectory,isSymlink,size}],truncated}`를 반환한다 — Remote 클라이언트는 경로 문법을 소유하지 않는다. look-ahead 한 건이 있으면 1,000개로 잘라 `truncated:true`로 알린다. 따라서 매우 큰 디렉터리의 Remote 목록은 전체 디렉터리의 전역 정렬 prefix가 아니라 bounded sample의 정렬 결과이며, 전체 스캔보다 자원 상한을 우선한다. 오버레이의 디렉터리 모드는 zoom/download를 숨기고 엔트리 이름을 항상 `textContent`로만 그리며, 디렉터리 탭은 list 재요청, 파일 탭은 기존 render 경로다. 헤더 경로의 선택 영역은 남는 헤더 폭이 아니라 경로 텍스트의 고유 폭까지만 차지하며, 검증된 현재 경로는 옆의 copy 버튼으로 클립보드에 복사한다. explorer를 경유해 연 파일에만 back 버튼이 보이고 back은 마지막 디렉터리를 다시 list 요청한다(목록 캐시 없음). 탐색 상태는 표시 상태라 어디에도 저장하지 않으며, 늦게 도착한 응답은 render와 같은 revision + lease/capability 스냅샷 규칙으로 버린다. 헤더 폴더 버튼은 lease+capability 보유 시에만 노출된다(비활성 대신 숨김).

**Remote 는 데스크톱의 typed preview 렌더러를 확장하지 않는다([ADR-0109](../adr/0109-file-viewer-typed-preview-renderers.md)).** `previewDocument`를 만들 수 있는 것은 document 계열(`html`/`markdown`)뿐이며, bridge 는 그 판정에 데스크톱 분류기(`filePreviewKind`)가 아니라 document 전용 분류기(`documentPreviewKind`)를 쓴다. JSON/CSV/diff/log/source 같은 structured 종류는 Remote 에서 지금까지와 동일하게 원문 text 로 내려가고 오버레이가 `textContent` 로 표시한다 — structured 렌더러는 React 컴포넌트라 프레임워크 없는 임베드 자산인 `remote-app.js` 로 옮길 수 없고, 두 벌을 동기화하는 비용이 이득을 넘는다. 데스크톱과 Remote 의 표시 능력 격차는 의도된 것이며 Remote 클라이언트 통합 시 재검토한다. 공유 커맨드가 만들어 Remote 에도 도달하는 `pdf`/`archive` 는 오버레이에서 렌더하지 않고 파일 종류와 개수를 알리는 플레이스홀더로 표시한다(알 수 없는 응답으로 실패하지 않도록).

오버레이는 반드시 사용자의 button/일반 Enter action 또는 검증된 path link 클릭에서 열린다. IME 조합 중 Enter와 legacy `keyCode=229`는 제출하지 않고, host path 입력은 모바일 키보드가 대소문자를 바꾸지 않도록 자동 대문자화를 끈다. 열기 요청은 revision으로 무효화하며 닫힌 뒤 또는 lease/capability가 바뀐 뒤 도착한 응답은 그리지 않는다. 닫기는 `Escape`(capture 단계에서 소비 — 그러지 않으면 ESC가 PTY로 흘러간다)·닫기 버튼·backdrop 클릭이고 파일 본문 클릭은 닫지 않는다. 오버레이는 자기 줌을 가진다: 이미지는 실제 width, 텍스트는 font-size로 확대하며(transform은 스크롤 컨테이너가 확대분에 닿지 못해 쓰지 않는다) 두 손가락 pinch·툴바 버튼·Ctrl+Wheel로 조작하고 파일마다 100%로 초기화되며 저장하지 않는다. 비동기 MCP/desktop viewer 변경은 Remote 입력을 자동 갱신하지 않으며, 사용자가 `From host`를 다시 눌러 명시적으로 가져온다.
...
파일 모드의 Download 옆 Open은 같은 원본 바이트 endpoint·lease·capability·상한을 재사용한다([ADR-0266](../adr/0266-remote-file-open-on-device.md)). Android는 `LaymuxNative.openRemoteFile(name, mediaType, base64)`로 현재 문서 세대와 크기를 검증한 뒤 전용 `cache/remote-open` 하위에 파일별 사본을 만들고 FileProvider URI + 임시 읽기 권한의 ACTION_VIEW로 연결 앱을 연다. 새 열기 시 24시간 지난 캐시를 정리하며 앱 없음·쓰기 실패는 안내한다. 구버전 APK는 업데이트 안내를 표시한다. 브라우저는 클릭 안에서 빈 탭을 예약해 opener를 제거하고 PDF는 Blob으로 이동, 표시 가능한 HTML·텍스트·JSON·XML·이미지는 스크립트·동일 origin 권한 없는 sandbox iframe으로 표시한다. ZIP·일반 바이너리 등 지원 MIME 목록 밖의 파일은 빈 탭을 닫고 Download 사용을 안내한다. CSP frame-src만 blob:을 추가 허용한다. HTML 상대 리소스와 스크립트 실행은 보장하지 않는다. 열기와 다운로드는 전송 상태를 공유하고 전환·인증 변경 후 늦은 응답은 버린다.

Android 저장·열기 호출은 주입된 `LaymuxNative` 객체의 메서드로 실행한다. 메서드만 분리해 호출하면 Java bridge가 수신 객체를 확인하지 못해 `non-injected object` 오류로 거부한다. 다운로드 응답을 기다리는 동안에도 브리지 객체 참조를 보존한다.
...
두 라우트 모두 기존 remote guard + active controller lease(body `leaseId` 또는 `X-Laymux-Remote-Lease`) 필수이며 Android E2E exact allowlist 에 이 두 POST 만 추가돼 있다. 범용 localhost 프록시가 되지 않도록 forward 대상 호스트·포트·경로는 오직 begin 시점의 auth URL 이 결정하고, 검증 실패·만료·id 불일치는 모두 `409`/`400` 으로 fail closed 한다. Android 앱은 네이티브 `beginOauthRelay` 가 auth URL 의 `redirect_uri` 를 다시 파싱해 전달받은 포트·경로와 일치하는지 확인하고, `localhost`/`127.0.0.1`은 IPv4 loopback, `[::1]`은 IPv6 loopback 에 1회용 리스너(절대 deadline 10분, 비일치 요청으로 연장 불가)를 연다. redirect 도착 즉시 브라우저에 "앱으로 돌아가라" 정적 페이지를 응답한 뒤 콜백을 보관한다 — OS 브라우저 전면 동안 E2E 세션이 suspend 라 새 RPC 가 거부되므로(ADR-0146) `forward` 는 앱 복귀(onStart) 후 Remote 문서가 backoff 재시도로 수행한다. 일반 브라우저 Remote 는 Start 클릭 시 동기로 연 빈 창을 begin 후 auth URL 로 이동시키고(팝업 차단 회피), 로그인 후 주소창의 `http://localhost:{port}/...` 전체를 붙여넣는 수동 폴백 모달로 같은 `forward` 를 태운다.

### 13.4 Terminal Control

새 기기의 기본 구성은 [ADR-0265](../adr/0265-pc-remote-first-use-defaults.md)를 따른다. 설정 진입은 PC 상단 톱니바퀴 또는 `Ctrl+,`, Remote drawer → Settings이며 Remote의 표시·입력 선호는 PC `settings.json`이 아니라 기기별 `localStorage`가 소유한다. 호스트 `settings.remote`는 연결·보안과 폰트·위젯 공개 정책을 소유한다.
...
Remote Settings의 `Floating` 탭은 커서·패널/알림 플릭 패드 2종을 각각 켜고 크기·불투명도를 조절하며, 기존 입력 액션에서 일반 탭 버튼을 여러 개 추가·삭제한다([ADR-0239](../adr/0239-remote-pad-hold-move-opacity.md), [ADR-0265](../adr/0265-pc-remote-first-use-defaults.md)). 배치는 `laymux.remote.keybar.floating = { pads: { dpad, navPad }, buttons: [{ id, actionId, enabled, size, opacity, x, y }] }`에 기기 로컬로 저장한다. 각 패드도 `enabled, size, opacity, x, y`를 가지며 새 기기 기본은 둘 다 활성이다. 방향 패드는 `(x: 0.9108609136460442, y: 0.5274580464716007)`, 탐색·알림 패드는 `(x: 0.10478285610595374, y: 0.5206913907023182)`에 둔다. 두 패드 모두 기본 64px·불투명도 0.5이며 일반 플로팅 버튼 목록은 비어 있다. 크기는 44–128px, 불투명도는 0–100%(저장값 0–1)다. X/Y는 드래그 위치를 저장하는 가용 화면 안의 0–1 좌표이며 설정 입력에서 제외한다. 설정에는 중앙을 길게 눌러 이동한다는 안내를 표시한다. 플로팅 컨트롤은 브라우저 기본 탭 하이라이트를 제거하고 Composer보다 위, 메뉴·뷰어보다 아래에 쌓인다. 사용자 요청에 따라 플로팅 버튼은 원형으로 표시하며 일반 방향 패드는 기본 Lucide Move와 사용 중 ArrowUp/Down/Left/Right를, pane/알림 패드는 공용 아이콘 경계에서 항상 `fill="currentColor"`로 채운 GamepadDirectional과 사용 중 SquareChevronUp/Down/Left/Right를 사용한다. 사용 중 아이콘은 각 방향에 맞춰 기존 플릭 힌트에 배치한다. 키보드는 Keyboard 아이콘을 사용한다. 일반 버튼은 직접 드래그, 패드는 중앙 반경 25% 안에서 450ms 길게 누른 뒤 드래그로 움직인다. 길게 누르기 전에 8px 이상 움직이면 플릭으로 처리한다. 이동·취소는 입력을 보내지 않고 취소는 원래 위치를 복원한다. 일반 플로팅 방향키도 탭만 실행하며 패드의 기존 플릭/반복은 유지한다. visual viewport 변화 시 화면 안으로 재배치한다. 메뉴·pane 식별자 복사·파일 탐색·스킵·PC 모드는 Hidden 목록과 플로팅 선택지에 제공하며 기존 원본 버튼의 활성 조건과 실행을 공유한다. API 추가 없이 브라우저 DOM 조작과 스크린샷으로 설정·이동을 검증한다.

SquareChevron 방향 힌트는 공통 Remote CSS에서 사각형을 `currentColor`로 채우고 내부 꺾쇠는 힌트 배경 토큰 `--panel` 색의 선으로 표시한다. 선택된 방향의 사각형은 기존 강조색을 상속한다. Lucide 원본 도형과 일반 Arrow 힌트는 유지한다.
...
`/remote/v1/display-settings` 계약은 제거됐고 Android E2E native HTTP allowlist에도 포함하지 않는다. host는 terminal 목록의 `appearance`로 font family/assets, cursor, theme만 제공한다. 폰트 바이너리 전송과 위젯 데이터 공개는 기기 표현이 아니라 host 데이터 공개 정책이므로 각각 `settings.remote.serveTerminalFont`와 `settings.remote.widgets`가 계속 소유한다.
...
브라우저 remote의 모바일 터치 스크롤/선택은 surface-local 처리다. Remote HTML은 Pointer Events 기반 gesture layer를 두고 일반 한 손 드래그를 텍스트 선택에 쓰지 않는다. normal buffer이며 mouse tracking mode가 꺼진 shell/log 화면에서는 한 손 세로 스와이프가 xterm scrollback을 움직이고, alternate buffer 또는 mouse tracking mode에서는 한 손 스와이프를 TUI 앱 내부 스크롤 입력으로 전달한다. scrollback을 위로 올리면 데스크톱 TerminalView와 같은 하단 이동 버튼을 띄우고, 누르면 해당 remote xterm viewport만 live tail로 이동한 뒤 버튼을 숨긴다. 움직임 없이 long-press가 성립하면 xterm의 단어 선택 서비스를 직접 호출해 누른 셀의 단어 전체를 seed로 선택 모드에 들어가며, 공백 셀은 같은 행의 연속 공백을 선택한다. 이 경로는 `mousedown`을 만들지 않으므로 xterm helper textarea에 대한 focus 요청도 만들지 않는다. 이후 드래그 또는 표시된 선택 핸들 이동은 그 단어를 유지한 채 셀 단위로 선택 범위를 확장한다. 선택 핸들은 선택 경계를 가리키는 위쪽 꼭짓점과 둥근 몸체를 가진 물방울 모양이며 기본 크기는 22px이다. 사용자는 Display 설정에서 14~32px로 조절하고, 값은 다른 Remote 표시 설정과 함께 surface-local `localStorage`에 저장한다. double/triple tap의 단어/줄 선택과 링크 활성화를 위해 합성하는 `mousedown`/`mouseup`은 현재 포커스된 입력 표면(composer 에디터 또는 xterm helper textarea)을 빼앗지 않는다. xterm의 element `mousedown`이 helper textarea로 포커스를 옮기면 이미 열린 소프트 키보드가 내려가고 visualViewport·IME padding·crop 창이 바뀌어 처음 고른 셀 좌표가 무너진다. Android도 Linux로 판정되어 xterm의 primary selection 경로가 `textarea.select()`로 포커스를 옮길 수 있으므로, 합성 구간에서는 helper `textarea.focus`와 `textarea.select`를 함께 막고 합성 전에 포커스돼 있던 노드만 되돌린다. 롱프레스의 selection service 직접 경로도 `refresh()`로 화면만 갱신하고 Linux primary selection 등록은 요청하지 않는다 — 선택 제스처가 IME를 올리지 않는 기존 동작은 유지한다(#482와 같은 입력 표면 불변식). 첫 단일 탭은 `.xterm-screen`의 링크 hit-test로 전달하며 평문/OSC 8 HTTP(S) 링크 셀일 때만 Linkifier의 mouse down/up 활성화 경로를 합성한다. 링크가 아닌 plain 단일 탭은 기존 선택을 해제하고 Direct mode에서는 xterm helper, Composer mode에서는 보이는 Composer editor에 같은 gesture focus를 준다. double tap은 단어 선택, triple tap은 줄 선택으로 처리하며 Composer·Direct 모두 입력 포커스를 바꾸지 않는다. `pointercancel`은 미완료 탭을 실행하지 않는다. 터치·펜의 `contextmenu`와 진행 중 선택 제스처의 네이티브 메뉴 이벤트는 terminal host의 capture 단계에서 차단해 xterm 우클릭 처리의 helper focus/select를 막는다. pointer 종류를 제공하지 않는 coarse-pointer 메뉴 이벤트도 같은 경로로 처리한다. 두 손가락 세로 스와이프는 현재 surface에서 가능한 스크롤 경로로 라우팅한다. mouse tracking mode에서 선택 또는 링크 활성화 합성 이벤트를 만들면 force-selection modifier를 실어 TUI로 입력이 전달되지 않게 한다. 선택된 텍스트는 별도 버튼 없이 선택 interaction이 끝나는 시점에 브라우저 클립보드로 복사한다. 마우스 선택을 terminal 밖까지 끌고 놓는 경우도 xterm의 document-level `mouseup` 선택 확정 뒤 복사를 예약한다. Clipboard API가 없거나 거절되면 `execCommand("copy")`가 발생시킨 네이티브 `copy` 이벤트의 `clipboardData`에 `text/plain`을 설정하고 기본 복사를 취소한다. 이 capture listener는 호출 구간에만 설치하고 성공·실패 모두 제거한다. 임시 textarea 생성·focus·blur·select는 하지 않는다 — 포커스를 잠깐 빌렸다 복원해도 열린 IME가 내려가거나 시스템 Back으로 닫힌 IME가 다시 올라올 수 있기 때문이다. 이 규칙은 롱프레스 시작·드래그·양쪽 핸들 이동·손을 뗀 뒤 자동 복사까지 동일하며, 컴포저의 초안·선택 범위·조합 상태도 보존한다. 브라우저가 복사를 거절하면 기존 오류 표시로 끝내고 입력 포커스로 우회하지 않는다. Chromium의 실제 xterm·네이티브 copy 이벤트로 포커스 무변경을 검증하며, Android/iOS 실제 OS 키보드는 별도 실기 검증 대상이다. 로컬 모바일 iframe은 `clipboard-write` 권한을 명시한다. 이 동작은 Remote API 계약이나 PTY 전역 상태를 바꾸지 않는다.
...
Remote 입력 action은 기본행(`main`)과 `Keys` 확장행(`expanded`) 각각의 `left`·`center`·`right` 정렬 구역에 배치하며, 어느 구역에도 없으면 `Hidden` 이다([ADR-0213](../adr/0213-remote-input-action-segment-placement-and-user-keys.md), [ADR-0186](../adr/0186-remote-input-action-three-zone-layout.md), [ADR-0265](../adr/0265-pc-remote-first-use-defaults.md)). 구역은 폭이 남는 기기에서 순서만으로 표현할 수 없는 좌·우 정렬을 표현하기 위한 것이며, 폭이 모자라면 행 내부 가로 스크롤로 자연히 되돌아간다. 새 기기 기본값은 기본행 `left: ^C · Q · Esc · /clr` / `right: Keyboard · Keys · Send`, 확장행 `left: Composer 전환 · P↕N↔ · 가장 오래된 알림 · Tab` / `right: ^U · ^L · ^T · ↕↔ · PgUp · PgDn · Attach file`이다. Send는 저장된 배치와 무관하게 Composer mode일 때만 렌더한다. `Keys`는 기본행의 세 구역 또는 `Hidden` 만 가능하고 자기가 여는 확장행에는 둘 수 없는 구조 토글이다. Keys를 숨기면 열린 확장행을 즉시 닫아 `expanded=false`로 저장하되 확장행 구성과 순서는 보존한다. 그 밖의 고정 action(`Keyboard`, `Send`, `Composer`, `Attach file`)과 모든 소프트키는 두 행의 여섯 구역과 `Hidden` 사이를 자유롭게 오간다. 배치 편집은 key bar 안의 팝오버가 아니라 항상 접근 가능한 Remote drawer의 Settings → `Input bar` 탭에 둔다.

`laymux.remote.keybar`의 `zones`(`{main:{left,center,right}, expanded:{left,center,right}}`)가 배치와 구역 내 순서의 SoT다. **배치가 곧 활성화다** — 두 행 중 어딘가에 배치된 action만 렌더하고, 배치되지 않은 것이 `Hidden` 이다. `Hidden` 목록은 저장하지 않고 미배치에서 파생하며, 세트/커스텀 선택(`sets`/`custom`/`usedCustom`)과 전역 `order` projection은 없다(ADR-0040의 사용자 순서는 구역 내 순서가 계승한다). 저장값이 이 shape이 아니면 — ADR-0186의 평탄 배열 포함 — 마이그레이션 없이 기본 배치로 되돌리되 현재 내장 액션과 사용자 키 목록의 `knownIds`에 없는 기본 사용자 키 참조도 걸러낸다. 정상 shape의 명시적 빈 배치·커스텀 순서는 보존하고 미지·중복 ID만 항목 단위로 버린다. 미배치 action을 기본값으로 채워 넣지 않으며 버전 필드도 두지 않는다. 배치·확장 상태와 사용자 등록 키는 `localStorage`에만 저장하는 surface-local UI 상태이며 `settings.json`, Remote API와 PTY 계약은 바꾸지 않는다([ADR-0004](../adr/0004-settings-vs-ui-state-separation.md), [ADR-0028](../adr/0028-remote-soft-key-toolbar.md), [ADR-0213](../adr/0213-remote-input-action-segment-placement-and-user-keys.md), [ADR-0265](../adr/0265-pc-remote-first-use-defaults.md)).

기본 제공 Ctrl 조합은 `^C ^J ^U ^T ^L` 다섯 개이고 라벨 표기는 전부 `^X` 계열로 통일한다(전용 `Ctrl+C` 버튼은 없다 — 같은 `\x03`을 보내는 `^C` 소프트키가 그 자리를 대신한다). 기본 `Q` 소프트키는 단일 바이트 `q`를 보낸다. 그 밖의 조합키는 사용자가 등록한다: `userKeys: [{id,label,seq,submit}]`에 저장하고 `id`는 `/^u-[a-z0-9]{1,24}$/`, `label` 1–8자, `seq` 1–32 code unit, 최대 24개를 정규화가 항목 단위로 강제한다. 등록 경로는 조합 피커(Ctrl / Alt / Ctrl+Alt + 베이스 키, Shift는 Alt를 포함할 때만 유효 — `Ctrl+Shift+X`는 `Ctrl+X`와 같은 바이트다)와 raw 시퀀스 입력(`\e \xNN \r \n \t \0 \\`)이다. 역슬래시 키가 없는 모바일 키보드를 위해 raw 입력은 `\e`·`\r`·`\t`·`\n`·`\x`·`\\`를 현재 선택 범위에 넣는 보조 버튼도 제공한다. 등록된 키는 `keyDef()` 단일 조회를 거친다. raw 폼의 `Send Enter` 체크박스는 기본 off이며 `submit`이 정확히 `true`일 때만 파싱된 seq를 text로 기존 `/input`의 `submit:true`에 보낸다. 누락·다른 타입은 false로 정규화한다. 체크 시 기존 Send와 같이 끝 줄바꿈 제거·본문 정규화·bracketed paste·별도 Enter 쓰기를 백엔드에 맡긴다. 클라이언트는 pending raw 입력을 flush하고 같은 write chain에 제출을 넣어 앞뒤 키의 요청 순서를 보존하며, terminal/lease가 바뀐 queued 입력은 폐기한다. 새 API나 OS 추측 분기를 만들지 않는다([ADR-0234](../adr/0234-remote-user-key-explicit-submit.md)).
...
Remote 설정 화면도 Windows Terminal형 개별 행과 제목 계층을 사용한다. 설정 drawer 안에서만 시스템 UI 폰트·최소 14px 라벨·12px 설명·36px 입력 높이를 적용하며, 터미널 글꼴과 키 바·워크스페이스 밀도는 변경하지 않는다. 설명은 라벨 아래에서 줄바꿈하고 토글은 오른쪽 같은 축에 정렬한다. Display는 글자 크기·Composer 불투명도·워크스페이스 메뉴·히스토리/스크롤·선택/위젯으로 묶고, 44px 탭 줄은 가로 스크롤과 기존 키보드 탐색을 유지한다. 기기 로컬 저장·즉시 적용·연결 전 편집·기존 DOM ID는 그대로다.
...
`Input bar`는 Main·Keys 배치 카드를 먼저 보여 주고, 미배치 키 목록은 기본 접힌 `Add keys`, 사용자 키 등록은 `Create custom key`, 행별 배율은 `Button sizes`에서 펼친다. 선택한 키의 이동 조작만 해당 구역 옆에 표시하며, 새로 추가한 키는 조작부까지 스크롤한다. 펼침 상태와 미완성 등록 입력은 편집 중 재렌더에 유지하지만 별도 영속하지 않는다. 접힌 `Add keys`도 숨김 드롭 대상이고, `Hidden` 이동·한 번 탭해 Keys 행에 추가·드래그·키 시퀀스 계약은 그대로다(ADR-0213 직접 적용).

모바일 remote의 app 높이는 `visualViewport.height`를 CSS 변수 `--remote-viewport-height`로 반영한다. 브라우저가 지원하면 viewport meta의 `interactive-widget=resizes-content`도 함께 사용한다. 같은 관측값은 터치 Composer의 keyboard-visible 판정에도 쓰되, DOM focus를 proxy로 삼지 않는다. `VirtualKeyboard.boundingRect`가 있으면 이를 우선하고, 없으면 폭별 keyboard-closed 최대 높이와 현재 높이의 차이가 80px 및 기준 높이의 15% 이상일 때만 열린 것으로 본다. 폭 변경은 기준 높이를 재설정해 회전을 키보드로 오인하지 않는다([ADR-0207](../adr/0207-remote-composer-recall-observes-soft-keyboard-geometry.md)). footer는 viewport 폭과 무관하게 입력 action 버튼만 시각적으로 표시한다. terminal title/profile/CWD 상태 문구는 기존 상태 계산·갱신에는 유지하되 live region으로 만들지 않고 `sr-only`로 두어, 반복 알림이나 가로 화면 버튼 행의 공간 점유를 만들지 않는다. 이 값들은 모두 surface-local DOM geometry이며 Remote API/PTY 소유권 계약을 바꾸지 않는다.

terminal host의 geometry 변화는 fit 정책([ADR-0038](../adr/0038-remote-height-shrink-surface-crop.md))을 거친다. xterm은 host 내부의 clipping wrapper 속 sizer 요소에 마운트되고, 폭 변경 또는 높이 증가만 fit + PTY resize를 전파한다. 폭이 그대로인 높이 축소(native keyboard 열림, 키바 표시, URL bar)는 normal buffer에서 PTY geometry를 유지한 채 sizer를 마지막 fit 픽셀 높이로 고정해 crop한다 — scrollback reflow형 TUI(Codex CLI 0.128.0+)가 SIGWINCH마다 전체 트랜스크립트를 재출력하는 flood를 막기 위해서다. Composer는 terminal 위의 overlay라 표시·숨김이나 높이 조절이 host 높이를 줄이지 않으며 이 crop 분기를 만들지 않는다([ADR-0203](../adr/0203-remote-composer-overlays-terminal-output.md)). crop 창은 화면 바닥이 아니라 **live tail 행**(커서 행과, 커서 아래 마지막 비어 있지 않은 렌더 행 중 아래쪽)에 정렬한다([ADR-0056](../adr/0056-remote-crop-window-anchors-live-tail.md)). tail 아래의 빈 행은 창 밖으로 밀려나므로 출력이 화면보다 적어도 프롬프트가 composer 바로 위에 붙는다. 이동량이 잘린 높이를 넘어 sizer 상단이 host 상단보다 내려가면 clipping wrapper가 노출되므로, wrapper 배경은 활성 터미널 appearance의 xterm 테마 배경을 따르는 `--terminal-surface-bg`로 칠한다. 커서와 tail 사이가 창 높이보다 길어 tail 정렬로도 커서 행이 창 위에 남는 경우에는 커서 가시성 보정이 우선해 커서·IME UI가 화면 밖에 남지 않게 한다. tail 재계산은 crop이 활성일 때만 렌더·스크롤·커서 이동에 대해 프레임당 한 번으로 합친다. 터치 선택 핸들은 host에 직접 붙어 clipping 밖에 남는다. 시작 핸들은 선택 경계의 왼쪽 아래, 끝 핸들은 오른쪽 아래로 벌어지는 물방울 모양이며 경계에 닿는 윗모서리만 각지다. 첫·끝 열처럼 바깥 공간이 부족하면 경계 꼭짓점은 유지하고 물방울 몸체만 안쪽으로 반전해 터치 영역이 화면 밖으로 잘리지 않게 한다. 핸들을 잡은 위치와 선택 경계의 오프셋을 유지해 첫 이동에서 선택이 튀지 않게 한다. 직접 핸들을 끄는 동안만 손가락 위에 1.8배 확대경을 표시한다. 확대경은 기존 xterm DOM 행과 선택 표시 중 보이는 주변 행만 복제해 글꼴·ANSI 색상·전각 폭을 유지하고, 입력 요소나 별도 터미널 인스턴스를 만들지 않는다. 갱신은 드래그·렌더·스크롤을 animation frame당 한 번으로 합친다. 확대경은 포커스·포인터·접근성 탐색을 받지 않으며 릴리스·취소·pointer capture 상실·창 blur·문서 숨김에서 제거한다. 화면 가장자리에서는 visualViewport 안으로 제한하고 위쪽 공간이 부족하면 손가락 아래에 배치한다. crop 중에도 새 attach·터미널 전환은 보존된 `cols/rows`를 PTY에 재전송한다(fit 생략과 resize 전송은 별개). alternate buffer는 scrollback flood가 없고 전체 화면 앱이 상단 행을 필요로 하므로 항상 fit을 전파하며, crop 중 buffer 전환이 일어나면 즉시 재평가한다. 높이 축소가 입력 표면이 아니라 **chrome 행**(위젯 스트립)의 등장 때문이면 crop이 아니라 fit 기준선 재설정이다([ADR-0129](../adr/0129-remote-chrome-row-rebases-fit-baseline.md)): 표시 상태가 뒤집힐 때 `fittedHostHeight`를 버리고 refit해 새 host geometry를 채택한다. 재설정하지 않으면 attach fit이 기록한 스트립 없는 높이가 기준선으로 남아, 폰트 로드처럼 나중에 도착하는 refit이 전부 crop 분기에서 막힌다.
...
host 데이터 공개 설정에서 해석한 표시 전용 계약이다. 포함 범위는 profile의 `fontFamily`, xterm
option으로 바로 적용 가능한 `cursorStyle`, 선택적 `cursorWidth`, `theme`, 그리고 정책상 허용된
선택적 `fontAssets`다. `fontSize`와 스크롤 민감도는 포함하지 않고 Remote client가 기기 로컬 값을
합성한다. Windows Terminal 색상 스킴의
...
screen checkpoint의 직렬화 길이는 원본 PTY sequence에 없으므로 V1 wire sequence에는 그 길이만큼 offset을 둔다. snapshot header의 `seqEnd - seqStart`와 binary 길이는 계속 같고 후속 delta도 같은 offset을 적용한다. 원본 PTY 경계는 `sourceStartSeq`/`sourceSeq`가 따로 나타낸다. 기기 로컬 `snapshotMaxKib`는 scrollback 보존량을 고르는 최초 요청 소프트 예산이며 현재 viewport·alternate buffer·복원 mode를 담은 최소 checkpoint는 이를 넘을 수 있다. 값을 보내지 못하는 구형 client에는 서버가 4 KiB fallback을 사용하고, 모든 checkpoint에는 1 MiB 절대 상한을 적용한다. slow consumer queue overflow·generation retire·sequence gap이면 socket을 닫는다. Cloud host도 같은 checkpoint/subscription 경로와 wire offset을 쓰고 공통 계약을 relay에 전달한다. 일시적인 checkpoint bridge/race/gap은 retryable `terminal_output_gap`, terminal 소멸은 `terminal_not_found`, malformed/절대 상한 위반은 `terminal_output_unavailable`로 전달한다. Remote page는 재접속이나 workspace 전환 시작에 기존 xterm을 먼저 비우지 않고, 새 snapshot header/binary pair를 완전히 검증한 직후 reset하고 적용한다. 사용자 주도 최초 연결·pane 전환은 replay 뒤 live tail에 착지하지만, 같은 terminal의 Output WebSocket 재접속과 자동 lease reclaim은 replay 전 surface-local viewport의 live tail 기준 행 오프셋을 복원한다. 새 checkpoint가 보존한 scrollback보다 오프셋이 크면 xterm 경계에서 clamp한다. 확장 metadata가 없는 과거 raw V1 snapshot은 pre-attach resize geometry에서만 legacy 호환으로 재생하고, `snapshotKind:"screen"`은 generation/source/geometry 전체를 필수 검증한다.

scroll-top history expansion([ADR-0182](../adr/0182-remote-scroll-top-history-expansion.md)): output attach는 선택적 history 예산을 함께 보낼 수 있다. Direct/Cloud는 output WebSocket 쿼리 `historyKib`, Android E2E는 암호화 open record의 선택 필드 `historyKib`다. bundled Remote page는 최초 attach부터 기기 로컬 `snapshotMaxKib`를 보낸다. 서버가 쓰는 실제 예산은 `max(4 KiB fallback, clamp(historyKib, 1..=1024))`이며 파싱 불가능하거나 누락된 값은 세 transport 모두 fail-open으로 4 KiB attach가 된다. 1 MiB 절대 상한과 checkpoint 검증 경로는 그대로다. Remote page는 normal buffer이면서 mouse tracking이 아닐 때, 스크롤 제스처(위쪽 wheel·위쪽 드래그·Shift+PageUp)가 viewport를 최상단으로 *도달*시키거나 그 상태로 위쪽 드래그/wheel이 오면 재attach하며(제스처 없이 출력이 밀어낸 최상단 도달과 포인터 누름은 무시, 연결 중단 알림 중에는 요청 안 함), 요청 예산은 지금 들고 있는 화면에서 유도한다 — `clamp(현재 snapshot bytes × 4, 64 KiB, 1024 KiB)`. 재attach는 재접속 경로와 동일하게 live tail 기준 행 오프셋을 복원해 읽던 줄을 유지한다. 각 요청은 요청 id로 자기 attach의 snapshot만 정산하고, 끼어든 attach(재접속·pane 전환·조기 실패·소켓 종료·타임아웃)는 요청을 취소하면서 예산을 요청 직전 값으로 되돌린다. 전달된 snapshot의 binary 길이가 직전보다 크지 않으면 그 terminal에서는 더 이상 요청하지 않는다(행 수는 replay geometry와 이후 fit reflow 때문에 attach 간 비교 불가). 이미 client 상한(1024 KiB)으로 요청한 뒤 다음 요청이 다시 상한을 넘게 계산되고 직전 checkpoint가 그 예산을 실제로 채웠다면, attach 없이 "더 큰 checkpoint를 요구할 수 없다"는 별도 문구로 알리고 같은 상태로 들어간다(예산을 못 채운 checkpoint는 데스크톱이 먼저 바닥난 것이므로 "이전 출력 없음"으로 알린다). 두 한계 문구는 2초 동안만 표시한 뒤 현재 terminal 제목으로 돌아가며, 그 사이 다른 상태가 표시되면 덮어쓰지 않는다. 예산·소진 상태는 attach 대상 terminal에 귀속되어 pane 전환·연결 해제 시 초기화되고, 사용자가 직접 다시 attach하는 경우(같은 pane 재선택 포함)도 기기 예산에서 시작한다. 자동 재접속만 확장 예산을 유지하며, 소켓이 연속으로 열리지 않으면 기기 예산으로 내려가면서 소진 상태도 함께 푼다. `LaymuxNative.supportsOutputHistoryBudget()`이 없는 구버전 Android 커넥터에는 예산 필드를 보내지 않는다. 커넥터는 범위 밖 `historyKib`를 stream 실패가 아니라 "확장 없음"(0)으로 떨어뜨린다. `snapshotKind`가 없는 legacy 무순번 host는 attach 전에 알 수 없어 요청 1회를 쓴 뒤 확장 대상에서 제외된다.

Remote 입력 UI의 명시적 선호는 `laymux.remote.inputMode`에만 저장한다. 저장값이 없으면 포인터 종류와 관계없이 composer가 기본이다([ADR-0265](../adr/0265-pc-remote-first-use-defaults.md)). terminal별 현재 모드와 draft/revision/in-flight token은 페이지 runtime Map에만 있고 reload 시 사라진다. Android의 in-place foreground transport 복구는 문서를 reload하지 않으므로 draft/revision을 보존한다. Background 전에 native 실행에 들어간 단일 HTTP는 exact 재개 응답으로 성공·실패를 확정하고, 실행 전에 취소되거나 queue에서 폐기된 요청만 transient 실패 처리한다. V1 snapshot state와 synthetic 최종 mode 적용이 끝나기 전에는 composer action과 Direct clipboard paste를 fail-closed한다. 입력 포커스는 사용자가 시작한 진입만 변경하며, 그 진입도 pointer 축으로 한 번 더 갈린다([ADR-0196](../adr/0196-remote-coarse-pointer-attach-defers-input-focus.md)). fine pointer에서는 Connect·terminal 선택으로 시작한 attach가 완료 시 입력 surface를 `focus()`한다. coarse pointer에서는 attach 완료가 `focus()`하지 않는다. 그 기기의 입력 포커스는 `Keyboard` 버튼, 입력 surface 직접 탭, 명시적 모드 전환, Send 직후 복귀, 파일 뷰어 닫기처럼 gesture의 transient user activation 창(같은 task 또는 그 직후 `requestAnimationFrame`) 안에서만 만든다. 최초 claim 뒤 navigation이 active terminal id를 확정하기 전에는 draft 소유자를 정할 수 없으므로 xterm을 만들지 않는다. Composer mode의 terminal touch bridge는 plain 단일 탭을 보이는 Composer editor로 같은 pointerup에서 동기 라우팅하고, 바로 다음 `requestAnimationFrame`에서 xterm helper 또는 그 blur가 남긴 body만 한 번 재조정한다. 링크·선택 gesture의 focus는 훔치지 않으며, xterm helper textarea만 blur해 native keyboard는 열렸지만 입력 대상은 없는 상태를 만들지 않는다. 첨부 업로드 완료의 focus 복원은 pointer와 무관하게 유지한다 — 업로드 중 editor `disabled`가 만든 blur를 되돌리는 것이고 대기도 동일 오리진 POST 하나다. 소프트 키보드는 그 창 안의 focus로만 열리므로, `await` 뒤의 focus는 DOM focus만 남기고 IME는 닫힌 상태를 만들어 DOM focus를 키보드 상태 proxy로 쓰는 `Keyboard` 버튼의 위상을 뒤집는다. Output WebSocket 재접속, snapshot replay 완료, heartbeat 만료 뒤 visible-document 자동 reclaim 같은 복구 경로는 pointer와 무관하게 bytes·geometry·lease만 복원하고 입력 surface를 `focus()`하지 않는다. 따라서 사용자가 시스템 키보드나 브라우저 동작으로 내린 키보드는 transport 복구 때문에 다시 열리지 않는다. PC WebView도 동일한 입력 상태 전이 계약을 사용하되 선호 키는 `laymux.desktop.inputMode`, 최초 기본값은 direct이며 Tauri `write_terminal_input`과 아래 desktop output v3 계약을 사용한다.
...
Remote composer도 데스크톱과 동일하게 두 가지 과거 입력 recall 경로를 노출한다(issues #504/#505, [data-flow §8.8](./data-flow.md)). (1) 빈·포커스 초안에서 **Tab** 은 최신순·중복 제거·blank skip·최대 N개(기본 8)로 만든 listbox 를 에디터 위에 띄우고, (2) 비어 있지 않은 초안 타이핑 중에는 prefix(대소문자 무시)·최신순·중복 제거·`value` 완전 일치 제외이되 `send:true` 별표는 유지·cap 자동완성 dropdown 을 같은 자리에 띄운다. 두 목록은 초안 길이(0 vs >0)로 상호 배타이며, keydown 에서 자동완성 블록을 Tab-open 블록보다 앞에 둬 비어 있지 않은 초안의 Tab 이 팝업을 열지 않고 추천을 채우게 한다. 자동완성은 초기 강조가 없어(activeIndex=−1) 일반 Enter 가 계속 Send 이고, ↑/↓ 로 강조가 생겨야 Enter 가 선택으로 바뀐다. **과거 입력 텍스트(history)와 일반 초안은 페이지 runtime Map(history 는 `Map<scopeKey, string[]>`, 버킷당 최대 200개)에만 두고 `localStorage`/`sessionStorage`/디스크 어디에도 영속하지 않는다. 단, 사용자가 선택한 text 파일과 5 KiB 초과 clipboard paste는 ADR-0181의 명시적 첨부 정책에 따라 bounded host cache 파일이 된다.** 이외의 초안은 ADR-0029 의 미전송·전송 문자열 비영속 경계를 Remote 로 그대로 확장한 것으로, 셸에 입력한 비밀번호 등 민감 문자열이 recall 표면이나 페이지 unload 뒤로 새지 않게 하는 보안 경계다. `commitComposer` 성공 콜백에서 전송된 텍스트만 이 Map 에 append 한다.

[ADR-0226](../adr/0226-composer-stars-are-host-global-persistent-state.md)·[ADR-0229](../adr/0229-composer-starred-entries-have-label-and-send.md)에 따라 사용자가 recall/자동완성 행의 별표 버튼을 명시적으로 누르거나 Desktop Settings의 Terminal 섹션에서 직접 추가·편집한 항목만 비영속 경계의 예외다. SoT는 `settings.json`의 backend-owned `terminal.composerStarredEntries: {value,label,send}[]`이며 workspace·pane·history scope와 무관한 호스트 전역 ordered set이다. 정체성은 `value`다. 레거시 `string[]`은 `{value,label:"",send:false}`로 읽는다. 최대 200개, `value` UTF-8 16 KiB, `label` 256바이트이고 빈 `value`·중복·한도 초과는 거절한다. 일반 settings patch에서는 `/terminal/composerStarredEntries`가 sensitive read-only이고 frontend checkpoint는 디스크 최신값을 보존한다. Desktop Settings는 전체 목록을 표시하며 추가·삭제·행 클릭 편집을 일반 Save 초안과 분리해 즉시 전용 IPC로 반영한다. Desktop IPC `set_composer_starred_entry(text,starred,label?,send?,previousText?)` 반환과 `composer-starred-entries-changed` event payload는 객체 배열이다. bearer/IP/origin guard + active controller lease를 모두 요구하는 `GET /remote/v1/composer/starred?leaseId=...`, `POST /remote/v1/composer/starred`만 Remote 진입점이다. POST 본문은 `value`(JSON `text`는 alias)·`starred`·선택적 `label`/`send`/`previousValue`다. `text`와 `value`가 둘 다 있고 다르면 400이다. 생략된 `label`/`send`는 기존 항목이면 유지하고 신규면 `""`/`false`다. `previousValue`가 있으면 그 항목을 제자리에서 이름 변경하고, 새 `value`가 다른 항목과 겹치면 400이다. GET/POST 응답은 `{revision:number,entries?:{value,label,send}[]}`이고 mutation은 atomic replace 뒤 event를 내보낸다. Remote는 claim 직후 목록을 읽고 `localStorage` 사본을 만들지 않는다. 자동완성은 별표 최신순을 runtime history보다 먼저 합성하고 value/label prefix로 매칭한다. exact-query 제외는 `value` 완전 일치이며 `send:true`면 제외하지 않는다. 수락 시 초안을 `value`로 채우고 send면 Composer Send를 이어서 호출한다. 자동완성 롱클릭(500ms)은 Settings 행과 같은 편집기를 연다. 롱클릭 문턱을 넘긴 pointer는 pick/Send를 소비한다. 별표 버튼 pointer action은 행 선택을 막고 editor focus를 유지한다. 아래의 “Remote는 host settings를 읽지 않는다”는 설명은 이 좁은 별표 전용 endpoint를 제외한 범용 settings surface가 없다는 뜻이다.

자동완성 항목과 그 별표 버튼은 공용 `keepInputSurfaceFocus`로 `mousedown`·`pointerdown`의 기본 포커스 이동을 모두 막는다. 따라서 터치로 값을 채우거나 별표를 토글해도 Composer가 포커스와 열린 소프트 키보드를 유지한다. 실제 동작은 계속 `pointerup`/`click`이 담당한다.

Remote 별표 GET/POST 응답은 runtime 단조 revision과 선택적 전체 목록 `{revision:number, entries?:{value,label,send}[]}`을 반환한다. GET의 `revision` query가 현재값과 같으면 `entries`를 생략한다. Remote는 claim 직후 전체 목록을 읽고 heartbeat 성공 뒤 조건부 GET으로 Desktop 변경을 수렴시키며, 더 낮은 revision의 늦은 응답은 적용하지 않는다. lease를 잃으면 surface revision과 목록을 함께 폐기한다. `dismissTopLayer()` 순서는 OAuth relay modal → FileViewer overlay → Composer 별표 편집기 → drawer 하위 화면 → Dock disclosure → drawer → Composer recall/autocomplete 다.

터치 입력에서 빈 editor 탭으로 history 목록을 여는 보조 경로는 탭 시작 시점에 소프트 키보드가 이미 보였을 때만 동작한다. 따라서 focus만 남고 키보드가 내려간 상태에서 키보드를 올리는 첫 탭은 목록을 열지 않는다. history 또는 자동완성 목록이 보일 때 editor의 빈 영역을 탭하면 목록만 닫고 초안은 바꾸거나 보내지 않는다([ADR-0207](../adr/0207-remote-composer-recall-observes-soft-keyboard-geometry.md)).

Remote 는 host `settings.json` 을 읽지 않으므로(설정 endpoint 없음) 두 recall 기능 on/off 는 surface-local `localStorage` 키 `laymux.remote.composerHistoryPopup`·`laymux.remote.composerAutocomplete`(기본 on, 없거나 `"0"` 이 아니면 on)에만 저장하며, 토글 UI는 Remote drawer Settings의 "Composer" 섹션에 둔다. 같은 섹션의 `Hide unused agent input`은 **Remote Composer가 입력 surface인 동안 중복되는 에이전트 고유 입력부를 숨기는** 시각 전용 선택이며, surface-local `laymux.remote.composerHideAgentInput`에 저장하고 기본 on(명시적 `"0"`일 때만 off)이다. Claude/Codex/Grok별 숨길 줄 수는 `laymux.remote.composerHiddenAgentInputLines`에 각각 저장하며 기본값은 3/4/2, 허용 범위는 0~24다. Composer를 열 때 현재 pane activity가 해당 에이전트면 설정된 줄 수에서 overlay가 실제로 덮는 terminal 행 수를 차감하고, 남는 줄만 live tail에서 위로 이동해 사용되지 않는 에이전트 입력부를 viewport 밖으로 제외한다. 따라서 overlay가 설정된 줄을 이미 모두 덮으면 viewport는 live tail에 머문다. Composer를 접거나 Direct input으로 전환하거나 이 선택을 끄면 live tail로 돌아가 에이전트 입력부를 즉시 다시 표시한다. 이 전이는 PTY 크기·출력·activity 상태·Remote API에는 관여하지 않는다. 같은 섹션의 `History sharing` select 는 데스크톱 `terminal.composerHistoryScope` 와 같은 세 값(`global`(기본)/`workspace`/`pane`)을 `laymux.remote.composerHistoryScope` 에 저장하며([ADR-0055](../adr/0055-composer-history-scope-setting.md)), 알 수 없는 값은 `global` 로 해석한다. Remote 의 버킷 키 도출도 `composerHistoryBucketKey()` 한 곳뿐이고 workspace 를 모르는 dock 터미널은 `pane:` 키로 좁게 fallback 한다. 스코프를 바꾸면 열려 있던 recall 목록을 닫고 새 버킷에서 다시 읽는다(병합·이관 없음). 데스크톱 설정과 자동 동기화하지 않는다 — 별개 surface 계약이다. 이 recall 은 새 Remote API/endpoint 를 만들지 않고 기존 `/input` 전송 경로만 사용한다.

위 숨김이 적용된 Remote Composer에서 `N = max(0, 설정 줄 수 - overlay가 덮는 행 수)`줄 위의 viewport는 **Composer 전용 숨김 경계**다. `N > 0`이고 사용자가 그보다 위의 scrollback을 보는 중 최하단 버튼을 누르면 먼저 이 경계로 이동하고, 버튼은 실제 live tail이 아니므로 계속 표시한다. 경계에서 다시 누르면 실제 live tail로 이동해 버튼을 숨긴다. 버튼의 bottom offset은 현재 Composer 높이를 반영해 overlay 상단보다 위에 유지한다. Composer editor의 수직 resize는 surface-local ResizeObserver가 Composer 높이·가린 행 수·숨김 경계를 다시 계산한다. 실제 tail을 본 뒤에도 Composer가 다시 focus를 받으면 숨김 경계로 자동 복귀하고 버튼을 다시 표시한다. `N = 0`이거나 Direct input·접힌 Composer·숨김 off·지원하지 않는 activity에서는 이 2단계 경계를 만들지 않고 기존처럼 한 번에 live tail로 이동한다.
...
Android E2E 세션 레지스트리도 `AppState`의 번호 락보다 앞서는 외부 predecessor다. 일반 검증은 레지스트리 guard를 놓고 `remote_access`/`remote_control`을 읽지만, E2E claim 커밋은 세션 교체와 소유권 설치 사이의 TOCTOU를 없애기 위해 짧은 구간 동안 `Android E2E registry → remote_access → remote_control` 순서로 중첩한다. pairing lifecycle까지 함께 잡는 경로의 전체 상대 순서는 `pairing lifecycle → Android E2E registry → remote_access → remote_control`이다. 어떤 번호 락을 보유한 상태에서도 E2E 레지스트리로 역진입하지 않는다([ADR-0208](../adr/0208-android-e2e-file-viewer-typed-capability.md)).
...
**xterm binary mouse 입력**: desktop 전용 `write_terminal_binary_input(id, generation, data: number[])`은 xterm 6.0.0 DEFAULT mouse `onBinary`의 `CSI M Pb Px Py`를 문자열 UTF-8 경계 없이 전달한다([ADR-0216](../adr/0216-xterm-legacy-mouse-binary-input-boundary.md)). UI는 각 code unit이 `0x00..0xff`인지 검사하고 Rust는 정확한 6바이트 형식, current PTY generation과 Local human-control owner를 다시 검증한다. command는 입력 IPC/FIFO 순서를 유지하는 plain command이며 Automation/Remote API에는 노출하지 않는다. Linux는 payload의 모든 바이트를 보존하고, Windows는 UTF-8인 ConPTY 경계에서 바뀌는 `0x80..0xff` payload를 enqueue 전에 거부한다. 실패는 재시도하지 않는 human-input delivery failure이며 SGR `onData`, protocol reply, replay와 최근 사용자 입력 모델에는 영향을 주지 않는다.
...
- 모바일 터치 선택 핸들과 드래그 확대경은 사용자가 요청한 플랫폼 선택 affordance를 따르므로 물방울·둥근 렌즈 형태를 예외로 둔다.
