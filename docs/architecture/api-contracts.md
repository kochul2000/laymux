...
**에이전트별 수집 경로는 분리한다.** Claude는 profile/config dir를 가진 PTY probe를 쓰며, Codex는 CLI가 제공하는 app-server 계정 API를 쓰고, Grok은 `/usage` TUI를 읽는 headless PTY probe를 쓴다([ADR-0154](../adr/0154-grok-first-class-agent.md)). provider는 원시 snapshot만 만들고 화면 규칙은 공통 `UsagePresentation`이 소유한다.
...
Grok Build 관련 동작(세션 복원, 셀렉터 상태 메시지 구성)을 제어한다. 필드 집합은 Codex와 같다 — `syncCwd`와 `sessionLimit*`는 두지 않는다([ADR-0154](../adr/0154-grok-first-class-agent.md)).
...
pairing manager main document·script·style은 APK `assets`와 `WebViewAssetLoader`의 `https://appassets.androidplatform.net/assets/`에서 적재한다. 이 secure WebView는 APK local origin과 아래 AEAD resource용 synthetic origin만 허용하고 CSP도 `default-src 'none'`, `connect-src 'none'`을 사용한다. ACK 네트워크 요청은 JavaScript가 아니라 Kotlin native 계층만 수행한다. `LaymuxNative` bridge는 scan·ACK 재시도 요청, 비밀이 아닌 pairing metadata 조회, 생체 보호 설정, key 확인/삭제, E2E Remote HTTP/output transport만 제공하며 secret byte를 반환하지 않는다. Cloud dashboard가 선택한 instance가 있으면 저장된 confirmed pairing 또는 새로 스캔한 QR의 `instance`가 정확히 일치해야 ACK와 session open을 진행한다.
