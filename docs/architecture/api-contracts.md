...
pairing manager main document·script·style은 APK `assets`와 `WebViewAssetLoader`의 `https://appassets.androidplatform.net/assets/`에서 적재한다. 이 secure WebView는 APK local origin과 아래 AEAD resource용 synthetic origin만 허용하고 CSP도 `default-src 'none'`, `connect-src 'none'`을 사용한다. ACK 네트워크 요청은 JavaScript가 아니라 Kotlin native 계층만 수행한다. pairing manager의 `LaymuxNative` bridge는 scan·instance별 ACK 재시도 요청, 비밀이 아닌 pairing metadata 목록 조회, 생체 보호 설정, instance별 key 확인/삭제를 제공하며 secret byte·pairing ID·client nonce를 반환하지 않는다. PC 제공 Remote 문서로 navigation하기 전에는 같은 이름의 interface를 E2E HTTP/output transport, 현재 session disconnect, 외부 링크 열기만 가진 별도 객체로 교체하고, pairing 상태 callback도 Remote surface에서는 실행하지 않는다. secure WebView는 다중 창을 켜지 않고 off-origin navigation을 거부하므로 Remote 문서의 터미널 링크는 `openExternalUrl(url)` 하나로 native에 넘긴다. native는 절대 URI·비opaque·`http`/`https` scheme·host 존재·userinfo 부재를 다시 검증하고 scheme만 소문자로 정규화한 뒤, 현재 local surface가 Remote일 때만 `ACTION_VIEW`+`CATEGORY_BROWSABLE`+`FLAG_ACTIVITY_NEW_TASK`로 OS 브라우저를 시작한다. 검증 실패는 무시하고 처리할 activity가 없으면 toast로 실패 닫힘하며, 앱은 `http`/`https` intent filter를 선언하지 않는다([ADR-0162](../adr/0162-android-remote-link-opens-os-browser.md)). pairing manager는 저장된 PC 목록과 confirmed/pending 상태를 표시하고 각 레코드를 독립적으로 확인·삭제한다. Cloud dashboard가 선택한 instance가 있으면 그 instance와 정확히 일치하는 저장된 confirmed pairing 또는 새로 스캔한 QR만 ACK와 session open에 사용한다. 선택한 PC의 confirmed 레코드가 없으면 다른 저장 pairing을 대체 선택하지 않고 QR을 요구하며, 기존 pending 레코드의 ACK 재시도는 pairing manager의 명시적 동작으로만 제공한다. 선택 변경은 기존 native E2E session을 먼저 닫는다([ADR-0154](../adr/0154-android-multi-instance-pairing-vault.md)).
