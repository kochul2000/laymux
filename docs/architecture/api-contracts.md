...
보이는 Remote Composer의 불투명도 상태는 raw 상태를 한 함수에서 다음 우선순위로 계산한다. 입력이 disabled면 `idle`, 초안·history/autocomplete 목록·IME 조합·전송 중 하나라도 활성화되면 `active`, 빈 textarea가 DOM focus를 가지면 `focused`, 나머지는 `idle`이다. `.terminal-composer[data-opacity-state]`가 이 결과를 반영하고 surface 전체에 해당 CSS 변수 값을 적용한다. Direct mode와 접힌 Composer는 계속 `hidden`이며 상태별 opacity가 새 레이아웃이나 terminal geometry를 만들지 않는다. Active 기본값은 100%라 입력과 추천 목록을 완전히 불투명하게 유지한다.
