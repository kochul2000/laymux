...
| `parse_error` | JSON 구문 오류 또는 최상위가 객체가 아님 | 전부 기본값 | 모달 알림 + 저장 영구 차단 |

**타입 오류 부분 복구.** 값 하나의 타입이 틀렸다고 파일 전체를 버리지 않는다. `settings/lenient.rs` 가 `serde_json::Value` 로 먼저 읽고, `serde_path_to_error` 로 실패 경로를 얻어 그 경로를 트리에서 **제거**한 뒤 재시도한다. 제거된 필드는 자신의 `#[serde(default)]` 로 채워지므로 복구 경로는 기본값 테이블을 따로 갖지 않는다. 값 보정(`"2"` → `2`)은 하지 않는다. 객체 필드는 키를, 배열 원소는 원소를 제거하며, 기본값이 없는 필수 필드가 깨지면 손실이 부모로 번진다(예: pane 의 `view.type` 타입 오류 → 그 pane 하나가 사라짐). 제거 횟수 상한은 64.

**손실 정책.** 드롭이 한 건이라도 있으면 `recovered` 상태가 되고, `useSessionPersistence` 가 `setBlockPersist(true)` 로 모든 `settings.json` 쓰기를 막은 채 기동한다. 사용자가 `SettingsRecoveryModal` 에서 확인을 누를 때 차단이 풀린다 — 그전까지는 레이아웃 변경도 저장되지 않으며 원본 파일은 손대지 않는다. 로더 자체는 어떤 경우에도 복구 결과를 디스크에 되쓰지 않는다. 모달은 드롭된 경로를 전부 나열한다(개수 요약만으로는 무엇을 잃었는지 알 수 없다).
