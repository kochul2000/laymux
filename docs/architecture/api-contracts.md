...
Remote drawer의 File viewer는 데스크톱에서 현재 열린 파일 상태와 호스트 경로 직접 입력을 제공하고, 결과를 별도 브라우저 탭에서 표시한다([ADR-0040](../adr/0040-remote-served-file-viewer.md)).

| Endpoint | Method | 용도 |
|---|---|---|
| `/remote/v1/file-viewer/status` | GET | active lease로 데스크톱 `useFileViewerStore`의 `{open,path}` 조회 |
| `/remote/v1/file-viewer/render` | POST | active lease로 현재 viewer 파일 또는 명시한 호스트 경로를 bounded web payload로 렌더 |

두 endpoint 모두 Remote bearer token/IP/Origin gate와 active controller lease를 요구한다. lease는 `X-Laymux-Remote-Lease` 헤더로 전달하고 render body는 `{ "source": "current" }` 또는 `{ "source": "path", "path": "..." }`다. `current`는 client가 보낸 path를 무시하고 desktop `useFileViewerStore`의 현재 path만 사용한다. 닫힌 current viewer, 빈 path, 알 수 없는 source는 실패한다.

`render`는 Rust route가 고정한 8 MiB `maxBytes`를 frontend async bridge에 전달한다. `readFileForViewer`는 image에도 상한을 적용한 bounded read를 수행한다. 응답은 `{path,kind:"text",content,truncated,previewKind?,previewDocument?}`, `{path,kind:"image",dataUrl}`, `{path,kind:"binary",size}` 중 하나다. HTML/Markdown `previewDocument`는 데스크톱 FileViewer와 같은 sanitizer/CSP builder의 결과이며 새 탭은 sandbox iframe `srcdoc`으로만 표시한다. 일반 text는 `textContent`, image는 `data:image/*`만 사용한다. Remote에서는 settings의 `extensionViewers` shell 매핑을 실행하지 않고 항상 이 built-in web renderer를 사용한다.

새 탭은 반드시 사용자의 button/Enter action에서 `window.open("/remote/viewer/")`으로 먼저 연다. child가 exact same-origin `laymux:file-viewer-ready` 메시지를 보내면 opener는 해당 `Window` 객체가 자신이 연 pending child인지 확인하고, token·lease·source/path를 `laymux:file-viewer-session` 메시지로 한 번 전달한다. child URL(query/fragment)·bootstrap DOM·localStorage/sessionStorage에는 token·lease·path를 기록하지 않는다. child는 exact origin과 `event.source === window.opener`를 확인해 최초 한 세션만 받고 즉시 opener 참조를 끊는다. 비동기 MCP/desktop viewer 변경은 heartbeat 성공 후 status polling으로 drawer에 반영하되 브라우저 popup 정책 때문에 자동으로 새 탭을 만들지 않는다.
