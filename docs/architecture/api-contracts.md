...
parsed frontier는 visible xterm callback과 rendererless checkpoint xterm이 같은 contiguous source prefix를 모두 완료한 교집합이다. `TerminalWriteBatchQueue`가 visible physical byte FIFO, zero-copy `Uint8Array.subarray()` descriptor의 materialization, callback/discard를 계속 소유한다. 두 xterm parser lane은 pane당 하나의 app-global owner를 공유하며 focused visible 4 / visible unfocused 2 / hidden 1의 latest-state weight와 age promotion으로 admission되고, 동시에 active인 desktop physical `xterm.write`는 최대 하나다([ADR-0092](../adr/0092-app-wide-terminal-write-round-robin.md), [ADR-0098](../adr/0098-terminal-parser-weighted-starvation-free-admission.md)). 이 교집합만 기존 `acknowledge_terminal_output(id,generation,token,seq)`를 전진시킨다. 같은 identity/payload 재전송은 idempotent하고 stale generation/token/envelope/grant completion은 현재 slot·grant·frontier를 바꾸지 않는다.
