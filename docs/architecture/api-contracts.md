...
Remote drawer 본문은 workspace 기본 화면과 Hidden workspaces·Notifications·Connection·Settings 하위 화면을 가진다. 연결된 상태에서 drawer를 열면 매번 workspace 기본 화면으로 시작하며 기존 File viewer·workspace 목록·Dock terminals를 렌더한다. Hidden workspaces·Notifications·Connection은 헤더의 각 버튼으로, 기기-로컬 Display와 설치 권유는 Settings 버튼으로 진입하고 뒤로가기는 workspace 기본 화면으로 돌아가 진입 버튼에 focus를 복원한다. Hidden workspaces와 Notifications의 상태는 Settings 업데이트 표시와 같은 5px 점으로만 나타내며 숫자 badge를 그리지 않는다. exact hidden/unread 개수는 각 버튼의 접근성 이름과 title에 유지한다([ADR-0187](../adr/0187-remote-drawer-status-dots-and-hidden-subview.md)). 기본은 기존 floating drawer이며 상단 메뉴 버튼이나 바깥 scrim으로 닫는다. `navigationPinned`를 요청하고 `window.innerWidth > navigationPinCutoff`일 때만 drawer를 terminal 옆 열에 고정하고 scrim·메뉴 버튼·close 동작을 비활성화한다. 컷오프 이하에서는 요청값을 보존한 채 floating drawer로 폴백한다. `navigationWidth`는 두 모드에 공통 적용하며 이 세 값은 연결·lease와 무관한 기기 로컬 표시 설정이다([ADR-0233](../adr/0233-remote-workspace-menu-pinning.md)). 아직 lease가 없거나 확정적으로 제어권을 잃어 drawer가 열릴 때만 Connection을 바로 표시한다. 하위 화면 선택은 ADR-0015의 surface-local drawer 상태이며 Remote API나 host 설정에 저장하지 않는다.
...
Display 탭의 `Edge swipe drawers`는 모바일 Remote 터미널의 양쪽 edge 제스처를 함께 켠다. 선택은 surface-local `localStorage["laymux.remote.edgeSwipeDrawers"]`에 저장하며 키가 없거나 값이 `"0"`이 아니면 기본 on이다. `mobileLayout`에서 xterm 영역의 왼쪽 24px 안에서 시작해 오른쪽으로 56px 이상 이동하고 수평 이동이 수직 이동의 1.25배를 넘으면 기존 workspace drawer를 연다. 오른쪽 24px 안에서 시작한 대칭 제스처는 active lease와 FileViewer capability가 있을 때 기존 Remote file explorer를 현재 terminal CWD(terminal이 없으면 host home)에서 연다. 열린 floating workspace drawer는 패널을 왼쪽으로, 열린 file explorer는 디렉터리 목록을 오른쪽으로 같은 임계값만큼 flick하면 닫힌다. 반대 방향 또는 수직 우세 이동은 기존 한 손가락 스크롤 경로로 되돌리고, 데스크톱 pointer에서는 이 제스처를 적용하지 않는다([ADR-0209](../adr/0209-remote-display-preferences-are-device-local.md), [ADR-0198](../adr/0198-remote-file-explorer-overlay.md)).
...
Structured input body는 `{ "leaseId": "...", "text": "...", "submit": true|false }`다. Rust는 LF/CRLF/CR을 CR로 정규화하고 terminal별 authoritative bracketed-paste state가 켜진 경우 text 부분을 `CSI 200~`/`CSI 201~`로 감싼다. `submit=true`의 최종 CR은 bracketed envelope 뒤에 둔다. 최종 인코딩 payload는 공통 1 MiB 상한을 적용한다. Composer는 Send action 하나만 노출하며 항상 `submit=true`를 사용한다. PC WebView와 fine-pointer Remote에서는 일반 Enter가 Send이고 Shift+Enter가 textarea 줄바꿈이다. coarse-pointer Remote에서는 Enter가 줄바꿈이고 Send 버튼만 제출한다. IME 조합 중 Enter는 어느 surface에서도 action이 아니다. Remote soft key(`^C` 포함)는 계속 raw `/write`를 사용한다. Direct clipboard paste는 `submit=false`로 `/input`을 사용해 browser xterm의 stale bracketed-paste mirror를 권위 상태로 사용하지 않는다([ADR-0034](../adr/0034-single-send-terminal-composer.md)).
