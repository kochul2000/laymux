...
`/remote/v1/workspaces/{id}/visibility`와 `/remote/v1/panes/{id}/visibility` body는 `{ "hidden": true|false, "leaseId": "..." }`이며 같은 lease header 대체를 허용한다([ADR-0153](../adr/0153-remote-hidden-item-visibility-controls.md), [ADR-0180](../adr/0180-remote-hidden-workspace-header-icon.md)). 재시도에도 상태가 뒤집히지 않도록 toggle이 아니라 목표 상태를 받는다. workspace 요청은 frontend bridge `ui.setWorkspaceHidden`을 통해 PC의 `setWorkspaceHiddenWithFallback`을 호출하므로 active workspace를 숨기기 전 다음 visible workspace로 착지하고 마지막 visible workspace 숨김을 거부한다. pane 요청은 존재하는 workspace pane만 `ui.setPaneHidden`으로 바꾸며 dock pane은 대상이 아니다. 복원 시 기존 store가 관련 auto-close eviction을 함께 지운다. 둘 다 성공 시 `workspace-state-changed`를 발행하고 Android E2E 내부 HTTP exact allowlist에도 포함된다. Remote drawer는 visible workspace quick-hide eye, 드로어 최상단의 crossed-eye 개수 아이콘으로 여는 workspace-only shelf, pane별 eye를 그리며 성공 뒤 navigation snapshot을 다시 읽는다. PC/Automation 호환용 `ui.toggle*Hidden`은 유지하지만 Remote API는 사용하지 않는다.
