...
- 최종 코드 `65f2e3be`를 다시 빌드한 dev PID 45972에서도 Windows 대화가 정확히 자동 복원되었다. WSL Astra의 새 빈 대화는 `fresh/idle`, PowerShell은 실제 명령 실행 후 `noAgent`였으며 전체 critical checkpoint를 두 번 통과했다(commit ID 3·4). 화면에서도 Windows 완료·일반 셸 완료·WSL 빈 입력 대기를 확인했다. 실제 사용자 프로세스 두 개의 읽기 전용 검사도 최종 코드로 두 번 통과했다.
