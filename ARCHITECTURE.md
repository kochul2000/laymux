...
[External Tool (curl)]
    │  HTTP request
    ▼
[Rust axum HTTP Server :19280 (release) / :19281 (dev)]
    │
...
dev와 release 빌드는 별도의 설정 디렉터리를 사용하여 동시 실행 시 충돌을 방지한다.

| 빌드 | Windows | Linux |
|---|---|---|
| Release | `%APPDATA%\laymux\` | `~/.config/laymux/` |
| Dev | `%APPDATA%\laymux-dev\` | `~/.config/laymux-dev/` |

각 디렉터리에 `settings.json`, `automation.json`, `paste-images/` 등이 독립적으로 저장된다.
`cfg!(debug_assertions)` 컴파일 타임 플래그로 결정되며, 단일 소스(`config_dir` 모듈)에서 관리한다.

### 12.3 포트 디스커버리

- **Windows**: `%APPDATA%\laymux\automation.json` (release) / `%APPDATA%\laymux-dev\automation.json` (dev)
- **Linux**: `~/.config/laymux/automation.json` (release) / `~/.config/laymux-dev/automation.json` (dev)
- 환경변수: `LX_AUTOMATION_PORT` (터미널 spawn 시 자동 주입)

| 빌드 | 우선 포트 범위 | 비고 |
|---|---|---|
| Release | 19280~19289 | 19280이 기본 |
| Dev | 19281~19289 | 19280을 건너뜀 — release와 동시 실행 시 충돌 방지 |

```jsonc
{
  "port": 19281,    // dev 예시 (release는 19280)
  "pid": 12345,
  "version": "0.1.0"
}
```
