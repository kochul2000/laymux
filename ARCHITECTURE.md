...
Layout은 Workspace 생성 시점에만 사용되는 **템플릿**이다. 생성 후 Workspace는 독립적으로 존재하며, Layout과의 영구 참조를 유지하지 않는다.

```
Layout (Template — 생성 시점에만 사용)
├── id
├── name
...
| 액션 | 동작 |
|---|---|
| Create from template | Layout 템플릿으로 새 Workspace 생성. 이후 연결 끊김 |
| Export as new template | 현재 Workspace의 pane 구조를 새 Layout으로 저장 |
| Export to existing template | 현재 Workspace의 pane 구조로 기존 Layout을 덮어쓰기 |
| Apply template | 선택한 Layout을 현재 Workspace에 적용 (pane 구조 초기화) |
...
| POST | `/api/v1/workspaces` | 워크스페이스 생성 (layoutId로 템플릿 지정) |
| PUT | `/api/v1/workspaces/:id` | 이름 변경 |
| DELETE | `/api/v1/workspaces/:id` | 삭제 |
| POST | `/api/v1/workspaces/apply-template` | 현재 워크스페이스에 레이아웃 템플릿 적용 |
