...
`computeCommandStatus` 함수 자체에 앱별 분기가 없다. 단, TUI 앱(Claude Code 등)은 OSC 133;D를 발행하지 않으므로, 격리된 앱 전용 모듈(§15.5)이 working→idle 전환 시 합성 exitCode를 Rust 백엔드에서 설정할 수 있다. 표시 함수는 이 합성 exitCode를 일반 exitCode와 동일하게 취급한다. `claudeMessage`는 텍스트 필드에만 영향을 미치며, 아이콘/색상은 변경하지 않는다.
...
- **빈도 기반 감지 (Burst Detection)**: 단일 DEC 2026h 이벤트만으로는 활성으로 판정하지 않는다. 포커스 리드로(DEC 1004 → `\x1b[I]` → 앱이 1회 리드로)나 키 입력 에코(키스트로크 → 앱이 1회 리드로)는 모두 1회성이다. **`DEC2026_BURST_WINDOW_MS`(2초) 내에 `DEC2026_BURST_THRESHOLD`(3회) 이상의 DEC 2026h가 감지되어야** 이벤트를 발행한다. 실제 TUI 작업(Claude 응답 생성, neovim 편집)은 초당 수십 회 프레임을 보내므로 임계값을 즉시 넘는다.
- **Throttle**: 임계값 충족 후에도 이벤트는 터미널당 최대 1회/초(`OUTPUT_ACTIVITY_THROTTLE_MS`)로 throttle하여 이벤트 폭주를 방지한다.
...
[Rust PTY 콜백]
  ...
  │  count ≥ 3 (DEC2026_BURST_THRESHOLD)?
  │    → Yes: AtomicU64 throttle (1초/터미널) 후 app.emit("terminal-output-activity")
  │    → No:  무시 (포커스 리드로 / 키 에코)
  ▼
[Frontend: useSyncEvents]
...
| 키 입력 에코 (타이핑) | 키당 1회 | 무시 (일반 타이핑 속도로는 2초 내 3회 미달) |
...
상태 아이콘, 알림 등 핵심 시스템에는 특정 앱(Claude Code 등)에 대한 전용 분기를 두지 않는다. 앱 전용 편의 기능(향상된 상태 메시지 등)은 별도 모듈에 격리하고, 설정 플래그(`claude.enhancedStatus` 등)로 활성화/비활성화할 수 있어야 한다. 해당 모듈을 import하지 않으면 편의 기능이 완전히 제거되어야 한다. 특수한 앱별 분기는 별도 모듈에 구현하고, 사용처에서 통합할 때 기존 로직을 손상시키지 않도록 세심하게 조율한다. 관련 테스트를 반드시 추가하거나 기존 테스트를 실행하여 기존 로직의 손상이 없음을 검증한다.
