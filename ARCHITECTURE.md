...
아이콘/색상 결정에 앱별 분기(Claude idle, vim 등)가 없다. `claudeMessage`는 텍스트 필드에만 영향을 미치며, 아이콘/색상은 변경하지 않는다.
...
| 작업 중 | 스피너 문자 접두어 (`✶✻✽✢`) | `✢ Working on task` |

**종료 판단**: 타이틀에 `"Claude Code"` 문자열이 없고 **동시에** 스피너 접두사 문자(`✶✻✽✢✳`)로도 시작하지 않을 때만 Claude Code가 종료된 것으로 판단한다. 스피너 접두사만 있는 타이틀(예: `✢ Working`)은 여전히 Claude Code 실행 중이다.
...
상태 아이콘, 알림 등 핵심 시스템에는 특정 앱(Claude Code 등)에 대한 전용 분기를 두지 않는다. 앱 전용 편의 기능(향상된 상태 메시지 등)은 별도 모듈에 격리하고, 설정 플래그(`claude.enhancedStatus` 등)로 활성화/비활성화할 수 있어야 한다. 해당 모듈을 import하지 않으면 편의 기능이 완전히 제거되어야 한다.
