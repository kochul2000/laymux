...
공식 `rmcp` SDK를 사용하여 MCP (Model Context Protocol) 서버를 Automation API에 직접 내장한다. 별도 바이너리 없이 `POST /mcp` 엔드포인트로 Streamable HTTP MCP 프로토콜을 제공한다.
...
| Tool | 구현 방식 | 설명 |
|------|-----------|------|
| `list_terminals` | bridge_request | 터미널 목록 조회 |
| `write_to_terminal` | AppState 직접 | PTY 입력 전송 |
| `read_terminal_output` | AppState 직접 | 출력 버퍼 읽기 |
| `focus_terminal` | bridge_request | 터미널 포커스 |
| `get_terminal_states` | AppState 직접 | 활동 상태 감지 |

**워크스페이스 (4)**:

| Tool | 구현 방식 | 설명 |
|------|-----------|------|
| `list_workspaces` | bridge_request | 워크스페이스 목록 |
| `get_active_workspace` | bridge_request | 활성 워크스페이스 |
| `switch_workspace` | bridge_request | 워크스페이스 전환 |
| `create_workspace` | bridge_request | 워크스페이스 생성 |

**그리드/팬 (3)**:

| Tool | 구현 방식 | 설명 |
|------|-----------|------|
| `get_grid_state` | bridge_request | 그리드 상태 |
| `focus_pane` | bridge_request | 팬 포커스 |
| `split_pane` | bridge_request | 팬 분할 |

**유틸리티 (3)**:

| Tool | 구현 방식 | 설명 |
|------|-----------|------|
| `take_screenshot` | bridge_request → image content | 스크린샷 캡처 |
| `list_notifications` | bridge_request | 알림 목록 |
| `send_notification` | bridge_request | 알림 생성 |
...
`scripts/setup-mcp.sh`를 실행하면 discovery 파일에서 port/key를 읽어 `.mcp.json`을 자동 생성한다.
...
# 프로젝트별 등록
./scripts/setup-mcp.sh --project /path/to/project
