...
| IDE CLI | `ide` 바이너리 (Rust, Tauri 동봉) |
...
Layout과 Workspace를 분리하여 1:N 관계로 설계한다.
...
├── name
├── layoutId         →  Layout 참조
├── cwd (optional)   →  워크스페이스 홈 디렉터리
└── panes: [ { x, y, w, h, viewInstanceConfig } ]
```

### 4.1 저장 액션 (Pane 내 View 변경 후)

| 액션 | 동작 |
|---|---|
| 저장 | 현재 Workspace만 수정 반영 |
| 저장 및 전파 | 동일 Layout을 참조하는 모든 Workspace에 반영 |
| 다른 이름으로 저장 | 새 Layout 생성, 현재 Workspace가 새 Layout 참조 |
| 원래대로 | 수정 내용 버리고 마지막 저장 상태로 복원 |
  ...
  "workspaces": [
    {
      ...
      "name": "프로젝트A",
      "layoutId": "dev-split",
      "cwd": "/home/user/dev/project-a",
      "panes": [
        { "x": 0.0, "y": 0.0, "view": { "type": "TerminalView", "profile": "WSL",        "syncGroup": "프로젝트A" } },
        { "x": 0.0, "y": 0.6, "view": { "type": "TerminalView", "profile": "PowerShell", "syncGroup": "프로젝트A" } },
...
hook에 의해 전파된 명령은 `IDE_PROPAGATED=1` 환경변수가 설정된 상태로 실행되며,
...
사용자는 별도 설정 없이 쉘에서 바로 `ide` 명령을 사용할 수 있다.

```bash
# IDE가 터미널 spawn 시 자동 주입
export IDE_SOCKET=/tmp/ide-{session}.sock   # IDE와 통신하는 Unix 소켓
export IDE_TERMINAL_ID=terminal-{id}        # 현재 터미널 인스턴스 ID
export IDE_GROUP_ID=group-{id}              # 현재 SyncGroup ID
export IDE_WORKSPACE_CWD=/home/user/proj    # 워크스페이스 홈 디렉터리 (설정 시)
export PATH="$PATH:/usr/local/ide/bin"      # ide 바이너리 경로
```

**셸 단축키**: 워크스페이스에 `cwd`가 설정되면, 셸에서 `cd =` 입력 시 해당 디렉터리로 이동한다. 미설정 시 `$HOME`으로 폴백. 새 터미널은 워크스페이스 CWD에서 자동 시작된다.

**커맨드 목록**

```bash
ide sync-cwd [path]                     # 그룹 내 CWD 동기화
ide sync-cwd [path] --all               # 모든 터미널에 전파
ide sync-cwd [path] --group [name]      # 특정 그룹에 전파
ide sync-branch [branch]                # 그룹 내 브랜치 동기화
ide notify "[message]"                  # IDE 알림
ide notify --level error "[message]"    # 레벨 지정 알림 (info|error|warning|success)
ide set-tab-title "[title]"             # 탭 제목 변경
ide set-command-status --command "[cmd]" # 실행 중인 명령 기록
ide set-command-status --exit-code N    # 명령 종료 코드 기록
ide open-file [path]                    # 에디터에서 파일 열기
ide send-command "[cmd]" --group [name] # 그룹 터미널에 명령 전송
ide get-cwd                             # 현재 CWD 조회
ide get-branch                          # 현재 브랜치 조회
ide get-terminal-id                     # 현재 터미널 ID 조회
```
...
```jsonc
{
  ...
  "hooks": [
    ...
    },
    {
      ...
      "param": "D",
      "when": "exitCode !== '0'",
      "run": "ide notify '명령 실패 (exit $exitCode)'"
    },
    {
      ...
      "run": "ide sync-branch $(git branch --show-current)"
    }
  ]
}
```
...
| 마지막 명령 + 결과 | OSC 133 E/D → `ide set-command-status` | 있을 때 |
| 최신 알림 텍스트 | OSC 9/99/777 또는 `ide notify` | 읽지 않은 알림 있을 때 |
...
[Shell: cd /foo]
    ...
    ▼
[TerminalView: PTY 수신]
    │  OSC 7 파싱 → hook 매칭 → run: "ide sync-cwd /foo"
    ▼
[ide 바이너리]
    │  IDE_SOCKET 경유 → IDE 본체에 JSON 전달
    ▼
[IDE: Terminal Manager]
    │  syncGroup 조회 → 대상 터미널 필터링
    │  IDE_PROPAGATED=1 플래그 설정 (루프 방지)
    ▼
...
- 환경변수: `IDE_AUTOMATION_PORT` (터미널 spawn 시 자동 주입)
...
| PUT | `/api/v1/workspaces/:id/cwd` | 워크스페이스 홈 디렉터리 설정 |
