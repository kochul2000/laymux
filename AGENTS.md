...
> 섹션 번호(§4.2, §8.4, §12.7, §14, §15 …)는 구 `ARCHITECTURE.md` 기준을 보존했고, 각 번호는 위 세 living doc 중 정확히 한 곳에 있다.
...
dev 터미널에서 `claude` 로 Claude Code 를 자동 실행할 때:

1. `claude\r\n` 전송 후 **10초 대기**(초기화). 제출은 반드시 `\r`(CR), `\n` 은 줄바꿈만.
2. 출력(`GET /api/v1/terminals/:id/output`)에서 **"trust" 확인** — "Yes, I trust this folder" 프롬프트면 `y\r\n` 으로 통과.
3. **"Claude Code" 타이틀 폴링**(3초 간격, 최대 60초). `GET /api/v1/terminals` 의 `title` 에 "Claude Code" 포함 또는 activity 가 `{"type":"interactiveApp","name":"Claude"}` 면 준비 완료.
4. 종료: Ctrl+C(`0x03` ETX)를 0.3초 간격 2회. 5초 후 activity 가 `shell` 로 바뀌었는지 확인.
