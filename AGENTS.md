...
- **React 훅 패턴** — 렌더 본문에서 `*Ref.current` 읽기·쓰기 금지(concurrent 렌더에서 찢어진다). 최신 props 미러링은 `useLayoutEffect`(같은 커밋에서 동기 실행 → passive effect·DOM 이벤트보다 먼저), 외부 스토어 시딩+구독은 `useSyncExternalStore`, 파생 가능한 값은 렌더 중 계산. `react-hooks/refs`·`react-hooks/set-state-in-effect` 는 `error` 이며 `eslint-disable` 로 억제하지 않는다.
